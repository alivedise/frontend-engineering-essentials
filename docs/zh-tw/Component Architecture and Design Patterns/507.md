...
併工具將元件的行為 props 套用到它上面。消費者寫 `<Button asChild><a href="/page">前往</a>
</Button>`，`Button` 的 `onClick`、`aria-*` 和 class 處理就會應用到 `<a>` 上，而 `Button`
不需要知道它渲染的是哪種元素。型別系統的好處是消費者明確提供了元素，TypeScript 無需泛型即可
