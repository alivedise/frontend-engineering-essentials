...
第四個設計考量是 prop 介面的版本控制策略。添加的 props 很容易做到不破壞性（以預設值添加）。
重新命名或移除的 props 需要主要版本升級或棄用期。在大型程式碼庫中廣泛使用或作為函式庫發佈的元
件，應將每個新 prop 視為潛在的長期承諾。一個有用的紀律是「三個使用情境」規則：在添加 prop 之前，
識別至少三個不同的、真實的呼叫點會使用它。如果 prop 只有一個呼叫點需要，正確答案幾乎總是在呼
叫點本身處理變化，而非將其編碼在元件的永久 API 中。

第五個設計考量是 `asChild` 模式——由 Radix UI 普及的一種多型替代方案。與其透過 `as` prop
使元件具有多型性（這需要複雜的 TypeScript 泛型才能正確型別化 ref 和所有轉發的 props），
`asChild` 接受任何單一 React 元素作為 `children`，並使用 `cloneElement` 或基於 slot 的合
併工具將元件的行為 props 套用到它上面。消費者寫 `<Button asChild><a href="/page">前往</a></Button>`，`Button` 的 `onClick`、`aria-*` 和 class 處理就會應用到 `<a>` 上，而 `Button` 不需要知道它渲染的是哪種元素。型別系統的好處是消費者明確提供了元素，TypeScript 無需泛型即可
正確推斷其 props。限制是 `asChild` 對不熟悉此模式的消費者不直觀，應在元件層級清楚地記錄。
...
**必須（MUST）使用 `{...rest}` 將元件未明確使用的所有 props 轉發至底層原生元素。** 在元件內部消費所有傳入 props 而不轉發剩餘部分，會破壞 ARIA 屬性、測試套件使用的 `data-testid` 屬性，以及呼叫者依賴的原生事件處理器。消費所有 props 不是保護措施，而是一個陷阱。

**必須（MUST）在任何包裹原生 DOM 元素的元件上支援 `ref` 轉發。** 不轉發 ref 的元件會迫使呼叫者使用 `document.querySelector` 直接存取 DOM，這很脆弱且在 SSR 情境中失敗。

**禁止（MUST NOT）預設讓每個元件都成為多型。** `as`/`asChild` 模式只應在元素彈性的需求是真實且反覆出現的情況下採用。對根元素永遠不會合理變更的元件進行過早的多型化，只會增加實作和型別的複雜性而毫無收益，不得在沒有具體多情境需求的情況下引入。
