...
**單一根層級 Boundary。** 在 `App` 根層級用一個 `<ErrorBoundary>` 包裹整個應用程式，意味著任何小工具 — 圖表、推薦輪播、活動動態 — 拋出渲染錯誤時，整個應用程式的 UI 都被根層級的降級畫面取代。從使用者的角度來看，應用程式已崩潰。修復方法是系統性的：審計每個獨立有價值的 UI 元件，確保它有自己的 Boundary。根層級 Boundary 作為最後手段的安全網保留，但在正確結構化的應用程式中，它幾乎不應該啟動。
...
**靜默地吞掉錯誤。** 渲染降級畫面卻不在 `componentDidCatch` 中呼叫可觀測性服務的 Error Boundary，意味著生產崩潰是隱形的。使用者看到降級的體驗，工程團隊什麼也看不到，錯誤在沒有人察覺的情況下持續累積，直到足夠多的使用者抱怨。始終將 `onError` 回呼連接至真實的可觀測性管線。即使在開發和測試環境中，至少也要將錯誤和元件堆疊記錄至主控台，讓開發團隊能看到哪些 Boundary 正在啟動。
