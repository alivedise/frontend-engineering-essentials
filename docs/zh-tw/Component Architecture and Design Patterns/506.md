...
**必須（MUST）實作 `getDerivedStateFromError`，以便在子元件拋出錯誤時將元件轉換至錯誤狀態。** 應該（SHOULD）同時實作 `componentDidCatch`，以便將錯誤回報至可觀測性服務。只渲染降級 UI 卻不回報錯誤的 Boundary，在運維上是盲目的。

**禁止（MUST NOT）靜默地吞掉錯誤。** `componentDidCatch` 必須在每次捕捉到錯誤時呼叫已設定的可觀測性端點，並包含 `info.componentStack` 參數，不得省略此回報步驟。

**必須（MUST）使用 `try/catch` 明確處理事件處理器與非同步錯誤。** Error Boundary 不會捕捉事件處理器錯誤、非同步程式碼（`setTimeout`、Promise）、SSR 或 Boundary 本身內部的錯誤。期望事件處理器錯誤被 Boundary 捕捉是 React 程式碼庫中最常見的韌性設計錯誤之一，不得依賴 Boundary 攔截這些失敗模式。
