...
工程師應該（SHOULD）在 context 值的各部分以不同頻率變更時，將其拆分為獨立的 provider。一個同時
包含 `user` 物件（在登入/登出時變更）與 `login`/`logout` 函式（穩定參考）的 auth context，會在
每次登入/登出時觸發所有消費者重新渲染，即使某些消費者只使用了穩定的函式。將其拆分為
`AuthUserContext` 與 `AuthActionsContext`，讓消費者得以只訂閱自己所需的部分，消除只讀取穩定
actions 的消費者所產生的多餘重新渲染。

工程師必須（MUST）對作為物件或陣列字面值的 context 值進行記憶化，以防止消費者在每次 provider
元件渲染時都重新渲染。將 `{ user, setUser }` 作為 context 值傳入，會在 provider 元件每次渲染
時建立一個新物件。每個消費者都會在 provider 渲染時重新渲染，無論 `user` 或 `setUser` 是否真的
發生了變化。`useMemo(() => ({ user, setUser }), [user, setUser])` 將重新渲染限制在實際值發生
改變的時候。這兩條規則——按變更頻率拆分、對物件值記憶化——涵蓋了生產環境 React 應用中遇到的
大多數 context 效能問題。即使消費者目前數量不多，也應從一開始就養成記憶化的習慣，因為隨著
功能擴展，消費者數量可能快速增加，而屆時要定位效能問題的根源往往比預防問題更費力。
