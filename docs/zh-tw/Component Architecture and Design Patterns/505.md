...
隨著 React 應用規模擴大，某些行為會在不相關的元件之間反覆出現：渲染頁面前的身分驗證檢查、使用者查看某個區塊時的分析事件觸發、主題注入、權限閘道、錯誤邊界、功能旗標切換。這些關注點橫向地切穿整棵元件樹，並不遵循任何父子關係。挑戰在於只需實作一次就能一致地套用每個關注點，不重複邏輯，也不把不相關的關注點糾纏進個別元件，同時讓元件樹對開發者每天倚賴的除錯工具保持清晰。
...
最後，HOC 非常適合框架整合點，在那裡元件包裹是慣用的 API 形式。React Router 的 `<Route>` 元素使用 render props，但許多路由層級的關注點（權限、分析、捲動恢復）以 HOC 形式包裹路由元件，比逐路由設定更乾淨。類似地，測試函式庫通常提供 HOC 風格的包裹器，用於在單元測試中注入測試 provider（`withStoreProvider`、`withThemeProvider`）。在這些情境中，HOC 是框架或工具所預期的 API 形狀，而不是你在做的設計選擇。

**React 19 與 ref 作為普通 prop 的變更。** React 19 移除了 function component 必須使用 `React.forwardRef` 的要求：function component 可以直接在 props 解構中宣告 `ref`，React 會正確處理它。這對純 React 19 程式碼庫的 HOC 撰寫體驗有所簡化。回傳的包裹函式可以直接在 props 中接受 `ref`，不再需要外包一層 `React.forwardRef`。然而，這項變更只是改變了機制；轉發 `ref` 的義務仍然存在。在 React 19 中忽略 `ref` 的 HOC，對任何持有包裹元件 ref 的父元件來說仍然是損壞的。實務上，大多數生產環境的 HOC 在 React 18/19 過渡期間仍需保持 `React.forwardRef` 相容性，而任何包裹 class component 的 HOC 則必須繼續使用 `React.forwardRef`，因為 class component 不接受 `ref` 作為普通 prop。ref 轉發契約是語意上的義務，而非 API 細節。滿足它的機制會隨版本改變，但義務本身不會改變。

**生產函式庫中的真實 HOC 範例。** HOC 模式的持久生命力，體現在它在基礎 React 函式庫中的持續使用。React Redux 的 `connect(mapStateToProps, mapDispatchToProps)(Component)` 是最經典的 HOC：它訂閱 Redux store、選取元件所需的 state 切片，並以 props 形式注入。元件本身從不直接從 store 匯入。Apollo Client 的 `withApollo(Component)` 將 Apollo client 實例作為 prop 注入，讓 class component 能夠在不使用 render-prop 或 hook 的情況下命令式地執行查詢。React Router（v6 以前）的 `withRouter(Component)` 則注入 `history`、`location` 和 `match` props，使樹中的任何元件都能存取路由 context，而不需要直接消費 `<Route>`。這些實作都遵循相同的結構契約：一個工廠函式、一個透過 hook（或訂閱）從 context 讀取狀態的包裹器、注入衍生 props，以及明確的 `displayName` 和 `forwardRef` 支援。
...
**使用 `hoist-non-react-statics` 提升靜態方法。** 當 HOC 包裹一個元件時，工廠回傳的包裹器函式不會繼承原始元件上定義的任何靜態方法。這是一個靜默的問題：`withAuth(Dashboard)` 上不會有 `Dashboard.getLayout`、靜態設定的 `Dashboard.defaultProps`，或任何其他你定義的靜態成員。在 Next.js 中，失去 `getLayout` 會導致頁面在沒有預期版面包裹器的情況下渲染。在元件函式庫中，失去靜態的 `componentType` 鑑別器會破壞消費端程式碼中的 switch-on-type 邏輯。`hoist-non-react-statics` 套件自動化了這個複製過程：在回傳包裹器之前呼叫 `hoistNonReactStatics(Wrapper, WrappedComponent)`，所有非 React 靜態成員（除了 `displayName`、`defaultProps`、`propTypes`、`contextType` 以及其他 React 保留鍵之外的所有靜態成員）都會被複製。如果你不想增加依賴，就針對應用程式實際使用的屬性明確複製靜態成員，刻意為之而非偶然遺漏。每個新 HOC 的程式碼審查清單項目應包含：「包裹元件定義了哪些靜態成員？包裹器上是否都有？」
...
**當堆疊無可避免時，使用 `pipe` 或 `compose` 工具組合 HOC 工廠。** 如果必須對單一元件套用兩個 HOC，優先使用明確的 `compose` 呼叫，而非巢狀呼叫。巢狀呼叫由內而外閱讀（`withAuth(withAnalytics(Dashboard))`），若不仔細閱讀，執行順序容易產生歧義。`compose(withAuth, withAnalytics)(Dashboard)` 則以從左到右（或從右到左，取決於 `compose` 的實作）閱讀，讓優先順序變得明確。大多數工具函式庫（lodash、ramda）提供 `compose` 或 `flowRight` 輔助函式；或者，一個專案本地的雙引數 `compose` 只需六行 TypeScript，就能完全消除歧義。即使使用 `compose`，原則章節中的兩個 HOC 上限仍然適用。`compose` 改善了可讀性，但不會減少樹中包裹元素的數量。
...
**必須（MUST）對其元件引數是純函式。** 給定相同的 `WrappedComponent`，每次呼叫都必須回傳帶有相同增強行為的元件。禁止修改 `WrappedComponent` 的原型或 `defaultProps`。這樣做會在同一個基底元件被傳入多個 HOC 時，引發不可預期的行為。

**禁止（MUST NOT）在單一元件上堆疊超過兩個 HOC。** 每一層都在 React 樹中增加一個包裹元素，讓 DevTools 的雜訊成倍增加，並建立 props 穿透通道。當需要套用超過兩個橫切關注點時，應提取單一多關注點包裹器，或將這些關注點重構為元件直接呼叫的 hook。

**應該（SHOULD）對橫切的非渲染關注點優先採用 custom hook。** 分析、認證狀態讀取、權限檢查、資料預抓取和 store 訂閱不需要包裹元素，不在 React DevTools 中產生匿名條目，且組合時不帶 HOC 堆疊的 props 穿透代價。當橫切關注點需要包裹或替換渲染輸出時，必須（MUST）使用 HOC。
...
替代方案 B 通常是最清晰的：元件的原始碼讀起來就像它所套用的關注點的一份清單，不帶包裹巢狀、DevTools 雜訊或 props 穿透。替代方案 A 適合當關注點確實必須包裹渲染輸出（本例中的主題 context 注入必須圍繞元件，這是渲染層面的需求），且在避免堆疊的同時進行合併仍然是值得的時候。

## TypeScript 中的 HOC 型別模式

HOC 型別是 React 程式碼庫中最常被問到的 TypeScript 主題之一，因為要保留完整型別推斷所需的泛型串接並不直觀。三種模式涵蓋了大多數真實世界的情境。

**模式一：從外部型別中排除注入的 props。** 最常見的 HOC 會注入一個或多個 props，而消費者不應該需要提供它們。對外部元件的 props 型別使用 `Omit`：
...
外部元件接受 `Omit<P, keyof WithAuthProps>`，這意味著消費者無法（也不需要）傳遞 `currentUser`。TypeScript 會在呼叫端強制執行這一點。

**模式二：無注入 props 的透傳 HOC。** 某些 HOC 在不注入 props 的情況下添加行為（例如在掛載時觸發分析事件）。外部型別與 `P` 相同：
...
**模式三：推斷 ref 型別。** 當 ref 型別很重要時（例如消費者持有指向 DOM 元素的 ref），為 ref 元素型別新增第二個泛型參數：
...
這更為冗長，但能讓消費者得到正確型別的 `ref.current: HTMLInputElement | null`。
...
- [FEE-503：Headless 元件與無渲染模式](./503.md)：共同動機是將橫切邏輯排除在元件主體之外
