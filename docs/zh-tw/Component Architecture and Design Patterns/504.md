...
第三個維度是函式庫設計。為外部消費者——或同組織中的多個產品團隊——建立元件時，你無法預測每個
消費者的使用情境。對只想要自我管理下拉選單的消費者強制使用受控模式是阻力；對需要將下拉狀態與
URL 參數同步的消費者強制使用非受控模式是阻礙。雙模式模式消除了這兩個問題。Radix UI 在所有
primitives 中一致地實作此模式：`Dialog`、`Popover`、`Select`、`Accordion`、`Collapsible`
等，均同時接受受控 props 與 `default*` props。`@radix-ui/react-use-controllable-state` hook
已以獨立套件的形式開放，將偵測與同步邏輯封裝在約 50 行程式碼中。

第五個維度是 TypeScript 型別工程學。完全受控的 API 將 `value` 的型別收窄至實際值型別並要求
`onChange`；TypeScript 編譯器會同時強制兩者。雙模式 API 通常使用多載簽名（overloaded signature）
或差異化聯合型別（discriminated union）來表達：只有在 `value` 不存在時才允許 `defaultValue`，
且非受控時 `onChange` 是選用的。正確實作此型別的函式庫，讓呼叫者在編譯時就收到錯誤，而非在執行
期才看到警告——這是在型別化程式碼庫中暴露 API 誤用的正確位置。
...
**必須（MUST）在受控模式下完整從 props 取得顯示值。** 受控元件不得在內部 state 中保存值的本地副本。每次可能改變值的使用者互動必須透過事件 callback 向上傳遞；父層單獨負責決定是否以及如何更新 props。若父層不更新 props，元件再次渲染相同的值——這是預期行為，不是錯誤。

**必須（MUST）在非受控模式下接受 `defaultValue` 以進行初始化，並且僅在掛載時讀取一次。** 元件不得在後續渲染中重新讀取 `defaultValue`。在掛載後變更 `defaultValue` 無效，呼叫者不得依賴它來重設元件。

**禁止（MUST NOT）將受控 props 與對應的 `defaultValue` props 同時傳遞給同一個元件實例。** 這樣做會產生不確定的行為。當元件處於受控模式時，不得在回應使用者互動時更新內部 state——props 是唯一的真實來源，每次變更必須透過 callback 傳遞出去。
...
- [TypeScript: Function Overloads](https://www.typescriptlang.org/docs/handbook/2/functions.html#function-overloads) — 實作雙模式型別簽名時，函式多載是最常見的型別工程手段，官方文件提供了完整範例
- [Radix UI: useControllableState 原始碼](https://github.com/radix-ui/primitives/blob/main/packages/react/use-controllable-state/src/useControllableState.tsx) — 約 50 行的實作，涵蓋模式偵測、受控/非受控的 state 同步，以及開發模式下的警告，是雙模式 hook 的標準參考實作
- [Mantine: Controlled and Uncontrolled guide](https://mantine.dev/guides/controlled-uncontrolled/) — 另一個成熟元件庫如何在設計文件中說明雙模式 API 設計，含 `useUncontrolled` hook 的設計理由
