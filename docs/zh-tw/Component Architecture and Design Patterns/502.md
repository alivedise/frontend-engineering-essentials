...
路徑仍會通知元件。在 Solid 中，在計算中讀取同一個 signal 兩次仍然只是一個訂閱。執行期會進行
去重複處理。
...
一個值得注意的發展是 TC39 Signals 提案，旨在將原語 signal 類型標準化為 JavaScript 語言本身的一部分。提案層級的說明（響應式 JavaScript 的歷史、無閃爍求值演算法、跨框架委員會與 polyfill 整合配方）由 [FEE-10005](../Web%20Platform%20Proposals/TC39%20and%20JS%20Proposals/10005.md) 涵蓋。對元件架構而言，可帶走的重點是：基於 signal 的細粒度響應式是跨框架的長期方向。本文所述的模式（計算值、條件訂閱、effect 邊界）會出現在每個採用 signal 的框架中，與團隊選擇何種框架無關。
...
signal 程式碼庫中記憶體洩漏的常見來源，症狀是效能隨著使用者操作的累積而緩慢降低，因為
無法被垃圾回收的訂閱列表不斷增長。
