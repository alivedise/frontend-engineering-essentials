...
**禁止（MUST NOT）在 headless 層嵌入 `className`、內聯樣式、DOM 元素選擇或與版面相關的標記。** 一旦 headless 層假設它將渲染 `<ul>`，任何需要基於 `<div>` 滾動容器的消費者都必須對抗這個假設。不得將渲染細節洩漏至 headless 層中。

**應該（SHOULD）在相同互動邏輯需要服務三個或更多視覺情境，或在建立消費者控制所有樣式的元件庫時，採用 headless 模式。** 在單一應用程式中只有一種視覺形式的元件不需要 headless 層——抽象成本是真實的，而收益為零。

**禁止（MUST NOT）過度抽象。** 在整個產品中外觀始終一致的 `<Button>` 不是 headless 候選者。過早的 headless 提取會製造沒有回報的間接層，不得在沒有具體多情境需求的情況下強行引入。
...
**將無頭層綁定到特定框架原語。** 直接引用 `useState` 的無頭 hook 僅限於 React。若相同邏輯可能需要在 Vue 專案中使用，應考慮是否能將核心狀態機（開啟/關閉、篩選、選取）與框架響應層分離。這是一個進階考量，但對跨框架元件函式庫至關重要。
