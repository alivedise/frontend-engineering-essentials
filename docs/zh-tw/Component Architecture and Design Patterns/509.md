...
**必須（MUST）將 `src/` 組織為以下順序的六個 FSD layer：`app`、`pages`、`widgets`、`features`、`entities`、`shared`。** 依賴規則是無條件的：任何 layer 中的模組禁止從其上方的任何 layer 或同一 layer 中的同級 slice 匯入。這個邊界是可由機器強制執行的不變量，而非軟性慣例。

**不應（SHOULD NOT）對路由數量少於約 10 個或活躍開發人員少於 3 人的應用程式應用 FSD。** 低於這個規模，layer 開銷超過了組織效益。組織良好的 `src/components/`、`src/hooks/`、`src/utils/` 和 `src/pages/` 結構對小型應用程式已經足夠。

**必須（MUST）保持 `shared` 領域無關。** `shared` layer 禁止包含對業務領域的引用——沒有 `useProductStore`、沒有 `CartItem` 型別、沒有領域特定的格式化器。每個 `shared` 的消費者都耦合到其內容；領域概念在 `shared` 中會成為每個從 `shared` 匯入的 layer 的傳遞依賴，不得讓業務領域概念滲入 `shared` 層。
