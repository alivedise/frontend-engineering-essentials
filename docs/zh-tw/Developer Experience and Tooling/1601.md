...
Linting 就是自動化的程式碼審查。在每次儲存時觸發的 lint 規則，比任何 PR 審查流程都更可靠地捕捉特定類別的錯誤，因為它對每一行程式碼、每一次儲存都執行，永遠不會有狀態不佳的時候。
...
Biome 最適合全新（greenfield）專案以及規則需求在其內建覆蓋範圍內的團隊。如果你正在啟動一個沒有遺留外掛依賴的新 TypeScript 專案，Biome 的零設定速度優勢是真實的且值得採用。已將 Biome 格式化工具標準化（取代 Prettier）的團隊，也能從擁有單一工具處理 linting 和格式化中獲益，消除了兩份獨立設定之間的協調開銷。
...
使用帶有明確匯入的 `eslint.config.js`。為原始碼檔案、測試檔案和設定檔案分別定義設定物件——各自有自己的規則集。這讓哪些規則適用於哪些檔案一目了然，並避免需要使用行內停用注解來消除測試工具或設定腳本中的誤報。

陣列結構直接對應到檔案範圍：陣列中的前幾個物件為所有檔案建立基準，後續帶有 glob 模式的物件則針對特定檔案類型縮小或覆蓋規則。這是可讀的、可預測的，且不需要了解目錄遍歷層疊。

### 支援型別的規則

設定 `parserOptions.projectService: true`——typescript-eslint v8+ 的現代 API，取代了需要手動指定 tsconfig 路徑的舊版 `parserOptions.project`。`projectService` 讓 typescript-eslint 建立和管理自己的 TypeScript 語言服務，它會根據 TypeScript 自己的專案解析邏輯，自動為每個檔案找到正確的 `tsconfig.json`。

在 `projectService` 旁邊始終設定 `tsconfigRootDir: import.meta.dirname`，以將 tsconfig 搜尋錨定到包含 `eslint.config.js` 的目錄。若沒有這個設定，tsconfig 的發現可能因 `eslint` 命令在哪個目錄執行而產生不同結果。

### 外掛選擇

React + TypeScript 專案的完善外掛集：

- **`eslint-plugin-react`** — React 專用規則：prop types、display names、列表中的 JSX key。
- **`eslint-plugin-react-hooks`** — 強制執行 Hooks 規則。在任何 React 專案中，這個外掛都是不可或缺的；遺漏 `useEffect` 依賴項會導致難以引入且難以診斷的微妙錯誤。
- **`eslint-plugin-jsx-a11y`** — JSX 的無障礙規則：要求圖片有 `alt` 文字、對互動元素強制鍵盤可導覽性、捕捉 ARIA 誤用。
- **`eslint-plugin-security`** — 捕捉引入 XSS、注入和原型污染漏洞的模式，包括帶有未消毒值的 `dangerouslySetInnerHTML` 和 `eval` 使用。
- **`eslint-plugin-unicorn`** — 廣泛的現代 JavaScript 慣用法集合：偏好 `Array.from` 而非 `Array.prototype.slice.call`、避免將 `null` 作為預設參數、在正規表示式中使用命名捕獲群組。該外掛在整個程式碼庫中強制執行一致的現代 JavaScript。

每個外掛服務於不同的關切事項。全部納入，而不是有選擇地挑選——完整的集合捕捉的問題多於各部分的總和。

### 規則嚴重程度紀律

對絕不能進入正式環境的問題使用 `error`：未使用的變數（`no-unused-vars`）、正式環境建置中的 console 陳述式（`no-console`）、TypeScript 中的明確 `any`（`@typescript-eslint/no-explicit-any`）以及未處理的 promise（`@typescript-eslint/no-floating-promises`）。這些規則捕捉的是錯誤，而不是風格偏好。

`warn` 僅用於積極遷移中的問題——你正在向有現有違規的程式碼庫引入的規則。設定遷移截止日期，遷移完成後移動到 `error`。

使用 `off` 明確停用規則，而不是讓規則在設定中缺席。缺席的規則是模糊的：不清楚這個規則是被考慮後拒絕了，還是從未被遇到過。設定中明確的 `"rule-name": "off"` 是一個文件化的決策。

### 從 `.eslintrc` 遷移到 flat config

如果你的專案使用現有的 `.eslintrc` 格式設定——無論是自己撰寫的還是繼承的共享設定——你不需要一次全部重寫。`@eslint/eslintrc` 套件提供了一個 `FlatCompat` 工具，可以橋接這兩個系統：它將舊版 `.eslintrc` 格式的設定包裝成 flat config 物件，讓你能夠逐步遷移。
...
export default [
  // 包裝舊版共享設定或 .eslintrc 檔案
  ...compat.extends("some-legacy-config"),
  // 在舊版包裝器旁邊新增原生 flat config 物件
  {
    rules: {
      "no-console": "error",
    },
...
`FlatCompat` 處理舊版層疊系統自動執行的外掛解析和規則合併。`baseDirectory` 選項將解析錨定到包含 `eslint.config.js` 的目錄，與舊版 `.eslintrc` 查找行為一致。一旦舊版設定透過 `FlatCompat` 正確載入，你就可以逐一將各個外掛和規則遷移到原生 flat config 物件，每完成一個就移除對應的 `compat.extends()` 呼叫。

這個遷移路徑對於依賴其他團隊或廠商發布的共享設定的團隊尤為重要。並非所有共享設定都已更新為匯出 flat config 物件——`FlatCompat` 讓你無需等待每個上游依賴發布 flat config 版本，就能採用 `eslint.config.js`。

### CI 整合

在 CI 中執行 `eslint --max-warnings 0`。這將任何警告視為失敗，防止開發者在本機學會忽略的未處理警告累積。結合 lint-staged 的 pre-commit hook（FEE-1603），這建立了兩道關卡：警告在提交前於本機被捕捉，任何漏過的都在合併前於 CI 被捕捉。
...
### 將應該是 `error` 的規則設為 `warn`

開發者很快就會學會忽略黃色底線。警告累積。幾週後，編輯器的問題面板顯示數十個警告，沒有一個被處理，而團隊的實際政策是這些模式是可接受的——因為工具從不阻擋它們。

如果一個模式值得偵測，它就值得被阻擋。將其設為 `error`。如果程式碼庫目前有無法立即處理的違規，在已知違規上使用有針對性的 `eslint-disable` 注解，提出 issue 來移除它們，並將規則設為 `error`，這樣從一開始就阻擋新的違規。

唯一的例外：在首次將嚴格規則引入現有程式碼庫期間，短暫使用 `warn` 來顯示違規數量，而不立即中斷 CI。設定一個具體的截止日期——最好在同一個衝刺（sprint）內——然後移動到 `error`。
