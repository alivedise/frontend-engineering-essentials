...
Lint 錯誤必須視為阻斷性問題，而非建議事項。將所有可操作的規則設為 `error`；`warn` 僅保留給正在逐步導入或淘汰的規則，一旦遷移完成便應立即移除。在 CI 中執行 `eslint --max-warnings 0` 以強制執行此契約。開發者習慣忽略的 lint 警告不是品質關卡——而是讓工程師學會忽略工具的雜訊。
