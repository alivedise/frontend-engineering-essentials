...
除錯應該（SHOULD）從具體的假設出發，而非探索性的日誌記錄。假設決定了使
用哪個 DevTools 面板和哪種中斷點類型。對於互動式除錯，中斷點應該（SHOULD）
優先於 `console.log`，因為它們在精確的感興趣位置停止執行，暴露完整的呼叫
堆疊，並允許即時檢查所有作用域內的變數，而無需修改原始碼。在優化任何感
覺緩慢的互動之前，必須（MUST）使用效能面板，因為感知的緩慢通常是由開發
者所懷疑程式碼路徑之外的長任務引起的。
