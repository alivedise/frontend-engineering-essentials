...
**應該（SHOULD）記錄 `--no-verify` 的明確繞過政策並透過程式碼審查文化執行。** 未共享個人分支上的進行中提交和活躍事件期間的緊急熱修復是唯一可接受的繞過情境，兩者都應該（SHOULD）產生後續票據。因 hook 太慢而繞過是 hook 配置需要優化的信號，而非繞過是適當的；因規則不便而繞過禁止（MUST NOT）被接受——應該修正 ESLint 配置中的規則，或修復違規。
