...
團隊必須（MUST）使用 Git hooks 在提交時自動化品質關卡。替代方案——依賴開發者的自律，在每次提交前手動執行 linting、格式化和驗證——會產生不一致的結果，並為程式碼審查增加摩擦。

Hook 各階段必須（MUST）用於符合其延遲預算的工作。`pre-commit` 必須（MUST）在 10 秒內完成；任何更慢的事都會訓練開發者用 `--no-verify` 繞過它。`commit-msg` 應該（SHOULD）在 1 秒內完成；它執行 commitlint，幾乎是即時的。`pre-push` 可以（MAY）執行較慢的檢查（型別檢查、單元測試），因為它執行頻率較低——每次推送一次，而非每次提交一次。

Hooks 必須（MUST）為新團隊成員自動安裝。`package.json` 中的 `prepare` 生命週期腳本（`"prepare": "husky"`）在 `npm install` 時執行並安裝 hooks，無需任何手動步驟。需要手動安裝的 hook 設定，就是將會在新貢獻者環境中缺失的設定。

機密掃描必須（MUST）是 `pre-commit` hook 的一部分。進入 git 歷史的機密——即使在從未合併的分支中——也已洩露。Git 歷史並不容易修訂。在提交之前掃描已暫存的內容，是防止這種情況發生的最後可靠機會。

`--no-verify` 不是被禁止的，但其使用必須（MUST）是有記錄的團隊決定，而非個人逃脫口。可接受的使用：個人分支上的進行中提交、每分鐘都至關重要的緊急熱修復提交。不可接受的使用：因為 hooks 太慢而繞過（修復速度問題），因為某個規則令人惱火而繞過（處理那個規則）。
...
將 `"prepare": "husky"` 加入 `package.json` 的 `scripts` 區段。這個腳本在每次 `npm install`、`yarn install` 和 `pnpm install` 時執行，安裝 Git hooks，無需新貢獻者的任何手動步驟。

沒有這個，克隆程式庫並執行 `npm install` 的開發者沒有 hooks。他們可以在沒有 lint 檢查、沒有提交訊息驗證、沒有機密掃描的情況下提交。品質關卡對其他所有人存在，但對他們不存在。
...
對於 pnpm 專案，由於 pnpm v8 起預設不會在 `pnpm install` 時自動執行 `prepare`。請在 `.npmrc` 中加入 `enable-pre-post-scripts=true` 以恢復此行為，或指示開發者在首次安裝後手動執行 `pnpm prepare`。

一個邊緣情況：`prepare` 腳本也在 CI 中的 `npm install` 期間執行。要在 CI 中跳過 hook 安裝，在 CI 環境變數中設置 `HUSKY=0`。Husky 在啟動時檢查這個變數，如果設置了就立即退出。

### lint-staged 以提升速度

在 `package.json`（或 `lint-staged.config.js`）中配置 lint-staged，模式匹配到相關工具。TypeScript + React 專案的標準配置：
...
每個模式陣列中的順序很重要。ESLint 先執行（`--max-warnings 0` 確保任何警告都被視為錯誤）。Prettier 第二執行並重新格式化檔案。lint-staged 在工具執行後重新暫存重新格式化的檔案，使提交包含格式化版本。

`--max-warnings 0` 旗標很重要。沒有它，即使有警告 ESLint 也以零退出，hook 通過。有了它，任何警告都會導致非零退出，阻止提交。這確保 `pre-commit` hook 和 CI 行為一致（CI 也應該執行 `eslint --max-warnings 0`）。

### Hook 階段紀律

將工作保持在正確的階段：

- **`pre-commit`**：lint-staged（對已暫存的檔案執行 ESLint + Prettier）、機密掃描（gitleaks）。目標：5 秒以內。
- **`commit-msg`**：commitlint。目標：1 秒以內。
- **`pre-push`**：`tsc --noEmit`（完整型別檢查）、`vitest run --passWithNoTests`（單元測試）。目標：60 秒以內。

在任一方向跨越階段邊界都會產生問題。將型別檢查移到 `pre-commit` 使 hook 太慢。將機密掃描移到 `pre-push` 意味著機密可以在被捕獲之前進入本地提交歷史（仍可補救，但更混亂）。將測試移到 `pre-commit` 是最常見也最嚴重的錯誤配置。

### 機密掃描

將 gitleaks 加入 `pre-commit` hook。`gitleaks protect --staged` 命令只掃描已暫存的差異，而不是整個程式庫，使它足夠快以符合 `pre-commit` 的延遲預算。
...
gitleaks 需要單獨安裝（它不是傳統意義上的 npm 套件——它是一個 Go 二進位檔）。在程式庫的入職指南中記錄安裝需求。對於使用 macOS 的團隊：`brew install gitleaks`。對於 Linux：gitleaks GitHub releases 頁面提供預構建的二進位檔。對於 CI 環境：將 gitleaks 作為 CI 步驟加入，除了（而非替代）pre-commit hook。

`.gitleaks.toml` 配置檔案允許為符合 gitleaks 規則但不是真正機密的模式添加允許清單項目（測試固定裝置、範例值、已知非機密的高熵字串）。沒有允許清單，誤報會導致開發者頻繁使用 `--no-verify`。

### 優雅的繞過機制

記錄 `--no-verify` 可接受的條件並傳達給團隊。建議的政策：

- **可接受**：個人功能分支上分享之前的進行中提交。開發者在推送之前會修復 lint 錯誤並重新格式化。
- **可接受**：活躍事件期間每分鐘都至關重要的緊急熱修復提交。開發者開一個後續票據來處理任何被繞過的品質問題。
- **不可接受**：因為太慢而繞過 hooks。這是 hook 配置需要優化的信號，而不是繞過是正確解決方案。
- **不可接受**：因為特定規則令人惱火而繞過 hooks。要麼規則不正確（透過 ESLint 配置處理），要麼規則正確，開發者需要修復違規。

團隊對繞過政策的協議比技術執行更重要。`--no-verify` 無法被停用——它是 Git 的內建旗標。可以執行的是圍繞其使用的文化，以及被繞過的提交在程式碼審查中被標記的期望。
...
### 在 pre-commit 中執行完整測試套件

這是最常見的 hook 錯誤配置。意圖是正確的——確保在提交之前測試通過——但實作使 `pre-commit` 如此緩慢以至於開發者繞過它。

修復方法：將測試移到 `pre-push`。測試每次推送執行一次，而不是每次提交。`pre-push` hook 有更寬裕的延遲預算（30–60 秒是可接受的）。測試可以在那裡執行，不影響提交速度。

如果團隊在提交時想要一些測試覆蓋，只對正在修改的特定檔案執行單元測試。一些團隊配置 lint-staged 執行 `vitest related`（只執行與修改檔案相關的測試）作為折衷方案。這比完整套件快，但仍能捕捉提交程式碼中的回歸。

### 不使用 `prepare` 自動安裝 hooks

需要手動步驟的 hook 設定對某些貢獻者來說將是不完整的。新開發者克隆程式庫，執行 `npm install`，沒有 hooks——因為 README 說要執行 `npx husky`，而他們要麼沒有閱讀那一節，要麼忘記了。

修復方法：`package.json` 中的 `"prepare": "husky"`。沒有文件，沒有手動步驟，沒有出錯的餘地。每次 `npm install` 都安裝 hooks。

一個細節：`prepare` 也在 CI 中執行。在 CI 環境變數中設置 `HUSKY=0` 以在那裡跳過 hook 安裝。CI 有自己的等效檢查；在 CI 中執行 hooks 是多餘的。
