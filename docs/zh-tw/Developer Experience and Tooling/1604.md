...
`@semantic-release/commit-analyzer` 預設使用 `@commitlint/config-conventional` 類型映射。只有 `feat`、`fix` 和 `BREAKING CHANGE` 觸發發布。所有其他類型如果生成了則會包含在變更日誌中，但不觸發版本提升。
...
flowchart TD
    ...
    C -- 是 --> E{scope 已定義且在允許的 scopes 中嗎？}
    E -- 否 --> F[Hook 拒絕：scope 不在列舉中]
    E -- 是 --> G{description 非空且標頭 <= 72 字元嗎？}
    G -- 否 --> H[Hook 拒絕並顯示字元數]
    G -- 是 --> I[提交被接受]
