...
每個團隊認可的規範，在開發者第三次手動建立該類型檔案之前，必須（MUST）被編碼到生成器中。重複是信號：如果開發者已手動建立兩次相同的檔案結構，第三次時生成器必須（MUST）存在。

生成器應該（SHOULD）只編碼最小可行結構——過度鷹架會產生開發者立即刪除的檔案。一個在只需要四個檔案時卻產出十二個檔案的生成器，比沒有生成器更糟糕：開發者學會忽略那些噪音，而生成器作為規範權威的信用度也隨之喪失。

生成器模板必須（MUST）在規範變更的同一個 Pull Request 中更新。反映上一季規範的生成器不是安全網——它是帶著官方工具權威性的過時模式來源。每個更動檔案規範的 PR，必須（MUST）包含對應的模板更新。

`plopfile.js`（或 Hygen 的 `_templates/` 目錄）必須（MUST）提交到版本庫。生成器是專案基礎設施，而非個人工具。它們應該（SHOULD）記錄在專案 README 中，並透過 `package.json` 腳本公開，讓任何開發者都能執行 `npm run generate` 而無需知道底層工具。

AI 輔助程式碼生成應該（SHOULD）被視為對已提交生成器的補充，而非替代。AI 生成看似合理的程式碼；已提交的生成器生成符合慣例的正確程式碼。兩者服務不同目的：AI 適用於新穎或一次性的模式，生成器適用於必須符合專案標準的重複性檔案結構。
...
**Plop** 將所有生成器整合在 `plopfile.js` 中。每個生成器——其名稱、提示和操作——都在一個文件中可見。這使審計變得直觀：一次文件審查就能揭示存在哪些生成器、它們提示什麼、以及它們產出什麼。Plop 使用 Handlebars 作為模板語言，內建的大小寫轉換輔助工具（`properCase`、`camelCase`、`kebabCase`、`dashCase`）可以從單一輸入推導出所有命名變體。Plop v4+ 使用 ESM 格式（`export default function`）。
...
當模板過時時，生成器成為所有規範偏差來源中最危險的一個——它以官方工具的權威性產出過時的模式。執行 `npm run generate` 的開發者信任輸出是正確的。如果元件模板從已廢棄的工具匯入、使用了已移除的 prop 模式，或以早於當前測試設定的方式構建測試檔案，每個生成的檔案都從技術債開始。

症狀是隱晦的：新生成的檔案在第一次提交時觸發 Lint 警告或需要立即編輯。這是生成器的問題，而不是開發者的問題。

解決方法是流程：讓模板更新成為任何規範變更 PR 的必要清單項目。在 PR 模板中新增段落：「如果此 PR 更改了檔案模式（測試結構、元件結構、匯入路徑），請更新對應的生成器模板。」

### 過度鷹架：生成開發者會刪除的檔案

生成十二個檔案但工作流程只需要四個的生成器，教會開發者忽視它。如果生成的四個檔案中有兩個被立即刪除——因為專案不使用 CSS 模組，或者 Story 只為共用 UI 元件而非頁面元件撰寫——生成器就已失去校準。

信號是刪除模式。如果程式碼審查顯示生成的檔案在生成它們的同一個提交中被移除，生成器範圍是錯誤的。審計開發者從生成輸出中實際保留的內容，並將生成器縮減到那個集合。生成完全符合開發者需要的生成器建立信任；產出廢物的則會失去信任。

過度鷹架往往在生成器為理想中的工作流程而非實際工作流程撰寫時發生。生成器作者想要每個元件都有 CSS 模組、桶形匯出和 Story。但團隊實際上使用 CSS-in-JS、沒有桶形匯出，以及只為共用 UI 撰寫 Story。以現實校準生成器，而非以理想。
