...
開發環境中使用的 Node.js 版本必須（MUST）固定為與 CI 和正式環境相同的
版本，並必須（MUST）提交至版本庫，使所有開發者自動使用同一版本。每個專
案的環境變數應該（SHOULD）以 `direnv` 搭配提交至版本庫的 `.envrc` 檔案
進行管理，機密值則從密鑰管理系統取得，而非以明文提交。本地服務應該
（SHOULD）以 Docker Compose 協調，以消除手動安裝步驟，並確保服務版本與
正式環境設定一致。上手流程必須（MUST）可自動化為單一指令或一連串線性步
驟，讓新成員無需尋求協助即可完成設定。
