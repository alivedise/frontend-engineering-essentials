...
格式化是一種協作協議，而非美學偏好。團隊必須（MUST）指定一個具有最終權威的格式化工具，並將其設定提交至程式庫。所有貢獻者必須（MUST）使用相同工具的相同版本進行格式化。格式化決策不應（MUST NOT）出現在程式碼審查的討論中——格式化工具是裁判，其輸出就是正確答案。

格式化工具設定應該（SHOULD）盡量精簡。每新增一個選項，就是團隊集體做出並需要共同維護的一個決策。代表真實團隊偏好的選項（引號風格、分號）屬於設定範圍。沒有團隊共識、只反映個人品味的選項，應該（SHOULD）保留格式化工具的預設值。

編輯器中的儲存時格式化（format-on-save）應該（SHOULD）為每位貢獻者設定。透過 lint-staged 在提交時格式化（format-on-commit）必須（MUST）作為安全網存在。CI 必須（MUST）執行 `prettier --check` 並在任何未格式化的檔案上失敗。這三層形成一個漸進式的執行鏈：立即回饋、提交時關卡、合併時關卡。
...
Biome 是一個基於 Rust 的前端工具鏈，在單一二進位中結合了格式化和 linting。它的格式化器在大多數情況下與 Prettier 相容，並且在基準測試中處理檔案的速度比 Prettier 快約 100 倍。對於大型程式庫，這個差異是實際的：Prettier 可能需要幾秒鐘來格式化所有檔案；Biome 在一秒內完成。
...
從沒有 `prettier.config.js` 開始。Prettier 的預設值經過精心選擇：80 字元每行、雙引號、分號、ES5 尾隨逗號。對許多團隊來說，這些預設值無需修改即可接受。接受預設值意味著零設定面——沒什麼可爭論的、沒什麼需要維護的、環境之間也沒有差異。

當團隊同意某些預設值不適合專案時，只新增那些選項。常見的偏差：

- `singleQuote: true`——在有大量 JSX 的專案中首選，其中屬性中的雙引號與雙引號字串衝突。
- `semi: false`——遵循 ASI 感知風格的團隊首選。
- `trailingComma: "all"`——確保函式參數上的尾隨逗號，在新增參數時產生更乾淨的 diff。
- `printWidth: 100`——適合有寬螢幕和密集 TypeScript 泛型的團隊。

每個新增的選項都是一個決策。只在有真正的團隊共識時新增選項，而不是當某個開發者有個人偏好時。

### ESLint 與 Prettier 的共存

安裝 `eslint-config-prettier` 並將其作為 ESLint 設定陣列中的最後一個項目新增：
...
這一個變更消除了重疊。ESLint 不再對格式化有意見。Prettier 是程式碼樣貌的唯一權威。兩個工具可以按任意順序執行而不會衝突。

永遠不要使用 `eslint-plugin-prettier`。它將 Prettier 作為 lint 規則在 ESLint 內部執行，速度較慢、產生較難閱讀的錯誤輸出，並在輸出中將格式化與正確性混為一談。`eslint-config-prettier` 方法——關閉衝突的 ESLint 規則——是 Prettier 和 ESLint 團隊都推薦的方法。

### CI 格式化檢查

在 CI 中新增一個格式化檢查步驟，在測試之前執行：

```bash
# 檢查：如果任何檔案與 Prettier 的輸出不同則失敗
prettier --check .

# 大型程式庫使用快取（快取按每次 CI 執行儲存，以檔案哈希為鍵）
prettier --check --cache .
```

`--check` 旗標使 Prettier 在任何檔案因格式化而改變時以非零代碼退出。這是一個關卡，而不是自動修復：CI 不應該提交程式碼變更。開發者負責在本機執行格式化工具；CI 驗證他們有這樣做。

`--cache` 旗標儲存每個檔案的哈希值以及 Prettier 的格式化決定。在後續執行中，哈希值未更改的檔案會被跳過。在大型程式庫上，這將格式化檢查時間從幾秒減少到未更改檔案的毫秒級。

### 儲存時格式化與提交時格式化

儲存時格式化在編輯器中設定，提供立即回饋。當開發者儲存檔案時，編輯器執行 Prettier 並在緩衝區寫入磁碟之前套用格式化輸出。開發者立即看到結果——無需等待 CI，無需 pre-commit hook 延遲。

提交時格式化透過 lint-staged 作為 pre-commit hook 的一部分執行。lint-staged 只對已暫存的檔案執行 Prettier，即使在大型程式庫上也很快。這是一個安全網：它捕捉從未透過配置了儲存時格式化的編輯器儲存的檔案（腳本、生成的檔案、透過命令列編輯的檔案）。

兩種機制服務於不同的失敗模式。兩者都要使用。
...
每個專案必須（MUST）提交一個 `.editorconfig` 檔案。最低需要的設定：
...
`end_of_line = lf` 設定在跨平台團隊中最為重要。使用 `core.autocrlf = true` 的 Windows 開發者在某些工作流程中可能仍會產生 CRLF 檔案。EditorConfig 指示編輯器無論 OS 預設值如何都使用 LF，在行結尾污染到達程式庫之前就防止它。

`[*.md]` 區段應該覆蓋 `trim_trailing_whitespace` 為 `false`。Markdown 使用尾隨空格來換行（`  \n` 在渲染的 Markdown 中產生硬換行）。去除它們會改變文件的渲染輸出。
...
`eslint-config-prettier` 停用來自 `@typescript-eslint`、`eslint-plugin-react` 和所有其他處理格式化的外掛的規則。它不需要知道載入了哪些外掛——它無論如何都會停用所有格式化相關的規則。
...
### 使用 `eslint-plugin-prettier` 而非 `eslint-config-prettier`

`eslint-plugin-prettier` 將 Prettier 作為 ESLint 規則執行。每個檔案在 lint 過程中由 Prettier 格式化，檔案當前狀態與 Prettier 輸出之間的任何差異都被報告為 lint 錯誤。其目的是在 ESLint 輸出中顯示格式化問題。

問題是實際的。第一，速度：將 Prettier 作為 lint 規則在每個檔案上執行，比將 Prettier 作為單獨的流程執行更慢。在大型程式庫上，這為每次 lint 執行增加了可測量的時間。第二，錯誤訊息：格式化差異以 ESLint 規則違規的形式出現，規則 ID 如 `prettier/prettier`，產生看起來像邏輯錯誤但實際上是空白 diff 的錯誤訊息。這對不熟悉此設定的開發者來說很令人困惑。第三，概念上：它在單一工具輸出中將格式化與 linting 混為一談，使區分正確性問題與風格問題變得更難。

正確的方法是 `eslint-config-prettier`：在 ESLint 中停用所有格式化規則，單獨執行 Prettier。工具是獨立的，各自的輸出是可獨立閱讀的，效能也更好。

### 沒有將 `eslint-config-prettier` 放在設定的最後

`eslint-config-prettier` 透過停用 ESLint 規則來工作。如果陣列中在它之後出現的設定物件重新啟用任何那些規則，衝突就會重新出現。常見的失敗模式：外掛的 `recommended` 設定物件在 `eslint-config-prettier` 之後展開，而那個外掛包含格式化規則。

修復是機械性的：`eslint-config-prettier` 始終是 ESLint 設定陣列中的最後一個項目。沒有例外。

```js
export default tseslint.config(
  js.configs.recommended,
  tseslint.configs.recommendedTypeChecked,
  reactPlugin.configs.recommended, // 可能包含格式化規則
  // ... 其他外掛設定 ...
  eslintConfigPrettier, // 最後——停用上面所有內容的格式化規則
);
```
...
- [FEE-1603：Git Hooks 與提交前自動化](./1603.md) — lint-staged 在每次提交前對已暫存的檔案執行 `prettier --check`
