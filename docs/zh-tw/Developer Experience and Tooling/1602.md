...
從沒有 `prettier.config.js` 開始。Prettier 的預設值經過精心選擇：80 字元每行、雙引號、分號、所有位置的尾隨逗號（v3 預設的 "all" 設定）。對許多團隊來說，這些預設值無需修改即可接受。接受預設值意味著零設定面——沒什麼可爭論的、沒什麼需要維護的、環境之間也沒有差異。

當團隊同意某些預設值不適合專案時，只新增那些選項。常見的偏差：

- `singleQuote: true`——在有大量 JSX 的專案中首選，其中屬性中的雙引號與雙引號字串衝突。
- `semi: false`——遵循 ASI 感知風格的團隊首選。
- `trailingComma: "es5"`——若程式庫必須支援函式參數尾隨逗號會造成問題的環境，可退回到僅限 ES5 的尾隨逗號（少見；Prettier v3 預設為 `"all"`）。
