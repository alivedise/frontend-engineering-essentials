...
Knip 出現之前，這個領域由三個前輩切割：ts-prune 在 TypeScript 專案中尋找未被引用的 export（Vanderkam，「Finding dead code (and dead types) in TypeScript」，2020）；depcheck 檢視 `package.json` 中未使用或未列出的相依套件；unimported 則找出沒有任何 entry point 能抵達的檔案。每一個都只看到一個切片。這三個工具如今都已封存，連同較新的 tsr 也是如此，作者們紛紛將 Knip 指為後繼者（Knip 文件，「Comparison and migration」）。Knip 自陳的使命是同時找出並修復「未使用的相依套件、export 與檔案」，因為「更少的程式碼與相依套件帶來更佳的效能、更低的維護成本與更容易的重構」（webpro-nl/knip README）。整合之所以重要，在於這四個偵測軸彼此耦合：移除一個未使用的 export 經常會讓某個檔案隨之變成未使用，而那個檔案的消失又會讓某個相依套件變成未使用，單一次的分析就能同時收斂這四個面向。

Knip 填補的缺口是跨檔案的。ESLint 的 `no-unused-vars` 一次只看一個檔案，因此一個沒有其他模組消費的 export，從它自身檔案內部看起來仍像是有在使用。打包工具的 tree-shaker 會在建置時把死碼從編譯後的 bundle 中拋棄，卻不會產生任何報告：某個檔案的最後一個 import 早在數月前就被刪除，它卻仍隨原始碼版本控管與 `package.json` 一起原封不動地出貨，因為沒有任何機制告訴團隊可以安心刪除它。Knip 從宣告的 entry point 走訪整個 import 圖，標記它抵達的每一個檔案、export 與相依套件，藉此補上這個缺口；凡是沒被標記到的，就是死的。Dan Vanderkam 在 Knip 出現之前就推薦用 ts-prune 做同樣的工作，他形容這兩個工具都在執行「同一種 mark-and-sweep 演算法」（Vanderkam，「Use knip to detect dead code and types」，2023）。

Knip 將其問題分類發布為一組規則 key：`files`、`dependencies`、`devDependencies`、`optionalPeerDependencies`、`unlisted`、`binaries`、`unresolved`、`exports`、`types`、`enumMembers`、`namespaceMembers`、`duplicates`、`catalog` 與 `cycles`（Knip 文件，「Rules and filters」）。`catalog` 涵蓋 pnpm、Yarn 與 Bun 的 workspace catalog：也就是宣告在 `pnpm-workspace.yaml`、`.yarnrc.yml` 或 `package.json` 中、集中管理的相依套件版本表。當某個 catalog 項目已定義，卻沒有任何 workspace 透過 `catalog:` 協定引用它時，這項問題就會被觸發，且可自動修復（Knip 文件，「Catalogs」；「Reference: issue types」）。`cycles` 標記循環引用，預設等級是 `warn` 而非 `error`（Knip 文件，「Rules and filters」），因此循環會被印出但不會讓 CI 失敗，除非團隊自行調高等級。另外還有兩個 key，`nsExports` 與 `nsTypes`，涵蓋命名空間層級的 export 與型別，但預設為關閉。這份分類就是設定的表面：每一種都附帶 `error` / `warn` / `off` 的調整旋鈕。本文將說明 Knip 如何組成這個表面、其外掛模型如何自動發現你已在使用的工具，以及設定上的常見地雷藏在哪裡，特別是在 monorepo 裡。
...
**自動修復範圍。** `--fix` 會移除未使用 export、re-export 與 export 型別上的 `export` 關鍵字；移除未使用的 `export default` 關鍵字；剔除未使用的 enum 與 namespace 成員；並從 `package.json` 移除未使用的 `dependencies`、`devDependencies` 與 catalog 項目（Knip 文件，「Auto-fix」）。刪除檔案則是另一項獨立的選用功能：加上 `--allow-remove-files` 才會一併刪除沒有任何東西 import 的檔案。`--fix` **不會**新增未列出的相依套件或執行檔（那需要人類意圖：那次 import 是不小心，還是真的需要？），也不會修復重複的 export。這項不對稱是刻意的：刪除是安全的，新增則需要意圖。

**`--workspace` 語意。** 鎖定某個 workspace 時，也會一併檢查它的祖系 workspace、它所依賴的 workspace，以及依賴它的 workspace。祖系 workspace 可能宣告了目標 workspace 所使用的相依套件，被依賴的 workspace 可能提供目標所仰賴的設定或原始碼，而依賴目標的 workspace 則可能 import 目標的 export（Knip 文件，「Monorepos and workspaces」）。把 `--workspace` 想成「這個 workspace，加上它所依賴的那段圖」，而非嚴格的篩選器。
