...
**必須（MUST）啟用 `strict: true` 以及三個額外的嚴格性旗標：`noUncheckedIndexedAccess`、`exactOptionalPropertyTypes` 和 `noImplicitOverride`。** 這三個旗標不在 `strict` 傘形結構中，但能捕獲重要的錯誤類別。`noUncheckedIndexedAccess` 為所有陣列索引存取和索引簽名查找新增 `| undefined`，強制進行顯式的空值檢查。`exactOptionalPropertyTypes` 區分屬性設為 `undefined` 和屬性不存在，防止一類賦值錯誤。`noImplicitOverride` 要求覆寫基底類別方法的方法上有 `override` 關鍵字，防止基底類別方法被重命名時的靜默失敗。

**必須（MUST）在任何使用僅轉譯建置工具的專案中啟用 `verbatimModuleSyntax: true`。** 這個旗標要求僅類型 import 使用 `import type`，僅類型重新匯出使用 `export type`，給予轉譯器一個明確的信號，以便在不執行模組副作用的情況下剝離類型 import。從舊版 `importsNotUsedAsValues: "error"` 設定遷移的團隊必須（MUST）知道 `verbatimModuleSyntax` 是一個超集，還強制執行對僅類型重新匯出使用 `export type`；遷移是機械性的——執行 `tsc`，修復報告的錯誤，提交。

**必須（MUST）使用 `vite-tsconfig-paths` 同步 `tsconfig.json` 和 Vite 之間的路徑別名，而不是手動重複。** 在 `resolve.alias` 中手動重複別名製造了一個同步危險：在 `tsconfig.json` 中添加別名而不更新 `vite.config.ts` 會產生類型檢查成功和運行時失敗。`vite-tsconfig-paths` 外掛直接從 `tsconfig.json` 讀取別名，消除了這整類錯誤。

**必須（MUST）以三層結構配置 TypeScript：`tsconfig.base.json`、`tsconfig.json` 和 `tsconfig.test.json`。** Base 檔案定義嚴格性、編譯器行為和模組系統設定，不含 `include` 或 `exclude` 項目，並在 monorepo 的所有套件間共享。應用程式設定繼承 base 並新增 `baseUrl`、`paths` 和應用程式源碼的 `include`。測試設定必須（MUST）繼承 base 而非應用程式設定，並在與應用程式路徑隔離的情況下新增測試框架類型和測試檔案模式。

**應該（SHOULD）在有多個 TypeScript 套件的 monorepo 中使用帶有 `composite: true` 和 `declarationMap: true` 的 project references。** Project references 允許 `tsc --build` 使用專案圖只重新建置已更改的套件及其依賴項，並在建置消費端應用程式時呈現函式庫套件中的類型錯誤。`declarationMap: true` 使「跳轉到定義」導航到源文件而不是 `.d.ts` 聲明——在套件與使用它的應用程式並行開發的 monorepo 中，這對開發者體驗至關重要。
