...
**使用 CSS Modules 作為元件作用域樣式的預設選項。** CSS Modules 在來源中為每條規則產生一個未雜湊的類別，但在建構時期產生唯一識別碼，讓你擁有低特異性選擇器（無屬性開銷、無行內樣式特異性）、零執行時期成本和確定性輸出。對任何 React 或框架無關的程式碼庫，SHOULD 將其作為起點。只有當動態主題化需求無法透過 CSS 自訂屬性滿足時，才偏向執行時期 CSS-in-JS；只有在向未知消費者分發元件時，才使用 Shadow DOM。
