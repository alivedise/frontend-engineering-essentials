...
你的團隊正在啟動一個新專案，討論該如何處理樣式。手寫原生 CSS 給予完全控制，但需要從頭建立規範。工具優先的 Tailwind 省去命名決策，讓樣式與標記共置。元件框架 Bootstrap 則提供現成的 UI 模式。開發效率、打包體積與設計彈性之間的取捨，取決於你選擇哪種框架模式。
...
**在撰寫第一個 utility class 之前，先設定 Tailwind 的 content 掃描路徑。** Tailwind 的 purge（v3）或 `content`（v4 的 `@source`）配置決定了哪些檔案會被掃描以偵測使用到的類別名稱。MUST 在正式建構前涵蓋所有檔案類型 -- HTML 模板、JS/TS 元件、`.vue`、`.svelte`、MDX 等。遺漏任何路徑會靜默地從輸出中刪除整組 utility，導致在開發環境正常運作、卻在正式環境失效的樣式問題。
...
**根據設計系統的成熟度選擇框架，而非以流行程度為準。** MUST 在選擇框架前先審視現有的設計約束 -- 色彩調色盤、間距比例、字型比例、動態令牌。若設計系統已完整規範，Tailwind CSS 或 Panda CSS 可精確地編碼這些約束；若令牌仍在演進，Open Props 提供不強加結構的原始 primitives。AVOID 先選框架再讓設計系統去配合其預設值，這會產生大量任意值覆寫，消除框架原本應帶來的一致性保證。
