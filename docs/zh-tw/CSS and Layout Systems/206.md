...
**將設計令牌提取為與框架無關的 CSS 自訂屬性作為單一事實來源。** SHOULD 在純 CSS 層或共用令牌檔案中定義核心令牌（`--color-brand`、`--space-4`、`--font-size-lg`），再由所採用的框架引用這些令牌。此做法將視覺決策與建構工具解耦，讓令牌可被原生 CSS、Web Components 及任何未來框架遷移使用，並使 Figma 到程式碼的同步保持可行。Tailwind v4 的 `@theme` 區塊、Panda CSS 的 `tokens`，以及 Open Props，皆支援引用外部定義的自訂屬性。
