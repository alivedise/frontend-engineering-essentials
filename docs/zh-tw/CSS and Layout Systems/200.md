...
你的團隊已上線一款產品兩年，共用的樣式表已超過 4,000 行且仍在持續增長。修改任何一個選擇器都可能在其他地方引發意外的破壞，新成員需要花好幾天才能搞清楚某個元素究竟套用了哪些規則。這些問題——特異性衝突、全域作用域洩漏、未記錄的繼承關係——並非這個專案獨有。CSS 的設計給了你一套分層工具來應對它們：層疊、特異性、自訂屬性、以及作用域策略，各自解決問題的不同面向。
