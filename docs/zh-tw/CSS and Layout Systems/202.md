...
**錯誤做法：** 保留預設的 `content-box` 尺寸計算，然後在 `width: 100%` 加上 padding 導致元素溢出容器時感到困擾：

```css
.sidebar {
  width: 300px;
  padding: 20px;
  /* 實際渲染寬度：340px -- 溢出 300px 的容器 */
}
```

**修正方式：** 在樣式表頂部套用全域 `border-box` 重置。每個主要框架和重置（normalize.css、Tailwind、Bootstrap）都這樣做：
...
### 3. 用 overflow: hidden 建立 BFC，而 flow-root 更乾淨

**錯誤做法：** 使用 `overflow: hidden` 來包含浮動或防止外距摺疊，然後對內容被裁切感到意外：

```css
.container {
  overflow: hidden; /* 建立 BFC，但裁切溢出的內容 */
}

.container .tooltip {
  position: absolute;
  /* Tooltip 被 overflow: hidden 裁切了！ */
}
```

**修正方式：** 使用 `display: flow-root` 建立無副作用的 BFC：

```css
.container {
  display: flow-root; /* BFC 但不裁切 */
}
```

### 4. z-index 在沒有定位/堆疊上下文的情況下不生效
