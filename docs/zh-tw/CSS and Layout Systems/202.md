...
也要留意無意間建立的堆疊上下文。在祖先元素上添加 `transform`、`opacity < 1` 或 `filter` 會建立堆疊上下文，困住所有後代的 z-index。這是「z-index: 9999 不生效」最常見的原因。完整的觸發條件清單與繪製順序見 [FEE-211 Stacking Context、繪製順序與 Top Layer](./stacking-contexts-paint-order-top-layer.md)。
...
| [FEE-211 Stacking Context、繪製順序與 Top Layer](./stacking-contexts-paint-order-top-layer.md) | 深入展開本文的堆疊模型：stacking context 完整觸發清單、CSS 2 附錄 E 繪製順序，以及取代堆高 z-index 的 top layer。 |
...
- [MDN: display](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Properties/display) -- display 屬性的完整參考，包含雙值外部/內部語法
