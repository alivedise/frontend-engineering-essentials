...
你需要建立一個卡片格線：卡片填滿可用寬度、換行到新列，且無論內容長短，同一列的卡片高度必須相等。Float 需要處理清除問題，inline-block 需要小心處理空白字元。Flexbox 解決了等高與自動換行的需求；Grid 在需要同時控制列與欄的二維排版時更進一步提供明確的放置能力。選擇哪一種，取決於排版是一維還是二維的問題。
