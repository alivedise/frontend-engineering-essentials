...
**錯誤：** 使用綁定視窗寬度的媒體查詢為可重用元件設定樣式：

```css
/* 每張卡片在 768px 視窗時都切換為水平排列，
   即使在窄容器中的卡片也是如此 */
...
卡片無論容器大小如何行為都相同。在 1400px 桌面上的 250px 側欄中，卡片得到「寬」樣式 -- 導致溢位或擁擠的排版。
...
將媒體查詢保留給頁面級排版決策（顯示/隱藏側欄、切換欄位數）。

### 2. 忘記在父元素設定 container-type
