...
工程師應該（SHOULD）將 `backdrop-filter` 限制在任一頁面上同時可見的少數元素。每個
設有 `backdrop-filter` 的元素，都需要瀏覽器在每次重繪時對其下方的圖層進行合成，這
會增加 GPU 負載。在 GPU 資源有限的行動裝置上，過度使用 `backdrop-filter` 會在捲動
與動畫過程中造成掉幀。單一 modal 背景、一個固定的頁首模糊效果，或是小型提示框的模
糊，是典型的可接受用法；將其套用至大型列表中的每一張卡片則不然。

工程師禁止（MUST NOT）僅憑 `mix-blend-mode` 來向使用者傳達意義。混合模式會以可能
使對比度低於 WCAG 無障礙標準的方式改變文字和圖片的視覺呈現，尤其是當背景顏色因內
容在混合元素下方捲動而發生變化時。若 `mix-blend-mode` 被用於文字的裝飾目的，工程師
必須（MUST）針對所有可能的背景值驗證對比度，而不僅僅針對初始狀態下的背景進行檢查。
