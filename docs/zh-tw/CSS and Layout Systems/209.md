...
變化時（無論是文字節點被更新，或是 class 被切換），瀏覽器可能需要遍歷同層元素、
...
context）：浮動元素與邊距都會被限制在元素邊界之內，不會溢出或穿透折疊。元素內部的
任何內容都不會影響外部元素的版面配置位置。瀏覽器可以在此元素的邊界停止版面配置的重
新計算：在被包含子樹內部完成的工作不會向上傳播至文件根節點，也不會橫向影響同層元素。
對於頻繁更新的元件（例如在資料刷新時重新渲染列的資料表格），這意味著瀏覽器只需在元
件的子樹內重新執行版面配置，而無需遍歷文件的其餘部分。
...
自包含的元件通常是版面配置樹中的葉節點。在將包含性與基於格線的版面配置組合使用
時，仍值得留意這一點。
...
這是已廣泛用於限定堆疊內容的慣用模式，因此實際影響通常是中性的。
...
元素進入視窗時，瀏覽器再依需求進行渲染。這與圖片或資源意義上的懶加載不同：不會推遲
任何網路請求。瀏覽器只是跳過計算尚未可見內容的版面配置與繪製工作。
