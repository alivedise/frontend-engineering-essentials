...
工程師應該（SHOULD）將 `contain: layout paint` 應用於作為獨立渲染單元的元件——
輪播器、資料表格、富文字編輯器、無限捲動項目，以及其他內部變化不影響同層版面配置的
小工具。這些包含性提示讓瀏覽器能夠將重新計算的範圍限制在被包含的子樹內。其帶來的效
益與被包含子樹的複雜度及更新頻率成正比；一個靜態且鮮少更新的元件從包含性中獲益甚微，
不加分辨地套用並非有意義的最佳化手段。

工程師必須（MUST）在對任何足夠大的元素使用 `content-visibility: auto` 時，同步設
定 `contain-intrinsic-size`，以避免元素從跳過狀態切換至渲染狀態時造成版面位移。若
未設定 `contain-intrinsic-size`，瀏覽器會將被跳過的元素視為零尺寸；當這些元素在捲
動時被渲染，文件高度就會改變，導致捲動位置跳躍。`contain-intrinsic-size` 的
`auto <length>` 語法會記憶元素最後一次已知的實際尺寸，使佔位尺寸在後續渲染過後保
持準確。
