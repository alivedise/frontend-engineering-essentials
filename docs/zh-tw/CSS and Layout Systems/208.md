...
你正在建立一個卡片排版，每張卡片包含標題、內文和頁腳。在標準格線中，每張卡片的內部元素各自是獨立的 flex 或 block 容器，因此同一排卡片的標題高度不會對齊。子格線讓卡片的子元素參與父層格線的列軌道，不需要 JavaScript 量測或固定高度，便能讓跨卡片的內部元素自動對齊。
...
- MDN：CSS 子格線 — https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Grid_layout/Subgrid
