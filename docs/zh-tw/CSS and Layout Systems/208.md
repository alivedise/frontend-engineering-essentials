...
工程師必須（MUST）在巢狀元素需要讓其子元素對齊父層格線軌道時，使用
`grid-template-rows: subgrid` 或 `grid-template-columns: subgrid`。以固定高度或
JavaScript 量測對齊達到相同效果的做法是脆弱的——內容變動時排版會悄悄損壞，而修復
需要重新執行量測邏輯或在多個地方手動更新寫死的數值。子格線是專為這個問題設計的
語意解法，在任何支援子格線的環境中必須（MUST）優先於各種變通方案。

工程師應該（SHOULD）在子格線對齊是重要使用者體驗考量、且應用程式需支援不具備子
格線能力的瀏覽器時，提供 `@supports (grid-template-rows: subgrid)` 回退機制。
回退排版不必與子格線排版完全一致；一個堆疊的欄式排版，或接受卡片間輕微未對齊的
排版即可。目標是讓所有使用者都能使用功能正常的排版，而非讓所有使用者都獲得像素
完美的對齊效果。回退應該（SHOULD）先以基準規則撰寫，再將子格線增強層疊在
`@supports` 區塊內，確保層疊（cascade）結果可預測，且增強排版無法破壞回退排版。
