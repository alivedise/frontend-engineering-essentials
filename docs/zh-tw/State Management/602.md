...
> **SHOULD**（建議）從既有狀態衍生值，而非同步獨立的狀態變數。

這個原則直接對應**單一真相來源（Single Source of Truth）**規則。當你將衍生值儲存為獨立狀態時，你創造了第二個必須與第一個保持同步的真相來源。每個同步點都是潛在的錯誤。

### 同步稅（The Synchronization Tax）

考慮一個帶有搜尋篩選的產品列表。「同步」方法同時儲存完整列表和篩選後的列表：
...
每次 `products` 或 `searchQuery` 變更，你都必須記得更新 `filteredProducts`。遺漏任何一個更新路徑，UI 就會顯示過時的資料。「衍生」方法完全消除了 State B：
...
### 1. 手動同步狀態而非衍生（Syncing State Manually Instead of Deriving）
...
function addItem(n) {
  const next = [...items, n];
  setItems(next);
  setTotal(next.reduce((a, b) => a + b, 0)); // 必須記得同步
}
...
// 正確 -- 衍生這個值
const [items, setItems] = useState([1, 2, 3]);
const total = items.reduce((a, b) => a + b, 0); // 永遠正確
```

同步版本有一個 bug：`removeItem` 更新了 `items` 但忘記更新 `total`。衍生版本不可能有這個 bug，因為 `total` 在每次 render 都從 `items` 重新計算。

### 2. 昂貴計算未使用 Memoization（Expensive Computation Without Memoization）
