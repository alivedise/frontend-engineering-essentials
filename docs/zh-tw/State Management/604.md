...
這就是**伺服器狀態**的根本挑戰：客戶端永遠不擁有真實來源（source of truth）。它持有的是一個快取（cache）-- 一個可能反映也可能不反映現實的本地副本。問題不在於快取*是否*會變得過時，而在於*多快*變得過時，以及*你怎麼處理*。
...
專用的伺服器狀態函式庫 -- TanStack Query、SWR、Nuxt 的 `useFetch`，以及框架層級的資料載入（Remix loaders、SvelteKit `load`、React Server Components）-- 之所以存在，是因為這類問題與客戶端狀態管理截然不同，值得有專門的工具來處理。

## 原則（Principle）

> **SHOULD NOT**（不建議）將伺服器回應儲存在客戶端狀態管理器（Redux、Zustand、Pinia）中作為真實來源。

伺服器資料屬於由專用函式庫管理的伺服器狀態快取。客戶端狀態管理器是為同步的、客戶端擁有的資料設計的 -- 主題偏好、側邊欄開關、表單草稿。混合兩者會造成維護負擔：你必須為每個端點手動追蹤 loading、error、過時狀態、重新擷取和快取失效。專用的伺服器狀態工具會自動處理這一切。
...
這就是 **stale-while-revalidate** 模式：立即提供快取，然後在背景靜默更新它。使用者立即看到內容，如果有任何變化則在片刻後自行修正。這種方法借鑑了 HTTP 快取語義（RFC 5861），是現代伺服器狀態管理的基礎。
...
- **React Server Components** 在伺服器上執行。它們可以直接 `await` 資料庫查詢或 API 呼叫。結果以已渲染的 HTML 串流傳輸 -- 無需客戶端 fetch、無 loading 狀態、初始載入不需要快取項目。
...
這些方式不是取代 TanStack Query 或 SWR -- 它們是互補的。伺服器渲染的資料處理初始載入；客戶端快取函式庫處理後續互動、mutations 和即時更新。
...
沒有 loading 狀態。不需要用 fetch 失敗的 error boundary（Remix 有路由層級的 error boundaries）。不需要管理客戶端快取。資料隨 HTML 一起到達。在透過 Remix `action` 函式進行 mutation 後，loaders 自動 revalidate。

## 常見錯誤（Common Mistakes）

### 1. 將 API 回應儲存在 Redux/Zustand 作為真實來源
...
將客戶端 store 用於客戶端擁有的狀態（主題、側邊欄、表單草稿）。將 TanStack Query、SWR 或框架層級的資料載入用於伺服器擁有的狀態。

### 2. 沒有 Loading 或 Error 狀態
