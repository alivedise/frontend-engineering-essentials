...
工程師應該（SHOULD）對等冪（idempotent）或容易逆轉且失敗率低的 mutation 套用樂觀更新。按讚
文章、重新排列列表、更新偏好設定或封存項目都是典型的候選情境：mutation 的結果可以預期且邊界
明確、回滾只是直接的狀態還原，且失敗情形少見到回滾路徑幾乎是罕見的邊界情況，而不是例行的使用
者流程。適合使用樂觀更新的關鍵指標包括：mutation 不太可能因應用層面的原因失敗（相對於網路失敗，
後者對所有 mutation 影響相同）、對結果的樂觀假設是可靠的，以及在回滾前短暫不一致的代價較低。

結果無法預期的 mutation 不得（MUST NOT）使用樂觀更新。付款、庫存預留、檔案上傳和權限變更的失敗
率過高，不適合樂觀確認；回滾路徑太混亂，無法乾淨地實作；後果嚴重性也太高，不能冒著向使用者顯示
誤報的風險。工程師必須（MUST）為每一個樂觀更新實作回滾邏輯，並在回滾發生時顯示可見的錯誤通知。
靜默回滾 — UI 直接還原到前一個狀態卻沒有任何說明 — 在使用者眼中就像是一個 bug，並會摧毀對這個
功能的信心。回滾通知必須說明發生了什麼，並在適用時提供恢復路徑，例如重試按鈕或說明頁面連結。
...
### 樂觀寫入前未呼叫 cancelQueries

一個常見的實作錯誤是在未先取消相同快取鍵的進行中查詢的情況下，就將樂觀更新套用到快取。如果
背景重新擷取正在進行，並在樂觀寫入之後才解析，TanStack Query 的預設行為是將過期的伺服器回應
合併進快取 — 以 mutation 前的資料覆蓋樂觀值。結果是可見的閃爍：UI 樂觀地更新、當過期的重新
擷取到達時短暫還原，然後在 `onSettled` 觸發新鮮重新擷取後再次更新。對使用者而言，這看起來像
是雙重更新的 bug。

```js
// 錯誤：缺少 cancelQueries — 過期的重新擷取可能覆蓋樂觀狀態
onMutate: async (postId) => {
  const previous = queryClient.getQueryData(['post', postId]);
...
`await` 是不可省略的。底層取消是非同步的；跳過 await 會留下競態條件，即使呼叫了 `cancelQueries`
也是如此。

### 靜默回滾而不給使用者任何回饋

正確實作回滾 — 在 `onError` 中還原快照 — 只是需求的一半。呼叫
`queryClient.setQueryData(key, context.previous)` 卻什麼也不做的 `onError` handler，會產生
一個靜默地自行還原的 UI。從使用者的角度來看：他們點擊了，UI 改變了，然後它在沒有解釋的情況下
改回去了。這明確地讓人感覺是一個 bug，而不是錯誤處理。它侵蝕信任、導致使用者反覆重試動作，
並產生支援工單。
...
每個樂觀更新的 `onError` handler 都必須顯示說明發生了什麼並為使用者提供前進路徑的通知。
「無法更新 — 請再試一次」是最低限度的可接受訊息。帶有重試操作入口的具體訊息是強烈建議的選擇。

### 對高風險 Mutation 套用樂觀更新

按讚按鈕和付款按鈕不是同一種 mutation，即使兩者都觸發 POST 請求。按讚按鈕的應用層面失敗率
幾乎為零；樂觀確認不需要付出任何代價，並且每次互動節省 150-300ms 的感知延遲。付款按鈕可能
因餘額不足、卡片過期、詐欺旗標和服務降級而失敗；失敗率足夠高，以至於樂觀確認付款會製造有害的
使用者體驗。
...
onMutate: async (paymentData) => {
  // 切勿在伺服器確認之前顯示「付款成功」
  setPaymentStatus('success'); // 使用者在伺服器回應前看到這個
  return {};
},

// 正確：對高風險 mutation 使用載入中狀態
// 顯示 spinner、停用提交按鈕、等待伺服器確認
// 只在伺服器回應後才顯示成功或失敗
```

顯示「付款成功」然後回滾到「付款失敗」，迫使使用者重新評估他們的錢是否已被扣除，損害對應用
程式的信任，並可能需要支援介入。對於高風險或頻繁失敗的 mutation，帶有清晰進度指示器的載入中
狀態是正確的選擇。樂觀更新是高信心、容易逆轉互動的精準工具 — 而不是通用的降低延遲技術。
