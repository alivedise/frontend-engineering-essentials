...
當應用程式狀態較小（每個快照低於數 KB）且已採用不可變狀態管理時，工程師應該（SHOULD）優先使用快照
模型來實作復原與重做。以 Immer 或 Redux 風格的 reducer 管理的不可變狀態，會產生結構共享
（structural sharing）：連續快照之間未變動的子樹是共用的，因此實際記憶體用量遠低於表面數字。
Immer 的 `produceWithPatches` 函式同時產生下一個狀態與反轉變更所需的補丁，結合了兩種模型的優點。
兩個模型都假設只有單一使用者工作階段；多寫入者的協作復原需要 OT 或 CRDT 語義，超出了此原則的範疇。

工程師必須（MUST）將復原歷史限制在有界深度——通常為 50 至 200 步——以避免長時間編輯階段中記憶體
無限增長。沒有深度上限的歷史紀錄，其記憶體消耗會隨工作階段長度線性增加。生產力軟體中，使用者對復原
步數的一般期望落在 20 至 100 步；提供超過 200 步的效益遞減，而記憶體成本持續增加。深度上限的數值
應以具名常數表示——例如 `const MAX_HISTORY = 100`——並在文件中說明選擇這個數值的理由，以便日後
維護人員理解並在必要時調整，而非猜測一個散落在程式碼中的魔法數字的含義。
