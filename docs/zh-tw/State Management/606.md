...
**避免在本地或伺服器狀態中重複 URL 狀態。** 當 URL 是真實來源時，MUST NOT 同時在 `useState` 或 store 中儲存相同的值。描述相同事物的兩個來源最終會產生不一致 -- 最常見的是當使用者點擊返回按鈕時，URL 改變了但元件狀態沒有。讓 URL 擁有該值；元件透過 router hook（`useSearchParams`、`useRoute`）讀取它，並透過導航寫入它。
