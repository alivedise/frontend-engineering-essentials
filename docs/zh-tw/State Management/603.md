...
> **SHOULD NOT**（不建議）使用 context 作為高頻變化狀態的狀態管理器。

Context 是一個**傳輸機制（transport mechanism）**。它解決的是「一個值如何從 A 到達 D 而不經過 B 和 C？」的問題。它不解決「如何有效率地更新狀態並只重新渲染受影響的元件？」的問題。那是狀態管理庫的工作。

當你將快速變化的值（表單輸入、游標位置、動畫幀）放入 React Context 時，該 context 的每個消費者都會在每次變更時重新渲染 -- 不論它們讀取的特定資料片段是否實際改變。這就是**重新渲染全部陷阱（re-render-all trap）**，也是 React Context 最常見的效能錯誤。

Vue 的 `provide`/`inject` 預設就避免了這個陷阱，因為 Vue 的響應式系統在細粒度層級追蹤依賴。但即使在 Vue 中，context 最好還是保留給不常變化的值：設定、服務和橫切關注點。
...
表單狀態在每次按鍵時都會改變。將它放入 React Context 意味著每個消費者在每次按鍵時都重新渲染。改用 local state、表單函式庫（React Hook Form、Formik）或狀態管理庫。

### 2. 不分割 Context

將不相關的值（主題、驗證、語系、feature flags）合併到單一 context 會造成不必要的耦合。當使用者登入時，主題消費者重新渲染。當語系變更時，驗證消費者重新渲染。始終按變更頻率和領域分割 context。
...
沒有 `useMemo`，`ThemeProvider` 的每次渲染都會建立新物件，導致所有消費者重新渲染，即使 `mode` 沒有改變。

### 4. 混淆 Svelte Context（非響應式）與 Vue provide/inject（響應式）
