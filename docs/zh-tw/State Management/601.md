...
### 2. 將衍生值存為狀態（Storing Derived Values in State）

```jsx
// 錯誤 -- 冗餘狀態
const [items, setItems] = useState([1, 2, 3]);
const [total, setTotal] = useState(6); // 從 items 衍生

function addItem(n) {
  const newItems = [...items, n];
  setItems(newItems);
  setTotal(newItems.reduce((a, b) => a + b, 0)); // 必須保持同步
}
...
如果一個值可以從既有狀態計算得出，它就不是狀態 -- 它是衍生值（derivation）。將它另外儲存會產生同步錯誤。在 React 中，在渲染期間計算。在 Vue 中，使用 `computed()`。在 Solid 中，使用衍生 signal。

### 3. 太多個別的 useState 呼叫（Too Many Individual useState Calls）
...
### 5. 過早提升狀態（Lifting State Prematurely）

```
在提升狀態之前，問自己：
1. 目前是否真的有超過一個元件需要這個狀態？
2. 還是我只是「以防萬一」某個未來元件可能需要它而提升？

如果只有一個元件使用這個狀態，就保持本地。
只在第二個消費者出現時才提升。
```

過早提升違反了就近原則（co-location），增加了耦合度，並使父元件和子元件都更難理解。「你不會需要它」（YAGNI）原則適用於狀態放置，就像適用於程式碼一樣。
