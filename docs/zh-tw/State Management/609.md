...
**應該（SHOULD）**對任何有三個或更多欄位、驗證需求或非同步提交的表單使用表單函式庫。驗證和
提交的生命週期——必填欄位、格式驗證、伺服器端錯誤、提交期間的載入狀態、提交後的重新驗證——
其複雜度足以從函式庫一致且經過測試的處理中獲益。在產品的每個表單上手動重新實作這個生命週期，
隨著表單數量增加會引入差異與維護負擔。

**必須（MUST）**在客戶端和伺服器端都進行表單輸入驗證。客戶端驗證為使用者提供即時回饋，但無法
阻止繞過瀏覽器的惡意或格式錯誤的提交。每個接受表單輸入的 API 端點，都必須獨立套用相同的驗證
規則，不依賴客戶端驗證是否已執行。

**禁止（MUST NOT）**僅依賴客戶端表單驗證來保護安全性相關的輸入。在瀏覽器中執行的驗證可以被
任何 HTTP 客戶端繞過。授權檢查、速率限制和資料完整性約束必須在 API 層強制執行，無論客戶端
驗證向使用者傳達了什麼。客戶端驗證是使用者體驗的提升，不是安全性的替代品。
這個區別對所有接受使用者輸入的 API 端點都成立，無論前端是否已實作驗證邏輯。
