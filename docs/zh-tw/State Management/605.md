...
**將不可能的狀態建模為不可能 -- 使用區別聯合（discriminated union），而非鬆散的布林值。** MUST NOT 以獨立的布林旗標建模互斥的模式。四個布林值（`isIdle`、`isLoading`、`isError`、`isSuccess`）產生 16 種可能的組合，其中只有 4 種是合法的。單一的區別聯合（`status: 'idle' | 'loading' | 'error' | 'success'`）精確產生 4 個值。型別系統強制正確性；任何執行期的防守都無法堵住所有 12 種不可能的組合。

**使用守衛條件（guard conditions）限制轉換。** SHOULD 為有前置條件的轉換加入守衛條件：在觸發擷取前驗證 URL 非空、在已經 loading 時防止重複提交。守衛是一個純粹的、同步的函式，回傳 `true` 或 `false`。AVOID 在兩個狀態中定義相同的轉換，當從其中一個狀態直接省略它就能防止無效路徑時。

**將機器定義與元件程式碼分開。** SHOULD 在專用的檔案或模組中定義狀態機，而非內聯在元件內部。在元件外部定義的機器可以被測試、Stately 視覺化工具和多個元件實例匯入。將機器內聯在元件中會將其定義與元件生命週期綁定，並使視覺化圖表無法獨立存取。

**確保每個非最終狀態至少有一個出口轉換。** MUST NOT 讓任何狀態成為死路。每個 error 狀態都需要至少一個 `RETRY` 或 `RESET` 轉換，讓使用者無需重新整理頁面就能恢復。XState 視覺化工具使死路狀態立即可見（沒有出箭頭的節點）-- 在發布前使用它來驗證完整性。
...
- [Stately Editor & Visualizer](https://stately.ai/registry/new) -- 建立、編輯和模擬狀態機的視覺化工具
