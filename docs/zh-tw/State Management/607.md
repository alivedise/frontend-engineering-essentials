...
:::info
狀態管理函式庫是帶有主張的工具。每一個都嵌入了關於狀態應該如何被結構化、更新和觀察的哲學。Redux 將狀態變更視為記錄的事件。MobX 將狀態視為活的試算表。Jotai 由下而上建構狀態，一次一個 atom。選定的範式會決定往後的除錯、測試與重構方式，並跟著程式碼庫存續多年；之後要換掉，代價是改寫 store 邏輯與重新訓練團隊。本文比較七個函式庫，以範式為主，API 表面為輔。
:::

## 背景

這些函式庫要解決的問題，也就是讓多個 view 與共享資料保持一致，早於 React 就已經存在。Knockout 在 2010 年 7 月推出了具備自動依賴追蹤的 observable，Backbone 緊接著在同年 10 月推出了 model 與 collection，讓 view 可以訂閱其變更事件。AngularJS（2010）透過 scope 與可注入的 service 來組織共享狀態。Elm 這個語言於 2012 年推出；從 action 與先前狀態計算出新狀態的 model-view-update 模式源自早期的 Elm 程式，並在 2015 年前後由社群將其正式化。Redux 自己的「Prior Art」文件將這個模式歸功於 Elm，指出 Elm 的 updater「與 Redux 中的 reducer 具有相同的作用」。

自此之後，相關的工作沿著多條平行軌道發展，而非單一時間軸：

- **React。** Flux（2014）引入了單向資料流，Redux（2015）則將其精煉為由純函式 reducer 更新的單一 store。React Hooks（2019）涵蓋了許多先前需要 store 才能處理的情況；Redux 的共同創造者 Dan Abramov 早在 2016 年就寫過「You Might Not Need Redux」，Hooks 讓這個建議對更多應用程式變得可行。Recoil（2020，儲存庫已於 2025 年 1 月封存）和 Jotai 加入了原子化、由下而上的狀態。React Server Components 與 Next.js App Router（2023）將資料擷取移到伺服器端，縮減了留在客戶端的狀態量。
- **Vue。** Vuex（2015）將 Flux 模式改編套用到 Vue 的響應式系統上。Pinia 於 2019 年首次發布，並在 2022 年 Vuex 進入維護模式後成為 Vue 的官方推薦，移除了 mutation 層，改為直接在 action 中寫入。
- **Angular。** Angular 應用程式透過可注入的 service 與 RxJS observable 共享狀態，不需要額外的函式庫。NgRx（首個 `@ngrx/store` 版本發布於 2015 年 12 月）在此之上疊加一個 Redux 風格的 store，供想要明確事件日誌的團隊使用。
- **框架無關。** MobX（2015，最初以 Mobservable 之名發布）提供搭配自動依賴追蹤的可變 observable。XState（2017）將狀態建模為明確的狀態機。Nanostores（2021）提供不到 1KB 的原子化 store，可跨框架運作。Zustand（2019）與 Valtio（2020）皆來自 Poimandres 集體，是以 React 為主、但也能在 React 之外運作的最小化 store，因此同時橫跨 React 與框架無關這兩條軌道。

這些軌道彼此疊加累積，沒有誰取代誰。採用程度一直都是局部的：在任何時間點，多數應用程式都是用框架內建機制或元件本地狀態管理狀態，完全不用這些函式庫。在有使用函式庫的專案中，Redux 依然被廣泛採用（Redux Toolkit 讓它的 API 現代化），MobX 也仍然驅動著大型應用程式。npm 註冊表顯示了這種共存的當前樣貌：截至 2026 年 7 月，Zustand 每月下載量約為 1.76 億次，相較之下 `@reduxjs/toolkit` 約為 9,800 萬次，這是新專案傾向的一個訊號。下載量衡量的是採用程度，而非品質。

跨框架的匯流正在進行中：Solid、Vue、Preact 和 Angular 各自提供了 signal 風格的細粒度響應式原語，TC39 Signals 提案（自 2024 年 4 月起處於 Stage 1）則試圖標準化跨框架共用的核心機制。API 比較與底層追蹤演算法請見 [FEE-611](/zh-tw/State%20Management/framework-signals)；本文聚焦於它之上的 store 層級函式庫。

本文檢視橫跨這些軌道的七個函式庫的設計哲學：Redux、Zustand、Pinia、Jotai、MobX、Nanostores 和 Valtio。目標是幫助你辨認出哪種心智模型適合你的情境。
...
衍生狀態與非同步狀態用的是同一套機制：修改 proxy，每個讀取過該屬性的訂閱者就會重新渲染。這裡沒有 reducer 或 selector，也沒有另一套詞彙用來標記「這次更新恰好是非同步的」。`refreshPrice` 讀起來就像一個普通的 async 函式，因為它本來就是。
...
Jotai 採取與 Redux 相反的做法，捨棄單一的由上而下 store，改用微小且獨立的狀態單元 **atom**，由下而上建構狀態。
...
Nanostores 以縮小 bundle 體積與降低框架綁定為核心目標：
