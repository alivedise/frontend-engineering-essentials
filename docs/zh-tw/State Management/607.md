...
```tsx
// 過度工程 -- 用 Redux 做一個只有 3 個元件的 todo 應用
// configureStore、createSlice、Provider、useSelector、useDispatch
// 用於從未離開一個頁面且沒有複雜更新邏輯的狀態

// 適當 -- local state 或輕量級函式庫
function TodoApp() {
...
Redux 在具有複雜狀態互動、middleware 需求和團隊協調的大型應用程式中才能回收投資。對於只有 local state 的小型應用，它增加了間接層卻沒有帶來好處。正如 Dan Abramov 所寫：「在你遇到 vanilla React 的問題之前，不要使用 Redux。」

### 2. 使用原始 Redux 而非 Redux Toolkit
