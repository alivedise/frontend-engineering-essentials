...
**禁止（MUST NOT）在任何情況下將機密——API 金鑰、token、密碼、憑證——提交進版本控制系統。** git 歷史中的機密應視為已永久洩漏：每一個 clone、每一個 fork 和每一份 git hosting 封存都有一份副本。團隊必須（MUST）在發現後立即輪換任何在歷史中被發現的機密，並在第一次 commit 前將所有 `.env*` 檔案格式加入 `.gitignore`。

**禁止（MUST NOT）將敏感憑證放入 `VITE_*` 或 `NEXT_PUBLIC_*` 環境變數中。** 這些前綴在建置時執行字串替換——該值作為字面字串內嵌進傳送到每個瀏覽器的 JavaScript bundle 中。團隊必須（MUST）將這些前綴限制用於真正公開的值（analytics ID、Stripe publishable key、公開 CDN URL），並必須（MUST）以不帶前綴的環境變數將機密傳遞給 build 步驟，使其僅在伺服器端存在。
