...
有一個細節需要注意：對於由 `main` 推送觸發的 workflow（合併後），取消通常是錯誤的。你希望每個推送到 `main` 的 commit 都被完整測試，並可能觸發部署。正確的模式是：`cancel-in-progress: true` 只應用於 PR 觸發的執行，對 `main` branch 執行省略或設為 `false`。條件式表達式 <code v-pre>cancel-in-progress: ${{ github.ref != 'refs/heads/main' }}</code> 可以在單一 workflow 檔案中處理這種情況。
...
**必須（MUST）以 lockfile hash 作為 `node_modules` 快取的 key，且必須（MUST）在每次快取還原後搭配 `--frozen-lockfile`。** 冷啟動的 `pnpm install` 需要 60–90 秒；快取命中只需 3–5 秒。以 `hashFiles('**/pnpm-lock.yaml')` 作為 key，能從設計上保證快取失效的正確性——任何依賴套件變更時，lockfile 改變，hash 改變，快取自動失效。若未搭配 `--frozen-lockfile`，restore-key 的部分匹配可能靜默地安裝與 lockfile 不符的套件。

**必須（MUST）在快取 key 中包含 `runner.os`。** Node modules 包含因平台而異的二進位檔和 native addon，在 Linux 與 macOS 之間有所不同。在 Linux runner 上建立的快取絕不應（MUST NOT）被 macOS runner 使用，因為為一個作業系統建立的 `node_modules` 無法在另一個上正確運作。請務必將 key 組合為 <code v-pre>node-modules-${{ runner.os }}-...-${{ hashFiles('**/pnpm-lock.yaml') }}</code>。

**必須（MUST）為 PR workflow 設定 `cancel-in-progress: true`，且絕不（MUST NOT）在 main 分支上設定它。** 當開發者推送修正後的 commit 時，必須（MUST）立即取消已被取代的執行，避免浪費 runner 時間和污染 PR 的 status check 列表。然而，取消 `main` 上的執行意味著某些 commit 永遠不會被完整測試，違背了合併後 CI 的目的。在單一 workflow 檔案中使用 <code v-pre>cancel-in-progress: ${{ github.ref != 'refs/heads/main' }}</code> 正確處理兩種情況。

**應該（SHOULD）在所有 CI workflow 中加入 `workflow_dispatch`。** 沒有這個觸發器，就無法在不推送新 commit 的情況下手動對一個分支重新執行 workflow。團隊應該（SHOULD）將它作為除錯 runner 環境問題或驗證修復時的零成本逃生出口。

**應該（SHOULD）固定工具版本，而非使用 `latest`。** 在 `pnpm/action-setup` 或 `actions/setup-node` 中使用 `version: latest` 意味著一次主版本升級可能在你的程式碼毫無改動的情況下靜默地破壞 CI。團隊應該（SHOULD）固定到主版本（`version: 9`）並有意識地升級。

**應該（SHOULD）為所有上傳的 artifact 設定 `retention-days`。** GitHub 預設的 artifact 保留期限為 90 天。PR 建置 artifact 應該（SHOULD）使用 `retention-days: 7` 以避免消耗過多儲存配額。可能需要部署後除錯的 release artifact 應使用更長的期限（如 30 天），但務必明確設定一個值，而非依賴平台預設值。
