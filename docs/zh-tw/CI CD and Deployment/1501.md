...
**必須（MUST）在快取 key 中包含 `runner.os`。** Node modules 包含因平台而異的二進位檔和 native addon，在 Linux 與 macOS 之間有所不同。在 Linux runner 上建立的快取絕不應（MUST NOT）被 macOS runner 使用，因為為一個作業系統建立的 `node_modules` 無法在另一個上正確運作。請務必將 key 組合為 `node-modules-${{ runner.os }}-...-${{ hashFiles('**/pnpm-lock.yaml') }}`。

**必須（MUST）為 PR workflow 設定 `cancel-in-progress: true`，且絕不（MUST NOT）在 main 分支上設定它。** 當開發者推送修正後的 commit 時，應該（SHOULD）立即取消已被取代的執行，避免浪費 runner 時間和污染 PR 的 status check 列表。然而，取消 `main` 上的執行意味著某些 commit 永遠不會被完整測試，違背了合併後 CI 的目的。在單一 workflow 檔案中使用 `cancel-in-progress: ${{ github.ref != 'refs/heads/main' }}` 正確處理兩種情況。
