...
> **SHOULD** 在測試之前執行 lint 和 type-check——fail-fast 順序優先暴露低成本訊號，避免在未通過靜態分析的程式碼上浪費昂貴的 runner 分鐘數。

> **SHOULD** 在 CI 中將 `tsc --noEmit` 作為獨立步驟，與 build 步驟分開——esbuild 和 SWC 等現代 bundler 在 transpile 時不進行型別檢查，這意味著構建成功並不代表型別正確。
...
**在 lint 和 type-check 之前執行測試。** 測試的成本遠高於靜態分析。在循序 pipeline 中將測試放在首位，意味著開發者要等待 2 分鐘才能發現一個 15 秒的 job 本可立即捕捉到的 lint 錯誤。

**假設構建成功就意味著型別正確。** Vite、Next.js（使用 SWC 時）以及基於 esbuild 的設置都在 transpile TypeScript 時不執行型別檢查器。從 CI 中移除 `tsc --noEmit` 會讓專案失去型別安全關卡。

**設定過高的覆蓋率閾值並將其作為關卡。** 80% 的硬性閾值會激勵開發者編寫能觸碰程式碼行但不驗證行為的低價值測試。以覆蓋率百分比設定關卡的團隊，往往最終擁有比不設任何閾值的團隊更差的測試套件——因為硬性底線創造了錯誤的優化目標。
...
**不上傳 build artifact。** 如果 build job 成功但沒有上傳 artifact，下游 job（size-limit、部署預覽）必須從原始碼重新構建。將 `dist/` 目錄作為 GitHub Actions artifact 上傳，並在下游 job 中恢復，能在每次執行時節省完整的 build 時間。

**在 CI 安裝時跳過 `--frozen-lockfile`。** 沒有 `--frozen-lockfile`（pnpm）或 `--ci`（npm），package manager 可能以與 lockfile 指定不同的方式解析依賴範圍。這會產生非確定性的安裝——CI 環境可能擁有與開發者機器不同的 package 版本。在 CI 上始終傳入 `--frozen-lockfile`。

**在 affected 指令中不設定 `fetch-depth: 0`。** Nx 和 Turborepo 需要完整的 git 歷史記錄來計算當前分支與基礎分支之間的差異。如果 `actions/checkout` 中沒有 `fetch-depth: 0`，repository 是淺克隆（shallow clone），基礎 commit 可能不存在，導致 affected 計算回退到執行所有內容。
