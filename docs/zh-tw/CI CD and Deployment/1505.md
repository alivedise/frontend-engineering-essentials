...
**應該（SHOULD）確保在伺服器端渲染的應用程式中，伺服器端和客戶端的旗標評估結果一致。** 若伺服器以旗標開啟的狀態渲染，但客戶端在 mount 時重新評估旗標並產生關閉的狀態，使用者會看到錯誤內容的閃爍。團隊應該（SHOULD）將旗標評估上下文從伺服器正確 hydrate 到客戶端，確保兩次渲染產生相同的結果。
