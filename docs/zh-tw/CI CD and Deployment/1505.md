...
**應該（SHOULD）對高風險功能使用 feature flag，而非直接部署給所有使用者。** 旗標讓功能能在數秒內停用而無需任何部署，提供了回滾策略在速度上無法匹敵的安全網。團隊應該（SHOULD）將高風險變更合併在旗標後面、先對內部使用者在正式環境中驗證，並在達到 100% 之前以漸進方式擴大曝光。

**必須（MUST）在建立 feature flag 時設定移除日期，並將其視為一等公民的工程任務。** 超過其用途仍留在程式碼中的旗標，會成為永久性的條件分支，阻礙重構並讓新工程師感到困惑。團隊必須（MUST）在建立時排定旗標移除的時程，並在 CI 中透過債務檢查腳本強制執行——當旗標移除日期已過時，讓 build 失敗。

**必須（MUST）在為前端實作金絲雀流量分流時設定 session affinity。** 被路由到金絲雀版本 `index.html` 的使用者，必須（MUST）從相同版本取得所有後續資源——JavaScript、CSS。混合版本會破壞應用程式，因為資源 URL 和 API 合約在不同部署之間不可互換。使用 Cloudflare Load Balancing、Nginx upstream weights 或 edge middleware 進行金絲雀路由時，務必透過 cookie 或 header 設定黏性 session。

**應該（SHOULD）在評估 feature flag 時始終定義安全的預設值。** Feature flag SDK 從遠端服務取得定義。若該服務不可用，SDK 應該（SHOULD）回退到安全的預設值，而非拋出讓應用程式 crash 的例外。大多數 SDK 接受預設值作為評估呼叫的第二個參數——請務必明確提供它。

**應該（SHOULD）在伺服器端渲染的應用程式中，將旗標評估上下文從伺服器 hydrate 到客戶端，以防止版本不一致的閃爍問題。** 若伺服器以旗標開啟的狀態渲染，但客戶端在 mount 時重新評估旗標並產生關閉的狀態，使用者會看到錯誤內容的閃爍。團隊應該（SHOULD）將旗標評估上下文從伺服器正確 hydrate 到客戶端，確保兩次渲染產生相同的結果。

**應該（SHOULD）在藍綠部署涉及資料庫 schema 變更時使用 expand-contract migration 模式。** 藍綠要求新舊版本應用程式能夠同時在相同的資料庫上運作。移除舊版本正在讀取的欄位的 migration 會破壞回滾能力。團隊應該（SHOULD）先新增欄位（expand）、部署新版應用程式，然後只有在舊版本完全退役後才移除舊欄位（contract）。
...
- Unleash Documentation — [Activation Strategies](https://docs.getunleash.io/concepts/activation-strategies)：開源 Unleash 旗標平台中的漸進式推出、使用者 ID 定向和自訂策略
- Vercel Documentation — [Deployments](https://vercel.com/docs/deployments)：不可變部署產出物與 `vercel promote` 指令，用於藍綠正式環境切換
