...
回滾決策的時效性與技術可行性同等重要。許多組織能夠在技術上完成回滾，但
在決策過程中耗費過多時間。清晰的升級路徑——誰有權授權回滾、誰負責執行、
誰負責對外溝通——應在事故發生前就已確立，並記錄於事故回應手冊中。在事
故壓力下臨時討論授權問題會延誤回滾，進而讓更多使用者受到影響。五分鐘內
完成回滾決策的目標，不僅需要快速的技術工具，也需要清晰的組織決策流程。
...
**必須（MUST）**確保每次生產部署在不重新執行建構管線的情況下可回滾。CDN 指標切換模式——保留前次部署資源並替換 HTML 入口點——滿足此要求，且可將回滾時間縮短至 60 秒以內。

**應該（SHOULD）**在確認生產事故後五分鐘內做出回滾決策，而非在長時間診斷之後。當錯誤率峰值與部署事件相關聯時，恢復服務的最快路徑是回滾；更深入的根本原因分析屬於事後分析（post-mortem），不得在事故應對期間進行。
...
- [Sentry：版本追蹤](https://docs.sentry.io/product/releases/)
- [AWS S3：版本控制](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Versioning.html)
