...
選擇主要來源區域需要同時衡量三個因素：距離最大使用者族群的物理距離、距
離來源資料庫的網路距離，以及法律資料主權要求。以歐洲使用者為主的 B2B
SaaS 應用程式，通常選擇法蘭克福或都柏林作為主要來源區域——這兩個位置同
時滿足低延遲和 EEA 資料主權要求。當應用程式必須服務多個大洲市場時，增
加第二來源區域的效益，取決於必須到達來源的流量比例。若 CDN 靜態資源快
取命中率已超過 95%，增加第二來源區域主要改善的是 HTML 文件和 API 呼叫
延遲，相對於運營複雜度的邊際效益必須明確評估，而非預設認為多區域架構一
定優於單一區域。
...
**禁止（MUST NOT）** 在邊緣函式中處理或儲存受地理分區限制的個人資料，除非該邊緣節點位於適用司法管轄區所要求的地區內。受資料駐留要求約束的資料，不得在不符合規定的邊緣節點上進行處理或儲存，必須專門路由至適用區域內的邊緣節點或代理至符合合規要求的來源區域；無論資料來自何處，在不符合規定的邊緣位置進行處理均違反適用的資料駐留法規。
...
- [Vercel：邊緣網路基礎設施](https://vercel.com/docs/edge-network/overview)
