...
CDN 清除不是瞬間完成的操作。Cloudflare、Fastly 和 CloudFront 的清除傳播通常在數秒至數分鐘之間完成，確切時間取決於邊緣節點數量、清除請求的路徑模式，以及各 CDN 供應商的內部基礎設施狀態。在此傳播窗口期間，不同地理位置的使用者可能收到不同版本的 HTML 入口點，進而引用不同版本的 JavaScript 和 CSS 資源。對於多數應用程式而言，這個數秒到數分鐘的窗口是可以接受的。但對於金融交易、即時協作或有嚴格 API 版本相依性的應用程式，則需要設計機制，讓仍在執行舊版本的客戶端能夠優雅地降級或自動重新載入，而非靜默地產生執行時錯誤。版本感知的 API 設計——透過 URL 版本號或請求標頭協商 API 版本——可以在 CDN 清除傳播期間提供額外的緩衝。

基於標籤的失效策略需要在部署管線中提前規劃。CDN 標籤必須在快取資源時設定，通常透過
`Cache-Tag` 或 `Surrogate-Key` 回應標頭傳遞。這意味著應用程式的後端必須在每個 API 回應
或 HTML 文件上主動輸出標籤，而非事後補充。將標籤邏輯嵌入中介軟體或 API gateway 層，
可以確保標籤一致地套用於所有相關回應，而無需每個 API 端點各自處理。缺乏一致標籤的應
用程式最終只能依賴 URL 模式清除，在 URL 數量龐大的應用程式中這是不可擴展的方式。
...
**禁止（MUST NOT）依賴使用者清除瀏覽器快取作為失效失敗的變通方案。** 快取管理是基礎設施責任；指示使用者清除快取，表明部署管線中存在缺失或損壞的清除步驟，不得以此代替正確的結構性修復。
...
- [MDN：Cache-Control](https://developer.mozilla.org/zh-TW/docs/Web/HTTP/Reference/Headers/Cache-Control)
...
- [Fastly：即時清除](https://www.fastly.com/documentation/reference/api/purging/)
- [Google Cloud CDN：快取失效](https://docs.cloud.google.com/cdn/docs/cache-invalidation-overview)
