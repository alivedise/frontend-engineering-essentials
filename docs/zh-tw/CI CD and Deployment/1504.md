...
> **SHOULD** 對 HTML 進入點（例如 `index.html`）設定 `Cache-Control: no-cache`，讓瀏覽器在提供快取副本之前，必須先向 origin 重新驗證。這確保使用者始終能取得最新的部署版本。

> **SHOULD** 對含有內容雜湊（content hash）的靜態資源（JavaScript bundle、CSS 檔案、檔名包含 hash 的圖片）設定 `Cache-Control: max-age=31536000, immutable`，使瀏覽器與 CDN edge node 能夠無限期快取這些資源而無需重新驗證。Content hash 保證了唯一性：若內容改變，檔名也會改變，舊的快取項目就再也不會被請求。

> **SHOULD** 使用提供 atomic deployment 保證的平台 — 部署中的所有檔案同時被推進，讓使用者不會遇到某些請求來自舊部署、某些來自新部署的狀態。
...
**對 `index.html` 設定積極的長期快取。**
對 `index.html` 設定 `max-age=31536000` 意味著擁有快取副本的使用者，即使快取過期前（最長一年）都不會看到新的部署。請對 HTML 進入點始終使用 `no-cache`。

**靜態資源沒有使用 content hash。**
若檔名不含 content hash，就無法安全地對資源使用長期 TTL。若你對 `main.js`（不含 hash）設定 `max-age=31536000`，更新 `main.js` 並不會改變 URL，快取了舊版檔案的使用者永遠不會取得新版本。請使用能啟用 content hash 的 bundler（Vite 預設啟用；請確認沒有被關閉）。

**在 S3 上以錯誤的順序上傳檔案。**
若你在帶有 hash 的資源就位之前先上傳 `index.html`，就會有一段時間讓使用者收到新版 HTML 但無法取得新版 JavaScript（因為帶有新 hash 的 bundle 尚不存在於 S3）。請始終先上傳帶有 hash 的資源，最後才上傳 `index.html`。
...
**部署新的 `index.html` 後忘記 invalidate CloudFront。**
CloudFront 會在 edge 快取回應。若你將新的 `index.html` 上傳至 S3 但沒有 invalidate CloudFront edge 快取，使用者會持續收到快取的舊版本，直到 TTL 過期。每次 S3+CloudFront 部署都必須對 `index.html` 執行 invalidation。
