...
**應該（SHOULD）對所有 commit 訊息使用 Conventional Commits 格式。** `type(scope): description` 結構是自動化版本計算的機器可讀輸入。工具必須（MUST）能夠解析每個 commit 的語義意圖——`feat`、`fix` 或帶有 `BREAKING CHANGE` footer——才能確定性地計算下一個版本號，並生成有意義的 CHANGELOG。

**應該（SHOULD）自動化版本號升級與 CHANGELOG 生成，而非手動進行。** 單一套件儲存庫和應用程式使用 `semantic-release`；monorepo 或有多個獨立版本套件的函式庫使用 `@changesets/cli`。這兩種工具都必須（MUST）配置為在 CI 中運行，以確保版本控制的一致性與可稽核性。

**禁止（MUST NOT）在自動化工具就位後手動編輯 `package.json` 中的版本欄位或手寫 CHANGELOG 條目。** 手動編輯將被工具覆蓋，並造成令人困惑的衝突。自動化工具應該（SHOULD）是版本和 CHANGELOG 的唯一來源。

**必須（MUST）只從 CI 發布套件，絕不從開發者本地機器發布。** CI 發布的套件具有可重現性、可稽核性，且不受開發者環境差異的影響。個人 npm 發布權限應該（SHOULD）被撤銷，要求所有發布都通過 CI workflow 進行。
