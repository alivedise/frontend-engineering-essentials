...
`semantic-release` 分析這些結構化訊息的清單，並套用確定性規則集：任何 type 為 `feat` 的 commit 觸發 minor 版本升級；任何 type 為 `fix`、`perf` 或 `refactor` 的 commit 觸發 patch 升級；任何帶有 `BREAKING CHANGE:` footer 或 type 後有 `!` 的 commit 觸發 major 升級。自上次發布以來所有 commit 中，適用的最高等級升級決定新版本號。沒有任何歧義，沒有委員會決策，沒有「這應該是 1.5 還是 2.0？」的爭論。commit 訊息在變更當下就已做出決定。
...
flowchart TD
    ...
    I --> J["@semantic-release/git\n將更新後的 CHANGELOG.md\n和 package.json commit 回 main"]
    J --> K["@semantic-release/github\n建立 git tag v1.4.0\n建立附有發布說明的 GitHub Release"]
    K --> L["套件以 @myorg/pkg@1.4.0\n在 npm 上發布"]
    K --> M["GitHub Release 顯示\n變更日誌與發布資產"]
...
`changesets/action` bot 維護一個名為「chore: version packages」的單一開放 PR，它會累積所有待處理的 changeset。PR body 顯示每個套件將升級多少版本，以及每個 CHANGELOG 中會出現什麼內容。當這個 PR 被合併後，action 執行 `pnpm changeset publish`，它會：

1. 執行 `pnpm changeset version`——升級 `package.json` 版本並寫入 CHANGELOG 條目
2. 將所有具有新版本的套件發布到 npm
3. 為每個發布的套件建立 git tag（例如 `@myorg/button@2.1.0`）
4. 為每個 tag 建立 GitHub Release
