...
state: draft
slug: release-automation-semantic-versioning-changelogs
...
它在整個團隊中強制執行紀律：每一個有意義的變更都被分類，分類結果驅動版本號，而版本號向使用者傳達相容性保證。整合你函式庫的下游團隊可以清楚知道 `2.0.0` 代表什麼（破壞性變更，需閱讀遷移指南），以及 `1.4.2` 代表什麼（僅 patch 修復，可安全升級）。
...
`semantic-release` 分析這些結構化訊息的清單，並套用確定性規則集：任何 type 為 `feat` 的 commit 觸發 minor 版本升級；任何 type 為 `fix` 或 `perf` 的 commit 觸發 patch 升級（注意：`refactor` 觸發 patch 版本是上方 `.releaserc.json` 中的自訂規則，非 semantic-release 預設行為）；任何帶有 `BREAKING CHANGE:` footer 或 type 後有 `!` 的 commit 觸發 major 升級。自上次發布以來所有 commit 中，適用的最高等級升級決定新版本號。沒有委員會決策，也沒有事後才出現的「這次發布該是 1.5 還是 2.0？」爭論：commit 訊息在變更當下就已做出決定。
...
在一個擁有 `@myorg/button`、`@myorg/input` 和 `@myorg/theme` 套件的 monorepo 中，像 `feat(button): add loading state` 這樣的單一 commit 影響 `@myorg/button`，但對 `@myorg/input` 或 `@myorg/theme` 沒有任何影響。`semantic-release` 針對單一發布分支分析整個 commit 日誌，它沒有內建「哪些 commit 影響哪個套件」的概念，這使得它在缺乏大量客製化的情況下並不適合 monorepo。
...
當發布時機到來，`pnpm changeset version` 消耗所有待處理的 changeset 檔案，按最高宣告嚴重性升級每個套件，並用 changeset 檔案中的描述更新每個套件的 CHANGELOG。`.changeset/*.md` 檔案隨後被刪除。後續的 `pnpm changeset publish` 發布所有具有新版本的套件。Changesets GitHub Action（`changesets/action`）在 `main` 分支上維護一個開放的 PR，顯示待處理的版本升級：「Version Packages」PR。合併這個 PR 就是發布。
...
自動化 CHANGELOG 生成使這一切無需額外成本。commit 訊息（或 changeset 描述）已經寫好了；工具將它們組裝成格式化文件並 commit 結果。這省去了手動「記得更新 changelog」的步驟。因為 changelog 在每次發布時都會重新生成，它不會產生偏差，也不會有未附說明文件就發布的版本。
...
**應該（SHOULD）對所有 commit 訊息使用 Conventional Commits 格式。** `type(scope): description` 結構是自動化版本計算的機器可讀輸入。工具應該（SHOULD）能夠解析每個 commit 的語義意圖（`feat`、`fix`，或帶有 `BREAKING CHANGE` footer），才能確定性地計算下一個版本號，並生成有意義的 CHANGELOG。

**應該（SHOULD）自動化版本號升級與 CHANGELOG 生成，而非手動進行。** 單一套件儲存庫和應用程式使用 `semantic-release`；monorepo 或有多個獨立版本套件的函式庫使用 `@changesets/cli`。這兩種工具都應該（SHOULD）配置為在 CI 中運行，以確保版本控制的一致性與可稽核性。

**禁止（MUST NOT）在自動化工具就位後手動編輯 `package.json` 中的版本欄位或手寫 CHANGELOG 條目。** 手動編輯將被工具覆蓋，並造成令人困惑的衝突。自動化工具必須（MUST）是版本和 CHANGELOG 的唯一來源。

**必須（MUST）只從 CI 發布套件，絕不從開發者本地機器發布。** CI 發布的套件具有可重現性、可稽核性，且不受開發者環境差異的影響。個人 npm 發布權限必須（MUST）被撤銷，要求所有發布都通過 CI workflow 進行。

**必須（MUST）在執行發布工具的 workflow 中設定 `fetch-depth: 0`。** `semantic-release` 和 changesets 都需要讀取完整的 git tag 歷史，以確定自上次發布以來有什麼變更。使用預設的淺層 clone（`fetch-depth: 1`）將導致工具無法看到前次 release tag，從而計算出不正確的版本。

**應該（SHOULD）在 release commit 訊息中加入 `[skip ci]` 標記。** `@semantic-release/git` 會將一個 commit 推送回預設分支；若沒有 `[skip ci]`，release workflow 將在 release commit 上再次觸發，浪費 CI 時間。每次自動化工具生成 release commit 時都應該（SHOULD）包含此標記。

**應該（SHOULD）對所有版本化套件和函式庫優先採用 publish-on-tag 而非 publish-on-merge。** 明確的 git tag 代表刻意的發布意圖；在每次合併到 `main` 時發布，會讓每個 commit 都成為潛在的發布，並消除在發布前批次累積變更的能力。Publish-on-tag 同時建立穩定的稽核軌跡，使每個 npm 版本都對應至精確的 git tag 與 GitHub Release。
