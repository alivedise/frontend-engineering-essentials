...
> **SHOULD** 對所有影響 UI 的 PR 使用 preview deployment — 利害關係人、設計師和 QA 應能在真實 URL 上審查真實的 build，而非截圖或本機設置說明。

> **SHOULD NOT** 在前端程式碼中硬編碼 API base URL — 使用環境變數，使 preview deployment 能在不修改原始碼的情況下指向正確的 API endpoint。
...
**硬編碼 API base URL。** Preview deployment 中最常見的配置錯誤。如果 `VITE_API_URL` 不是環境變數，每個 preview 環境都會呼叫相同的硬編碼 endpoint——通常是 production。這意味著測試 API 合約變更的 preview 將針對舊合約通過，review 將無法捕捉到 regression。

**後續推送時未更新 PR 留言。** 簡單的 GitHub Actions bot 每次推送都建立新留言。經過 10 次 force-push 的 PR 後，留言串充斥著 10 個 preview URL，其中只有一個是最新的。始終檢查現有 bot 留言並就地更新。
...
**忘記銷毀 preview 環境。** 在自行託管的設置中，preview 環境不會自動清理。如果部署在沒有清理 workflow 的情況下累積，託管成本和命名空間污染會隨時間增長。加入一個在 PR 關閉時觸發的 workflow 來刪除部署：
...
jobs:
  cleanup:
    runs-on: ubuntu-latest
    steps:
      - name: 刪除 preview deployment
        run: |
          # 範例：透過 branch 名稱刪除 Cloudflare Pages deployment
          curl -X DELETE \
...
**未在 preview URL 上停用搜尋引擎索引。** Preview deployment 可透過 URL 公開存取。如果不添加 `noindex` header 或 `robots.txt` disallow，搜尋引擎會對其進行索引。這會用開發版本污染搜尋結果。為所有 preview deployment 添加 `X-Robots-Tag: noindex` 回應 header。
...
**在 preview 環境中使用 secrets 而未限定範圍。** Preview deployment 由 pull request 觸發，包括來自 fork 的 PR。GitHub 預設不向 fork PR 觸發的 workflow 暴露 secrets——這是一個安全特性。如果你的 preview workflow 需要 secrets（API token、服務憑證），請確保使用帶有適當預防措施的 `pull_request_target`，或使用帶有所需審查者的環境層級 secrets 來控制存取。
...
- [FEE-1504 Production 部署策略](1504.md)
