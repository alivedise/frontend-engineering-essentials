...
- SHOULD 使用 WebP 作為預設圖片格式以獲得廣泛的瀏覽器相容性；SHOULD 在建置管線或 CDN 支援格式協商的情況下，提供 AVIF 作為下一代替代方案。
- SHOULD 在每個 `<img>` 上使用 `srcset` 和 `sizes`，為使用者的視窗和顯示密度提供適當大小的圖片。
- SHOULD 為所有折疊線以下的圖片加上 `loading="lazy"`，以延遲它們的網路請求，直到使用者滾動至附近。
- MUST 為 LCP 圖片（初始視窗中最大的圖片）加上 `fetchpriority="high"`，確保瀏覽器以最高優先級獲取它。
- MUST 為每張圖片加上描述性的 `alt` 文字，以符合無障礙性和 SEO 要求。
- SHOULD 為每張圖片設定明確的 `width` 和 `height`（或在 CSS 中使用 `aspect-ratio`），以防止圖片載入期間的 Cumulative Layout Shift。
- SHOULD 在框架專案中使用框架圖片元件（Next.js `<Image>`、Nuxt `<NuxtImg>`、Astro `<Image>`）——它們預設強制執行正確行為。
...
- **在 WebP 足夠的情況下仍使用 JPEG 或 PNG。** WebP 獲得了普遍的瀏覽器支援，在同等品質下可減少 25--40% 的檔案大小。在 2024 年及以後，對於新的圖片上傳，除非舊版系統要求，否則沒有理由預設使用 JPEG。

- **懶加載 LCP 圖片。** LCP 圖片根據定義在初始視窗中。為其加上 `loading="lazy"` 會延遲獲取，直到瀏覽器解析了足夠多的頁面來確定該圖片在視窗中——這會顯著增加 LCP。LCP 圖片應具有 `fetchpriority="high"` 且無 `loading` 屬性（或 `loading="eager"`）。

- **不為圖片添加明確的 width 和 height。** 沒有尺寸，瀏覽器在版面中不為圖片分配空間。圖片載入時，周圍的內容移動，產生 CLS。設定 `width` 和 `height`（匹配固有大小或顯示的長寬比）讓瀏覽器立即預留正確的空間。
...
- **在 picture 元素或 CDN 中跳過格式協商。** 生成 WebP 但不生成 AVIF（或反之）會錯失壓縮效益。如果 CDN 或建置管線支援 AVIF 編碼，始終提供 AVIF 作為第一個 source，WebP 作為後備。
