...
核心洞察是：這些策略構成的是一套工具箱，而非排名。架構良好的應用程式通常同時使用多種策略。一個 Next.js 電商網站可能首頁使用 SSG、產品頁面使用 ISR、搜尋結果使用 SSR + Streaming、購物車使用 CSR，全部在同一個程式碼庫中。

## 最佳實踐

**根據內容動態性和 SEO 需求選擇渲染策略。** 絕對不應在未進行充分分析的情況下，預設將每個路由設定為 SSR 或 CSR。對於很少變更的靜態內容，SHOULD 使用 SSG 以最大化傳遞速度並最小化基礎設施成本。含有每位使用者個人化資料或即時資料的路由，MUST 使用 SSR 或 CSR，因為預先渲染無法同時為匿名訪客和已登入使用者產生正確的輸出。

**使用串流式 SSR 改善資料相依頁面的 TTFB。** 當一個頁面有多個獨立且延遲不同的資料來源時，傳統 SSR 會等待最慢的查詢才回應整個頁面。SHOULD 將慢速資料取得的元件包裹在 `<Suspense>` 邊界中，讓伺服器能立即輸出頁面外殼和首屏內容，並在慢速區段解析完成時串流進來。這在不增加基礎設施成本的情況下，降低了使用者感知的載入時間。

**對無快取策略的個人化內容避免使用全頁面 SSR。** 對每個請求都渲染個人化 HTML 且沒有快取，在規模上的擴展性很差，並會增加與資料取得時間成正比的 TTFB 延遲。SHOULD 在建構完全動態的 SSR 之前，先評估使用 cache-key 變化（依使用者層級、地區或分群）的邊緣快取方案。對於個人化但非即時的內容，SSR 搭配短效快取或 stale-while-revalidate 模式，優於完全不快取的逐請求渲染。
...
設定 `revalidate: 3600` 後就不再理會，代表使用者可能看到最多一小時前的內容。對時間敏感的內容（活動頁面、限時特賣），這可能造成損害。修正方式：對關鍵內容使用按需重新驗證（webhook 觸發），僅對可以容忍過期的內容使用基於時間的重新驗證。

### 2. 伺服器/客戶端分歧導致的 Hydration 不匹配
...
- [Rendering: Server Components -- Next.js Docs](https://nextjs.org/docs/app/getting-started/server-and-client-components) -- Next.js 關於 React Server Components、串流與 App Router 渲染模型的文件
