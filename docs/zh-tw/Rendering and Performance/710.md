...
工程師**應該（SHOULD）**在 CSS 動畫與過渡中，僅對 `transform` 與 `opacity` 屬性製作動畫。
這是瀏覽器合成執行緒唯一能在不涉及主執行緒、不觸發繪製的情況下以 GPU 處理的兩個屬性。
對 `width`、`top`、`background-color` 或 `box-shadow` 製作動畫，
會在每個動畫影格觸發版面配置與繪製，執行於主執行緒且受 JavaScript 阻塞。
在 60fps 的播放速率下，每個影格的時間預算為 16.7ms；
在任何具有非零散 DOM 結構的頁面上，每個影格都執行版面配置與繪製，
可能消耗大部分預算，使 JavaScript 沒有足夠時間執行而導致掉幀。
若設計方案要求對觸發版面配置的屬性製作動畫，
應優先與設計師討論是否可以透過視覺上等效但不觸發版面配置的屬性替代，
這是在架構層面解決問題而非在實作層面修補症狀的正確做法。
`transform` 與 `opacity` 的動畫可完全繞過這兩個階段，
即使主執行緒在持續高負載下，合成執行緒仍可獨立於主執行緒排程運作，
穩定維持 60fps 的流暢播放。

工程師**禁止（MUST NOT）**對目前未在播放動畫或即將播放動畫的元素套用 `will-change`。
禁止將 `will-change` 設定為靜態 CSS 規則，
因為該屬性一旦設定，便會立即將元素提升至合成圖層，
並在屬性持續存在的整段期間持續佔用 GPU 記憶體，
即使該元素在使用者整個瀏覽期間從未觸發任何動畫。
禁止對清單卡片等重複元素使用靜態 `will-change: transform`，
這會為每個 DOM 元素建立一個合成圖層，
使 GPU 紋理記憶體的消耗量隨 DOM 元素數量倍增，
無論其中有多少元素正在實際播放動畫。
在虛擬捲動（virtual scroll）情境下，
即使 DOM 中同時只有少數元素，也必須確保 `will-change` 跟隨元素的
可見與不可見狀態動態開啟與關閉，而非套用在容器元素上。
應改為在 `:hover` 狀態或 JavaScript 觸發動畫前動態套用，
並在動畫結束後透過設定 `will-change: auto` 將其移除。
