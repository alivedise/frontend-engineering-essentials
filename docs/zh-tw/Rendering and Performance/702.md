...
Svelte 的洞察：如果框架是一個編譯器而非執行期函式庫，它可以提前分析程式碼並生成只針對可能改變的 DOM 節點的更新函式。不需要建立樹、不需要 diffing、不需要 patching -- 只有直接賦值。
...
在 Solid 中，元件函式只執行一次。Signal 建立 reactive subscription，直接更新 DOM -- 沒有重新渲染、沒有 diff。
...
渲染列表時沒有 key，React 和 Vue 會退回到基於 index 的啟發式方法，假設項目順序不變。在開頭插入一個項目會導致之後的每個元素被「更新」（因為 index 移位，其 props 改變），造成不必要的 DOM 異動和元件狀態損壞。

**修正：** 永遠提供從資料衍生的穩定、唯一的 `key`（資料庫 ID、UUID、slug）-- 當列表可以重新排序、篩選或有項目插入/移除時，永遠不要使用陣列 index。

### 2. 對可重新排序的列表使用陣列 index 作為 key

基於 index 的 key 對於靜態、只有 append 的列表本身沒有錯。當項目可以重新排序、在中間插入或被移除時，它們變得危險。reconciler 會將 index 0 對應到第一個 DOM 節點，不管它代表哪個資料項目，導致狀態（input 值、focus、動畫）黏在錯誤的元素上。

**修正：** 使用項目本身的 identity 作為 key。如果資料沒有自然的 ID，在項目建立時生成一個（不是在渲染期間）。

### 3. 假設 Virtual DOM 很「快」
...
在 React 中，在 render function 內建立新的物件、陣列或函式會在每次渲染時產生新的參考。因為 React 的 diffing 透過參考比較 props，即使值在邏輯上沒有改變，子元件也會重新渲染。

```jsx
// 每次渲染都建立新的 style 物件和新的 handler 函式。
// 接收這些作為 props 的子元件將總是重新渲染。
function Parent() {
...
**修正：** 使用 `useMemo` 穩定物件/陣列的參考，使用 `useCallback` 穩定函式的參考。在 React 19+ 中，React Compiler 可以自動處理許多情況。
