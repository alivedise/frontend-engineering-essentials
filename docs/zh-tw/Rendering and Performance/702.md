...
**避免在 render 中建立匿名物件和函式。** 當這些表達式作為 props 傳遞給已記憶化的子元件時，SHOULD NOT 在 JSX 中內聯寫 `style={{ color: 'red' }}` 或 `onClick={() => handler(id)}`。每次渲染都會建立新的參考，導致 React.memo 和 PureComponent 的比較永遠失敗，使記憶化失效。SHOULD 使用 `useMemo` 穩定物件和陣列的參考，使用 `useCallback` 穩定函式的參考，或採用 React Compiler（React 19+）自動處理這些情況。
