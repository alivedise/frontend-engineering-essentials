...
**避免 hydration 不匹配 -- 伺服器端與客戶端必須渲染出相同的輸出。** MUST 確保初始渲染在伺服器端和客戶端產生相同的 HTML。任何在渲染期間使用的非確定性值 -- `Date.now()`、`Math.random()`、與語系相關的格式化，或瀏覽器專屬 API 如 `window` 和 `localStorage` -- 都會在伺服器端和客戶端之間產生差異，導致框架丟棄伺服器渲染的 DOM 並完全從客戶端重新渲染。SHOULD 將瀏覽器專屬的邏輯隔離在 `useEffect`（React）或 `onMounted`（Vue）內，讓它們只在 hydration 完成後才執行。

**對非關鍵元件使用漸進式 hydration。** SHOULD 使用 `<Suspense>` 邊界（React 18+）將頁面的獨立區段包裹起來，讓每個區段作為獨立單元進行 hydration。沒有 Suspense 邊界的單一根節點會強制整個頁面作為整體進行 hydration，在所有元件處理完成之前阻塞主執行緒。應優先對使用者最可能首先互動的元件進行 hydration；將較不關鍵的區段推遲到背景 hydration 流程中處理。

**對首屏以下的內容延遲 hydration。** SHOULD NOT 為初始載入時不可見的元件支付 JavaScript 下載和執行的成本。使用 `client:visible`（Astro）、結合 Intersection Observer 的 `React.lazy`，或 `defineAsyncComponent`（Vue）來延遲 chunk 的請求，直到元件接近視窗。這縮短了 hydration 差距（TTI 減去 FCP），並減少造成內容豐富頁面 INP 分數不佳的主執行緒阻塞。
...
## 深入剖析：Hydration 不匹配偵測與全量重新渲染的代價

當 React 對伺服器渲染的頁面進行 hydration 時，它同步走訪現有的 DOM 樹和元件樹，附加事件監聽器並協調狀態。如果它遇到伺服器渲染的 HTML 與元件在客戶端產生的輸出不匹配的節點，就會記錄一個 hydration mismatch（不匹配）。

**React 17 的行為。** 在 React 17（及更早版本）中，不匹配會導致 React 靜默地丟棄不匹配的子樹並在客戶端從頭重新渲染 -- 在生產環境中不警告就替換掉伺服器 HTML。在開發環境中，錯誤會被記錄到主控台。最終結果是：任何不匹配都會清除該部分樹的伺服器渲染 HTML，重新引入內容缺失或不同的閃爍，並抵消 SSR 在該區段的 FCP 優勢。

**React 18 的行為。** React 18 改變了錯誤的可見性：在開發環境中，不匹配現在以警告的形式回報，並提供更精確的差異資訊（屬性值、文字內容、元素類型）。然而，在生產環境中，底層的代價並未改變：React 仍然丟棄不匹配子樹的伺服器渲染 DOM，並從客戶端重新渲染。沒有「聰明的合併」機制來整合伺服器和客戶端的輸出。與 React 17 的區別純粹在於開發期間對開發者的可見度 -- 對使用者的執行期結果仍是受影響子樹的完整客戶端重新渲染。

**為什麼這對效能很重要。** 對於頁面層級的不匹配（例如，在渲染期間讀取 `localStorage` 的佈局元件），整個頁面的伺服器 HTML 被丟棄，瀏覽器必須從客戶端渲染的輸出繪製。這在執行期將 SSR 轉變為 CSR -- 伺服器渲染的所有 TTFB 和 FCP 優勢都消失了，使用者可能看到一閃而過的空白內容，或者在客戶端輸出替換伺服器輸出時出現內容位移。
...
**實用的偵測策略。** 在開發環境中，React 18 記錄帶有差異值的不匹配。使用 `NEXT_PUBLIC_` 環境變數（Next.js）或等效方式，確保任何在渲染期間使用的值在伺服器端和客戶端都相同。對於必須不同的值（客戶端生成的 ID、瀏覽器偵測的特性），使用兩次渲染模式：第一次渲染輸出穩定的占位符，然後在 hydration 完成後的 `useEffect` 中更新為客戶端特定的值。

## 常見錯誤

### 1. Hydration 不匹配：伺服器與客戶端 HTML 不一致

當伺服器渲染的 HTML 與客戶端 JavaScript 在 hydration 過程中產生的輸出不同時，框架會拋出 hydration mismatch 錯誤。在 React 中，這會導致框架丟棄伺服器渲染的 DOM 並從客戶端重新渲染，抵消 SSR 的所有效能優勢。常見原因：
...
### 3. 沒有使用 Suspense 邊界進行漸進式 hydration
