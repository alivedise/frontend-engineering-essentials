...
- SHOULD 在正式環境中使用 field data（CrUX 或 web-vitals 函式庫）衡量 Core Web Vitals，而不是只依賴 Lighthouse lab 數據。
- SHOULD 為 LCP 圖片加上 `fetchpriority="high"`；在 Next.js 中使用 `<Image>` 的 `priority` prop。
- SHOULD 為所有圖片設定明確的 `width` 和 `height`，並為動態內容預留空間，以防止版面位移（CLS）。
- SHOULD 透過拆分長任務與延遲非必要的 JavaScript 來降低 TBT，這直接有助於改善 INP。
- SHOULD 將 TTFB 視為伺服器與 CDN 效能的前置指標；TTFB 不佳時，LCP 不可能達到良好標準。
- SHOULD 區分 lab data（Lighthouse、DevTools）與 field data（CrUX、web-vitals 函式庫），將兩者視為互補工具，而非可互換。
...
- **只最佳化 Lighthouse 分數，忽略 CrUX。** Lighthouse 在模擬的中階裝置和受限網路上執行。一個網站可能在 Lighthouse 得 95 分，但低階 Android 手機上的真實使用者體驗到很差的 LCP。部署修復後，務必在 CrUX field data 中驗證改善效果。

- **未將 LCP 圖片標記為高優先級。** 如果 LCP 候選元素是沒有 `fetchpriority="high"` 的 `<img>`，或由 JavaScript 注入而非存在於 HTML 中，瀏覽器將在腳本、樣式表和其他圖片之後才發現並獲取它——這是最常見的 LCP 退步原因。

- **圖片和 iframe 沒有明確尺寸。** 當瀏覽器在圖片載入前不知道其尺寸時，不會分配空間。圖片載入並進行版面計算時，周圍的所有內容都會下移。設定 `width` 和 `height`（或在 CSS 中設定 `aspect-ratio`）讓瀏覽器預留空間，消除位移。

- **在初始渲染後將橫幅或廣告內容注入 DOM。** 橫幅、Cookie 同意對話框和廣告位在畫面繪製後插入，會將現有內容向下推，產生大量 CLS 分數。在內容載入前用 `min-height` 佔位符預留空間，或對不影響文件流的覆蓋層使用 CSS `position: fixed`。

- **將 INP 誤認為「首次互動」指標（與 FID 混淆）。** FID 只衡量第一次互動的輸入延遲。INP 觀察每次互動並回報最差值。一個對第一次點擊回應良好，但複雜下拉選單或即時篩選器互動較慢的頁面，即使 FID 良好，INP 仍會很差。

- **主執行緒上的長任務。** 任何執行超過 50 ms 而不讓出的 JavaScript 都會阻塞輸入處理，提高 TBT 和 INP。使用 `scheduler.yield()`、`setTimeout(..., 0)` 或 `requestIdleCallback` 將長任務拆分成小塊，讓瀏覽器有機會處理待處理的互動。
