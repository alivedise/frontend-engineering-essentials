...
驗證步驟並非一次性活動。CDN 設定會改變，來源伺服器會升級，負載平衡器會被替換，協定協商設定也可能在不知不覺中退化。將協定驗證納入效能稽核的標準項目——透過 `nextHopProtocol` 檢查代表性資源、在 CDN 管理介面中確認 CDN 到來源的連線設定、並確認回應中包含 `Alt-Svc` 標頭——可確保 bundle 策略隨基礎架構演進而持續與傳輸協定保持一致。任何基礎架構變更（CDN 提供商切換、來源伺服器升級、負載平衡器替換）落地時，都應重新驗證協定協商，然後才認定該變更已完成。
基礎架構團隊有時會在不知情的情況下，因啟用某個功能而將 HTTP/2 降級為 HTTP/1.1（例如啟用了來源伺服器不支援的自訂協定），而在 CDN 邊緣到使用者這一跳仍呈現 HTTP/2。在與 bundle 策略變更相同的 PR 或部署中主動執行協定驗證，比事後數週才在效能指標異常時追溯發現問題，代價要低得多。
將協定驗證列為部署後檢查清單中的必要項目，確保這類無聲退化不會在生產環境中長期累積而未被察覺。
