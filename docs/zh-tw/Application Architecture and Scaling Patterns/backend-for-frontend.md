...
:::info
Backend-for-Frontend 是一個小型伺服器端元件，與恰好一種使用者體驗配對，由打造那個體驗的團隊擁有。它存在的理由是通用 API 對誰都不特別好用：網頁應用需要把三次呼叫合併成一個聚合端點、行動應用需要精簡的酬載，而兩者都得排隊等後端團隊的路線圖。BFF 把那層翻譯搬進前端團隊自己的地盤，讓它能跟 UI 一樣快地變更。這個模式後來多了第二份工作，如今往往才是採用它的決定性理由：IETF 針對瀏覽器應用的 OAuth 指引把 BFF 列為最安全的架構，因為 token 只活在伺服器上，瀏覽器裡沒有任何指令碼注入偷得走的東西。
:::

## 背景

這個模式源自 SoundCloud 在 2015 年前後逃離單體公用 API 的過程：當打造新用戶端的團隊擁有自己的 API 層時，它「能移動得快很多，因為不需要各方協調」，Phil Calçado 與 Sam Newman 把結果寫成了 Backends For Frontends。組織層面的洞見是刻意運用的 Conway 定律（系統會鏡射打造它的組織的溝通結構）：與其讓一個通用 API 團隊成為每個用戶端需求的瓶頸，不如給每種體驗一個單一用途的邊緣服務，聚合下游服務、為那一個 UI 塑形回應。十年後，許多前端團隊都在跑一個沒有掛名的 BFF，因為 meta-framework 的伺服器路由（API routes、server actions、loaders）就是 UI 團隊擁有的逐體驗伺服器程式碼，這滿足了模式對所有權的定義。真正改變的是安全那一半：OAuth 2.0 for Browser-Based Apps 草案（IETF 進行中的 Best Current Practice）把 BFF、token-mediating backend、browser-based OAuth client 明文列為「安全性遞減」的三種架構，讓「token 住哪裡」成為這份草案逼著前端 API 設計先回答的第一個問題。本文涵蓋這兩半，以及讓 BFF 不變成第二個單體的邊界紀律。
...
Token handler 的那一半：瀏覽器只拿到 `HttpOnly` session cookie，其他什麼都沒有；BFF 是機密（confidential）OAuth 用戶端（能以自己的憑證向授權伺服器證明身分的用戶端），把 access 與 refresh token 留在伺服器端，並注入代理出去的呼叫。頁面上的惡意指令碼可以在執行期間*使用*這個工作階段，卻沒有 token 可以外洩。`pkce: true` 代表 PKCE（RFC 7636），把授權碼綁定到發起請求的用戶端的逐次登入證明金鑰；真實的登入路由會在重導向之前先建立 verifier，範例略去了這一步：
...
export const auth = new Hono()
  ...
  .all("/api/proxy/*", async (c) => {
    // 變更操作在代理前先對工作階段驗證 X-CSRF-Token（此處略去）
    ...
    return fetch(downstreamUrl(c.req), {
      ...
      body: c.req.raw.body,
      duplex: "half", // Node 的 fetch 轉送 body 串流時需要這個選項
    });
  });
```

用戶端的後果主要是刪程式碼：不再需要儲存 token 或處理 refresh，請求變成帶上憑證的普通同源 fetch，外加一個 CSRF token，因為 cookie 在跨站請求上也會被順路帶走：
...
export async function api<T>(path: string, init?: RequestInit): Promise<T> {
  const res = await fetch(`/api${path}`, {
    ...
    // double-submit 模式：BFF 把 token 鏡射在一個可讀的 cookie 裡
...
- **必須（MUST）**一個 BFF 配一種體驗，抵抗把它通用化的誘惑；當兩個用戶端共用一個 BFF，每次變更都要跟兩個畫面談判，通用 API 的問題只是往上搬了一層重建。Newman 自己允許把幾乎相同的體驗合併（一個 mobile BFF 同時服務 iOS 與 Android）；把那當作光譜上可接受的一端，把本條規則當作體驗真正分歧時的校準。
...
- **應該（SHOULD）**把囉唆的呼叫序列收到伺服器端；BFF 坐在資料中心延遲上，瀏覽器坐在最後一哩延遲上，把三次呼叫的瀑布搬過這條線，等於用資料中心的往返換掉最後一哩的往返。
- **應該（SHOULD）**認出你已經擁有的那個 BFF：meta-framework 的路由處理器、server actions 與 loaders 就是 UI 團隊擁有的逐體驗伺服器程式碼，在那裡套用同樣的邊界紀律，而不是再加第二層邊緣。
- **應該（SHOULD）**在存在防腐映射時讓 BFF 承載它（見[前端 DDD](/zh-tw/frontend-ddd)）：在伺服器端翻譯上游 DTO，讓線路格式與瀏覽器 bundle 互不相干。
- **可以（MAY）**在各畫面資料形狀差異劇烈時，用 GraphQL 作為 BFF 的查詢表面；模式與協定是正交的。
- **可以（MAY）**在純網頁應用、API 已經是 UI 形狀、驗證已走 cookie 時完全跳過這個模式，Newman 自己劃的界線只講聚合那一半（純網頁應用若沒有大量伺服器端聚合，多半不需要）；再疊上 token 這一題，兩者都不需要時，BFF 就是一跳額外延遲加一份 on-call 輪值。
...
**安全的重新定框改變了採用的算術。**多年來 BFF 是開發者體驗模式，聚合之痛夠大才值得多養一個部署單位。OAuth 瀏覽器應用的工作把敏感應用的預設反轉了：瀏覽器持有 token 如今是需要你*主動辯護*的架構，XSS 爆炸半徑是你必須回答的問題，而 BFF 是參考答案。這份草案給了團隊採用它的新理由：即使聚合與塑形都不需要，也可以純粹為了 token 邊界而用。

**成本是一個服務，前端團隊應該誠實計價。**BFF 是有正常運行時間、密鑰、擴縮與 on-call 故事的部署單位，擁有者卻是專長在 UI 的團隊。Meta-framework 代管吸收了一部分營運重量，這是模式隨那些框架而非在它們之前普及的實際原因。

## 深入探討

**IETF 的架構階梯。**瀏覽器應用草案把三種架構按安全性遞減排序。*BFF*：後端是機密 OAuth 用戶端，所有資源請求經它代理，token 永不抵達瀏覽器；指令碼注入可以搭工作階段的便車，卻偷不走憑證。*Token-mediating backend*：後端取得 token，但把 access token 交給瀏覽器直接呼叫資源，以可被竊（雖短命）的 token 換掉代理跳數。*Browser-based OAuth 2.0 client*（草案對「純瀏覽器公開用戶端」的正式名稱）：SPA 自己跳完整支舞，每個 token 都離外洩只差一次 XSS。草案對商務與個人資料應用強烈建議第一種，並與 RFC 9700（OAuth 安全 BCP）搭配完成周邊加固。
...
**串流與長連線。**讓 SSE（Server-Sent Events）或 WebSocket 升級經 BFF 代理能保住 token 邊界，但會把 BFF 變成持有連線的層，擴縮輪廓從無狀態請求處理器變成帶逐使用者記憶的東西。常見的折衷：在 BFF 終結串流、以同源 SSE 重新暴露；或為那一條通道鑄造短命、窄範圍的 token 讓它直連，有意識地接受在安全階梯上往下走一階。
...
| Browser-based OAuth client | 瀏覽器 | Access + refresh token | 完全沒有後端可用；有意識地接受 XSS 暴露 |
| 無 BFF、cookie API | 伺服器 session，瀏覽器無 OAuth | Session cookie | 純網頁應用、第一方 UI 形狀 API：模式自己的「不必費事」案例 |

兩個關鍵判準問題能收斂多數爭論：「如果攻擊者在我們頁面上跑指令碼，他帶得走什麼？」與「這個畫面的酬載變更需要誰批准？」第一題決定行的排序；第二題決定這條邊緣到底屬不屬於前端團隊。

## 延伸閱讀

- [前端的 Domain-Driven Design](/zh-tw/frontend-ddd)
- [Server-Driven UI](/zh-tw/server-driven-ui)
- [身份驗證與 Token 儲存](/zh-tw/1202)
- [跨來源資源共享（CORS）與跨網站請求偽造（CSRF）](/zh-tw/1204)
- [伺服器端渲染（SSR）與伺服器元件安全](/zh-tw/1207)
