...
**無條件 precache 離線 fallback 頁面。** 在 service worker 的 `install` 事件 cache open 呼叫中加入 `/offline.html`。不要依賴執行期快取條目作為 fallback。fallback 必須在任何使用者訪問發生之前就已可用。

**在 DevTools 中測試離線 fallback。** 開啟 DevTools，前往 Application > Service Workers，勾選「Offline」，然後導航至未快取的路由。應該顯示離線頁面。如果出現瀏覽器的預設錯誤頁面，表示 fallback 未正常運作。在上線前進行此測試。

**區分離線指示器與離線 fallback。** `online`/`offline` 事件監聽器驅動的是 UI 指示器（橫幅、狀態標示），告訴使用者他們在目前頁面的網路狀態。離線 fallback 是用於替換失敗導航請求的整頁內容。這是兩種不同機制，服務於兩種不同目的；實作其中一個並不代表實作了另一個。

**將 sync 佇列保存在 IndexedDB 中，而非 sessionStorage 或記憶體中。** 儲存在記憶體中的佇列，在頁面重新整理時會遺失。儲存在 sessionStorage 中的佇列，在瀏覽器分頁關閉時會遺失。只有 IndexedDB 在頁面重載和 service worker 重啟後都能存活。`idb` 函式庫（Jake Archibald）提供了一個乾淨的 Promise-based 包裝器，包裝了原始的 IndexedDB API，並大幅減少了樣板程式碼。

**在 IndexedDB 寫入之後再註冊 sync tag，而非之前。** sync 事件可能在網路已可用的情況下，於註冊後幾乎立即觸發。如果 tag 在資料寫入 IndexedDB 之前就被註冊，handler 會在空的 store 上執行，並回傳一個 resolved Promise，讓瀏覽器認為同步完成。已排隊的資料之後孤立在 IndexedDB 中——沒有已註冊的 sync tag 來處理它。

**在兩端都通知使用者佇列狀態。** 當寫入在離線時被排隊，顯示訊息：「已在本地儲存。連線恢復後將自動提交。」當同步完成（service worker 透過 `postMessage` 通知頁面），更新 UI：「提交成功。」如果使用者從未看到第二個確認，他們可能在重新上線時再次提交表單，造成重複。完整的往返回饋迴路——排隊確認與同步確認——可防止重複並建立信任。

**為所有瀏覽器提供手動重試按鈕。** Background Sync 是漸進式增強，不是基線功能。使用功能偵測包裝 sync 註冊（`'SyncManager' in window.ServiceWorkerRegistration.prototype`），並始終在 UI 中為不支援的瀏覽器使用者渲染「重試」按鈕。重試按鈕在點擊時直接從頁面讀取 IndexedDB 佇列並重放它。

**在永久失敗時回滾樂觀狀態。** 定義每個操作的「永久失敗」含義：通常是不可重試的 HTTP 狀態碼（400、401、403、404、409、422）或重試預算耗盡。當偵測到永久失敗時，回滾本地狀態變更，並向使用者顯示清楚的錯誤訊息，提供重試選項。絕不靜默丟棄失敗的寫入。
...
**沒有在 `install` 事件中 precache `/offline.html`。** 如果離線 fallback 只在執行期新增到快取中——即使用者首次在線上瀏覽頁面時——那麼安裝了 PWA 但在首次導航之前就斷線的使用者，將看到瀏覽器的預設錯誤頁面，而非應用程式的 fallback。在 `install` 期間 precache 是唯一的保證。

**在 IndexedDB 寫入完成之前就註冊 Background Sync tag。** sync 事件可能在註冊後數毫秒內觸發，如果連線已可用的話。如果 tag 在 `db.add()` 呼叫 resolve 之前就被註冊，handler 會讀取到空的 store，回傳 resolved Promise，瀏覽器就將同步標記為完成。資料之後卡在 IndexedDB 中——它永遠不會被重試，因為該註冊已消失。始終在呼叫 `reg.sync.register(...)` 之前 `await db.add(...)`。

**當同步失敗時靜默丟棄樂觀更新。** 如果使用者提交了表單，看到成功狀態（樂觀），失去了連線，而背景同步永久失敗（伺服器回傳 409，或重試耗盡），應用程式必須回滾成功狀態並通知使用者。靜默失敗比錯誤訊息更糟——使用者認為操作成功了，並且當結果從未出現時會感到困惑。

**假設 Background Sync 普遍可用。** 它並非如此。iOS Safari 不支援它。Firefox 支援有限。任何使用 Background Sync 卻沒有手動重試 fallback 的應用程式，對相當比例的行動使用者來說是壞掉的。始終用功能偵測包裝 sync 註冊，並始終提供重試按鈕。
