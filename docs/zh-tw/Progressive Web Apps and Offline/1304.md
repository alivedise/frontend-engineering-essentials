...
**應該（SHOULD）優先選擇 `generateSW`；只在必要時切換到 `injectManifest`。** `generateSW` 模式完全是宣告式的，所有快取行為在 `vite.config.ts` 中以資料表達。當 service worker 需要自訂的請求變更邏輯（auth token 注入、A/B 路由、自訂分析）時，團隊禁止（MUST NOT）使用 `generateSW`；這類情況必須（MUST）使用 `injectManifest` 模式。
