...
**將每個策略函式的作用範圍限定在單一具名快取。** 在每個策略函式的呼叫端傳入快取名稱作為參數（`cacheFirst(request, STATIC_CACHE)`、`networkFirst(request, DYNAMIC_CACHE)`），而非在函式內部寫死，可讓快取的歸屬在呼叫點一目了然，並防止意外的跨快取污染。版本化靜態資源與執行期取得的 API 回應應存放在不同的具名快取中，以便能夠各自獨立地進行版本管理和清理。如果新部署將 `STATIC_CACHE` 升級至 `static-v2` 但 `DYNAMIC_CACHE` 維持 `dynamic-v1`，activate 處理器只會刪除 `static-v1`；動態快取則不受影響地延續。這種粒度只有在每個呼叫點的快取名稱都明確的情況下才能實現。

**存入快取之前，先確認回應狀態碼正確。** 能夠 resolve（不拋出錯誤）的 `fetch()` 仍然可能回傳 4xx 或 5xx 回應。如果 `cache.put()` 儲存了一個 404 回應，後續每次對該 URL 的快取命中都會提供 404，直到快取被清除為止，讓資源看起來永久損壞。在呼叫 `cache.put()` 之前，檢查 `response.ok`（即 `response.status >= 200 && response.status < 300`），若為 false 則跳過快取。網路錯誤路徑（`fetch()` 拋出錯誤）是由 `networkFirst` 的 `catch` 區塊處理的獨立路徑。回應層級的錯誤路徑（`fetch()` 以非 OK 狀態 resolve）則需要在 `cache.put()` 之前明確加上 `if (response.ok)` 的防護。

**為靜態快取名稱加上版本號，並在 activate 時刪除舊快取。** 在 Service Worker 檔案頂部定義 `const STATIC_CACHE = 'static-v1'` 和 `const DYNAMIC_CACHE = 'dynamic-v1'`。在 `activate` 處理器中，刪除每個名稱不在當前快取名稱集合中的快取。每當預先快取的資源列表變更時，遞增版本字串。這確保前一個 Service Worker 版本所快取的資源立即被清理，而不是無限期佔用儲存空間。

**在將回應存入快取之前，務必先複製（clone）回應。** `Response` 物件的主體是一個只能消費一次的可讀取串流。如果你將 `response` 傳給 `respondWith`，然後再嘗試呼叫 `response.clone()`，主體已被讀取，複製結果將是空的。立即複製：`const clone = response.clone()`；複製品存入快取，原始回應回傳給頁面。

**Cache-First 只能套用於具有內容雜湊 URL 的資源。** URL 中的內容雜湊是使 Cache-First 安全的不變量。沒有它，Cache-First 就是一個正確性風險。審查 Service Worker 程式碼時，最簡單的正確性檢查是查看每個 `cacheFirst()` 呼叫點，確認其處理的 URL 包含雜湊片段。

**將 Network-First 作為任何未明確分類的資源的安全預設。** 未被刻意分類的資源，以新鮮的網路回應提供比以可能陳舊的快取回應提供更好。Network-First 對於該資源而言不如 Cache-First 或 Stale-While-Revalidate 高效，但永遠不會出錯。先確保正確性，待路由驗證無誤後再進行效能優化。

**不要快取來自跨來源 no-cors 請求的不透明回應。** 在呼叫 `cache.put()` 之前檢查 `response.type === 'opaque'`，對不透明回應跳過快取。如果需要快取跨來源資源，向伺服器請求 CORS 標頭，並以 `mode: 'cors'` 發起 `fetch`。

**為動態快取實作有界淘汰策略。** 對於任何在執行期（而非安裝時）填充的快取，定義條目的最大數量，並在每次 `cache.put()` 後強制執行。用 `cache.keys()` 取得所有快取鍵值，如有需要按插入順序排序，並刪除超出限制的條目。沒有這個機制，高流量應用程式的動態快取將無限增長。

**明確測試離線行為，而非僅測試快取命中路徑。** Network-First 回退到快取、快取條目缺失時的 Cache-Only 錯誤，以及 Stale-While-Revalidate 的背景更新，都有只在網路不可用時才會執行的路徑。使用 Chrome DevTools Network 面板的「Offline」模式，驗證每種策略在網路請求失敗時（而非只有成功時）的行為符合規格。
...
**動態快取沒有快取淘汰策略。** 快取每個處理的回應的 Service Worker，會讓動態快取無限增長。在儲存配額有限的裝置（舊款 Android 裝置、隱私瀏覽模式）上，瀏覽器最終會同時淘汰該來源的所有快取，一口氣摧毀預先快取和動態快取。在動態快取的每次 `cache.put()` 後，實作有界的 LRU 淘汰：開啟快取、取得所有鍵值，並刪除超出限制的最舊條目。

**快取來自跨來源 no-cors 請求的不透明回應。** 無論底層伺服器回傳 200 還是 500，不透明回應的 `response.status` 始終為 `0`。快取一個實際上是伺服器錯誤的 0 狀態回應，意味著 Service Worker 會將錯誤回應（可能具有空主體）提供給後續對該資源的每個請求，而不會顯示任何可見的錯誤。在呼叫 `cache.put()` 之前，始終檢查 `response.type !== 'opaque'`，或者在伺服器支援時以 `mode: 'cors'` 發起 `fetch`。

**存入快取之前未複製回應。** 呼叫 `response.clone()` 不是可選的。`Response` 主體是一個可讀取串流，只能消費一次。如果 Service Worker 將回應傳給 `respondWith()`，然後嘗試再次讀取主體以供 `cache.put()` 使用，第二次讀取會回傳空串流。複製必須在任何副本被消費之前建立：在 `await fetch()` 呼叫之後，對回應進行任何其他操作之前，立即執行 `const clone = response.clone()`。
