...
選擇正確的快取策略不應憑直覺，而應基於三個關於資源本質的具體問題：這個資源多久變更一次？提供陳舊版本的後果有多嚴重？離線時完全無法取得資源的後果又有多嚴重？

這三個問題構成了一個決策框架，可以在不依賴記憶策略名稱的情況下，系統性地為每種資源類型選出最合適的策略。高更新頻率加上嚴重的陳舊後果指向 Network-First；低更新頻率加上輕微的陳舊後果指向 Cache-First 或 Stale-While-Revalidate；離線時完全不可用會造成核心功能破壞的資源，必須透過 Cache-Only 搭配預先快取來保障可用性。

有疑慮時，優先選擇新鮮度：將 Network-First 套用在本可用 Cache-First 的資源上是一種效能浪費，但永遠不會造成正確性錯誤；而將 Cache-First 套用在應該用 Network-First 的資源上，可能導致使用者無限期看到陳舊內容，這是一種難以排查的正確性失敗。保守選擇在效能上有代價，激進選擇在正確性上有代價，而後者通常更難被察覺和修復。

此外，快取策略的選擇也必須考量 HTTP 方法：非冪等的請求（POST、PUT、DELETE、PATCH）永遠不應被快取，無論資源類型為何。快取一個產生副作用的請求回應，並在後續請求中重播，意味著重複執行了原本只應執行一次的寫入操作，可能產生重複資料、雙重計費或狀態損壞等後果。`fetch` 處理器必須在決定是否快取之前，先確認請求方法為 `GET`。這個方法過濾器應該是處理器中最先執行的條件，而非最後執行的，以確保非 GET 請求在到達任何快取邏輯之前就已被排除。
...
**禁止（MUST NOT）** 在任何策略下快取非冪等請求（POST、PUT、DELETE、PATCH）的回應。快取一個變更回應並在後續請求中重播，會產生未定義且可能造成破壞性的行為——重複的寫入操作可能導致重複記錄、雙重扣費，或使應用程式狀態損壞。`fetch` 處理器在對任何請求呼叫 `cache.put()` 之前，必須（MUST）先檢查 `event.request.method === 'GET'`。
