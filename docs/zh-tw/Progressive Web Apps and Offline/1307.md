...
一個在開發環境通過 Lighthouse 稽核但在生產環境中靜默地無法更新的 PWA，比沒有 PWA 還要糟糕——使用者在毫不知情的情況下被服務著舊版內容，不知道新版本已存在。團隊 MUST 在上線至生產環境之前實作明確的 service worker 更新策略。瀏覽器的預設更新行為——新 SW 等待所有分頁關閉後才啟動——不是一個策略。它是一個分頁會被頻繁關閉的假設，而這對大多數 web 應用程式而言是不成立的。長時間存活的分頁是生產力工具、儀表板，以及使用者整個工作日都保持開啟的任何應用程式的常態。在預設行為下，此類使用者在新版本部署後可能會在舊版本上停留數天或數週。

團隊 MUST 使用版本化快取名稱，並在 `activate` 時刪除舊快取。沒有包含版本字串的快取名稱在新 service worker 啟動時無法可靠地清除：如果所有快取使用相同的名稱，`activate` 處理器無法區分哪些快取是過時的。快取名稱 SHOULD 遵循 `name-vN` 的格式（例如 `static-v3`、`runtime-v3`），其中 `N` 在快取策略或資源集以需要全新清除的方式變更時遞增。團隊 SHOULD 在上線前於實體 iOS 裝置上稽核已安裝的 PWA——macOS Safari 和 iOS 模擬器無法複製適用於真實 iOS 硬體的推播通知權限限制和背景 API 的限制。
...
**針對生產建置而非 dev server 執行 Lighthouse。** Vite 的 dev server 不會註冊 service worker。`next dev` 會 stub 或停用 SW 行為。針對 dev server 執行 Lighthouse 會產生誤導性結果——它可能對一個沒有真實 service worker 的應用程式報告 PWA 分數通過，或者對生產環境中會通過的檢查報告失敗。始終針對 `pnpm build && pnpm preview` 或等效的生產建置指令的輸出執行稽核，並使用 Chrome 的無痕模式避免來自先前測試執行的快取狀態。

**使用 prompt-to-reload 模式作為預設更新策略。** `skipWaiting()` 沒有使用者提示，只有在新 SW 的快取策略與進行中的請求向後相容，且資源集的變更不會破壞進行中的工作階段時才是安全的。對於大多數應用程式，更安全的預設是從 service worker 的 `install` 事件發送 `UPDATE_AVAILABLE`，並顯示一個不干擾的橫幅，讓使用者控制何時重新載入。

**每當快取策略變更時遞增快取版本字串。** 不要使用沒有版本後綴的快取名稱。名為 `app-cache` 的快取無法被 `activate` 處理器與名為 `app-cache` 的前版本快取區分開來。使用 `static-v1`、`static-v2` 等，並在 `activate` 期間清除除當前版本之外所有匹配命名空間前綴的舊版本。

**明確地預快取 `start_url`。** Lighthouse 離線檢查在停用網路的情況下導航至 `start_url`。如果 `start_url` 不在預快取中，此檢查失敗。更重要的是，將 PWA 安裝到主畫面後在離線狀態下啟動它的使用者，如果 `start_url` 沒有被快取，將看到瀏覽器錯誤頁面。這是損壞的主畫面安裝體驗最常見的原因。

**在提交至 Play Store 之前驗證 Digital Asset Links。** 如果 `.well-known/assetlinks.json` 檔案缺失、格式錯誤或返回錯誤的憑證指紋，Chrome 將在已安裝的 TWA 內顯示 URL 列。在 Play Store 安裝的「應用程式」中看到 URL 列的使用者，會得出應用程式損壞或品質低劣的結論。使用 Digital Asset Links API 驗證該檔案，然後再提交 APK。

**在實體 iOS 裝置上進行測試。** macOS Safari 不會強制執行 Web Push 通知的主畫面安裝需求。在 macOS 上測試推播通知訂閱會成功，但當使用者在 iPhone 上的 Safari 中開啟應用程式時，相同的程式碼會靜默地失敗，因為 `Notification.requestPermission()` 在瀏覽器分頁上下文中被忽略。始終在執行 iOS 16.4 或更新版本的實體 iPhone 上驗證推播訂閱流程。

**在不設定過期策略的情況下避免快取 opaque response。** 沒有 CORS 標頭的跨來源回應以 opaque response 的形式儲存。Cache Storage API 將其大小報告為 0 位元組，但瀏覽器可能以填充（padded）的大小計算其配額用量（Chrome 歷史上以 7 MB 填充 opaque response 進行配額計算）。在沒有最大條目數或 TTL 的情況下快取許多 opaque response，可能耗盡來源的瀏覽器儲存配額，觸發其他快取資源被驅逐。如果必須快取跨來源資源，請使用 Workbox 的 `ExpirationPlugin` 配置條目限制。

**提供有意義的離線 fallback 頁面。** Lighthouse 稽核接受空白頁面作為離線回應，但使用者理應得到更好的體驗。建立一個專用的 `/offline.html` 頁面，在 SW 安裝階段預快取，解釋使用者目前離線，並在可用時提供已快取內容的導航。當 SW 無法從網路履行導航請求且沒有快取版本時，回退到此頁面，而不是返回一般瀏覽器錯誤。
...
**針對 dev server 執行 Lighthouse PWA 稽核。** Vite 的 dev server 和大多數框架的 dev server 要麼完全停用 service worker，要麼以 stub 替代。針對 `localhost:5173`（Vite）的 Lighthouse 稽核將無法通過 service worker 檢查，或針對一個在生產環境中不代表真實行為的 no-op SW 進行報告。始終稽核 `localhost:4173`（Vite preview）或已部署的生產 URL。
...
**`skipWaiting()` 搭配 `clients.claim()` 但沒有遞增快取版本。** 當新 SW 啟動並接管所有開啟的分頁時，它開始服務請求——但如果快取名稱沒有變更，它將從與舊 SW 相同的快取中提供資源。如果舊快取包含與新應用程式程式碼不相容的過時資源，使用者將看到損壞的應用程式。始終將 `skipWaiting()` 策略與快取版本遞增配對使用。

**不在實體 iOS 裝置上測試。** macOS Safari 不強制執行 Web Push 的主畫面安裝需求。在 macOS 上測試推播通知訂閱會成功，但當使用者在 iPhone 上的 Safari 中開啟應用程式時，相同的程式碼會靜默地失敗，因為 `Notification.requestPermission()` 在瀏覽器分頁上下文中被忽略。始終在執行 iOS 16.4 或更新版本的實體 iPhone 上驗證推播訂閱流程。

**在提交至 Play Store 之前沒有設定 Digital Asset Links。** 如果 `assetlinks.json` 缺失或不正確，Chrome 在已安裝的 TWA 內顯示 URL 列。URL 列是對使用者表明包裝出了問題的訊號。若不發布更新的 APK 則無法修正。在首次提交至 Play Store 之前驗證 Digital Asset Links 配置。

**在各版本之間使用相同的快取名稱。** 名為 `app-cache` 的快取無法被 `activate` 處理器與前版本名為 `app-cache` 的快取區分開來。如果新 SW 使用 `app-cache` 儲存其預快取資源，它可能發現前版本已存在的條目並跳過抓取更新的資源。版本化快取名稱，並在 `activate` 期間清除舊版本。

**沒有過期策略地快取 opaque response。** 沒有 CORS 標頭的跨來源回應以 opaque response 的形式儲存。Cache Storage API 將其大小報告為 0 位元組，但瀏覽器可能以填充的大小計算配額（Chrome 歷史上以 7 MB 填充 opaque response 進行配額計算）。在沒有最大條目數或 TTL 的情況下快取許多 opaque response，可能耗盡來源的瀏覽器儲存配額，觸發其他快取資源被驅逐。如果必須快取跨來源資源，請使用 Workbox 的 `ExpirationPlugin` 配置條目限制。
...
- Google Chrome Developers — [Lighthouse PWA 稽核文件](https://developer.chrome.com/docs/lighthouse/pwa/)
...
- Apple Developer — [在 Apple 平台上啟用 Web Push API](https://developer.apple.com/documentation/usernotifications/sending_web_push_notifications_in_web_apps_safari_and_other_browsers)
