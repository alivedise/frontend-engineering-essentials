...
可安裝性是一種漸進式增強，MUST NOT 被用來限制核心功能。拒絕安裝 App 的使用者，或瀏覽器不支援安裝的使用者，MUST 獲得完整可用的 Web 體驗。manifest 和安裝提示的存在是為了改善回頭使用者的便利性和互動性 -- 它們不是發佈的付費牆。

團隊 MUST 提供 192x192 和 512x512 像素的 maskable 圖示，且 manifest 中宣告的 `start_url` MUST 被已註冊的 Service Worker 快取。離線載入失敗的已安裝 App 所呈現的體驗與壞掉的原生 App 無法區分。技術上存在但引用未快取資源或尺寸不正確圖示的 manifest SHOULD 視為無效；Lighthouse 會如此報告。如果團隊打算支援安裝，`display` 欄位 MUST NOT 設為 `"browser"` -- `"browser"` 模式明確退出獨立啟動體驗，並在基於 Chromium 的瀏覽器中抑制 `beforeinstallprompt`。
...
**在每個 HTML 頁面連結 manifest 和 theme-color。** `<link rel="manifest" href="/manifest.json" />` 和 `<meta name="theme-color" content="#3b82f6" />` 標籤必須出現在 App 每個頁面的 `<head>` 中，而不只是根頁面。瀏覽器在每次頁面載入時評估可安裝性條件；在某些頁面缺少 manifest 會導致從深層連結進入 App 的使用者出現不一致的行為。

**在 `start_url` 中使用 `/?source=pwa` 進行分析歸因。** 查詢參數被 App 的路由邏輯忽略，但被分析工具作為不同的流量來源捕獲。這讓團隊可以測量已安裝使用者從主畫面開啟 App 的頻率與從瀏覽器開啟的頻率，以及已安裝使用者是否比瀏覽器使用者有不同的留存率或轉化率。

**延遲安裝提示至有意義的轉化之後。** 使用者與 App 的第一次互動是顯示安裝提示的最差時機。他們還沒有了解 App 是做什麼的，或為什麼他們會想要在主畫面上有它。等到使用者完成一個表示意圖的操作 -- 儲存文件、新增任務、登入 -- 再顯示安裝按鈕。

**在發布前使用 maskable.app 測試圖示外觀。** 分別提供 `"purpose": "any"` 和 `"purpose": "maskable"` 條目。驗證 maskable 變體裁切為圓形時看起來正確。使用工具確認沒有有意義的內容落在圖示外側 20% 的區域。

**在上線前使用 Lighthouse 驗證 manifest。** Lighthouse 的 PWA 稽核檢查 manifest 完整性、圖示可用性、`start_url` 可達性和 Service Worker 註冊。在 CI 或 Chrome DevTools 中執行稽核，在發布前發現否則會阻止 `beforeinstallprompt` 觸發的問題。

**處理 `appinstalled` 事件以更新 UI 狀態。** 使用者接受安裝提示後，瀏覽器會在 `window` 上觸發 `appinstalled`。監聽此事件以隱藏安裝按鈕，並可選地顯示確認訊息。這確保按鈕在 App 已在主畫面上之後不會繼續顯示在 UI 中。
...
**圖示條目缺少 `"purpose": "maskable"`。** 沒有 maskable 圖示，Android 和部分桌面環境會應用預設背景並將標準圖示裁切為圓形，通常會裁掉放置在圖示邊緣附近的商標或文字。分別為 `"any"` 和 `"maskable"` 目的提供圖示條目，並在發布前在 maskable.app 驗證 maskable 變體。

**設定 `"display": "browser"`。** 這明確退出 standalone 模式，並阻止 `beforeinstallprompt` 在基於 Chromium 的瀏覽器中觸發。打算支援安裝的團隊必須使用 `"standalone"`、`"minimal-ui"` 或 `"fullscreen"`。

**在第一次頁面載入時顯示安裝提示。** 使用者尚未看到 App 的價值。立即提示會產生低接受率和糟糕的第一印象。延遲自訂安裝按鈕，直到使用者完成至少一個有意義的操作。

**未處理 `appinstalled`。** 如果使用者已安裝 App 後安裝按鈕仍然可見，會造成困惑，暗示 UI 沒有回應。始終在 `window` 上監聽 `appinstalled`，並在觸發時移除或隱藏按鈕。

**在單一圖示條目上使用 `"purpose": "any maskable"`。** 這告訴瀏覽器同一張圖片服務兩個目的。在不裁切圖示的平台上，maskable 圖片的外側內距將顯示為可見的空白。為每個目的提供單獨的條目。

**在深層連結頁面缺少 manifest 連結。** 如果 `<link rel="manifest">` 只出現在根頁面，透過直接 URL（電子郵件連結、搜尋結果）進入 App 的使用者可能不會在該頁面載入時觸發可安裝性評估。在每個頁面的 `<head>` 中包含 manifest 連結。
