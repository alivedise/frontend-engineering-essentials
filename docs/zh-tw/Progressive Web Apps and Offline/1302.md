...
### Service Worker 作為平台擴充點，而非快取機制

Service Worker 最常被介紹為快取工具，但這個定位掩蓋了其更深層的本質：它是瀏覽器在頁面與網路之間插入自訂邏輯的唯一正式機制。快取行為是 `fetch` 事件處理器邏輯的結果，而不是 Service Worker 的固有屬性。理解這個區別有助於避免架構上的誤判：一個沒有 `fetch` 處理器的 Service Worker 無法攔截任何請求，因此也無法實現任何快取或代理目的；而一個有 `fetch` 處理器但不進行任何快取操作的 Service Worker，可以作為身份驗證標頭注入器、請求去重器，或是基於瀏覽器的 API mock 層，完全不涉及快取。這個視角將 Service Worker 的能力從「使應用程式離線運作」拓展為「在 origin 層面控制所有網路請求的完整行為」。

此外，`fetch` 處理器的存在對瀏覽器的可安裝性評估有直接影響：Chrome 在決定是否觸發 `beforeinstallprompt` 事件時，會要求 Service Worker 必須包含 `fetch` 事件監聽器。這意味著即使一個應用程式的主要目標是成為可安裝的 PWA 而非攔截網路請求，也必須在 Service Worker 中至少宣告一個 `fetch` 處理器。忽略這個要求的應用程式將默默地無法觸發安裝提示，且這個失敗不會產生任何可見的錯誤訊息，使其成為 PWA 開發中最難診斷的問題之一。

從架構角度而言，「為何要有 Service Worker」的問題應在設計階段就明確回答，而非在實作時才臨時決定。不同的目標需要不同的 `fetch` 處理器策略：若目的是離線支援，則需要預先快取和回退邏輯；若目的是身份驗證標頭注入，則需要根據請求 URL 進行選擇性攔截；若目的是支援可安裝性，則至少需要一個最小化的 `fetch` 處理器以滿足瀏覽器要求。將這些目的混合在單一 Service Worker 中是合法的，但需要在 `fetch` 處理器內部維護清晰的分支邏輯，並確保每個分支都有明確的優先順序和退出條件。

最後，值得注意的是，Service Worker 的 origin 邊界是安全設計的一部分，而非可繞過的限制。一個 origin 上的 Service Worker 無法攔截另一個 origin 的請求，這保證了跨 origin 資源的隔離性。理解這個邊界有助於在設計 API 架構時做出正確決策——例如，決定是否透過代理路由將跨 origin 請求轉換為同 origin 請求，以便 Service Worker 能夠介入。

同樣地，Service Worker 的 scope 受限於其檔案在伺服器上的路徑，這意味著放置在 `/assets/sw.js` 的 Service Worker 只能控制 `/assets/` 路徑下的請求，而無法擴展至整個 origin，除非透過 `Service-Worker-Allowed` 回應標頭明確允許更寬廣的 scope。將 Service Worker 檔案保持在根目錄（`/sw.js`）是最直接且最不容易出現 scope 問題的部署方式，也是建置工具和文件範例中最常見的慣例。

### Service Worker 的生命週期

Service Worker 從首次註冊到完全控制頁面的網路請求，會經歷四個不同的階段。理解每個階段是撰寫在首次安裝、跨版本更新及多分頁情境下行為正確的處理器的前提。
...
**必須（MUST）** 在任何以支援可安裝性為目標的 Service Worker 上註冊 `fetch` 事件處理器。Chrome 要求存在 `fetch` 處理器才能觸發 `beforeinstallprompt` 事件；沒有 `fetch` 處理器的 Service Worker 既無法滿足可安裝性條件，也無法達成任何有意義的網路攔截目的，**不應該（SHOULD NOT）** 被部署到正式環境。
