...
**在註冊前始終檢查 Service Worker 是否受支援。** `'serviceWorker' in navigator` 的檢查可防止在不支援此 API 的環境中出現執行時期錯誤，包括某些較舊的瀏覽器和非安全上下文（其中 Service Worker API 被封鎖）。在 `DOMContentLoaded` 或 load 事件監聽器中進行註冊，以避免阻塞關鍵渲染路徑。

**在 `install` 和 `activate` 處理器中，對每個非同步操作呼叫 `event.waitUntil()`。** 沒有 `waitUntil`，瀏覽器可能在非同步工作完成前終止 Service Worker 執行緒。使用 `Promise.all()` 將多個並發非同步操作包裝成傳給 `waitUntil` 的單一 promise。

**明確為快取名稱加入版本號，並在 `activate` 處理器中刪除舊快取。** 從不清理舊快取的 Service Worker 會在各版本部署間積累過期條目。模式是在檔案頂部定義 `const CACHE_NAME = 'app-v1'`，並在 `activate` 處理器中刪除所有名稱不匹配 `CACHE_NAME` 的快取。當預快取資源列表變更時，遞增版本字串。

**當需要立即控制時，在 `activate` 中使用 `self.clients.claim()`。** 沒有 `clients.claim()`，新啟用的 Service Worker 只控制在啟用後導航的頁面。現有開啟的分頁繼續由前一個 Service Worker 服務，直到它們重新載入。當你需要新 Service Worker 立即接管所有開啟分頁時，呼叫 `clients.claim()`。

**保持 Service Worker 檔案精簡，透過 `importScripts()` 或動態 import 引入大型邏輯。** Service Worker 檔案在瀏覽器每次檢查更新時都會被解析和評估。大型檔案會增加更新檢查的開銷。將大型處理器 registry、fixture 資料（用於 MSW 風格的設定）或複雜的路由表移至在 Service Worker 中 import 的獨立模組，讓主檔案保持為薄薄的入口點，其位元組比較檢查能快速完成。

**在 `window.addEventListener('load', ...)` 回呼中進行 Service Worker 的註冊。** 這確保 Service Worker 的註冊網路請求不會與頁面在初始載入期間的關鍵資源競爭。這是慣用寫法：讓瀏覽器在開始背景的 SW 安裝之前，先完成首次渲染所需的所有資源載入。

**透過 `postMessage` 傳送 auth token，而非寫入 SW 檔案本身。** Service Worker 檔案不是存放密鑰的安全位置。正確的模式是使用 `navigator.serviceWorker.controller.postMessage({ type: 'SET_AUTH_TOKEN', token })` 從頁面傳送 token 給 Service Worker，並將其儲存在 SW scope 中的變數裡。Token 不會在 SW 重啟後持續存在 -- 請在每次頁面載入時重新傳送。

**在 `fetch` 處理器中，在修改請求前仔細對請求 URL 和方法進行過濾。** 無條件攔截所有請求的 `fetch` 處理器會捕獲導航請求、跨 origin 請求和瀏覽器內部請求。在決定是否以修改後的請求回應之前，先測試 `event.request.mode`、`event.request.method` 和 `new URL(event.request.url).origin`。未正確過濾可能會以意想不到的方式破壞導航、字型載入和 analytics 呼叫。安全的常見模式是對任何 origin 不匹配 `self.location.origin` 的請求提前 return，將處理器的效果限制在同 origin 請求。

**刻意測試 Service Worker 更新流程，而不只是正常路徑。** 大多數 Service Worker 的錯誤不是在初次安裝時出現，而是在更新週期中：新 SW 已安裝但舊版仍活躍、新快取填入了與舊版不同的資源名稱，或 `clients.claim()` 導致分頁在工作階段途中切換 Service Worker 版本。撰寫一個明確的測試情境：在一個分頁中開啟 App，部署更新後的 Service Worker 檔案，開啟第二個分頁，並確認第一個分頁在第二個分頁使用新 Service Worker 的同時仍能正常運作。Chrome DevTools Application 面板的「waiting」指示器是可觀察的訊號。

**處理 `error` 事件於註冊 promise 上。** `navigator.serviceWorker.register()` 回傳的 promise 在 Service Worker 腳本解析失敗或拋出頂層錯誤時會 reject。許多環境中未處理的 rejection 是靜默的。附加一個 `.catch()` 處理器以記錄失敗，讓有問題的 Service Worker 部署能夠浮現在錯誤監控中，而非靜默降級離線行為。

**不要在 Service Worker scope 或 Cache Storage 中儲存密鑰。** Cache Storage 可以透過 `caches.open()` 被同 origin 上任何執行的 JavaScript 存取。儲存在快取條目中的密鑰（如私鑰或 refresh token），可以被 origin 上任何其他腳本讀取，包括注入的第三方腳本。對於 auth token，使用透過 `postMessage` 填充的記憶體內 Service Worker 變數，並在伺服器可以強制執行時，依賴 `HttpOnly` cookie 進行工作階段管理。

**在生產環境中，避免在沒有使用者觸發的情況下呼叫 `skipWaiting()`。** 在 `install` 處理器中無條件呼叫 `skipWaiting()` 會導致新 Service Worker 在舊版控制的分頁仍然開啟時啟用。如果新的快取策略與那些分頁已載入的資源不相容，可能在工作階段途中產生破損狀態。建議改為透過 `registration.waiting` 提供「重新載入以更新」的提示。
...
**在 `install` 或 `activate` 處理器中省略了 `waitUntil()`。** 沒有 `waitUntil`，瀏覽器會認為處理器已同步完成，並可能在非同步快取操作完成前終止 Service Worker 執行緒。症狀是間歇性的：有時快取會被正確填入（當瀏覽器恰好讓 SW 執行緒存活足夠長時），有時則不會。始終將 promise 傳給 `waitUntil`，即使它只包裝了單一個 `caches.open()` 呼叫。

**在未檢查 `'serviceWorker' in navigator` 的情況下註冊 Service Worker。** 在不支援 Service Worker 的瀏覽器中呼叫 `navigator.serviceWorker.register()` -- 包括 HTTP 上下文（非 localhost）、部分注重隱私的設定，以及舊版 Safari -- 會拋出執行時期錯誤。始終用功能偵測檢查保護註冊呼叫。
