...
### 為何 Shell 與內容的分離是架構關切，而非效能技巧

App Shell 模型通常被介紹為效能技術，但其更深層的價值是架構上的。將應用程式的結構框架與其動態內容分離，在 Service Worker 與系統其餘部分之間建立了清晰的契約：Service Worker 保證框架的即時可用性；內容層負責自身的新鮮度。

這種關注點分離允許在資源層級而非頁面層級推論離線策略。同一 URL 回應的不同部分可以以不同的快取語義管理——Shell 採用 cache-first 策略，內容採用 network-first——因為它們具有不同的更新頻率和過時後的不同後果。將它們合併為單一快取策略，強制在效能和新鮮度之間做出在邊界劃定正確時根本不存在的假選擇。

從實作角度而言，Shell 邊界的決策不僅影響使用者感知的效能，也影響快取管理的複雜度：邊界越窄，預先快取的有效載荷越小，Service Worker 更新時需要重新下載的資源也越少。每個被納入 Shell 的位元組都會增加初次安裝時間，並在每次 Shell 資源變更時延長更新週期。
...
**應該（SHOULD）** 對 Shell 邊界外擷取的路由級內容套用 NetworkFirst 或 StaleWhileRevalidate 快取策略。頻繁變化的內容——API 回應、使用者動態消息、搜尋結果——必須反映當前資料；以 CacheFirst 策略提供這些內容有展示過時資訊的風險。NetworkFirst 在網路可用時確保新鮮度，僅在使用者離線時才回退至快取；StaleWhileRevalidate 立即提供最後已知的回應，同時在背景更新，適合可接受單次請求延遲的資料。
