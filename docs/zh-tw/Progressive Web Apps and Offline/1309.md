...
團隊必須（MUST）選擇明確的更新策略，而非依賴瀏覽器的預設被動行為——
預設行為可能讓使用者無限期地停留在過時版本上。協作方法——偵測新的
Service Worker 正在等待並顯示使用者可見的重新載入提示——應該（SHOULD）
是預設選擇。`skipWaiting` 禁止（MUST NOT）在沒有理解其對進行中請求的
影響以及新舊快取資源混合版本狀態的情況下被呼叫——不得在未充分評估混合
版本風險的情況下直接配置 `skipWaiting: true`。
