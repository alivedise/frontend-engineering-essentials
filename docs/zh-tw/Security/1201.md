...
Content Security Policy（CSP）是一個 HTTP 回應標頭，用來告知瀏覽器哪些來源可以載入資源、哪些腳本可以在頁面上執行。它是瀏覽器內建的機制，用於限制跨站腳本攻擊（XSS）的破壞範圍：即使攻擊者成功將惡意腳本注入 HTML，正確設定的 CSP 仍可阻止該腳本執行。CSP 無法取代輸入清理（input sanitization）或輸出編碼（output encoding）——那些仍是第一道防線——但當第一道防線失守時，CSP 作為第二道防線可以有效限制損害。

CSP 以單一 HTTP 標頭交付，包含以分號分隔的指令列表。每個指令針對特定的資源類型：`script-src` 控制 JavaScript、`style-src` 控制 CSS、`img-src` 控制圖片、`connect-src` 控制 fetch 和 XMLHttpRequest、`font-src` 控制字型、`frame-ancestors` 防止點擊劫持（clickjacking）、`base-uri` 阻止 base 標籤注入、`form-action` 限制表單提交目標。`default-src` 作為所有未明確列出的資源類型的回退——但值得注意的是，`base-uri` 和 `form-action` **不**繼承 `default-src`，必須始終明確設定。
...
- **絕對不可**在 `script-src` 中使用 `'unsafe-inline'`，除非同時搭配逐請求（per-request）的 nonce 或 hash 使其失效
- **應該**在切換為強制執行模式之前，先部署 `Content-Security-Policy-Report-Only`
- **應該**在 `script-src` 中搭配 nonce 使用 `'strict-dynamic'`，而非維護網域白名單
- **應該**設定 `base-uri 'none'` 與 `frame-ancestors 'none'`（或特定的受信任來源列表）
- **應該**透過 HTTP 回應標頭交付 CSP，而非使用 `<meta>` 標籤
...
設定錯誤的 CSP 會在真實使用者面前悄悄地破壞合法腳本。它會讓你的登入按鈕、分析工具、客服聊天元件、錯誤追蹤工具失效——任何你忘記考慮到的腳本都可能受到影響。你不會事先知道，直到使用者開始回報問題。`Content-Security-Policy-Report-Only` 是唯一安全的新策略部署方式：它在不阻擋任何內容的情況下回報違規，讓你能在真實流量中迭代調整策略，再切換到強制執行模式。略過這個步驟，就是讓團隊在凌晨兩點回滾 CSP 標頭的主要原因。
...
2. **第三方腳本會載入它們自己的依賴項。** 你的分析腳本從 `analytics.example.com` 載入，而它又從 `telemetry.example.com` 載入遙測資料——這個你忘記加入白名單。分析工具因此失效，你又加了 `telemetry.example.com`，白名單就這樣無限增長。
3. **CDN 網址會改變。** 某個供應商從 `cdn.vendor.com` 換到 `assets.vendor.io`。你必須在推送更新之前先修改生產環境的 CSP 標頭。

`'strict-dynamic'` 以不同方式解決這個問題：一旦腳本透過受信任的 nonce 或 hash 被載入，它動態建立並插入 DOM 的所有子腳本也會被信任。你再也不需要將 CDN 網域加入白名單。信任從伺服器的明確背書中傳播，而不是來自一份可能過時的網域列表。

### 為何 `base-uri 'none'` 容易被遺忘，卻代價昂貴

能夠注入 `<base href="https://evil.example.com">` 標籤的攻擊者，可以將頁面上的所有相對 URL 重新導向到他們的伺服器——包括表單的 action 目標、腳本來源、圖片來源。即使你有嚴格的 `script-src` 策略，這個攻擊也有效，因為 base 標籤的重寫發生在 URL 解析之前，而相對 URL 並不像絕對 URL 那樣受到 CSP 來源檢查的約束。`base-uri 'none'` 或 `base-uri 'self'` 可以完全消除這個攻擊向量。只需一個指令、一個值，但它幾乎總是在現實世界的 CSP 策略中缺席。

## 最佳實踐

**正確產生 nonce。** Nonce 必須是密碼學上安全的隨機值，至少 128 位元，且絕對不能跨請求重複使用。在 Node.js 中：

```js
// 建議：randomUUID 在 Node 14.17+ 及所有現代瀏覽器中可用
const nonce = Buffer.from(crypto.randomUUID()).toString('base64');

// 替代方案：randomBytes 讓你控制長度
const nonce = crypto.randomBytes(16).toString('base64');
```

絕對不要從請求資料（時間戳記、使用者 ID、IP 位址）衍生 nonce——任何確定性函數都容易遭受預測攻擊。

**務必透過 HTTP 標頭設定 CSP，而非 `<meta>` 標籤。** `<meta http-equiv="Content-Security-Policy">` 的形式存在，但有幾個重要限制：
...
**從 `default-src 'none'` 開始。** 從所有資源都被封鎖開始，只添加應用程式確實需要的資源類型。這迫使你思考每一種資源類型。從 `'self'` 或寬鬆的預設值開始的策略，往往會默默地累積越來越多的例外。

**使用 csp-evaluator.withgoogle.com 進行測試。** 貼入你的策略並檢查高嚴重性警告。常見問題：白名單網域有 JSONP endpoint、沒有 nonce 的 `unsafe-inline`、缺少 `object-src`、缺少 `base-uri`。

**務必先部署 report-only 模式。** 即使是對已在強制執行模式的策略進行更新，也應先在 staging 環境切換到 report-only，然後在生產環境部署 report-only，收集至少 48 小時的資料，最後再切換到強制執行模式。
...
5. **在生產環境使用 report-only** — 將 report-only 標頭推送至生產環境。在不同瀏覽器和地區監控違規端點至少 48 小時。
...
這個順序對面向使用者的應用程式而言不是可選的。直接跳到強制執行模式，就是在尖峰流量時發現 Safari 使用者付款流程失效的主要原因。
...
### 2. 在 Next.js middleware 中產生 nonce 並設定 CSP 標頭
...
Next.js 從 middleware 設定的請求標頭中讀取 nonce，並在 SSR 過程中將其套用到它渲染的 `<script>` 標籤。
...
瀏覽器在發生違規時，會以 JSON 格式 POST 到該端點：
...
Hash 必須與腳本內容的確切位元組相符，包括空白字元。任何字元差異——即使是結尾的換行符號——都會使 hash 失效，腳本將被封鎖。

## 常見錯誤

**當腳本在 CSP 下失效時，以添加 `'unsafe-inline'` 作為「修復」方案。**
這是最常見、也是破壞性最大的錯誤。`'unsafe-inline'` 會讓所有沒有 nonce 的 CSP 失效，讓你回到零 XSS 防護的狀態。正確的修復方式是將內聯腳本移到外部檔案，或為其添加 nonce。如果腳本來自第三方且無法接收 nonce，`'strict-dynamic'` 通常可以處理它的入口點有 nonce 後動態載入的依賴項。
...
Nonce 必須在伺服器端逐請求產生。如果你的建置步驟生成以靜態資源形式提供的 HTML 檔案（沒有伺服器端渲染），在建置時嵌入的 nonce 對每個訪客都是相同的——實際上是一個常數，而非 nonce。對靜態資源使用基於 hash 的 CSP，或者切換到 SSR 來處理需要基於 nonce 策略的頁面。

**在沒有 report-only 階段的情況下，直接以強制執行模式部署 CSP。**
你會從使用者回報中發現你破壞了什麼，而不是從自己的測試中發現。任何你沒有考慮到的合法資源——第三方元件、瀏覽器擴充功能腳本、動態注入的分析標籤——都會在生產環境中靜默地失效。
