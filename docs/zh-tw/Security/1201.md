...
**必須（MUST）正確產生 nonce。** Nonce 必須（MUST）是密碼學上安全的隨機值，至少 128 位元，且絕對不能跨請求重複使用。使用 `crypto.randomUUID()`（Node 14.17+）或 `crypto.randomBytes(16)` 轉換為 base64。絕對不要從請求資料（時間戳記、使用者 ID、IP 位址）衍生 nonce——任何確定性函數都容易遭受預測攻擊。
