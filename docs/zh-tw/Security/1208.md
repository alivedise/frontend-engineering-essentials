...
SRI 的核心安全屬性在於雜湊值是由開發者在建置時計算並嵌入 HTML 中的，
而非由 CDN 或其他外部方提供。這意味著即使 CDN 基礎設施遭到入侵，
攻擊者也無法在不使瀏覽器偵測到不符的情況下替換資源位元組。信任錨點
從 CDN 的身份（透過 TLS 憑證驗證）轉移到資源內容（透過密碼雜湊驗證），
這是一個更強大的安全保證，因為它不依賴於 CDN 供應商的安全姿態。

SRI 解決的是供應鏈完整性問題中最難以靜態分析的部分：執行期資源替換。
即使應用程式程式碼本身沒有漏洞，從外部 CDN 載入的第三方資源仍可成為攻
擊向量。SRI 確保的不是來源的可信度，而是位元組層級的一致性——這是確保
外部資源自審查以來未被修改的最直接方式。
