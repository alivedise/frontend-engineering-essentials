...
**必須（MUST）在伺服器端維護 origin 白名單。** 必須（MUST）將受信任的 origin 集合寫死在程式碼中，只有當傳入的 `Origin` 在白名單內時才設定 `Access-Control-Allow-Origin`。不要用 regex 推導白名單——以 regex 做比對容易被繞過（例如 `https://example.com.evil.com` 匹配 `.*example\.com`）。
