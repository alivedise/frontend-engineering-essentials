...
開放重新導向漏洞在攻擊者的工具箱中扮演著放大器的角色。攻擊者本身不能直
接危害使用者，但可以利用應用程式的聲譽讓惡意連結看起來可信。當受害者看
到一個帶有合法域名的 URL 時，他們更容易信任連結，儘管最終目標是攻擊者控
制的網站。防止開放重新導向就是拒絕讓應用程式的可信度被武器化。

允許清單與封鎖清單之間的選擇不僅是風格偏好的問題，而是反映了系統安全
姿態的根本差異。允許清單編碼應用程式所信任的內容；封鎖清單編碼應用程式
所知道的危險內容。由於危險 URL 輸入的空間是無界的，且隨著新繞過技術的
發現而不斷演進，封鎖清單需要持續維護才能保持有效。相比之下，允許清單
只要受信任目標集合是正確的，就能保持穩健——未知輸入預設被拒絕。
