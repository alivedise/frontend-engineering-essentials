...
**在提交至 HSTS preload list 之前，確認所有子網域都已啟用 HTTPS。** 使用 hstspreload.org 的網域檢查器與 Mozilla Observatory 確認每個子網域都可透過 HTTPS 到達、回傳有效的憑證，且不在任何環節透過 HTTP 進行重新導向。在任何子網域仍提供 HTTP 的情況下 preload 網域，會導致該子網域對所有使用最新瀏覽器的使用者不可用，且無法快速復原。
...
**對 session cookie 設定 `SameSite=Strict`。** `SameSite=None; Secure` 僅用於明確的跨站嵌入使用案例——付款 iframe、聯合登入 widget、session 必須在跨站頂層導航中存續的 OAuth 回調流程。
...
`SameSite=None` 需要同時設定 `Secure`。若沒有 `Secure`，瀏覽器會完全拒絕該 cookie（Chrome、Firefox、Safari 都強制執行此規則）。
...
- [FEE-1201：內容安全政策（CSP）](./1201.md) — CSP 與這些標頭並列；`frame-ancestors` 取代 `X-Frame-Options`
- [FEE-1202：身份驗證與 Token 儲存](./1202.md) — session cookie 的 `Secure`、`HttpOnly`、`SameSite` 屬性
- [FEE-1204：跨來源資源共享（CORS）與跨網站請求偽造（CSRF）](./1204.md) — `SameSite` cookie 屬性用於 CSRF 防禦
