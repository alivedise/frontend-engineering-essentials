...
- **SHOULD** 在所有正式網域上設定 `Strict-Transport-Security: max-age=31536000; includeSubDomains`
- **MUST** 在所有回應中設定 `X-Content-Type-Options: nosniff`
- **SHOULD** 設定 `Referrer-Policy: strict-origin-when-cross-origin`
- **SHOULD** 透過 `Permissions-Policy` 限制未使用的瀏覽器 API
- **MUST** 在所有 session cookie 上設定 `Secure` 與 `HttpOnly`
- **SHOULD** 在 session cookie 上設定 `SameSite=Strict`；需要跨站導航進入 session 的應用程式則使用 `SameSite=Lax`

MUST 規則代表最低可接受的基準——省略 `X-Content-Type-Options` 或 cookie 的 `Secure`/`HttpOnly` 屬性是有文件記錄的已知漏洞。SHOULD 規則代表所有正式環境部署的最佳實踐，應視為必要，除非有特定且有文件記錄的例外原因（例如，某個尚無法提供 HTTPS 的舊版子網域是延後 HSTS preloading 的合理例外，但不是完全省略 HSTS 的理由）。
...
**在提交至 HSTS preload list 之前，確認所有子網域都已啟用 HTTPS。** 使用 hstspreload.org 的網域檢查器與 Mozilla Observatory 確認每個子網域都可透過 HTTPS 到達、回傳有效的憑證，且不在任何環節透過 HTTP 進行重新導向。在任何子網域仍提供 HTTP 的情況下 preload 網域，會導致該子網域對所有使用最新瀏覽器的使用者不可用，且無法快速復原。在評估子網域時，不要忘記內部工具、CI/CD 儀表板、監控端點，以及合作夥伴整合的 hostname——頂層網域下的任何子網域都受到 `includeSubDomains` 影響。

**在 CDN 或反向代理層設定安全標頭。** 在應用程式程式碼中設定標頭，意味著只有當應用程式伺服器回應時標頭才會存在。Nginx 的錯誤頁面、快取靜態資源的 CDN 回應，以及重新導向回應，都會繞過應用程式程式碼。在基礎設施層設定標頭——Cloudflare transform rules、Vercel 的 `vercel.json` `headers` 設定、或 Nginx 的 `add_header` 指令——確保每個回應，無論來源為何，都攜帶正確的標頭。

**使用 securityheaders.com 測試標頭。** 這個掃描器會為你的標頭評分、識別缺少的條目，並標記設定錯誤的值。在正式環境和發布前的測試環境都執行一次。

**審計第三方 script 以校準 `Permissions-Policy`。** 在設定全面拒絕政策之前，先列出每個頁面上哪些第三方 script 實際需要 camera、microphone、geolocation 或 payment API。部分合法的 script——視訊通話 widget、基於地理位置的個人化、付款收集流程——確實需要這些 API，若政策拒絕它們而 iframe 上沒有對應的 `allow` 屬性，這些功能將靜默失敗。正確的做法是：在頂層政策中預設拒絕所有功能，再透過 iframe 上的 `allow` 屬性向特定 iframe 來源重新授予特定功能（`<iframe allow="payment 'self' https://checkout.stripe.com">`）。這提供最細緻的控制，並在標記中明確記錄哪些 frame 被授予哪些能力。

**在 cookie 上使用 `Max-Age` 而非 `Expires`。** `Max-Age` 是以瀏覽器收到回應的時刻為基準的相對秒數。`Expires` 是絕對日期時間，其行為取決於客戶端時鐘是否準確。當兩者同時存在時，`Max-Age` 優先於 `Expires`。使用 `Max-Age` 以獲得可預測的過期行為。

**除非明確需要子網域共用，否則避免使用 `Domain` cookie 屬性。** 若確實需要子網域共用，優先在需要 cookie 的子網域上設定，而非在頂層網域設定。

**根據應用程式是否使用外部重新導向，選擇適當的 `SameSite` 值。** `SameSite=Strict` 是最嚴格的設定：session cookie 永遠不會隨任何跨站請求發送，包括來自外部來源的頂層導航。這對於完全自足、沒有外部 identity provider 的應用程式是正確的選擇。

OAuth 和 SSO 重新導向流程在 `SameSite=Strict` 下會失敗。當授權伺服器在登入後將使用者重新導向回你的應用程式時，這個重新導向是一個來自授權伺服器網域的頂層 GET 導航。由於這是一個跨站導航，`SameSite=Strict` 會抑制 session cookie——應用程式收到重新導向但找不到 session，登入看起來就會失敗。`SameSite=Lax` 在頂層 GET 導航期間保留 cookie，同時仍然在跨站子資源請求和表單 POST 時封鎖它。如果應用程式使用任何 SSO、OAuth、SAML 或外部重新導向流程，`SameSite=Lax` 是正確的值。`SameSite=None; Secure` 僅用於明確的跨站嵌入使用案例——付款 iframe、聯合登入 widget——即 cookie 必須從第三方 frame 上下文發送的情境。
