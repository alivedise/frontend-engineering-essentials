...
**必須（MUST）根據應用程式是否使用外部重新導向，選擇適當的 `SameSite` 值。** `SameSite=Strict` 是最嚴格的設定：session cookie 永遠不會隨任何跨站請求發送，包括來自外部來源的頂層導航。這對於完全自足、沒有外部 identity provider 的應用程式是正確的選擇。
