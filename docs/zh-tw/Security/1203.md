...
**應該（SHOULD）使用 `RETURN_DOM_FRAGMENT` 搭配 `appendChild` 取代 `innerHTML` 賦值以獲得額外的安全性。** `DOMPurify.sanitize(input, { RETURN_DOM_FRAGMENT: true })` 回傳一個可附加到目標元素的 `DocumentFragment`。應該（SHOULD）優先使用此模式，以完全避免 `innerHTML` 賦值——那是 HTML 解析時執行 script 的機制。
...
**禁止（MUST NOT）將 `textContent` 和 `innerHTML` 互換使用。** `textContent` 賦值為文字值——對任何輸入都是安全的。`innerHTML` 解析 HTML 標記——會執行事件處理器和 script。為了「效能」或「避免雙重跳脫」而從 `textContent` 切換到 `innerHTML`，會引入 XSS。當值來自使用者輸入時，絕不可（MUST NOT）以 `innerHTML` 替代 `textContent`——它們是具有根本不同安全屬性的不同 API。
