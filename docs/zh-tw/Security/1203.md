...
跨網站指令碼（XSS）是網頁應用程式中最普遍的用戶端漏洞類型。取得 XSS 執行權的攻擊者可在受害者瀏覽器中、以應用程式的 origin 執行任意 JavaScript：讀取 localStorage 中的 token、竊取非 `httpOnly` 的 cookie、偽造已驗證的請求、劫持 session、甚至改寫整個頁面。攻擊面不只限於表單和留言框——任何從攻擊者控制的資料到 DOM sink 的路徑都是潛在的 XSS 向量。

React 和 Vue 等框架透過自動跳脫文字節點，讓許多常見模式變得更安全。但它們並沒有消除 XSS——只是將風險集中在特定位置。剩餘的風險集中在開發者為了渲染 rich text 而使用的逃生艙口：React 的 `dangerouslySetInnerHTML`、Vue 的 `v-html`，以及原生 JavaScript 中直接的 `innerHTML` 賦值。每一處使用這些 API 卻未事先清潔（sanitize）輸入的地方，都是一個作用中的 XSS 漏洞。
...
- **MUST** 在將所有使用者產生的 HTML 傳入 `dangerouslySetInnerHTML`、`v-html` 或 `innerHTML` 之前，必須先以 DOMPurify 進行 sanitize
- **MUST NOT** 在任何情況下，將未經 sanitize 的輸入傳入 `dangerouslySetInnerHTML` 或 `v-html`
- **SHOULD** 在將值賦給 `href`、`src` 或 `action` 屬性之前，驗證 URL 協定——允許清單應包含 `https:`、`http:`、`mailto:`；拒絕 `javascript:` 和 `data:`
- **SHOULD** 從 report-only 模式開始，透過 CSP `require-trusted-types-for 'script'` 採用 Trusted Types
...
逃生艙口——React 中的 `dangerouslySetInnerHTML` 以及 Vue 中的 `v-html`——完全繞過了這層保護。API 名稱中的「dangerously」是任何前端 API 中最直白的警告：這個詞並非裝飾性的。對未經 sanitize 的輸入使用它，等同於直接寫 `element.innerHTML = userInput`。Vue 的 `v-html` 帶有相同的風險，只是名稱沒有那麼明顯。
...
第三方函式庫讓問題更加複雜。Rich text 編輯器、Markdown 渲染器、圖表函式庫——任何一個都可能在內部使用 `innerHTML` 而未加以說明。框架的文字跳脫只適用於你透過 JSX 或模板語法渲染的內容，無法保護透過函式庫內部 DOM 操作繞過的內容。
...
輸入 sanitization 可能被繞過。直接的資料庫匯入、管理工具、遷移腳本、第三方整合——資料可以透過跳過輸入驗證層的途徑進入系統。

儲存的資料應只被視為資料，而非安全的 HTML。在渲染點進行 sanitize，在你確切知道需要哪種跳脫的輸出情境中進行。

## 最佳實踐

**以應用程式所需的最嚴格允許清單設定 DOMPurify。** 如果功能只需要粗體、斜體和連結，就準確地設定：`ALLOWED_TAGS: ['b', 'i', 'a', 'p', 'br']` 和 `ALLOWED_ATTR: ['href']`。每一個額外允許的標籤和屬性都擴大了攻擊面。不要從寬鬆的設定開始再逐步限制——從限制開始，依需要擴展。

**使用 `RETURN_DOM_FRAGMENT` 搭配 `appendChild` 取代 `innerHTML` 賦值以獲得額外的安全性。** `DOMPurify.sanitize(input, { RETURN_DOM_FRAGMENT: true })` 回傳一個可附加到目標元素的 `DocumentFragment`，完全避免了 `innerHTML` 賦值——那是 HTML 解析時執行 script 的機制。

**在將值賦給 `href`、`src` 或 `action` 之前，明確驗證 URL 協定。** 使用 `new URL(userUrl)` 解析 URL，並對照 `['https:', 'http:', 'mailto:']` 的允許清單檢查 `.protocol`。DOMPurify 的預設設定會去除 `href` 屬性中的 `javascript:` URL，但在完全未通過 DOMPurify 的情況下，對使用點的明確驗證是一個獨立的防禦層。

**先以 report-only 模式採用 Trusted Types。** 這遵循與 CSP 相同的推出模式。設定 `Content-Security-Policy-Report-Only: require-trusted-types-for 'script'` 並收集違規報告。每份報告都識別出一個需要路由經過政策的不安全 DOM sink。只有在正式環境的違規率降到零後，才切換到強制模式。

**建立單一 Trusted Types 政策並將所有 HTML 路由經過它。** 包裝 DOMPurify 的單一命名政策創造了一個單一控制點——你可以審計、更新和測試一個函式。分散在程式碼庫各處的多個政策，會製造與多個 sanitization 實作相同的維護問題。

**不要將 `textContent` 和 `innerHTML` 互換使用。** `textContent` 賦值為文字值——對任何輸入都是安全的。`innerHTML` 解析 HTML 標記——會執行事件處理器和 script。為了「效能」或「避免雙重跳脫」而從 `textContent` 切換到 `innerHTML`，會引入 XSS。它們是用於不同目的、具有根本不同安全屬性的 API。
...
**假設 `textContent` 和 `innerHTML` 可以互換。** 一個常見的效能優化嘗試：將 `textContent = value` 替換為 `innerHTML = value` 以避免「雙重跳脫」或感知到的額外負擔。`textContent` 設定文字值，對任何輸入都是安全的。`innerHTML` 解析 HTML 標記並執行事件處理器和 script。它們是具有根本不同安全屬性的不同 API。當值來自使用者輸入時，沒有任何情境下用 `innerHTML` 替代 `textContent` 是正確的。

**忘記 `javascript:` URL 是獨立的 XSS 向量。** `href` 或 `src` 屬性中的 `javascript:` 協定，在元素被觸發時（連結被點擊、圖片載入失敗）會執行 JavaScript。`<a href="javascript:alert(document.cookie)">點我</a>` 是合法的 HTML，它在點擊時執行，完全獨立於 `<script>` 標籤。HTML sanitizer 在 `href` 屬性中處理了這個問題，但從使用者輸入構建屬性的自訂 URL 處理程式碼可能沒有。在將值賦給 `href`、`src` 或 `action` 之前，務必明確驗證 URL 協定。

**使用帶有不安全 sink 的第三方函式庫而未加以審計。** WYSIWYG 編輯器、Markdown 渲染器、圖表函式庫——任何接受使用者提供內容並將其寫入 DOM 的函式庫，都可能在內部使用 `innerHTML`。檢查函式庫是否對其輸入進行 sanitize。如果沒有，在傳入函式庫之前進行 sanitize。如果它提供 sanitizer 設定選項，請使用它。
