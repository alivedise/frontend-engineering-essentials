...
公鑰演算法、對稱加密，以及金鑰衍生函式。這些基元以原生程式碼實作，
而非以 JavaScript 程式庫實作，並在二進位資料而非字串上運算。與通用的
JavaScript 加密實作相比，結果是執行速度更快，攻擊面也更小。

然而，擁有密碼學基元的存取權，並不意味著在瀏覽器中進行加密總是合適的。
密碼學很容易被以看似安全實則不然的方式誤用。一個根本的約束貫穿始終：
任何存在於瀏覽器端 JavaScript 中的秘密，都可以被該 JavaScript 環境讀取。
若 JavaScript 遭到入侵，無論是透過 XSS、惡意瀏覽器擴充功能，或被入侵的
CDN 指令碼，秘密都會暴露給攻擊者。這並不使瀏覽器加密毫無用處；它定義了
...
在同一頁面中執行的 JavaScript，包括注入的 XSS 有效載荷，都無法透過讀取
...
`crypto.subtle` 操作是非同步的，回傳 Promise。這讓瀏覽器能在主執行緒之外
執行密碼學操作而不阻塞 UI。對於大型資料緩衝區的操作（加密大型檔案、
生成大型金鑰），這對於回應性很重要。
...
執行的每次加密操作必須是唯一的。使用相同金鑰重複使用 IV，會讓攻擊者能從
兩個以相同金鑰-IV 組合加密的密文中恢復明文和驗證金鑰。IV 不是秘密；它與密文
