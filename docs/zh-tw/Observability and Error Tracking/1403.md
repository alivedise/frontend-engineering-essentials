...
- 應該（SHOULD）使用具備一致欄位的結構化 JSON 日誌（`timestamp`、`level`、`message`、`userId`、`sessionId`、`traceId`）——而非 `console.log`。
- 不應該（SHOULD NOT）記錄個人識別資訊（PII）：不記錄電子郵件地址、全名、密碼、支付卡資料或原始用戶輸入。
- 應該（SHOULD）在頁面卸載時使用 `navigator.sendBeacon` 刷新日誌——而非 `fetch` 或 XHR，因為它們會在導航時被取消。
- 應該（SHOULD）將日誌事件在記憶體中批次處理，並依定時間隔（例如每 5 秒）或緩衝區達到大小閾值時刷新——而非每個日誌事件觸發一個 HTTP 請求。
- 應該（SHOULD）以可設定的比率（例如 5–10%）對高流量日誌事件（詳細的用戶互動追蹤、導航事件）進行取樣，以避免日誌平台超載。
- 應該（SHOULD）使用 `visibilitychange` 搭配 `document.visibilityState === 'hidden'` 作為頁面卸載訊號——這比 `beforeunload` 更可靠，尤其在行動瀏覽器上。
- 應該（SHOULD）在伺服器端（Node.js / SSR）使用 Pino 或 Winston 進行日誌記錄，而非 `console.log`。
...
日誌記錄中的電子郵件地址、姓名和任何其他 PII，會產生 GDPR 資料保留義務，並擴大日誌平台資料外洩的影響範圍。永遠只記錄不透明識別碼。
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden') {
    fetch('/api/logs', { method: 'POST', body: JSON.stringify(buffer) }); // 可能被取消
  }
});

// 正確：sendBeacon 是射後不理的，在頁面卸載後仍能存活
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden') {
...
在每次呼叫的日誌方法內呼叫 `fetch('/api/logs', ...)`，而一次用戶互動期間可能呼叫 10 次，這是一個網路反模式。它產生 10 個連續的 HTTP 請求，增加延遲和伺服器負載。應緩衝事件，並在定時間隔或達到閾值時刷新。

### 在生產環境使用 `console.log`

`console.log` 的輸出在生產瀏覽器環境中不會被傳送到任何地方。它出現在用戶的 DevTools 中（如果他們有開的話），然後消失。它無法被查詢、聚合或用於告警。將生產環境的日誌記錄替換為結構化日誌記錄器，將記錄傳送到日誌攝取端點。

### 在生產環境使用 `debug` 層級記錄

Debug 層級的日誌包含內部狀態、元件生命週期事件和 API 請求細節，在開發時很有用，但在生產環境中會產生龐大的量。在生產環境中將最低日誌層級設定為 `info` 或 `warn`，以避免不必要的儲存成本和日誌平台中的雜訊。

### 使用 `beforeunload` 而非 `visibilitychange`

`beforeunload` 在行動瀏覽器上不可靠（iOS Safari 和許多 Android 瀏覽器在分頁切換或應用程式背景化時不會一致地觸發它）。使用 `visibilitychange` 搭配 `document.visibilityState === 'hidden'` 作為權威的頁面卸載訊號。它在行動裝置瀏覽器背景化時觸發，在桌面端分頁隱藏或關閉時也會觸發。

### 各日誌記錄缺少一致的欄位

一筆付款錯誤的日誌記錄包含 `userId`，而一筆導航事件的日誌記錄不包含，這使得跨記錄關聯變得不可能。在日誌記錄器實作中建立所有日誌記錄的基準綱要並強制執行——而非在呼叫端。`userId`、`sessionId` 和 `traceId` 欄位應在日誌記錄器設定中設定一次，並自動注入每筆記錄中。

## 相關 FEE

- [FEE-1400: Observability & Error Tracking Overview](./1400.md) — 類別背景與前端可觀測性的三大支柱
- [FEE-1401: Error Tracking with Sentry](./1401.md) — 用 Sentry 捕獲 JavaScript 例外；Sentry 與結構化日誌是互補的——Sentry 用於錯誤追蹤，結構化日誌用於稽核軌跡和業務事件
- [FEE-1404: OpenTelemetry Browser Tracing](./1404.md) — Trace ID 源自分散式追蹤層；在每筆日誌記錄中包含 `traceId` 以實現日誌與追蹤的關聯
- [FEE-1407: LogRocket Session Replay](./1407.md) — LogRocket 結合工作階段回放與日誌捕獲；結構化日誌補充回放，有助於除錯複雜的狀態問題
