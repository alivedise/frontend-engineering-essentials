...
**不應該（SHOULD NOT）僅依賴自動插樁作為可觀測性覆蓋率的唯一來源。** 自動插樁能捕獲 fetch 呼叫、XHR 請求和頁面載入——這些事件都有可觀察的瀏覽器 API 掛載點。它無法推斷操作的業務語意：「用戶開始結帳」、「購物車已更新」、「文件上傳已完成」。手動 span 增加了自動化工具無法推斷的語意上下文（操作名稱、用戶意圖、業務實體 ID），這使 APM 瀑布圖在除錯期間具有可解讀性。

**應該（SHOULD）為業務上下文附加 span 屬性**——`user.id`、`order.id`、`session.id`、功能旗標狀態——不記錄個人識別資訊（PII）。沒有業務上下文屬性的追蹤顯示「`POST /orders` 花了 2.3 秒」但不顯示是哪個用戶、哪個訂單、哪個功能旗標變體。附加業務屬性後，工程師可以按 `order.id` 篩選、按用戶區段分組，並立即定位受影響的實體，而無需反向關聯多個系統的日誌。
...
- [Grafana Tempo — OpenTelemetry](https://grafana.com/docs/tempo/v3.0.x/visualize-traces/)
