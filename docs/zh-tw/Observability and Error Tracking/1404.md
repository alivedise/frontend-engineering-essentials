...
async function handleSubmit() {
  ...
  try {
    ...
    const existing = performance.getEntriesByName('checkout-submit-duration')
    if (!existing.length) {
      performance.measure('checkout-submit-duration', 'checkout-submit-start', 'checkout-submit-end')
    }

    const entries = performance.getEntriesByName('checkout-submit-duration')
...
```ts
// 錯誤：如果拋出例外，span.end() 永遠不會被呼叫
const span = tracer.startSpan('my-operation')
const result = await riskyOperation()  // 可能拋出
span.end()  // 拋出例外時永遠不會到達這裡
...
永遠不結束的 span 會被 `BatchSpanProcessor` 無限期地保存在記憶體中，永遠不會被匯出。在長時間運行的應用程式中，洩漏的 span 會累積，最終造成記憶體壓力。

### 向第三方域名傳播 `traceparent`

```ts
// 錯誤：向所有域名傳播追蹤上下文
getWebAutoInstrumentations({
  '@opentelemetry/instrumentation-fetch': {
    propagateTraceHeaderCorsUrls: [/.*/],  // 匹配所有內容
  },
})
...
向 Stripe、Google Analytics 或其他第三方 API 的請求中注入 `traceparent`，會將你的追蹤 ID 傳送給外部方，並且如果這些服務不接受該標頭，可能會導致 CORS 預檢請求失敗。
...
```ts
// 錯誤：只監控超過 200ms 的任務，會遺漏累積導致 INP 差的任務
if (entry.duration > 200) { report(entry) }

// 正確：Long Tasks API 已經使用了規範中的 50ms 閾值
// 在客戶端報告所有長任務；在 APM 後端按嚴重性篩選，而非在客戶端
if (entry.duration > 50) { report(entry) }
```

50ms 閾值是規範定義的「正常」任務和長任務之間的邊界。51ms 到 200ms 之間的任務是常見的 INP 貢獻者，不應被靜默丟棄。

### 在瀏覽器套件中暴露 OTLP 收集器憑證
...
瀏覽器套件內容是公開可讀的。套件中的 API 金鑰實際上是公開的。改為透過你自己的後端代理 OTLP，並使用環境變數進行伺服器到收集器的驗證：
...
- [FEE-1400: Observability & Error Tracking Overview](./1400.md) — 類別背景；追蹤是前端可觀測性三大支柱之一，與錯誤追蹤和日誌記錄並列
- [FEE-1402: Core Web Vitals — INP and Long Tasks](./1402.md) — INP 直接由主執行緒上的長任務引起；本文提供了在生產環境偵測長任務的插樁方法
- [FEE-1403: Logging & Structured Logging](./1403.md) — 在每條結構化日誌記錄中包含 `traceId`，以在 APM 平台中實現日誌與追蹤的關聯
- [FEE-1406: Alerting on Trace Data](./1406.md) — 定義基於追蹤衍生指標的告警閾值：p95 結帳延遲、長任務頻率、按 span 名稱統計的錯誤率
