...
- 應該（SHOULD）使用 `web-vitals` 函式庫監測 Core Web Vitals，並回報至分析端點。
- 應該（SHOULD）在生產環境中監控 p75 LCP 和 INP，而非僅依賴 Lighthouse 分數。
- 不應該（SHOULD NOT）對 Web Vitals 取平均值——應使用百分位數（p75 為標準）。
- 應該（SHOULD）在大規模流量下使用取樣策略（1–5%），以避免分析成本過高。
- 應該（SHOULD）依裝置類型、連線類型和頁面範本對 RUM 資料進行分群，以發掘可行的改善方向。
- 應該（SHOULD）將 CrUX 和 RUM 的現場資料視為生產效能的真實來源；將 Lighthouse 視為開發階段的反饋工具。

## 設計思維

### 為何 RUM 在生產監控上優於 Lighthouse

Lighthouse 是一種合成監控工具：它在受控環境（Chrome DevTools Protocol、特定的 CPU 節流等級、特定的網路節流預設）中，於單一裝置上執行一次有腳本的頁面載入，在單一時間點產生結果。它具有確定性和可重現性，因此非常適合作為 CI 品質門檻和開發期間的開發者回饋工具。
...
`average(LCP)` 不是有效的效能指標。平均值 2.0s 可能代表雙峰分佈，其中一半用戶 LCP 為 0.5s，另一半為 3.5s。以 p75 作為主要操作指標。大多數分析平台原生支援百分位數聚合。在 ClickHouse 中：`quantile(0.75)(value)`。在 BigQuery 中：`APPROX_QUANTILES(value, 100)[OFFSET(75)]`。

### 將 Lighthouse 視為生產信號

Lighthouse 是開發和 CI 工具。其分數反映在受控條件下單一裝置上的效能。關於基礎設施、快取和最佳化的生產決策必須由現場資料驅動。如果 Lighthouse 顯示 92，但 p75 現場 LCP 為 4.5s，Lighthouse 分數具有誤導性——現場資料才是真實。

### 量測 FID 而非 INP

FID 已於 2024 年 3 月作為 Core Web Vital 被棄用，由 INP 取代。新的監測儀器應使用 `web-vitals` 函式庫中的 `onINP`。舊儀表板中的 FID 資料應僅視為歷史資料。如果你的團隊效能儀表板仍將 FID 作為主要指標，請遷移到 INP。

### 使用 `fetch` 而非 `sendBeacon` 回報指標

`fetch` 請求可能在用戶導航離開頁面時被取消。在工作階段後期收集的指標事件（特別是 INP，它在用戶互動時觸發）如果回報呼叫使用 `fetch`，就有遺失的風險。所有 RUM 事件提交都應使用 `navigator.sendBeacon`。`sendBeacon` 是一個即發即忘的 POST，瀏覽器會排隊處理並在頁面卸載時發送。

### 按指標取樣而非按工作階段取樣

對每個指標事件獨立做出取樣決策，會產生不一致的資料集。同一個工作階段的 LCP 事件和 INP 事件可能以不同的比率回報，使關聯它們成為不可能。每個工作階段使用 `sessionStorage` 做一次取樣決策，並將其統一應用於該工作階段的所有指標。

### 不對 RUM 資料進行分群

整個網站的單一 p75 LCP 隱藏了關鍵模式。主要在桌機上使用的頁面範本可能有優秀的 LCP，而同一範本在行動裝置上可能 LCP 很差。產品頁面可能很快，而結帳頁面可能很慢。至少按以下維度對 RUM 資料進行分群：頁面 URL 或範本、裝置類別（行動裝置 vs. 桌機）和瀏覽器。大多數效能退步在出現於聚合數字之前，在特定分群中就已可見。
...
- [Datadog RUM 文件](https://docs.datadoghq.com/real_user_monitoring/browser/)
