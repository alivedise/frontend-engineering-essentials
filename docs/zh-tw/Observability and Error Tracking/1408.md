...
瀏覽器發出的每個屬於使用者互動一部分的 HTTP 請求，應該（SHOULD）攜帶
`traceparent` 標頭，將其連結至瀏覽器建立的根 Span。後端服務必須（MUST）
配置為讀取並傳遞此標頭，使前後端的 Span 在可觀測性後端中出現在相同的
Trace ID 之下。手動建立 Span 應該（SHOULD）僅限於具有實質意義的使用者互
動——頁面導航、表單提交以及明確的資料載入操作——而非每個 DOM 事件，後者
只會產生追蹤雜訊，不具可觀測性價值。

取樣率必須（MUST）在 SDK 層級配置，在任何 Span 資料離開瀏覽器之前生效。
將每個使用者 Session 的所有 Trace 都傳送到後端，會壓垮資料攝取管道並
大幅增加成本。對於高流量應用程式，1% 到 10% 的取樣率是合適的選擇。
100% 取樣僅適用於開發環境，或需要針對特定 Session 進行目標除錯的情況。
