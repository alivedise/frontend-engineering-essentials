...
**應該（SHOULD）在捕獲每個分析事件時，標記所有活躍的功能旗標變體。** 若沒有每個事件的旗標標記，A/B 實驗分析就無法分割群組並在相同指標上比較 treatment 與控制組。使用 PostHog 的團隊可受益於每個捕獲事件的自動旗標標記；使用其他分析平台的團隊必須（MUST）在每個事件呼叫中明確添加旗標屬性。
