...
**必須（MUST）在執行任何特定於變體的程式碼之前，將活躍的功能旗標狀態附加到 Sentry 錯誤作用域。** 若旗標上下文在變體渲染之後才設定，則渲染期間拋出的任何錯誤都不會攜帶旗標標籤，調查將需要手動重現。團隊必須（MUST）在旗標評估時呼叫 `Sentry.setTag('flag.flagName', variant)` 或 `Sentry.setContext('feature_flags', flagMap)`，在對變體值進行分支之前完成此操作。

**必須（MUST）在啟動功能旗標之前，預先指定實驗假設、主要指標、最小可檢測效果（MDE）和所需樣本量。** 在達到預先指定的樣本量之前評估結果，會使假陽性率遠高於名義上 5% 的顯著性門檻（p-hacking）。團隊必須（MUST）在共享實驗簡報中記錄這些參數，並且禁止（MUST NOT）因結果看起來有利就提前停止實驗——只能在預先指定的樣本量時評估一次。

**必須（MUST）在啟動前為每個 A/B 實驗定義護欄指標——包括 p75 LCP 和 p75 INP——而非在觀察結果之後才定義。** 一個改善主要轉換指標但讓 LCP 退步 1.3 秒的實驗並非凈收益；它在主要指標未能捕捉的維度上傷害了使用者。團隊必須（MUST）將護欄突破視為阻止推出的條件：儘管護欄被突破仍推出實驗，會為「護欄是可選的」建立先例，使其失去意義。

**應該（SHOULD）在捕獲每個分析事件時，標記所有活躍的功能旗標變體。** 若沒有每個事件的旗標標記，A/B 實驗分析就無法分割群組並在相同指標上比較 treatment 與控制組。使用 PostHog 的團隊可受益於每個捕獲事件的自動旗標標記；使用其他分析平台的團隊應該（SHOULD）在每個事件呼叫中明確添加旗標屬性。

**應該（SHOULD）為每個實驗使用穩定且唯一的旗標鍵——禁止（MUST NOT）在連續實驗中重用旗標鍵。** 在相同鍵下參與早期實驗的使用者，其歷史事件附帶了舊的旗標分配。重用鍵會污染新實驗的群組成員資格。團隊應該（SHOULD）使用描述性的版本化旗標鍵（例如 `checkout-redesign-2026-q2`），並在實驗結束時退役舊鍵。

**應該（SHOULD）使用穩定的工作階段或使用者識別碼為回訪使用者持久化旗標分配。** 星期一看到控制變體、星期二看到 treatment 變體的使用者，會向兩個群組貢獻受污染的資料，並體驗到不一致的介面。團隊應該（SHOULD）使用能自動處理持久化的旗標管理 SDK（PostHog 透過 `localStorage+cookie`），或在首次訪問時分配穩定的 UUID 並跨工作階段持久化。
...
- [Sentry setContext 文件](https://docs.sentry.io/platforms/javascript/configuration/apis/)
