...
從 CDN 在套件旁邊提供 `.map` 檔案會暴露原始程式碼。在部署前檢查建置輸出是否包含 `.map` 檔案。若使用 Vite 或 webpack 插件，確認 `filesToDeleteAfterUpload` 已設定。若透過 `sentry-cli` 上傳，請在 CDN 同步步驟前從建置目錄中刪除 `.map` 檔案。用 `ls -la dist/ | grep .map` 驗證——正確的 CI 執行後輸出應為空。

### 在 `Sentry.init()` 中省略 `release` 欄位

沒有 Release 識別符，錯誤事件就無法與特定部署關聯。「哪個版本引入了這個問題？」這個問題在平台中就沒有答案。Release 必須與傳遞給插件或 `sentry-cli` 的字串相同。常見的做法是在建置步驟中將其作為環境變數注入（例如 `VITE_RELEASE=$(git rev-parse --short HEAD)`）。

### 跳過 `beforeSend` 設定

沒有雜訊過濾，新的 Sentry 安裝通常在正式環境流量的前幾天內就會達到事件配額。瀏覽器擴充功能錯誤、第三方腳本拒絕和良性瀏覽器怪癖都會產生事件。在上線前設定 `beforeSend`，而不是在配額耗盡後才補救。
...
### 登出時不呼叫 `Sentry.setUser(null)`

如果使用者登出後另一位使用者在同一個瀏覽器會話中登入（在共用電腦上很常見），第二位使用者的錯誤將被歸因於第一位使用者的身份，直到頁面被重新整理。請務必在登出處理器中呼叫 `Sentry.setUser(null)`。
...
- [FEE-1405：Feature Flag 可觀測性與 A/B 測試](./1405.md) — 將 Feature Flag 情境附加至錯誤事件，以將錯誤與 Flag 變體相關聯
- [FEE-1407：Session Replay 與正式環境除錯](./1407.md) — 將 Session Replay 與 Sentry 錯誤事件相關聯以進行根本原因分析
