...
:::info
Sentry 會捕獲 JavaScript 應用程式中未處理的例外、未處理的 Promise 拒絕，以及手動回報的錯誤，將它們分群為 Issue，透過已上傳的 source map 解析堆疊追蹤，並將每個 Issue 與引入它的 Release 及提交關聯起來。本文涵蓋 React、Vue 與純 JavaScript 的 SDK 設定；用於在雜訊耗盡事件配額前過濾雜訊的 `beforeSend` 與 `ignoreErrors` 模式；用於分診的使用者情境與麵包屑；透過建置插件與 `sentry-cli` 上傳 source map；以及將分群後的 Issue 轉為通知的警示規則。每個程式碼範例都以目前的 SDK 為準，並非虛擬碼。
:::

## 背景脈絡

每一個 JavaScript 應用程式都會在正式環境中拋出例外。DOM 操作與網路回應之間存在競爭條件、第三方腳本干擾全域狀態、舊版瀏覽器對規範的解讀各有差異，而使用者也總能找到開發者未曾預料的操作路徑。無論團隊是否密切監控，這些例外都會發生。

若沒有適當的監控工具，錯誤就是隱形的。使用者遇到空白畫面、關掉分頁、繼續下一件事。團隊只有在使用者主動發出客服工單、且工單被路由到工程部門時，才能得知問題的存在。大多數使用者不會填寫工單，因此團隊會誤以為系統一切正常，因為沒有收到任何損壞回報。
...
**Sentry** 是這個領域中廣泛使用的平台。它於 2008 年以 `django-db-log`（David Cramer 為 Django 專案撰寫的開源錯誤記錄器）起家，後來發展成本文範例所使用的全端錯誤追蹤平台；其背後的公司 Functional Software, Inc. 於 2012 年成立。其功能集包括：

- **開源起源與自架選項。** 核心平台為開源（sentry.io/from-source）。對資料所在地有嚴格要求的團隊可在自有基礎設施上運行完整平台。大多數團隊使用托管雲端版本，但開源選項消除了供應商鎖定的風險。
- **內建於 SDK 工作流程的 Source Map 支援。** 官方的 Vite、webpack 和 Rollup 插件會在建置步驟中自動上傳 source map，讓壓縮後的堆疊追蹤無需手動上傳步驟即可還原為原始碼位置。
...
完整的 Sentry 工作流程涵蓋 SDK 初始化、source map 上傳、麵包屑、使用者情境、版本追蹤、雜訊過濾與警示設定。Source map 上傳與版本追蹤同時也是 FEE-1402 與 FEE-1407 中 RUM 與 Session Replay 工作流程的前提條件。

## 設計思維

### 選擇錯誤追蹤平台

這個領域有多個平台可供選擇，包括 Datadog RUM、Rollbar、Bugsnag、LogRocket 與 Highlight。本文以 Sentry 作為範例平台，但無論團隊評估哪個平台，都有三項能力值得檢查：

**Source map 上傳整合至建置工具。** 能在建置期間自動上傳 source map 的插件（`@sentry/vite-plugin`、`@sentry/webpack-plugin`，或其他平台的對應方案），能將手動且容易出錯的上傳步驟，簡化為單一的建置設定項目。

**自架或地端部署選項。** 有資料主權要求、GDPR 義務，或安全審查流程限制第三方資料共享的團隊，需要能在自有基礎設施上運行的平台，而不只是 SaaS 產品。

**版本與提交的關聯性。** 若平台能將每次部署與提交範圍建立關聯，並回報哪些 Issue 在該 Release 中出現或重現，工程師就能直接問「哪個版本引入了這個錯誤？」，而不必手動交叉比對錯誤峰值與部署記錄。
...
- **使用者連線造成的網路錯誤。** 在不穩定行動連線上的使用者，如果在會話中途失去連線，會產生源自使用者網路、團隊無法採取行動的 `TypeError: Failed to fetch` 事件。
- **已知的良性錯誤。** 部分瀏覽器在正常運作中會拋出 `"ResizeObserver loop limit exceeded"` 或 `"Script error."`（無有用細節的跨來源腳本錯誤）。

廣告攔截器也會從另一個方向影響事件：許多攔截器會封鎖對 `*.ingest.sentry.io` 的請求，因為該網域符合常見的廣告追蹤器封鎖清單樣式，導致使用廣告攔截器的使用者所產生的事件根本無法離開瀏覽器。`Sentry.init()` 中的 `tunnel` 選項會將事件透過同源路徑傳送，再由你自己的伺服器或邊緣代理轉發至 Sentry，藉此完全避開該封鎖清單的比對。

若不進行過濾，這些事件會淹沒 Issue 追蹤器，埋葬真正的錯誤，並耗盡事件配額。`beforeSend` 是 `Sentry.init()` 中在每個事件發送前執行的回呼函式。它可以檢查事件並返回 `null` 以丟棄它，或在發送前修改事件。一個設定良好的 `beforeSend` 能過濾掉大部分來自擴充功能、第三方腳本與良性瀏覽器怪癖的雜訊，讓真正的錯誤不被埋沒，事件配額也不會被沒有人會處理的事件耗盡。

### 為何版本追蹤改變了除錯工作

診斷通常是事故應對中最耗時的部分。「哪一行壞了？」可以用堆疊追蹤回答。「這是什麼時候開始壞的？」需要將錯誤頻率的時間變化與部署時間戳進行關聯。「哪次部署引入了這個問題？」需要手動交叉比對錯誤峰值與部署記錄。

版本追蹤會自動回答後兩個問題。當一個 Release 被建立並關聯了提交記錄後，平台可以顯示：
- 該 Release 中部署的確切提交範圍。
- 哪些 Issue 在這個 Release 中首次出現。
- 哪些先前已解決的 Issue 在這個 Release 中重現。
- 負責引入新 Issue 的提交的團隊成員。

工程師看到錯誤、看到它出現在 Release `1.4.7` 中、點擊進入提交範圍，並在幾分鐘內找出有問題的變更。
...
**應該（SHOULD）在登入成功後立即呼叫 `Sentry.setUser()`，並在登出時呼叫 `Sentry.setUser(null)`。** 附加使用者情境能讓團隊篩選所有影響特定使用者的錯誤並主動聯繫受影響使用者。團隊應該（SHOULD）只記錄不透明的內部使用者 ID，絕不使用電子郵件地址或真實姓名，以避免在 Sentry 平台上產生 GDPR 資料保留義務。

**必須（MUST）在 `Sentry.init()` 中設定 `enabled: import.meta.env.PROD` 以防止開發工作階段消耗事件配額。** 開發流量對正式環境事故沒有診斷價值，卻會消耗與真實錯誤相同的配額。團隊必須（MUST）透過環境旗標閘控初始化，確保 Sentry 只在正式環境中發送事件，因為設定錯誤的本地環境可能在數小時內耗盡月度配額。

**應該（SHOULD）以錯誤率而非原始錯誤計數設定告警規則。** 原始計數閾值在每次流量高峰時都會觸發，無論應用程式是否真的壞了。團隊應該（SHOULD）設定快速燃燒告警（錯誤率 > 2% 持續 10 分鐘）和迴歸告警（已解決的 Issue 在新 Release 中重現），每條告警均附上 runbook URL。
...
Sentry.init({
  ...
  integrations: [Sentry.browserTracingIntegration()], // 預設未啟用
  beforeSend(event) {
    const message = event.exception?.values?.[0]?.value ?? "";
    if (message === "TypeError: Failed to fetch") return null;
    return event;
...
在登入成功後立即呼叫 `Sentry.setUser()`。這會將後續所有錯誤事件與已驗證的使用者關聯，讓你可以搜尋「所有影響使用者 X 的錯誤」，並主動聯繫受影響的使用者。只傳送不透明的識別符，例如內部使用者 ID 或角色；請勿傳送電子郵件地址或真實姓名，否則會在 Sentry 內部產生 GDPR 資料保留義務。
...
export function onLoginSuccess(user: { id: string; role: string }) {
  // 只使用 id 與 role。刻意省略電子郵件與真實姓名，
  // 以避免在 Sentry 中產生 GDPR 資料保留義務。
  Sentry.setUser({
    id: user.id,
    role: user.role,
  });
...
Sentry 會自動為 DOM 點擊、XHR 和 fetch 請求、`console.log` 呼叫，以及瀏覽器導航事件收集麵包屑。對於不會自動產生麵包屑的應用程式層級事件（例如使用者在多步驟表單中前進、收到 WebSocket 訊息、評估 Feature Flag），請手動添加。
...
在 React 渲染過程中拋出的錯誤，若沒有 React 錯誤邊界先行攔截，會導致元件樹被卸載。`@sentry/react` 匯出了 `Sentry.ErrorBoundary`，這個元件會將錯誤連同 React 元件堆疊回報至 Sentry，在失敗的子樹位置渲染備援 UI，並隔離該失敗，使樹的其餘部分不會被卸載。

```tsx
import * as Sentry from "@sentry/react";

function ErrorFallback() {
  return <p>發生錯誤，團隊已收到通知。</p>;
}
...
請將 `ErrorBoundary` 包在頁面中可獨立失敗的區域，例如單一小工具或單一路由，而不是整個應用程式。放在根層級的邊界，會讓任何渲染錯誤變成整頁空白，而非隔離的局部失敗。
...
# 為每個建置後的 .js 檔案及其 .js.map 標記 debug ID，
# 讓 Sentry 無需 --url-prefix 即可將堆疊訊框比對至對應的 source map
...
`sourcemaps inject` 透過 debug ID 將堆疊訊框比對至對應的 source map，這與上方 Vite 及 webpack 插件所使用的機制相同，因此一旦 source map 完成標記，就不再需要依賴檔名或 URL 前綴比對。
...
- [sentry-cli releases 文件](https://docs.sentry.io/cli/releases/)
- [Sentry beforeSend 文件](https://docs.sentry.io/platforms/javascript/configuration/filtering/)
- [Sentry 警示規則文件](https://docs.sentry.io/product/monitors-and-alerts/alerts/)
