...
**必須（MUST）在 `Sentry.init()` 中設定 `enabled: import.meta.env.PROD` 以防止開發工作階段消耗事件配額。** 開發流量是雜訊，不是訊號。團隊必須（MUST）透過環境旗標閘控初始化，確保 Sentry 只在正式環境中發送事件，因為設定錯誤的本地環境可能在數小時內耗盡月度配額。
