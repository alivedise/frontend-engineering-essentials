...
**必須（MUST）在工作階段重播設定中保持 `maskAllInputs: true`，且對於在 DOM 文字節點中渲染個人識別資訊的應用程式啟用 `maskAllText`。** 若在配置遮罩之前就捕獲了個人識別資訊，資料已經被錄製並受 GDPR 刪除義務約束，而這在操作上可能難以履行。團隊必須（MUST）將遮罩視為預設設定——選擇性錄製特定元素，而非事後逐一選擇遮罩欄位。

**必須（MUST）在 GDPR 適用司法管轄區，於取得使用者明確同意後才啟動工作階段重播。** 工作階段重播依 GDPR 構成個人資料處理，通常需要超出合法利益的合法依據。團隊必須（MUST）先初始化 Sentry 但不含重播整合，然後僅在使用者透過 Cookie 同意平台授予分析同意後，才動態新增 `Sentry.replayIntegration()`。對所有使用者啟用重播後才發現歐盟使用者在未同意的情況下被錄製，構成資料保護事件。
...
**應該（SHOULD）將 `replaysOnErrorSampleRate` 設為 `1.0`，以捕獲每個產生錯誤的工作階段的完整重播緩衝區。** 圍繞錯誤發生的 60 秒重播緩衝區，是將除錯工作流程從閱讀呼叫堆疊追蹤，轉變為觀看導致崩潰的操作序列的關鍵。團隊應該（SHOULD）將 `replaysSessionSampleRate` 設為較低值（0.05–0.1）用於隨機工作階段抽樣，但應該（SHOULD）保持 `replaysOnErrorSampleRate` 為 1.0，以確保絕不錯過由錯誤觸發的重播。
...
**不應該（SHOULD NOT）將工作階段重播資料保存超過資料保留政策的要求；應在重播工具儀表板中設定保留期限。** 許多重播提供商預設保留時間較長（例如 30 天或 90 天）。超過必要保留期限的重播資料仍然受到 GDPR 刪除義務的約束，而且在工具儀表板中管理個別刪除在操作上不可行。團隊應該（SHOULD）在完成初始設定後立即明確設定保留期限，而非依賴提供商的預設值。
