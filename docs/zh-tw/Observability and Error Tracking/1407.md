...
**必須（MUST）將隱私保護視為預設值，而非事後補救——採用選擇錄製（opt-in to recording）而非選擇遮罩（opt-in to masking）的設計。** 若在配置遮罩之前就捕獲了個人識別資訊，資料已經被錄製並受 GDPR 刪除義務約束，而這在操作上可能難以履行。團隊必須（MUST）在 `Sentry.replayIntegration()` 中保持 `maskAllInputs: true`，且對於在 DOM 文字節點中渲染帳號號碼、電子郵件地址或健康資料的應用程式，必須（MUST）設定 `maskAllText: true`。
