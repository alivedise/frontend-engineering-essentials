...
- 應該（SHOULD）以錯誤率（%）告警而非原始錯誤計數——率能對流量量進行正規化，在所有負載水準下都有意義。
- 應該（SHOULD）定義涵蓋錯誤率、p75 LCP 和 p75 INP 的前端 SLO，並追蹤錯誤預算燃燒率，而非時間點的閾值違規。
- 應該（SHOULD）將每個告警連結到 runbook URL，以便值班工程師在告警觸發時有文件化的響應程序。
- 應該（SHOULD）每季審查和精簡告警規則——任何每週觸發超過一次但不需要行動的告警都是移除或調整的候選。
- 不應該（SHOULD NOT）為不直接指示使用者受損或不需要立即行動的指標創建告警。
- 應該（SHOULD）同時使用快速燃燒（>5 倍）和緩慢燃燒（>2 倍持續）告警來應對 SLO 錯誤預算，以捕捉急性事件和慢性降級。
...
```
# 錯誤：在每次流量峰值時觸發，即使錯誤率正常
告警：最近 10 分鐘 JavaScript 錯誤 > 100

# 正確：對流量量進行正規化
告警：最近 10 分鐘 JavaScript 錯誤率 > 2%
      （error_count / total_sessions）> 0.02
```

0.1% 流量下的 100 個錯誤是災難。0.5% 流量下的 1,000 個錯誤在 SLO 範圍內。原始計數無法區分這兩者。

### 設置 SLO 目標但沒有錯誤預算

```
# 錯誤：SLO 作為二元通過/失敗，沒有預算概念
"可用率必須達到 99.9%，否則我們失敗了。"

# 正確：帶有明確錯誤預算的 SLO
"30 天內 99.5% 的工作階段必須無錯誤。
 錯誤預算 = 0.5% × 30d × 24h × 60m = 216 分鐘。
 當前預算消耗：47 分鐘（月份第 12 天，預算的 21.8%）。"
```

沒有錯誤預算，每次 SLO 違規都觸發相同的響應，無論嚴重程度如何。有了預算，團隊可以做出明智的權衡：當剩餘 180 分鐘時，可能消耗 20 分鐘預算的有風險部署，與剩餘 10 分鐘時是截然不同的對話。
...
annotations:
  summary: "前端錯誤率緊急"
  runbook_url: "https://wiki.example.com/runbooks/frontend-error-rate-critical"
  description: "錯誤率已超過 2% 達 10 分鐘。"
```

沒有 runbook 的告警，是在最糟糕的時機將文件責任委派給值班工程師。Runbook 不需要完美——一份列出三個最可能原因及其解決方法的草稿，遠比什麼都沒有要好得多。

### 從不精簡告警規則

告警規則會積累。團隊一開始有 5 個，事件後新增 3 個，引入新工具時再增加 4 個，六個月後有 30 個告警規則，以不同的頻率和嚴重性觸發。沒有人知道哪些仍然相關。季度審查流程強制執行紀律：

```
季度告警審查清單：
1. 列出過去 90 天中所有告警規則及其觸發頻率
2. 對於每個觸發超過 4 次的規則：有多少百分比需要行動？
3. 行動率 < 25% 的規則：調整閾值或刪除
4. 從未觸發的規則：仍然相關嗎？不相關就刪除。
5. 對於每個保留的規則：確認 runbook URL 是否是最新的
```

### 快速燃燒與緩慢燃燒視窗配置錯誤

```yaml
# 錯誤：只使用單個短視窗監控燃燒率——遺漏緩慢的慢性降級
- alert: SLOBurnRate
  expr: slo_burn_rate_1h > 5

# 正確：多視窗燃燒率能同時捕捉急性和慢性問題
# 快速燃燒：短視窗和長視窗都升高（降低假陽性）
- alert: SLOFastBurn
...
單一短視窗燃燒率告警在短暫的流量峰值期間會產生假陽性。多視窗方法要求短期峰值和長期趨勢同時存在，從而大幅降低了假陽性率。
