...
此設定能在每次 HMR 更新時提供快速的重建速度，同時提供指向你原始 TypeScript 或 JSX 原始碼的準確行號。`eval` 與 `eval-cheap-module-source-map` 之間的效能差距很小；但除錯體驗的差距卻很大。不能為了「加速建置」而在開發環境停用 source map——重建成本微不足道，卻會喪失大量除錯時間。

**生產環境絕對不能在 bundle 中保留 `//# sourceMappingURL` 參照，以暴露完整 source map。**
在 Webpack 中使用 `hidden-source-map`（`devtool: 'hidden-source-map'`），或在 Vite 的建置設定中使用 `sourcemap: 'hidden'`。將生成的 `.map` 檔案上傳到 Sentry 等錯誤追蹤工具，讓你的團隊能看到符號化的堆疊追蹤——而不讓終端使用者透過 DevTools 讀取你的原始碼。
...
框架外掛會自動為元件檔案處理 HMR 邊界。共享模組——API 客戶端、功能旗標儲存、設定物件——沒有框架管理的邊界。沒有明確的處理器，任何這些模組的變更都會向上蔓延相依鏈，退回為完整頁面重新整理，導致所有記憶體中的應用程式狀態遺失。
