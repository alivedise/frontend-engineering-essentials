...
URL 載入一個聯邦模組（federated module，例如一個元件、一個工具函式、甚至整個功能），而無需在
...
寫作 `import('ui/Button')`，其中 `ui` 對應遠端別名，`Button` 對應公開路徑別名。建置期不做任何
...
診斷聯邦故障需要在多個層次建立可見性。瀏覽器 console 是第一個信號：Webpack 聯邦執行期
會在共用依賴協商退回打包重複副本時發出警告，而 `remoteEntry.js` 載入的網路層失敗，若沒有
error boundary 包覆，則會以未捕捉的 promise rejection 形式出現。在 error boundary 的
`componentDidCatch` 中加入結構化的日誌記錄，捕捉遠端名稱、被請求的 URL，以及瀏覽器的
錯誤物件，並將此遙測資料轉送至應用程式的錯誤追蹤服務。缺少這項監測，聯邦故障看起來和
任何其他執行期錯誤沒有區別，值班工程師必須重現條件才能判斷原因是 CDN 中斷、版本協商失敗
還是遠端模組本身的錯誤。

在 CI 中執行真正驗證聯邦交握的整合測試（而非 mock 遠端模組），是在上線前發現版本協商問題
最可靠的方式。在 CI 裡使用與生產環境相同的 `remoteEntry.js` 產生方式執行本地聯邦拓撲，
能捕捉到以 mock import 進行單元測試時無法偵測到的共用依賴不匹配問題。這類整合測試應納入
CI 的必要閘門，使版本協商失敗在進入生產環境之前就成為可見的建置失敗，而非等到部署後才
被使用者回報。
...
- Zack Jackson: Practical Module Federation (official site) — https://module-federation.github.io/
- Zack Jackson: Practical Module Federation (book guide) — https://module-federation.github.io/practical-guide
