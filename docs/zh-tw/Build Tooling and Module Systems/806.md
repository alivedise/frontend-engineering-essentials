...
**絕對不可（MUST NOT）** 將密鑰（API 金鑰、私有 auth token、資料庫密碼、簽章金鑰）放入前端
環境變數。它們在構建時期就被替換進 bundle，任何人都可以透過 DevTools 讀取。

**必須（MUST）** 使用框架指定的前綴（Vite 用 `VITE_*`，Next.js 用 `NEXT_PUBLIC_*`），作為
每個要暴露給客戶端 bundle 的變數的明確宣告。

**應該（SHOULD）** 將 `.env` 和特定模式的 `.env.production` 提交到版控，其中只存放非敏感的
預設值。**絕對不可（MUST NOT）** 提交 `.env.local`——它依慣例被加入 `.gitignore`，專門用來
存放不應離開開發者本機的密鑰與本地覆寫值。

**應該（SHOULD）** 在 CI 中使用 GitHub Actions 的 `secrets`（而非 `vars`）來注入任何敏感值。
Secrets 會在日誌中被遮蔽；vars 則不會。
