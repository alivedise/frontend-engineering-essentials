...
修補版本之間是穩定的。將 `.tsbuildinfo` 的快取命中率納入 CI 效能監控，可以協助識別快取失效的模式，並在
TypeScript 升級或 `tsconfig.json` 頻繁異動時做出對應的調整。若快取命中率長期偏低，
應優先排查 `tsconfig.json` 是否因為動態生成的欄位（如 `include` 路徑基於環境變數展開）
導致快取金鑰頻繁失效，並在必要時改用穩定的靜態配置。
