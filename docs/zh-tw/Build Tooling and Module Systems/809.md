...
工程師**必須（MUST）**以 ES 模組形式撰寫函式庫程式碼——在 `package.json` 中設定
`"type": "module"` 或使用 `.mjs` 副檔名——並**必須（MUST）**使用具名匯出，而非將單一物件
作為預設匯出。Tree-shaking 需要可靜態分析的 import/export 綁定。
`export default { foo, bar, baz }` 是不透明的：打包工具看到的是一個值為物件的單一預設匯出，
而對該物件的屬性存取無法在靜態層面解析為具體的匯出，因此只要有任何地方引入了這個預設匯出，
整個物件都必須被納入。具名匯出，例如 `export function foo() {}` 與 `export function bar() {}`，
讓打包工具能夠在只有 `foo` 被引入時，只納入 `foo` 而排除 `bar`。CommonJS 模組
（`module.exports`、`require`）根本無法被 tree-shaken；CommonJS 模組系統的動態特性意味著
打包工具若不執行模組就無法確定其匯出表面。

工程師**必須（MUST）**在所有預計可被 tree-shake 的套件的 `package.json` 中宣告
`"sideEffects"` 欄位。省略此欄位將導致打包工具保守地處理該套件，即使其匯出未被引用，
也會納入所有可達的模組。對於完全無副作用的套件，正確的值為 `false`；對於包含有副作用
檔案（如 CSS 引入或 polyfill）的套件，則應設為 glob 模式陣列。應用程式層級的 `package.json`
也**應該（SHOULD）**為內部模組加入此欄位，讓應用程式自身的打包工具能以同等的信心消除未使用
的內部程式碼。
...
作者能保證沒有副作用的手寫工廠呼叫。永遠不要標記一個寫入全域的呼叫、發出網路請求的呼叫，
或改變共享狀態的呼叫——打包工具會靜默地丟棄它，副作用不會執行。
