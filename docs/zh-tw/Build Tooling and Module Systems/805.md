...
前端工程發展初期，每個專案都存放在各自獨立的 git repository 中。這種模型——polyrepo——
給予團隊充分的自治：獨立的部署週期、分離的 CI 流程、互不干擾的套件管理 lockfile。當各
套件獨立演進、變更很少跨越邊界時，這個模型運作良好。
...
並等待傳遞窗口。實際的程式碼變更——可能只有五行——卻被埋在一個跨多天、跨多團隊的協調
問題裡。Git submodule 是傳統的逃生出口，但眾所周知地脆弱：submodule 狀態不會自動同步，
...
根本無法擴展。套件管理工具加入了 workspace 支援（npm 7 於 2020 年，pnpm 和 Yarn 更早），
而專為此設計的任務協調器——Turborepo 和 Nx——則解決了剩下的問題：跨數十乃至數百個套件的
高效率、有快取、平行化任務執行。
...
共用工具函式和三個消費端的重構，需要橫跨四個 repository 進行協調變更、四個 PR 和四個
CI 流程——而且沒有辦法讓這個變更具備原子性。若重構採分階段進行（先出貨函式庫，再更新
消費端），就必須明確管理某些消費端使用新 API、某些仍使用舊 API 的中間狀態。在 monorepo
中，同樣的重構只是一次 commit、一個 PR、一次 CI 執行。變更預設就是原子性的。
...
Turborepo 刻意保持精簡。它在你現有的套件管理工具和現有的 `package.json` 腳本之上加入
任務 DAG 和快取層。只有一個設定檔（`turbo.json`）。它不強制規定專案結構，不產生程式碼，
也不在套件之間強制邊界。這使得增量採用變得容易——你可以在一個下午之內將 Turborepo 加入
現有的 pnpm 工作區。
...
都會變慢。在那種規模下運作的公司（Google、Meta）使用大多數團隊永遠用不到的特殊工具
（Mercurial、Eden 等虛擬檔案系統）。對大多數團隊來說——最多幾百個套件——配備 Turborepo
或 Nx 的 pnpm 工作區完全能應對這個規模。反對 monorepo 更有力的論點是團隊自治：所有團隊
共用同一個 CI 流程、同一個根目錄工具設定，以及同一個發布關卡。一個套件中不穩定的測試
會阻塞整個 CI 執行。透過 `CODEOWNERS` 建立明確的所有權，以及使用任務過濾（`turbo run
build --filter=./apps/web`）可以緩解這個問題，但協調成本是真實存在的。
...
目錄，消費端套件參照 `dist/`。這種方式模擬了套件對外發布時的使用方式，而且 Turborepo
可以快取建置結果——消費端應用程式只有在函式庫的原始碼改變時才需要重新建置。

## 視覺化

### Turborepo 任務 DAG

下圖顯示了一個小型 monorepo 中跨兩個應用程式和一個共用函式庫的三個任務。箭頭從依賴指向
被依賴——一個任務需等待箭頭指向它的所有任務完成。
...
若 `packages/ui` 的原始碼檔案自上次執行以來未有變更，步驟 1 就是快取命中——Turborepo
...
Yarn 支援相同的協定。npm workspaces 使用不同的語法（以 `"*"` 作為版本值），但 symlink
行為是相同的。
...
當 CI 時間開始讓人痛苦時再升級到 Turborepo——`turbo.json` 可以疊加在現有的 pnpm
工作區之上，不需要重新結構化任何東西。

### 6. Nx：專案圖與 affected 執行

Nx 不需要為每個腳本在 `nx.json` 中宣告任務——它從 `package.json` 依賴和原始碼 import
推斷出專案圖。
...
**沒有在 `turbo.json` 中宣告 `outputs`。**
若 `outputs` 被省略或為空，Turborepo 在成功執行後仍將該任務標記為已快取——但快取命中時
不會還原任何檔案。一個預期 `dist/` 存在的下游任務會失敗。請宣告任務所有寫入的目錄：
`"dist/**"`、`".next/**"`、`"build/**"`。Turborepo 將快取這些檔案，並在快取命中時還原
它們，使下游任務能找到所需的檔案。
...
**根目錄 `package.json` 沒有設定 `"private": true`。**
monorepo 的根目錄 `package.json` 是工作區清單，不是可發布的套件。沒有 `"private": true`，
在根目錄執行 `pnpm publish` 會意外地發布根目錄清單。所有工作區根目錄和任何僅供內部使用
的套件都應標記為 private。

**在套件目錄內執行 `pnpm install` 或 `npm install`。**
在工作區中，依賴必須從根目錄（或使用 `--filter`）管理。在 `packages/ui/` 內執行
`npm install` 會在該目錄中建立獨立的 `node_modules` 和 `package-lock.json`，這會破壞
工作區的 symlink 並造成獨立、相互衝突的依賴解析。請務必從根目錄管理工作區依賴：
`pnpm add lodash --filter @myrepo/ui`。

**快取有副作用的任務（網路呼叫、隨機種子、時間戳記）。**
Turborepo 的快取基於內容雜湊：相同的輸入檔案永遠產生相同的快取命中。若一個任務進行
網路呼叫（抓取即時資料、存取資料庫）或產生每次執行都會改變的輸出（含嵌入時間戳記的
建置產物），快取將產生過期的結果。請在 `turbo.json` 中將這類任務標記為 `"cache": false`。
...
- [為何選擇 Monorepo — Nx Blog](https://nx.dev/concepts/more-concepts/why-monorepos)
