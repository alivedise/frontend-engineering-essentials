...
本文版本範疇說明中提及的 React 19 ref 模型，值得在元件架構決策的脈絡下再次說明。React 19 將 `ref` 作為一般 prop 傳遞的模型，消除了對 `forwardRef` 的需求：一個將 `ref` 作為 prop 接受並傳遞給底層元素的元件，滿足了 `forwardRef` 原本解決的所有使用情境。遷移至 React 19 的團隊應採取漸進式策略：新元件使用新的 ref-as-prop 模型，既有的 `forwardRef` 用法保留原地，等待專屬的重構機會。遷移不需要改變底層的 ref 行為，只需改變包裝的 API，因此比 API 介面的變動看起來風險更低。
React 團隊提供了一個 codemod，處理從 `forwardRef` 包裝器到新的 ref-as-prop 模式的機械性轉換。在獨立分支上執行 codemod，審查差異以確認正確性，並處理浮現的任何 TypeScript 錯誤，是有許多元件使用 `forwardRef` 的團隊的推薦方法。在與其他功能工作隔離的情況下執行 codemod，讓差異可審查且遷移在發現非預期問題時可撤回；將 React 19 ref 遷移作為獨立技術債衝刺排程，而非伺機與功能工作合併，可產生最乾淨的提交歷史，並讓部署後發現相容性問題時回退變得直截了當。

Props 介面的匯出政策值得在函式庫層級進一步說明。匯出的 props 介面成為公開 API 契約的一部分：對介面的破壞性變更——重命名必填 prop、改變 prop 的型別、移除可選 prop——會破壞已匯入並使用該介面進行型別標注的呼叫方。函式庫作者應以與元件執行時行為相同的版本紀律對待匯出的 props 介面。這意味著以 ESLint 等工具追蹤哪些 props 介面是公開匯出的，並確保所有破壞性 props 變更都在語義版本號（semver major）升級中進行，而非在 minor 或 patch 版本中偷偷引入。
對於應用層程式碼，由於消費者是內部的，彈性較大，但建立將匯出 props 介面視為 API 介面的習慣，無論脈絡如何都是有價值的。
追蹤哪些介面是公開匯出的，可以透過為設計系統套件建立一份明確的「公開型別清單」文件，並在每次 API 審查時對照這份清單確認，防止非預期的公開介面在不知不覺中進入 semver 契約。
這種紀律的長期收益是：下游消費者可以信任地對匯出型別進行版本固定，在升級時能清楚知道哪些型別變更是破壞性的，進而做出有根據的升級決策，而非每次升級都需要手動測試所有使用點。

在函式元件寫法的一致性上，建立全程式碼庫的 ESLint 規則可以自動強制執行不使用 `React.FC` 的慣例。`@typescript-eslint/consistent-type-definitions` 配合 `interface` 選項，可強制 props 型別使用介面而非型別別名；自定義的 `no-react-fc` 規則或 `react/display-name` 搭配適當的設定，可在程式碼審查之前就在編輯器中即時標記違規。這些工具保護確保新加入的工程師不會在不知情的情況下引入 `React.FC`，而不需要依賴文件或審查人員的記憶。
元件型別化慣例（包括不使用 `React.FC`、使用 `ComponentProps` 派生、顯式宣告 `children`）若能在設計系統的入門文件和新成員 onboarding 的第一個任務中以範例呈現，可讓這些慣例從「需要記住的規則」轉變為「看了範例就知道這樣做」的自然模式，減少知識需要被反覆強調的情況。
最終，元件 TypeScript 型別化慣例的健康狀態反映在元件測試是否能以清晰的型別簽名作為介面進行撰寫。若測試中充滿了 `as` 型別斷言或 `any` 型別，通常是元件型別化不準確的症狀——從 `ComponentProps` 派生的準確 prop 型別讓測試固件的建立直截了當，不需要任何型別體操。以「我的元件型別是否讓測試容易撰寫？」作為型別化品質的快速啟發式檢驗，是工程師評估自己型別設計的實用自我問診工具。
這個啟發式方法也適用於整個元件函式庫的健康狀態評估：若測試套件中存在大量型別斷言或對 `@ts-ignore` 的依賴，通常指向系統性的型別不準確性，值得安排專項的型別重構工作，而非讓問題繼續累積。
定期以 `tsc --noEmit --strict` 在完整程式碼庫上執行的 CI 步驟，確保型別錯誤在引入時被立即發現，而非在下一次型別化改善工作前一直潛伏。
