...
**版本範疇：** 本文涵蓋 React 17 和 18 的寫法。React 19 引入了新的 ref 模型，允許將 `ref` 作為一般 prop 傳遞，無需使用 `forwardRef`，同時簡化了 ref API。本文的 `forwardRef` 寫法在 React 17-18 的程式碼庫及向下相容的元件中仍然有效，但 React 19 專案應參閱 React 19 遷移指南以了解更新的 ref 處理方式。
