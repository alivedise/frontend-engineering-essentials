...
Ref 帶來了第三種不同的型別化需求。`useRef` 有兩個根據初始值產生不同型別的簽名，選錯簽名會得到 React 無法安全指派給 DOM 元素的 ref 型別。這個差異很微妙 — 缺少 `null` 初始值會改變 ref 的整體語意 — 也是 React 程式碼中最常見的 TypeScript 錯誤來源之一，許多人靠試誤解決問題，而非真正理解底層模型。造成錯誤 `useRef` 呼叫的同一個概念缺口，也會導致 `forwardRef` 上的泛型型別錯誤，因此一旦理解 ref 型別模型，就能同時解決這兩個問題。

泛型元件（generic component）帶來了第四個需求，而這正是 `React.FC` 的限制所揭示的：元件的型別簽名有時需要攜帶一個型別參數，從輸入資料流向渲染輸出，TypeScript 的泛型函式語法是唯一能處理這種情況的工具。能夠約束 `value` 和 `options` 為 `User` 型別的 `<Select<User>>`，或讓 `renderItem` 存取 `Product` 型別項目的 `<List<Product>>`，必須是泛型函式，而非以固定 props 介面型別化的元件。理解元件就是函式，且 TypeScript 的完整泛型函式語法對元件同樣適用，是解鎖這個能力的關鍵。

本文涵蓋的四個關切點 — `React.FC` 替代方案、從 HTML 元素派生 prop 型別、DOM ref 型別化，以及泛型元件 — 在日常 React TypeScript 開發中反覆出現。理解每個模式背後的原理，而非單純記憶正確形式，使工程師能在新情境中應用正確的型別技術，並在遇到型別錯誤時診斷根本原因，而非依賴試誤。
...
對於使用 React Context API 的元件，型別化 context 值和 context provider 的 prop 是 FEE-1701 泛型基礎的自然延伸，且與本文的 `forwardRef` 和 `ComponentProps` 模式互補。
