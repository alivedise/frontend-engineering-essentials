...
工程師必須（MUST）對所有從外部來源進入應用程式的資料——API 回應、URL 查詢參數、localStorage 值、WebSocket 訊息以及環境變數——使用結構描述函式庫進行驗證，再將這些資料視為已宣告的 TypeScript 型別。對未驗證的外部資料進行型別斷言，不是驗證；它只是壓制了編譯器的警告，卻沒有確認執行時期的值是否符合預期的形狀。結構描述解析成功後所得到的已驗證具型別結果，才是 TypeScript 型別可以被信任為真正反映值之實際結構的最早時間點。

工程師必須（MUST）使用函式庫的推論工具從結構描述推導應用程式的 TypeScript 型別，而不是在每個結構描述旁另行維護一個獨立的 TypeScript 介面。當一個結構描述和一個介面作為同一形狀的獨立宣告共存時，修改其中一個就會與另一個產生靜默的偏差——結構描述拒絕型別所接受的資料，或是型別接受結構描述拒絕的資料。`z.infer<typeof schema>` 將結構描述與型別合併為單一宣告，只有一個維護點。工程師應該（SHOULD）在應用程式中處理外部來源資料的程式碼裡，優先使用 `safeParse` 而非 `parse`，因為 `safeParse` 回傳一個可與 TypeScript 控制流程收窄整合的差異化聯合型別，而非在每個呼叫點都必須捕捉的例外。
...
區分 `parse` 和 `safeParse` 的實用原則是：當驗證失敗應該崩潰或拋出時（初始化程式碼、測試固件），使用 `parse`；當失敗應該產生一個已處理的錯誤結果時，在其他所有地方使用 `safeParse`。在大多數應用程式執行時期的程式碼中——資料獲取、表單處理、URL 解析——`safeParse` 是正確的預設選擇。這個選擇的影響是全局性的：整個程式碼庫採用一致的錯誤處理風格，要嘛例外驅動，要嘛結果型別驅動，取決於哪種方式在第一個關鍵的 fetch 工具中被採用。
...
結構描述驗證與錯誤處理（FEE-1807）密不可分：`safeParse` 的失敗分支產生一個結構化的錯誤值，應用程式必須決定如何呈現、記錄或從中恢復。這兩篇文章合在一起，涵蓋了非同步資料的完整生命週期：從邊界處的驗證，到向使用者呈現已解析的資料或錯誤狀態。
