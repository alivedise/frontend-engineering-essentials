...
工程師在撰寫任何專案本地 `.d.ts` 宣告之前，必須（MUST）先確認是否已有現成的型別支援——包括透過 `package.json` 的 `types` 欄位提供的打包型別，以及 npm `@types` 命名空間提供的社群型別。為已有型別的函式庫撰寫宣告，會產生一個比原本要修正的編譯錯誤更難察覺的問題：編譯錯誤消失了，但型別的不準確性被悄悄引入。

工程師應該（SHOULD）優先將缺失或有誤的型別貢獻至 DefinitelyTyped，而非無限期維護專案本地宣告。維護於單一專案中的宣告只能惠及該專案，並隨函式庫持續演進而累積技術債；DefinitelyTyped 的貢獻則會被版本化、針對已發布的版本進行測試，並對整個生態系開放。當專案本地宣告是唯一可行的選項（例如內部套件或暫時性的阻塞解除），必須（MUST）將其範圍限制在最精確的模組名稱，並以注解說明其臨時狀態，將其視為待解決的工作事項，而非永久配置。

工程師禁止（MUST NOT）將 `declare module '*'` 作為無法解析的匯入錯誤的通用抑制器。萬用字元形式會消除所有無法解析的匯入的錯誤，而不只是促使添加它的那一個。具名的環境模組宣告——每個未型別化的模組一條——是有針對性地抑制的正確工具。

本原則中的三條規則——撰寫前先確認、優先貢獻上游、本地撰寫時精確限定範圍——共同定義了防止最常見且代價最高的宣告檔案錯誤所需的最低盡職調查。每條規則針對一種截然不同的失敗模式：第一條防止重複工作和型別覆蓋，第二條防止本地維護債務的積累，第三條在本地宣告不可避免時限制其影響範圍。
