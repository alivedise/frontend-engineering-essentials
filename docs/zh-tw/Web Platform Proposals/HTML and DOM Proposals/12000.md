...
藍色節點是分類總覽。其下每個子文章都是涵蓋一項前穩定 HTML 或 DOM 提案的 FEE 條目。三篇文章依編號順序列出，總覽之下沒有主題分組。未來規格可能隨新提案成熟而補充更多 12xxx 文章，編號從 12004 起依序延續。

三篇子文章涵蓋 HTML 平台的不同表面。

FEE-12001 記錄 `CustomElementRegistry` 建構與 `attachShadow({ registry })`，這是自訂元素定義的命名空間隔離機制。

FEE-12002 記錄 `documentPictureInPicture.requestWindow()`，可將任意 HTML 提升為浮動的永遠置頂視窗。

FEE-12003 記錄 `<script type="speculationrules">`，這是取代 `<link rel="prerender">` 的宣告式 prefetch 與 prerender 治理機制。

兩個原先規劃中的文章在本規格落地前已從此範圍畢業。Invoker Commands（`command` / `commandfor` 屬性）於 2025 年 12 月達到 Baseline 2025，由獨立的穩定範圍 FEE 追蹤。Navigation API 於 2026 年 1 月達到 Baseline 2026，同樣交由後續穩定範圍的規格處理，與 Popover 和 Invoker Commands 一併管理。12xxx 範圍記錄尚未達到 Baseline 穩定的提案；一旦某功能畢業，其文章便移至穩定的 0-9999 分類。

文章彼此獨立——讀者可以挑任一篇閱讀而不需理解其他文章。交叉引用出現在每篇文章的「內部參考」段落中，標示真正互動的提案。Scoped Custom Element Registry 涉及 Web Components 周邊工具；Document Picture-in-Picture 與影像和媒體 API 互動；Speculation Rules 與 Resource Hints 和 Core Web Vitals 互動。

不同讀者在不同文章進入此範圍，依據他們手上既有的問題：

- **微前端整合者** — 從 Scoped Custom Element Registry 開始。命名空間衝突問題（兩個 widget 定義相同的標籤名稱）是送他們進入此範圍的動機。
- **視訊會議或即時編碼產品工程師** — 從 Document Picture-in-Picture 開始。永遠置頂的視窗能力沒有其他平台原語可以近似替代。
- **效能工程師** — 從 Speculation Rules 開始。宣告式的 prefetch 與 prerender 治理取代臨時的 `<link rel="prefetch">` 用法，提供政策面介面。

上面建議的閱讀順序（編號順序）適用於沒有特定既有問題、正在盤點此範圍的讀者。手上已有問題的讀者應直接跳到上方對應的文章，其他文章作為選讀背景。對近期畢業的 Invoker Commands 或 Navigation API 有興趣的讀者，應在穩定範圍後續規格落地後至 100s 範圍查閱。

畢業追蹤獨立於本總覽。當個別提案達到背景段落定義的跨引擎出貨門檻時，由獨立的畢業規格把該文章內容移至對應的 0-9999 範圍，並在 12xxx 槽位留下重導向存根。12xxx 槽位仍被占用——FEE ID 不重複使用——以免其他文章的交叉引用失效。

下一個畢業候選是 Scoped Custom Element Registry，若 Firefox 以未設旗標方式出貨，12001 文章會畢業至 100s 範圍，並在 12001 保留存根。Document Picture-in-Picture 與 Speculation Rules 取決於 Safari 與 Firefox 的出貨進度，時程較難預測。Invoker Commands 與 Navigation API 雖原本排入此範圍的規劃文件，但在 12xxx 文章撰寫前已畢業——兩者直接寫入穩定的 100s 範圍，沒有經過 12xxx 中繼。

對瀏覽 12xxx 範圍的讀者而言，實務上的意義是：若原本規劃在此處的功能從索引中消失且未留存根，請先到穩定的 100s 範圍查找。FEE 專案的畢業政策僅為曾有完整 12xxx 文章的功能保留存根；在規劃期間畢業的功能（例如本處的 Invoker Commands 與 Navigation API）不留存根，直接移至穩定目的地。

這種模式——功能在前穩定文章撰寫前就畢業——是平台出貨節奏的健康訊號。12xxx 範圍捕捉的是「提案在某一引擎出貨」到「各主要引擎皆可使用」之間的時間窗。當此時間窗在 FEE 專案來得及撰寫文章前結束，文章便於其穩定目的地撰寫，避免撰寫兩次。

本總覽同時扮演入口與畢業追蹤者的角色。

讀者用它來定位提案全景並決定下一篇要讀哪一個子文章。FEE 專案用它作為檢查清單，告訴規格哪些文章符合畢業條件。把兩個角色放在同一份文件是刻意的——拆開會使維護成本加倍，也會把出貨狀態的事實分散到兩個表面。

分類目前的深度較淺，反映 HTML 與 DOM 提案畢業速度相對於 FEE 追上的速度。Popover（Baseline 2025）、Invoker Commands（Baseline 2025）與 Navigation API（Baseline 2026）皆由獨立的穩定範圍 FEE 規格追蹤。Scoped Custom Element Registry 是下一個可能的畢業候選，取決於 Firefox 是否以未設旗標的方式出貨。

建議的閱讀順序依循編號。Scoped Custom Element Registry 最先，處理多擁有者頁面的命名空間隔離；Document Picture-in-Picture 第二，提供新的視窗能力；Speculation Rules 最後，負責 prefetch 與 prerender 的治理。第一次接觸此範圍的開發者可依序閱讀來建立心智模型——先掌握元素註冊層級的新增，接著理解視窗 API，最後理解效能治理介面。

除了上列五篇子文章之外，12000 範圍還有空間容納尚未到達 WICG 或 WHATWG 的功能。Observable 基本元素（目前在 WICG 討論的 `Observable` 介面）可能最終到來；view-source 層級的變更（提議中結構化資料的 `source` 元素）可能進一步充實範圍。

保留編號（`12006` 起）可容納未來提案而不需要重新編號既有文章。FEE-ID 配置是單調遞增的——新起草的文章總是取範圍內下一個未用的 ID，與主題無關。這讓跨規格的交叉引用保持穩定，也避免新提案加入時重新搬動既有文章所需的協調成本。

## 此範圍的軌道

| ID    | 標題                           | 狀態            |
|-------|--------------------------------|-----------------|
| 12001 | Scoped Custom Element Registry | Limited 可用性  |
| 12002 | Document Picture-in-Picture    | Limited 可用性  |
| 12003 | Speculation Rules              | Limited 可用性  |

子文章依序編號，大致按讀者在實務中遇到提案的順序排列。Scoped Custom Element Registry 是元素註冊層級的新增；Document Picture-in-Picture 是視窗 API；Speculation Rules 控制 prefetch 與 prerender 提示。每篇文章在「內部參考」段落交叉連結會彼此互動的提案。
...
- [Interop Project](https://wpt.fyi/interop-2026) — 跨廠商年度功能相容性承諾；常含 HTML 與 DOM 功能於焦點清單上。
...
- [W3C TAG security-questionnaire](https://github.com/w3c/security-questionnaire) — 網路平台提案安全與隱私審查使用的檢查清單。
