...
五篇子文章涵蓋 HTML 平台的不同表面。FEE-12001 記錄 `command` 與 `commandfor` 屬性，將任何 `<button>` 上的命令式點擊處理器接線轉為宣告式標記。FEE-12002 記錄 `CustomElementRegistry` 建構與 `attachShadow({ registry })`，這是自訂元素定義的命名空間隔離機制。FEE-12003 記錄 `documentPictureInPicture.requestWindow()`，可將任意 HTML 提升為浮動的永遠置頂視窗。FEE-12004 記錄 `window.navigation`，SPA 框架現在於其上建立的平台路由基本元素。FEE-12005 記錄 `<script type="speculationrules">`，這是取代 `<link rel="prerender">` 的宣告式 prefetch 與 prerender 治理機制。

文章彼此獨立——讀者可以挑任一篇閱讀而不需理解其他四篇。交叉引用出現在每篇文章的「內部參考」段落中，標示真正互動的提案。Invoker Commands 與 Popover 直接互動，因為 `command="toggle-popover"` 是最常見的 invoker 使用情境；Navigation API 與 Speculation Rules 在框架同時使用兩者進行路由層級的 prefetch 時互動；Document Picture-in-Picture 與 Navigation API 在 PiP-aware 路由需要保留視窗狀態時互動。

不同讀者在不同文章進入此範圍，依據他們手上既有的問題：

- **盤點 UI 原語的應用工程師** — 從 Invoker Commands 開始。宣告式 `command` / `commandfor` 模型最接近日常表單與對話框工作，與 Popover 的整合涵蓋最常見的覆蓋層模式。
...
- **觸及路由的框架作者** — 從 Navigation API 開始。平台原語取代每個 SPA 框架過去十多年基於 history 的路由層。
- **效能工程師** — 從 Speculation Rules 開始。宣告式的 prefetch 與 prerender 治理取代臨時的 `<link rel="prefetch">` 用法，提供政策面介面。

上面建議的閱讀順序（編號順序）適用於沒有特定既有問題、正在盤點此範圍的讀者。手上已有問題的讀者應直接跳到上方對應的文章，其他文章作為選讀背景。
...
繼 Popover 之後的下一個畢業候選可能是 Invoker Commands，在 Firefox 移除旗標時，12001 文章畢業到 FEE-100 範圍，存根保留在原處。Scoped Custom Element Registry 若在 Firefox 出貨後會走類似路徑，使 12002 緊接其後進入畢業佇列。Document Picture-in-Picture、Navigation API 與 Speculation Rules 取決於 Safari 與 Firefox 的出貨進度，時程較難預測。

本總覽同時扮演入口與畢業追蹤者的角色。

讀者用它來定位提案全景並決定下一篇要讀哪一個子文章。FEE 專案用它作為檢查清單，告訴規格哪些文章符合畢業條件。把兩個角色放在同一份文件是刻意的——拆開會使維護成本加倍，也會把出貨狀態的事實分散到兩個表面。

分類目前的深度較淺，反映 HTML 與 DOM 提案畢業速度相對於 FEE 追上的速度。Invokers 與 Document Picture-in-Picture 是未來達到跨引擎穩定門檻時畢業到 FEE-100 或 FEE-400 的候選。Popover 已達到 Baseline 2025，另以獨立規格追蹤至穩定範圍的專屬 FEE。

建議的閱讀順序依循編號。Invoker Commands 最先，作為最立即實用的宣告式模式；Scoped Custom Element Registry 第二，處理多擁有者頁面的命名空間隔離；Document Picture-in-Picture 第三，提供新的視窗能力；Navigation API 第四，於平台層處理 SPA 路由；Speculation Rules 最後，負責 prefetch 與 prerender 的治理。第一次接觸 HTML/DOM 提案全景的開發者可依序閱讀來建立心智模型——先掌握較單純的宣告式新增，再理解影響架構範圍的功能。
