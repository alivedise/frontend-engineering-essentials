...
| `.indexed()` | **不在規格中。** 此方法在提案討論期間被探討過，但最終未納入 ECMAScript 2025。如果需要索引存取，請使用 `.map((v, i) => [i, v])` 或陣列上的 `entries()`。 |

注意：迭代器上的 `.map()` 不像 `Array.prototype.map` 那樣將索引作為第二個引數傳入。映射函式只接收當前值。
...
Iterator helpers 在 2026 年 1 月的 TC39 全體會議上達到 TC39 Stage 4，並作為 ECMAScript 2025（2025 年 6 月發布）的一部分正式發布。該提案由 Michael Ficarra、Yulia Startsev、Lea Verou 等人共同提倡。它在 Stage 3 停留了數年，等待瀏覽器實作跟上。
