...
此提案目前位於 TC39 Stage 1。任何瀏覽器或引擎都尚未實作。目前語法不存在 Babel 外掛。語法是實驗性的，將會改變。不得用於正式環境。請查閱 TC39 提案儲存庫以取得最新狀態。
:::

## 背景

JavaScript 的 `switch` 陳述式數十年來一直是主要的多分支分派機制。它適用於簡單的相等性測試，但一旦需要根據值的*結構*進行分支，就會迅速失效。有五個具體問題使 `switch` 不足以應對現代 JavaScript 的需求：
...
// 避免 — 將 if 巢狀在 arm 本體內會隱藏 guard
when { status: 429, let retryAfter }:
  retryAfter < 60 ? scheduleRetry(retryAfter) : renderError('retry window too long');
```

注意：多陳述式 arm 本體並非 proposal-pattern-matching 的一部分。請將邏輯萃取至輔助函式。多陳述式區塊需要獨立的 proposal-do-expressions 提案。
...
**不得在正式環境使用 `match`。** 此提案位於 Stage 1。語法是實驗性的，將會改變。目前語法不存在可用於正式環境的 Babel 外掛。請查閱 TC39 提案儲存庫以取得最新狀態。
...
// 注意：多陳述式 arm 本體並非 proposal-pattern-matching 的一部分。
// 請將邏輯萃取至輔助函式。多陳述式區塊需要獨立的 proposal-do-expressions 提案。
...
注意：`0` 使用 `SameValue` 語意，因此 `+0` 和 `-0` 是不同的值。請明確使用 `+0` 或 `-0` 以匹配特定的帶號零。
...
match (obj) {
  ...
  // 簡寫：{ foo } 等同於 { foo: let foo }
  // （將屬性 'foo' 的值綁定至同名的本地變數 foo）
  when { foo }: '...';
}
```
