...
現代瀏覽器已不再將表單控件的渲染委派給作業系統元件——Chrome 與 Firefox 使用各自的渲染引擎。然而，`<textarea>` 與 `<input>` 在 CSS 規範中仍屬於**替換元素（replaced element）**，在 `field-sizing` 出現之前，CSS 的本質尺寸模型並無機制依文字內容調整其尺寸。
