...
**跳過紅燈階段。** 執行立即通過的測試不是紅燈綠燈循環——那只是綠燈測試。在信任測試之前，確認它可以失敗。如果你無法從一個新測試產生失敗，要麼測試測的是錯誤的東西，要麼實作已經存在了。一個無法失敗的測試沒有任何價值。

**寫太多程式碼來達到綠燈。** 紀律是：最少量的程式碼來通過。如果測試期望 `add(1, 2)` 回傳 `3`，正確的第一個實作是 `return 3`。強迫自己寫下一個打破硬編碼回傳的測試。這聽起來很教條，但它教你讓測試逐步驅動設計，而非一次性實作。

**將重構步驟視為可選的。** 技術債會在有綠燈測試但沒有重構紀律的程式碼中積累。綠燈測試是重構的安全網——拒絕使用安全網意味著背負著持續積累的債務向前走。
...
**將探索與 TDD 混淆。** 當你真的不知道如何解決一個問題時，不帶測試地寫探索性程式碼。那是探索：它產生知識，而非正式程式碼。探索結束後丟棄它，然後用 TDD 實作真實的解決方案。將探索當作 TDD 會話來對待，會產生你還沒有充分理解到足以進行測試驅動的程式碼。
