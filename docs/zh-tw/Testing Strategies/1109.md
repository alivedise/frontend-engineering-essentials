...
這種重用性的架構意義在於：處理器值得被認真投入。能準確反映真實 API 形狀的處理器——使
用與實際伺服器相同的回應型別、狀態碼和錯誤信封——會在每個使用情境中帶來複利回報。MSW
的另一個優點是讓測試對未來的 HTTP 客戶端更換保持韌性：若應用程式從原生 `fetch` 遷移到
`axios` 或引入 React Query，MSW 的處理器定義不需要改變，因為 MSW 在網路層攔截，與客戶
端實作細節無關。

## 設計思維

MSW 的架構選擇——在網路邊界層而非模組層級進行攔截——對整合測試的覆蓋深度有根本性的影響。當模擬發生在網路邊界層時，應用程式的完整請求建構路徑都會被執行：URL 組裝、查詢字串序列化、請求標頭的組合、請求本體的序列化，以及回應解析和錯誤處理邏輯。這些都是真實 API 整合失敗的常見來源，若模擬發生在模組層級則完全不可見。

選擇 MSW 作為整合測試的模擬策略，也意味著接受一個隱含的紀律要求：處理器定義必須準確反映真實 API 的行為，包括錯誤條件、狀態碼語意、回應形狀，以及任何條件式邏輯（例如根據請求本體或查詢參數回傳不同結果）。一個僅覆蓋成功路徑的處理器套件，雖然能使測試通過，但無法驗證應用程式在真實環境中面對多樣化 API 回應時的行為。這正是為何處理器定義值得被當作第一類工程產出來投入——它們是整合測試所提供保障品質的上限。

從實際採用的角度來看，導入 MSW 的阻力通常來自初始設定成本的感知。然而，`setupServer` 的設定只需約十行程式碼，且在整個測試套件中只需設定一次——後續的整合測試只需定義處理器，不需要重複搭建基礎設施。採用 MSW 後，開發者通常發現他們可以在不啟動任何伺服器程序的情況下，完整測試涉及網路互動的功能，而這正是整合測試應有的效率目標。

另一個值得注意的採用優勢是，MSW 處理器天然地成為 API 契約的可執行文件。當後端 API 規格發生變更時，更新處理器定義不僅使測試反映新的行為，也同時更新了 Storybook story 中的展示資料，以及手動開發流程中的模擬回應。這種單一來源的更新流程，相較於維護多套分散的模擬資料，能顯著降低 API 契約變更所需的協調成本，並減少測試與真實 API 行為之間的隱性落差。
...
選擇 `setupServer` 還是 `setupWorker` 的決定，通常由測試執行環境而非應用程式架構決定：
Jest 和 Vitest 預設在 Node.js 中執行，因此使用 `setupServer`；在真實瀏覽器中運行的
Playwright 和 Cypress 整合測試使用 `setupWorker`。同一份處理器定義，不同的執行環境——
這是 MSW 的核心設計主張，也是它相較於其他網路模擬方案在長期維護上的主要優勢。對大多數
專案而言，從 `setupServer` 開始是最合理的起點，待有需要在瀏覽器環境中重用相同處理器時，
再引入 `setupWorker`。
...
- [MSW：快速入門](https://mswjs.io/docs/quick-start)
- [msw-storybook-addon](https://storybook.js.org/addons/msw-storybook-addon)
- [MSW：GraphQL 支援](https://mswjs.io/docs/api/graphql)
- [Testing Library：非同步工具](https://testing-library.com/docs/dom-testing-library/api-async/)
- [MSW：請求處理器 API](https://mswjs.io/docs/api/http)
- [Vitest：測試環境設定](https://vitest.dev/config/#environment)
