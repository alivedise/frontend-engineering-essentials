...
**應該（SHOULD）** 對 E2E 選擇器使用 `data-testid` 屬性，而非 CSS 類別名稱或元素結構。CSS 類別在 UI 重新設計時會改變；`data-testid` 屬性是元件與測試之間的明確契約。

**應該（SHOULD）** 在 CI 上配置測試重試（Playwright CLI 使用 `--retries 2`，Playwright 設定使用 `retries: 2`，或 Cypress 設定使用 `retries: { runMode: 2 }`）。網路時序和 CI 環境變異會導致間歇性失敗，而這些並非真正的錯誤；重試能吸收偶發的不穩定性，而不會遮蔽系統性失敗。

**應該（SHOULD）** 優先使用自動等待斷言（Playwright 的 `expect(locator).toBeVisible()`，Cypress 的 `cy.get(...).should('be.visible')`），而非睡眠或輪詢。這兩個框架在執行動作前都會自動等待元素出現；明確的睡眠是一種程式碼壞味道，表明存在框架本應處理的時序假設。

**不應該（SHOULD NOT）** 在 E2E 測試之間共享可變狀態。每個測試應設置並清理自己的資料，或使用測試隔離機制（每個測試使用獨立使用者、資料庫事務回滾、測試固定裝置）。
...
**在 E2E 測試中使用 CSS 類別選擇器。** 當設計系統重新命名類別、按鈕獲得修飾符類別，或重構中完全移除類別時，`cy.get('.btn-primary')` 或 `page.locator('.submit-button')` 就會失敗。改用 `data-testid` 屬性或明確穩定的可存取定位器（`getByRole`、`getByLabel`）。

**未在 CI 上配置重試。** 阻擋部署的單次不穩定失敗，比兩次額外重試的成本更具破壞性。將 `retries: 2` 設為 CI 的基準配置。如果特定測試偶爾重試，請調查根本原因——那將是真實的時序錯誤或測試隔離問題。

**撰寫依賴執行順序的測試。** 測試 A 建立一個使用者。測試 B 期望那個使用者存在。當測試執行器平行化或打亂測試時，測試 B 間歇性失敗。每個測試必須在 `beforeEach` 鉤子中建立自己的資料或使用測試固定裝置。

**使用睡眠而非斷言等待。** `page.waitForTimeout(2000)` 是固定的睡眠，使測試更慢，且當操作耗時 2001ms 時仍然失敗。使用 `expect(locator).toBeVisible()` 或 `cy.get(...).should('be.visible')`，兩者都會輪詢直到條件為真或超時。框架負責處理時序。
