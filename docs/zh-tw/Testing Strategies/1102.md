...
**應該（SHOULD）** 透過元素的可存取 role 或 label 查詢元素——使用 `getByRole`、`getByLabelText`——而非 `data-testid`。如果你無法透過 role 找到一個按鈕，該按鈕可能缺少其 ARIA role，這是真實的無障礙缺陷，而非僅僅是測試不便。

**應該（SHOULD）** 使用 `@testing-library/user-event` 的 `userEvent`，而非 `@testing-library/react` 的 `fireEvent`。`userEvent` 模擬真實使用者會產生的完整瀏覽器事件序列（pointerdown、mousedown、focus、keydown、input、keyup、change、blur）。`fireEvent` 僅派發單一合成事件——對簡單情況足夠，但不足以測試焦點管理、鍵盤導覽和輸入遮罩。

**不應該（SHOULD NOT）** 對元件的內部狀態、React ref 或實作層面的屬性進行斷言。測試 DOM 中渲染的內容以及發出的事件，而非元件如何在內部產生這些輸出。

**不應該（SHOULD NOT）** 將 `getByTestId` 作為首要查詢策略。它作為當沒有可存取查詢方式時的逃生艙口存在。優先使用它會產生即使無障礙性已損壞也能通過的測試。
...
**將 `getByTestId` 作為預設查詢。** 當 `data-testid` 無所不在時，即使元件完全無法存取，測試也能通過。如果你的按鈕有 `data-testid="submit"` 但沒有 `role`、沒有可見文字、也沒有 `aria-label`，測試是綠色的，但按鈕對螢幕閱讀器而言是不可見的。優先使用可存取查詢，只有在沒有可存取替代方案時才使用 `getByTestId`。

**在需要 `userEvent` 時使用 `fireEvent`。** `fireEvent.change(input, { target: { value: 'x' } })` 設定值並觸發一個 change 事件，跳過所有逐按鍵邏輯。使用 `onInput`、`onKeyDown` 或透過受控輸入強制執行字元計數的元件，在 `fireEvent` 下的行為將與真實使用者輸入不同。預設使用 `userEvent.type()`。

**用 `getBy` 斷言不存在。** `expect(screen.getByRole('alert')).not.toBeInTheDocument()` 若元素不存在，在 `expect` 執行之前就會拋出錯誤——因為 `getByRole` 在未找到時拋出。對不存在斷言使用 `queryByRole`：`expect(screen.queryByRole('alert')).not.toBeInTheDocument()`。

**沒有 `await` `findBy` 查詢。** `screen.findByRole('heading')` 回傳一個 Promise。忘記 `await` 會導致測試對未解析的 Promise 物件進行斷言，而該物件永遠是真值，這意味著無論元素是否出現，測試都會通過。
...
**測試內部狀態而非可觀察輸出。** 從元件外部存取 `component.state`、React ref 值或內部 hook 回傳值，會將你的測試與實作細節耦合。一旦你將該邏輯提取至自訂 hook、移至 context 或重命名內部變數，測試就會失敗——即使渲染的 UI 完全相同。測試使用者在 DOM 中觀察到的內容。
