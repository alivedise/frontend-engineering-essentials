...
**驗證使用者或模組消費者可觀察到的行為——絕不測試內部實作細節。** 斷言應放在使用者或呼叫端所觀察到的輸出上：元件渲染的文字、提交的 API 載荷、失敗路徑產生的錯誤訊息。測試必須（MUST）斷言可觀察的輸出，而非呼叫了哪個私有方法或內部屬性的值。當重構改變了實作但行為不變時，這類測試不應該（SHOULD NOT）失敗。

**將覆蓋率作為發現未測試程式碼的缺口偵測器，而非用來認證測試正確性的品質關卡。** 覆蓋率衡量的是執行路徑，不是斷言的正確性。100% 覆蓋率禁止（MUST NOT）被解讀為對正確性的信心。實際的閾值（70–80% 行覆蓋率）應用來標記完全沒有測試觸及的檔案，而不是激勵無斷言的測試填充。覆蓋率下降是尋找缺失測試的信號；覆蓋率上升不是測試品質好的證明。

**定期執行突變測試，以找到覆蓋率無法偵測到的行為缺口。** 突變測試問的是「有任何測試真正驗證了這一行的作用嗎？」，而非覆蓋率所問的「這一行被執行了嗎？」。你應該（SHOULD）每週執行一次突變測試，或僅對 PR 中更改的檔案使用 `stryker run --since` 執行，以可管理的成本獲得高信號的回饋。完全跳過突變測試，會讓整類測試品質缺口永久不可見。

**刪除脆弱的測試，而非讓它們保持被跳過狀態。** 被標記為 `.skip` 的測試是被忽略的測試，不是通過的測試。每個被跳過的測試都代表 CI 不再驗證的一個行為。你必須（MUST）選擇：修復不穩定性，或刪除該測試並寫一個更好的。持續增長的被跳過測試清單是測試套件腐化的跡象，必須（MUST）被正視處理。
