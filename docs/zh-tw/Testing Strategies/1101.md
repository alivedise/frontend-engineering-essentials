...
**應該（SHOULD）** 在所有新的 Vite 專案中使用 Vitest。它共用既有的 `vite.config.ts`、支援原生 ESM，並且冷啟動速度比 Jest 快得多。

**不應該（SHOULD NOT）** 以快照測試取代斷言式測試。一個渲染完整元件 JSX 樹的快照，任何人只要執行 `vitest --update-snapshots` 就會在未仔細閱讀的情況下批准它。優先使用明確的斷言，而非快照，來驗證使用者所見的內容。
...
**在錯誤的位置重置模擬。** `vi.fn()` 會累積呼叫歷史。如果你在測試 B 中檢查 `expect(mockFn).toHaveBeenCalledOnce()`，而測試 A 也呼叫了同一個模擬一次，你的斷言將因為錯誤的原因而通過。在 `beforeEach` 中使用 `mockClear()`，或在 `vitest.config.ts` 中全域設定 `clearMocks: true`。
...
**未閱讀就批准快照。** 在快照失敗後執行 `vitest --update-snapshots` 而不檢視差異，是快照最常見的侵蝕測試品質的方式。如果你說不清楚快照為何改變，這個測試帶給你的不是信心——而是虛假的覆蓋率感。

**以覆蓋率閾值作為目標。** `expect(fn()).toBeTruthy()` 斷言在覆蓋率目的上覆蓋了該函式，但幾乎沒有斷言任何有意義的內容。覆蓋率是發現未覆蓋程式碼的訊號，而非測試品質的證明。目標是當你引入合理的錯誤時測試會失敗，而不是讓百分比數字上升。
