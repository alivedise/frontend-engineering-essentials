...
**對所有新 Vite 專案必須（MUST）使用 Vitest 而非額外建立 Jest 設定。** Vitest 讀取現有 `vite.config.ts` 中的 `test` 區塊，路徑別名、插件和環境變數會自動繼承，不需要 `moduleNameMapper` 重複設定。這對新專案來說是必須（MUST）遵守的原則；獨立的 `jest.config.ts` 與 `vite.config.ts` 之間的設定漂移，應該（SHOULD）被視為維護負債，而非小問題。
