...
**使用整頁截圖而非範圍限定的元素截圖。** 用整頁截圖進行元件測試，就像在單元測試中使用 `document.body.innerHTML`：你得到的雜訊遠多於有效信號。動態的側邊欄、Cookie 提示橫幅，或包含即時資料的頁尾，每次執行都會造成假陽性。始終將截圖範圍限定在你關心的特定元件或區域。

**提交在開發者機器上生成的基準。** 在 macOS 上用 Retina 顯示器和特定字型平滑設定生成的基準，不會匹配 CI 執行器上 Ubuntu 的截圖。結果是永久性的假陽性。制定政策：基準始終從 CI 生成和儲存。如果需要更新基準，向 CI 推送一個提交並在那裡觸發基準擷取，或在本地使用 Docker 來匹配 CI 的渲染環境。

**將閾值設得太低或太高。** 0 個像素差異的閾值會在每條反鋸齒邊緣上持續失敗。10% 的閾值會讓實質性的佈局迴歸通過。從 `maxDiffPixelRatio: 0.01`（1%）開始，根據你在特定渲染環境中觀察到的雜訊進行校準。不同元件可能需要不同的閾值：文字密集的元件比純色塊更嘈雜。

**擷取前未停用動畫。** 在動畫循環中的隨機時刻擷取旋轉的載入 spinner，每次都會產生不同的截圖。任何在擷取截圖時正在運行的 CSS `transition` 或 `animation`，都會引入時序相關的差異。對視覺回歸擷取始終注入 `animation-duration: 0ms`，或使用 `waitForTimeout` 等待動畫完成——不過 timeout 本身也會引入不穩定性。
