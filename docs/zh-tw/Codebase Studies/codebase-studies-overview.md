...
以下是一篇 Codebase Studies 文章的形態，這是 FEE-1801 範例段落的單段摘錄，呈現以 commit 固定的原始碼引用：
...
此分類的核心校準是：**程式碼庫是見證者**，而非主題本身。讀完 FEE-1801 的人應帶走對 *dispose 生命週期模式* 的認識（可套用或可辨識的具名模式），Three.js 則是展示該模式的具體見證。文章的職責是教授該模式，並以 Three.js 的原始碼作為佐證材料。
