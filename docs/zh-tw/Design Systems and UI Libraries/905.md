...
深色模式是代幣層級的關注點，而非元件層級的關注點。元件**必須不**包含硬編碼的顏色值。每個顏色**必須**參照語義 CSS 自訂屬性。當此契約被遵守時，切換主題只需覆蓋一組 CSS 變數——零元件更改，零 JavaScript 重新渲染。

語義代幣與其解析值的分離，使主題切換在機制上變得簡單。渲染 `color: var(--color-text-primary)` 的元件不知道那在亮色模式下解析為 `hsl(222 47% 11%)`，還是在深色模式下解析為 `hsl(210 40% 98%)`。這是正確的狀態：元件擁有其語義意圖；主題擁有解析後的值。

原始代幣——如 `--color-blue-500: hsl(217 91% 60%)` 的原始值——**必須不**有特定主題的覆蓋。原始代幣代表值，而非意圖。只有語義代幣——`--color-interactive-default: var(--color-blue-500)`——需要深色模式 variant，因為只有語義代幣表達了可能因主題而改變的意圖。

各團隊**應該**同時實作 `prefers-color-scheme` 偵測（適用於尚未在應用程式中設定偏好的使用者）和基於 class 的切換（適用於想要覆蓋系統設定的使用者）。這兩種機制服務於不同的使用者：忽略前者會破壞設定了全系統深色模式偏好的使用者的體驗；忽略後者使使用者無法在應用程式中使用與 OS 偏好不同的模式。
...
**對主題切換使用 `transition: all`。** `transition: all` 對所有改變的 CSS 屬性做動畫——包括版面配置屬性。在主題之間內容改變時寬度改變的元件（例如重排的導覽列）會產生意外的版面配置動畫。只指定顏色相關屬性：`transition: color 150ms ease, background-color 150ms ease, border-color 150ms ease`。
