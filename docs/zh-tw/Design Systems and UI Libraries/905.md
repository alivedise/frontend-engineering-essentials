...
設計系統中的每個元件都會渲染顏色：背景色、文字顏色、邊框顏色、互動狀態顏色。元件必須使用顏色。問題在於這些顏色是以硬編碼值表達，還是以共享語義契約的參照表達。

當顏色被硬編碼（`bg-white`、`text-gray-900`、`border-gray-200`）時，深色模式需要修改每個元件。一個有 50 個元件的程式庫需要更新 50 個檔案，且有遺漏某個的風險。當顏色以 CSS 自訂屬性參照表達（`var(--color-surface-page)`、`var(--color-text-primary)`、`var(--color-border-subtle)`）時，深色模式只需要一次性覆蓋一組變數。零元件更改，零 JavaScript 重新渲染。
...
CSS 自訂屬性層疊是使主題運作的機制。亮色主題代幣在 `:root` 上定義，深色主題覆蓋在套用於 `html` 或 `:root` 元素的 `[data-theme="dark"]` 或 `.dark` 上定義。所有元件參照語義代幣。當 class 或屬性改變時，層疊將代幣解析為深色值，每個參照這些代幣的元素自動更新。不需要 JavaScript 重新渲染元件。
...
這是基準線。在 OS 偏好中設定了「深色模式」的使用者自動獲得深色主題，不需要任何應用程式內的使用者互動。沒有偏好需要儲存，也不需要執行任何 JavaScript。即使 JavaScript 載入失敗，它也能運作。
...
伺服器端渲染帶來了特定挑戰：伺服器在沒有存取使用者儲存主題偏好的情況下渲染 HTML。用戶端的首次繪製使用伺服器渲染的 HTML，反映的是預設（亮色）主題。幾分之一秒後，JavaScript 執行、讀取 `localStorage`，並套用深色 class。頁面從亮色閃爍到深色。這被稱為主題的 FOUC（Flash of Unstyled Content，無樣式內容閃爍）。
...
**只有語義代幣需要深色模式 variant。** 原始代幣是不會改變的調色板值。只有語義代幣，也就是表達意圖的代幣，才有特定主題的覆蓋。將原始代幣保留在 `.dark` 覆蓋之外。
