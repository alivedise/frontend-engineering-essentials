...
以下範例中，元件代幣為求簡潔而與語意代幣置於同一個 `semantic.json` 中。在正式的代幣系統中，元件代幣通常會獨立放在 `component.json` 檔案中，並加入 Style Dictionary 的 `source` 陣列。
...
- [W3C Design Token Community Group — Format Specification](https://www.designtokens.org/tr/drafts/format/) — DTCG JSON 格式的規範性規格，包含所有已定義的 `$type` 值、複合代幣結構和別名參照語法。
