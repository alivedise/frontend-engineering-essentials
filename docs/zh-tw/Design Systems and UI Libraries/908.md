...
工程師**應該（SHOULD）**使用變體設定函式庫（CVA、`tailwind-variants`）來表達元
件變體，而非在行內撰寫條件式 className 邏輯。具備三個或以上獨立變體維度的元件，
需要隨每個新維度呈二次方增長的條件邏輯；變體設定物件則在維度數量上呈線性增長，
並自動產生自文件化的 API 介面。變體設定方式還能自動生成 TypeScript 型別，無需
手動撰寫即可對變體 API 進行靜態驗證。

工程師**必須（MUST）**在元件層級的樣式中參照語意設計代幣，而非直接使用原始代
幣值。一個按鈕設定 `background-color: var(--color-blue-600)` 與特定顏色耦合，每
當調色盤更換就必須跟著修改。一個按鈕設定
`background-color: var(--color-action-primary)` 與語意概念耦合，主題更換、品牌
更新或深色模式切換時，元件原始碼完全不需要任何修改。
