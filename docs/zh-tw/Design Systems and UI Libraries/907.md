...
- [lucide-react 文件](https://lucide.dev/guide/react) — `lucide-react` 的官方文件，包含安裝、props 參考和 tree-shaking 說明。
- [@radix-ui/react-icons 文件](https://www.radix-ui.com/icons) — Radix 圖示集的官方文件，包含完整的圖示目錄。
- [MDN：aria-hidden](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-hidden) — 何時以及如何使用 `aria-hidden` 從無障礙樹中移除元素。
- [MDN：aria-label](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-label) — `aria-label` 如何為沒有可見文字的互動元素提供無障礙名稱。
