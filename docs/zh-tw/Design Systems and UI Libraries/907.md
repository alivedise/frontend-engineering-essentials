...
失敗模式：`import * as Icons from 'lucide-react'`。這會使 webpack 等不具備 Rollup 範圍分析的打包工具失去 tree-shaking 能力。Vite（使用 Rollup）仍可 tree-shake 命名空間匯入，但此行為因打包工具而異。優先使用具名匯入以在所有打包工具中確保消除效果。
...
**使用 `width` 和 `height` HTML 屬性設定可縮放圖示。** HTML 屬性在 SVG 元素上設定固定像素尺寸，會忽略 `em` 單位、響應式工具和 CSS 自訂屬性——`width="20"` 不會隨周圍字型大小縮放。對需要響應式或隨文字縮放的圖示，請改用 CSS——Tailwind 的 `h-4 w-4`、CSS 的 `width: 1rem; height: 1rem`，或透過 `size` prop 設定 CSS `width`/`height`。
