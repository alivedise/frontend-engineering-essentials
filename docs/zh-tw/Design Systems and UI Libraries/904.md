...
沒有 story 的元件就是沒有合約的元件。Story**必須**記錄每個有意義的狀態——不只是正常路徑。各團隊**必須**將 story 檔案與元件檔案放在同一位置。

正常路徑是每個開發者都知道的狀態。在生產環境失敗的是那些沒人記錄的狀態：非同步操作進行中時的 `disabled`、網路呼叫失敗時的 `error`、清單有零個項目時的 `empty`、資料正在獲取時的 `loading`。這些狀態有特定的視覺需求——具體到某人在設計審查中核准了它們。它們需要作為 story 存在，因為 story 是「這是元件在此狀態下應該呈現的樣子」的唯一持久記錄。

同位置放置不是風格偏好，而是防止文件漂移的維護慣例。當 `Button.stories.tsx` 與 `Button.tsx` 在同一個目錄時，每個開啟元件的開發者都能立即看到 story。當 story 存放在獨立的 `__stories__` 目錄或獨立的 `storybook/` 目錄樹時，在例行元件工作中它們是不可見的，是元件重構時最先失去同步的東西。

各團隊**應該**在每個 Storybook 配置中加入 `@storybook/addon-a11y`。無障礙問題在開發階段比在 QA 或收到投訴後更容易修復。這個外掛程式對每個 story 渲染執行自動化的 axe-core 稽核，在 Storybook UI 中直接呈現遺漏標籤、對比度不足和無效 ARIA 屬性等問題。
...
**只撰寫 Default story。** Default story 記錄了正常路徑，它沒有記錄元件在 disabled、loading 或發生 error 時的樣子。這些是有特定視覺需求的狀態，也是最先靜默回歸的狀態。從一開始就為每個有意義的狀態撰寫 story。
...
**不將 story 與元件放在同一位置。** 當 story 與它們記錄的元件存放在不同目錄時，它們在例行元件工作中是不可見的。開發者編輯元件、忘記 story 的存在，然後推送破壞 story 的更改。同位置放置是讓 story 可見並保持維護的慣例。
