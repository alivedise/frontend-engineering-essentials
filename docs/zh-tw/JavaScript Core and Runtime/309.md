...
四種弱原語 -- `WeakMap`、`WeakSet`、`WeakRef` 和 `FinalizationRegistry` -- 共享一個共同約束：鍵和被登錄的目標必須是物件（或在較新的引擎版本中，某些已登錄的符號）。字串、數字和布林等原始值不是 `WeakMap` 或 `WeakSet` 的有效鍵，因為原始值是值類型：沒有可以弱持有的身份，也沒有可以追蹤可達性的物件。這個限制在執行時期以 `TypeError` 強制執行。

ES2023 將 `WeakMap` 和 `WeakSet` 擴展為也接受已登錄符號（registered symbols）作為鍵，這些是 `Symbol.for(key)` 的結果 -- 它們具有全域身份，可以透過鍵查找。以 `Symbol()` 創建的未登錄符號不是有效鍵，因為它們沒有引擎可以追蹤用於 GC 目的的全域身份。

## 原則

工程師應該（SHOULD）在將私有元資料或快取與具有獨立生命週期的物件關聯時，優先使用 `WeakMap` 而非 `Map`。`Map<HTMLElement, EventData>` 會無限期保留所有曾見過的元素，在任何會建立並捨棄元素的應用程式中無限增長。`WeakMap<HTMLElement, EventData>` 在元素被回收後自動釋放條目，從結構上使記憶體洩漏不可能發生，且不需要呼叫者提供任何顯式清理程式碼。在 `Map` 和 `WeakMap` 之間的選擇，主要不是效能決策，而是關於所有權和生命週期的正確性決策。

工程師禁止（MUST NOT）將 `WeakRef` 或 `FinalizationRegistry` 作為顯式資源清理的替代方案。GC 的執行時機是不確定的。`WeakRef` 的目標物件可能在進程的整個生命週期內都存活，也可能在毫秒內被回收 -- 兩者都沒有保證，且規格刻意不承諾終結回呼的交付順序或時機。這些 API 適用於可以容忍過期條目的快取，以及偵測漏掉清理操作的除錯工具。檔案控制代碼、網路連線、事件監聽器移除和計時器取消都需要顯式的生命週期管理，無論是否涉及弱引用。

`Map` 和 `WeakMap` 之間的選擇不只是關於記憶體 -- 它也是關於語意的正確性。當你選擇 `WeakMap` 時，你在宣告：「這個元資料的生命週期與鍵物件的生命週期相同。」當你選擇 `Map` 時，你在宣告：「這個關聯需要顯式管理，且可能比鍵物件存活更長。」讓選擇反映你對所有權的實際意圖，而不是以哪個先出現在腦海中為准。
...
**必須（SHOULD）在將元資料與 DOM 元素、類別實例或其他物件關聯，且元資料沒有獨立生命週期時，使用 `WeakMap`。** 最常見的誤用是在元件管理器、事件追蹤層或動畫系統中使用 `Map<Element, Data>`。當元素在導航或條件渲染期間被移除並替換時，`Map` 仍然保留它們，因為 `Map` 對所有鍵持有強引用，在長壽命 SPA 中無限增長。症狀是記憶體使用在整個 session 中穩定增長，在 Chrome DevTools 記憶體面板中可見為沒有來自文件根節點存活路徑的保留 `HTMLElement` 節點。`WeakMap` 在元素被回收時自動釋放條目，從結構上使洩漏不可能發生。無需 `onDestroy` 鉤子，無需 `registry.delete(el)` 呼叫，無脆弱的生命週期耦合 -- 清理是自動的且不可能在重構或元件函式庫升級期間被意外省略。另一方面，`WeakMap` 無法查詢其當前大小或迭代：若你需要這些能力，搭配顯式生命週期管理的 `Map` 是正確選擇。
