...
工程師禁止（MUST NOT）將 `WeakRef` 或 `FinalizationRegistry` 作為顯式資源清理的替代方案。GC 的執行時機是不確定的。`WeakRef` 的目標物件可能在進程的整個生命週期內都存活，也可能在毫秒內被回收 -- 兩者都沒有保證，且規格刻意不承諾終結回呼的交付順序或時機。這些 API 適用於可以容忍過期條目的快取，以及偵測漏掉清理操作的除錯工具。檔案控制代碼、網路連線、事件監聽器移除和計時器取消都需要顯式的生命週期管理，無論是否涉及弱引用。`Map` 和 `WeakMap` 之間的選擇不只是關於記憶體，更是關於語意的正確性：選擇 `WeakMap` 即宣告元資料與鍵物件的生命週期相同；選擇 `Map` 即宣告該關聯需要顯式管理、且可能比鍵物件存活更長。
