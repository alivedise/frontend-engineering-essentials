...
**MUST 明確處理非同步錯誤。** 每個 promise chain 都 MUST 有 `.catch()`，或者在 `async` 函式中被包在 `try/catch` 裡。Unhandled rejection 會被 runtime 記錄，但不會阻止應用程式在損壞的狀態下繼續執行。

**SHOULD 使用 error `cause` 來保留上下文。** 在捕捉並重新拋出時，用 `new Error('message', { cause: originalError })` 包裝，這樣完整的錯誤鏈就可以用於除錯。在正式環境程式碼中，遺失原始錯誤是最常見的錯誤之一。

**SHOULD NOT 使用空的 catch 區塊。** 靜默地吞掉錯誤會讓 bug 變得隱形。如果你刻意忽略錯誤，請加上註解說明原因。

**SHOULD 在 UI composition boundary 定義 error boundary。** 每個獨立有意義的 UI 區段（側邊欄、資料表格、表單）都應該被框架的 error boundary 包裝，這樣一個區段的崩潰不會拖垮整個頁面。
...
try {
  ...
  // 刻意忽略：localStorage 在私密瀏覽模式中可能已滿或被停用。
  // 應用程式不需要持久化這個偏好設定也能正常運作。
}
```
...
### 3. 忘記 async 函式回傳的是 rejected promise
...
### 4. 不處理 promise rejection

```js
// Bug：rejected promise 沒有 handler
fetch("/api/data").then((res) => res.json());
// 如果 fetch 失敗，unhandledrejection 會靜默觸發

// 修正：永遠附加 .catch() 或在 try/catch 中使用 await
fetch("/api/data")
  .then((res) => res.json())
...
- [TC39 -- Safe Assignment / Try Operator Proposal](https://github.com/arthurfiorette/proposal-safe-assignment-operator) -- Stage 0 的 Result 風格錯誤處理 JavaScript 提案
