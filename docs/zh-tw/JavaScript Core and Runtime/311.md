...
工程師應該（SHOULD）使用 `Proxy` 集中處理橫切關注點 — 驗證、變更通知、存取
日誌記錄、延遲初始化 — 而非在每個 setter 或方法中重複這些邏輯。Proxy 的價值
在於它在物件層級攔截操作：單一個 `set` 陷阱涵蓋目標上的每一個屬性，而無需在
建立 proxy 時列舉屬性。這使得 Proxy 適用於具有動態屬性集的目標 — 即物件的形狀
事先未知，且新屬性可能在 proxy 建立後才加入的情況。相較之下，在每個 setter
方法中手動加入驗證或通知邏輯，不僅需要對所有現有方法進行修改，還需要對未來
新增的每個屬性記住重新加入這些邏輯 — 這是一個脆弱的、依賴開發者自律的約束。

工程師必須（MUST）在 Proxy 陷阱中使用 `Reflect` 方法執行預設操作，而非直接存
取目標物件。在 `get` 陷阱中直接存取 `target[prop]` 對於具有存取器屬性或原型
鏈的物件會產生錯誤。`Reflect.get(target, prop, receiver)` 能正確傳遞接收者，
這對於繼承的 getter 透過 Proxy 正常運作至關重要。當陷阱完全不需要修改操作時，
應直接呼叫對應的 `Reflect` 方法並回傳其結果，以保留規範對該操作要求的所有不
變性（invariants）。對於 `set` 陷阱，這意味著在驗證通過後必須呼叫
`Reflect.set(obj, prop, value, receiver)` 並回傳其布林結果，而非假設賦值永遠
成功並直接回傳 `true`；若目標屬性是不可寫的，`Reflect.set` 會正確地回傳
`false`，而直接回傳 `true` 則會違反規範的不變性。
