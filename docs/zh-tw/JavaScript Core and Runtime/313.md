...
工程師應該（SHOULD）將 `structuredClone()` 作為純資料物件的預設深複製函式 —
包括普通物件、陣列、Map、Set、Date 以及類似的值型別資料。JSON 迴路方式
（`JSON.parse(JSON.stringify(obj))`）禁止（MUST NOT）用作通用的深複製：它靜默地
丟棄 `undefined` 屬性、將 `Date` 物件轉換為字串、在循環引用時拋出錯誤，並且對
`Map`、`Set` 和 `RegExp` 無聲地失敗。這些靜默的資料損壞會引發難以診斷的細微錯誤，
因為產生的物件在結構上看起來是合法的，實際上卻攜帶了被損壞的值。`structuredClone()`
對其所支援的所有型別預設是正確的，對不支援的型別則拋出 `DataCloneError`，而非
靜默地損壞資料。

工程師禁止（MUST NOT）期待 `structuredClone()` 能複製類別實例並保留其方法。
`structuredClone()` 不保留原型鏈。以 `structuredClone()` 複製的類別實例，會變成一
個僅具有相同自有可列舉屬性的普通物件，而沒有任何定義在類別原型上的方法。對複製品
呼叫任何原型方法，都會拋出 `TypeError`。需要複製類別實例的團隊，必須實作明確的
`clone()` 方法，或使用複製建構子模式 — 即靜態的 `fromData(data)` 或 `clone()` 方法，
從來源物件的自有資料建構一個新的完整初始化實例。這個限制同樣適用於函式（無法跨
序列化邊界傳輸）和 DOM 節點（它們是無法以結構化資料表示的宿主物件）。
