...
工程師在將元資料或協議鉤子（protocol hooks）添加至跨函式庫與應用程式程式碼
共享的物件時，應該（SHOULD）使用 `Symbol()` 作為屬性鍵，以避免屬性名稱衝突
無聲地破壞行為。使用 `__processed` 之類的字串鍵會有衝突風險：函式庫與應用
程式程式碼可能同時讀寫同一個槽位，而 bug 只在行為出現難以溯源的分歧時才浮
現。Symbol 鍵的碰撞機率不是低，而是零 — 因為鍵本身就是唯一的身份識別，沒
有任何程式碼能在不持有參考的情況下產生相同的 Symbol。傳遞給 `Symbol('desc')`
的描述字串會出現在 `sym.toString()` 和除錯器輸出中，但對身份識別、相等性或
查找行為沒有任何影響。

工程師禁止（MUST NOT）在意圖建立唯一、非共享鍵時使用 `Symbol.for()`。
`Symbol.for(key)` 從全域登錄表中回傳相同字串鍵對應的同一個 Symbol — 這是明
確的共享，而非唯一。任何 realm 中的任何程式碼只要呼叫 `Symbol.for('myLib.state')`，
都會取得同一個 Symbol，包括恰好選擇了相同登錄表字串的不相關程式碼。若第三
方套件獨立使用了相同的 `Symbol.for` 鍵字串，它便能完整存取函式庫儲存在該鍵
下的任何屬性。私有唯一鍵請使用 `Symbol()`；只有在明確需要跨 realm 共享時才
使用 `Symbol.for()`，且應將鍵字串記錄為穩定的公開介面，因為它成為任何呼叫
方都可觀察或修改的共享命名空間。
