...
**MUST 在元件卸載時清理事件監聽器與訂閱。** 每一個 `addEventListener`、`subscribe` 或 `observe` 呼叫，都 MUST 在元件銷毀時有對應的移除操作。洩漏的監聽器會使其閉包 -- 以及閉包所參照的所有內容 -- 無限期存活。

**SHOULD 使用 AbortController 進行監聽器清理。** 將 `AbortSignal` 傳入 `addEventListener`，這樣只需呼叫 `controller.abort()` 即可一次移除所有相關監聽器，省去保留個別處理函式參照的麻煩。

**SHOULD 使用 WeakMap 將元資料（metadata）關聯至 DOM 元素。** 當需要將資料附加至 DOM 節點（如快取計算值、儲存狀態）時，以節點作為鍵使用 `WeakMap`。這讓節點及其元資料在節點從文件中移除後可以一同被回收。

**SHOULD NOT 依賴垃圾回收的執行時機來保證正確性。** 垃圾回收是非確定性的。絕對不要撰寫依賴 finalizer 及時執行或 `WeakRef.deref()` 在特定時間點回傳 `undefined` 的程式碼。
...
### 2. 在 SPA 導頁時新增事件監聽器卻未移除

```js
// BUG: every navigation adds another listener -- they accumulate
function onRouteEnter() {
  window.addEventListener("resize", recalculateLayout);
}

// FIX: use AbortController tied to route lifecycle
function onRouteEnter() {
  const controller = new AbortController();
  window.addEventListener("resize", recalculateLayout, {
    signal: controller.signal,
  ...
  return () => controller.abort(); // call on route leave
}
```
...
### 4. setInterval 在元件卸載時未呼叫 clearInterval

```js
// BUG: interval runs forever after component unmounts
useEffect(() => {
  setInterval(() => {
    fetch("/api/status").then((r) => r.json()).then(setStatus);
  }, 5000);
  // No cleanup -- interval keeps firing, calling setStatus on unmounted component
}, []);

// FIX: store the interval ID and clear it in the cleanup function
useEffect(() => {
