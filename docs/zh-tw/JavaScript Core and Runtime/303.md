...
**Prototype 污染（Prototype Pollution）。** 由於 `Object.prototype` 位於每個普通物件 prototype chain 的頂端，任何寫入其中的屬性都會對執行環境中的所有物件可見。Prototype 污染發生於攻擊者控制的資料——通常透過不受信任輸入的 `JSON.parse`——到達類似 `obj[key] = value` 的程式碼路徑，且 `key` 為 `"__proto__"` 或 `"constructor"` 時。如 `{"__proto__": {"isAdmin": true}}` 的 payload 可以將 `isAdmin` 注入 `Object.prototype`，使所有後續的 `obj.isAdmin` 檢查都返回 `true`。防禦措施包括：在賦值前驗證 `key` 不是 `"__proto__"`、`"constructor"` 或 `"prototype"`；對接受外部鍵值的累加物件使用 `Object.create(null)`；以及使用能封鎖 prototype 修改鍵值的 schema 驗證函式庫。V8 團隊自 Node 12 / Chrome 80 起已強化 `JSON.parse`，將 `__proto__` 視為資料鍵而非 prototype 設定器，但手動撰寫的合併函式和遞迴賦值模式仍存在風險。
...
**V8 隱藏類別（Hidden Classes）與行內快取（Inline Caches）。** JavaScript 引擎不會在熱路徑上的每次屬性存取都進行完整的 prototype chain 遍歷——那會極為緩慢。取而代之，V8 會編譯最佳化的機器碼，假設共享相同「形狀」（以相同順序擁有相同自身屬性集合）的物件也共享相同的隱藏類別。隱藏類別記錄每個屬性的位元組偏移量，讓屬性存取不需要雜湊查詢即可達到 O(1)。當引擎首次遇到 `obj.x` 時，會生成一個行內快取——一小段機器碼，檢查 `obj` 的隱藏類別，若匹配則直接從快取的偏移量讀取 `x`。關鍵影響在於：當物件在建構函式中初始化所有屬性，且之後不動態新增或刪除屬性時，效能最佳。每次這樣的變動都會觸發隱藏類別轉換。起始相同但以不同順序新增屬性的物件會得到不同的隱藏類別，無法共享行內快取，從而從最佳化路徑退化為「多態（megamorphic）」備用路徑。
...
**Object.freeze 與 prototype chain。** `Object.freeze(obj)` 使 `obj` 的自身屬性變為不可寫入且不可設定，並阻止新增屬性。但它不會凍結 prototype。prototype chain 上任何可變的屬性仍然可達且可寫入。若目標是真正不可變的值，需凍結整條鏈——`Object.freeze(obj)` 和 `Object.freeze(Object.getPrototypeOf(obj))`——或搭配凍結的初始化物件使用 `Object.create(null)`。
