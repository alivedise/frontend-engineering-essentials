...
**SHOULD 在新程式碼中優先使用 ESM 而非 CJS。** ESM 支援靜態分析（static analysis）、搖樹優化（tree-shaking）和原生瀏覽器載入。CJS 在舊版相依套件和特定 Node.js 工具中仍有必要，但新套件和應用程式碼 SHOULD 使用 ESM。

**MUST 在 `package.json` 中以 `"sideEffects": false` 標記無副作用（side-effect-free）的套件。** 缺少此標示，打包工具就無法安全移除未使用的匯出。副作用（side effect）是指在匯入時執行且影響全域狀態的程式碼（polyfills、CSS 匯入、註冊呼叫）。若你的套件沒有這類程式碼，請明確宣告。

**SHOULD 在函式庫程式碼中使用具名匯出（named exports）而非預設匯出（default exports）。** 具名匯出能提供更好的搖樹優化、IDE 自動匯入和重構支援。預設匯出適用於單一用途的模組（如一個 React 元件檔案），但在函式庫重新匯出多個項目時會造成摩擦。

**MUST NOT 在 ESM 檔案中使用 `require()`。** 在單一檔案中混用模組系統會導致令人困惑的錯誤。若需要從 ESM 載入 CJS 相依套件，使用 `import`（Node.js 可匯入 CJS）或以 `node:module` 的 `createRequire` 作為最後手段。

**SHOULD 使用動態 `import()` 進行程式碼分割（code splitting）。** 靜態匯入會預先載入所有內容。使用 `import()` 在路由邊界、功能旗標或重型相依邊界進行分割，以降低初始載入時間。
...
```js
// utils.js -- 令人困惑：同時匯出預設和具名
export default function formatDate(d) { /* ... */ }
export function parseDate(s) { /* ... */ }

// consumer.js -- 容易寫錯
import formatDate from "./utils.js";            // 可行（預設匯出）
import { formatDate } from "./utils.js";         // 錯誤：沒有名為 'formatDate' 的具名匯出
import { default as formatDate } from "./utils.js"; // 可行但彆扭
...
### 5. 假設搖樹優化能移除所有未使用的程式碼（sideEffects 欄位）
