...
**SHOULD 預設使用 `===`。** Strict equality 是可預測的：只有當型別和值都匹配時才回傳 `true`。這消除了一整類 bug。

**例外：`== null` 是有用的慣用語法。** 表達式 `value == null` 在 `value` 為 `null` 或 `undefined` 時為 `true`，其他所有情況為 `false`。這是刻意且被廣泛接受的 loose equality 用法 -- 這是大多數 style guide 允許的唯一 `==` 檢查。

```js
// 推薦：用一個表達式同時檢查 null 和 undefined
if (value == null) {
  return fallback;
}
...
**SHOULD 在 `-0` 或 `NaN` 語意重要時使用 `Object.is()`。** 在數值演算法、Map 鍵或 SameValue 比較（如 `Array.prototype.includes`）中，`Object.is()` 提供數學上正確的答案：
...
MDN 指出：「Strict equality 幾乎永遠是正確的比較運算。」除非有特定理由，否則所有相等性檢查都應使用 `===`。可透過 ESLint 的 `eqeqeq` 規則自動強制執行。

```js
// 正確：strict equality -- 不會有型別意外
user.role === "admin";
count === 0;

// 可接受的例外：== null 同時檢查 null 和 undefined
...
需要轉換型別時，使用 `Number()`、`String()`、`Boolean()` 進行明確轉換，而不是依賴隱式 coercion。明確轉換讓意圖對閱讀者一目瞭然，並防止 coercion 規則靜默地產生意外結果。

```js
// 隱式 -- 脆弱，依賴 coercion 規則
const n = +"42";         // number（一元正號）
const s = "" + 42;       // string（+ 串接）
const b = !!value;       // boolean（雙重否定）
...
全域的 `isNaN()` 在測試前會先將參數強制轉換為數字，因此 `isNaN("blabla")` 回傳 `true`，即使 `"blabla"` 是字串而非 `NaN`。`Number.isNaN()` 不進行強制轉換：只有當參數的型別是 Number 且值為 `NaN` 時才回傳 `true`。

```js
isNaN("blabla");          // true  -- 字串被強制轉換為 NaN，結果誤導性強
isNaN(undefined);         // true  -- undefined 被強制轉換為 NaN，結果誤導性強

Number.isNaN("blabla");   // false -- 根本不是 Number 型別
Number.isNaN(undefined);  // false -- 根本不是 Number 型別
Number.isNaN(NaN);        // true  -- 正確
Number.isNaN(0 / 0);      // true  -- 正確
```
