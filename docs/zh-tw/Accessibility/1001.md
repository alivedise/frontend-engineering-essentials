...
瀏覽器為每個頁面維護兩棵樹：**DOM 樹**（完整的文件模型）以及**無障礙樹**（一個平行的表示結構，去除純視覺資訊，並將每個元素的語義角色、名稱、狀態與屬性暴露給輔助技術（assistive technologies，AT））。

當螢幕閱讀器、開關裝置或語音控制軟體需要理解 UI 時，它們查詢的是無障礙樹，而非直接操作 DOM 或視覺渲染結果。平台無障礙 API（macOS 的 NSAccessibility，即「AX API」；Windows 的 IAccessible2/UIA）介於瀏覽器與輔助技術之間，將樹結構轉譯成 AT 可消費的介面。
...
ARIA 是無障礙樹的詞彙表。它只改變無障礙樹所回報的內容 — 帶有 `role="button"` 的 `div` 會被公告為按鈕，但仍然無視 Enter 和 Space。
...
| `<select>` | `combobox` | 設定 `multiple` 屬性或 `size` > 1 時為 `listbox` |
...
| `role="alert"` | 緊急、中斷式公告的 live region（live region 指內容一變更 AT 就會自動公告的元素，見下方「Live Region（即時區域）」；隱含 `aria-live="assertive"`） |
...
屬性描述固定或半固定的特性，補充或覆寫元素的計算無障礙名稱與描述。核心的命名與關聯屬性：
...
完整的屬性清單（`aria-owns`、`aria-haspopup`、`aria-level` 等）定義於 [WAI-ARIA 1.2 規格](https://www.w3.org/TR/wai-aria-1.2/#state_prop_def)。

### ARIA 狀態（States）

狀態描述在互動過程中會改變的動態條件，應即時反映。本文用到的狀態：
...
其他狀態如 `aria-busy`、`aria-invalid` 與 `aria-current`，定義於同一個 [WAI-ARIA 1.2 章節](https://www.w3.org/TR/wai-aria-1.2/#state_prop_def)。
...
下圖展示 DOM 元素如何讓輔助技術得以存取。瀏覽器解析 HTML、建立 DOM，並從中推導無障礙樹。ARIA 屬性修改無障礙樹層。平台無障礙 API（例如 macOS 的 NSAccessibility 或 Windows 的 UI Automation）將樹暴露給輔助技術。
...
<button aria-label="關閉">
  <!-- focusable="false" 只影響舊版 IE/Edge，這些瀏覽器會把 SVG 排入 tab 順序 -->
...
```html
<!-- polite（禮貌）live region：搜尋結果、狀態更新 -->
...
<!-- assertive（急切）live region：僅用於緊急錯誤 -->
<div role="alert">
  <!-- 僅用於無法等待使用者閒置的緊急錯誤 -->
...
注意：`role="alert"` 自動隱含 `aria-live="assertive"` 和 `aria-atomic="true"`；依 ARIA 規格，`role="status"` 也隱含 `aria-live="polite"` 和 `aria-atomic="true"`。像上方 `role="status"` 範例那樣同時加上明確的 `aria-live` 與 `aria-atomic` 屬性，就規格而言是多餘的 — 但這能提升與較舊螢幕閱讀器的相容性，因此仍是常見做法。`role="alert"` 範例則刻意省略它們：MDN 提醒，`role="alert"` 搭配明確的 `aria-live="assertive"` 可能導致 iOS VoiceOver 重複公告。
...
**必須（MUST）優先選用原生 HTML 元素，而非 ARIA 重新實作。** 必須（MUST）在使用 `<div role="button">`、`aria-label` 或 `<div role="heading" aria-level="2">` 之前，優先選用 `<button>`、`<label>` 和 `<h2>`。原生元素免費提供角色、名稱計算、鍵盤行為和焦點順序。每個僅使用 ARIA 的替代方案，都需要你重新實作瀏覽器已內建的功能，而且難免遺漏邊角情況。

**禁止（MUST NOT）對可聚焦元素或包含可聚焦子元素的元素套用 `aria-hidden="true"`。** 將元素從無障礙樹中隱藏，同時讓它保持可鍵盤到達，會產生「幽靈控制項」：鍵盤使用者可以聚焦它，但螢幕閱讀器使用者無法感知或理解它。若要同時對視覺和非視覺使用者隱藏互動內容，使用 `display:none` 或 `hidden` 屬性 — 兩者也都會將元素從 tab 順序中移除。若內容必須保持可見但不可互動（例如 modal 開啟時其背後的頁面），使用 `inert` 屬性，它會將整個子樹同時從焦點順序和無障礙樹中移除。

**禁止（MUST NOT）以不受支援的方式使用與元素原生語義相衝突的 ARIA 角色。** 不要在 `<button>` 上放 `role="heading"`，也不要在 `<table>` 上放 `role="button"`。角色必須（MUST）用於原生語義與其不衝突的元素上。衝突的角色會產生不一致且令輔助技術使用者感到困惑的無障礙樹。

**必須（MUST）隨時保持 ARIA 狀態屬性與元件實際狀態同步。** `aria-expanded`、`aria-checked`、`aria-selected` 等屬性，必須（MUST）在對應的視覺或互動狀態每次改變時同步更新。過時的 ARIA 狀態比沒有 ARIA 狀態更糟——它會主動誤導螢幕閱讀器使用者關於元件目前的狀況。
...
W3C 的 Using ARIA 文件說明：「如果你可以使用已內建所需語義和行為的原生 HTML 元素或屬性，而非重新利用一個元素並添加 ARIA 角色、狀態或屬性來使其無障礙，那麼請使用原生元素。」
...
在每一步，開發者都在手工建構 `<button>` 免費提供的功能。而在每一步，都有出錯的可能 — 最容易漏掉的是 Space：它必須同時觸發 click，並抑制預設的頁面捲動。原生元素不只是更方便；它更強健，因為它的行為在瀏覽器中實作，已針對數千個螢幕閱讀器、AT 配置和邊角情況進行測試。

把 ARIA 保留給 HTML 無法表達的情況，例如可編輯的 autocomplete combobox 或 tree。
...
對大多數正式應用程式而言，國際化論點是決定性的：`aria-labelledby` 參照已在你的本地化流程中的文字。`aria-label` 的字串字面值對大多數 i18n 工具鏈是不可見的 — 必須在正常的 i18n 流程之外另行處理。在一個有許多元件和許多語系的大型應用程式中，維護 `aria-label` 字串的負擔相當可觀。
...
在容器上加上 `role="listbox"` 或 `role="combobox"`，告訴螢幕閱讀器將其視為該 widget 類型。螢幕閱讀器使用者隨即期待 ARIA Authoring Practices Guide 中定義的鍵盤互動模式：方向鍵在選項間移動、Home/End 跳至第一/最後，以及輸入字元即時跳到相符選項（type-ahead）。如果鍵盤行為未實作，widget 對 AT 使用者的困惑程度比直接使用 `<select>` 還要高。
...
### 將 live region 連同其內容一起插入 DOM

```html
<!-- 錯誤：live region 建立時訊息就已經在裡面 -->
<!-- JavaScript：
     container.innerHTML =
       '<div role="status" aria-live="polite">檔案上傳成功</div>';
-->
<!-- 螢幕閱讀器公告的是「已經被它發現的 live region」內部的*變更*。
     插入時內容已經就位的區域，往往完全不會被公告。 -->

<!-- 正確：頁面載入時就渲染空的區域，稍後再更新其文字 -->
<div role="status" aria-live="polite" id="upload-status">
  <span aria-hidden="true">✓</span>
  <!-- 裝飾性圖示：在這裡使用 aria-hidden="true" 是正確模式 -->
</div>
<!-- JavaScript，在上傳完成後：
     document.getElementById('upload-status')
       .appendChild(document.createTextNode('檔案上傳成功'));
-->
```

Live region 的公告是由「輔助技術已註冊的區域」內部的內容變更所觸發。頁面載入時先渲染空的容器（或只含裝飾性內容），事件發生時再注入訊息文字。在區域內用 `aria-hidden="true"` 隱藏裝飾性圖示沒有問題 — 這是裝飾性內容在任何位置（包括 live region 內）的標準模式。
...
assertive 公告很少是適當的 — session 逾時、嚴重的身分驗證錯誤、阻止頁面運作的瀏覽器相容性警告，除此之外寥寥無幾。其他所有情況都應使用 `polite`。
...
- [HTML 與語義化標記總覽](/zh-tw/HTML%20and%20Semantic%20Markup/100)
- [鍵盤導覽與焦點管理](/zh-tw/Accessibility/1002)
- [無障礙元件模式](/zh-tw/Accessibility/1007)
