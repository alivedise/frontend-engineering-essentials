...
**使用原生 HTML 元素，而非 ARIA。** 應該（SHOULD）在使用 `<div role="button">`、`aria-label` 或 `<div role="heading" aria-level="2">` 之前，優先選用 `<button>`、`<label>` 和 `<h2>`。原生元素免費提供角色、名稱計算、鍵盤行為和焦點順序。每個僅使用 ARIA 的替代方案，都需要你重新實作瀏覽器已內建的功能，而且難免遺漏邊角情況。
...
**應該（SHOULD）隨時保持 ARIA 狀態屬性與元件實際狀態同步。** `aria-expanded`、`aria-checked`、`aria-selected` 等屬性，必須（MUST）在對應的視覺或互動狀態每次改變時同步更新。過時的 ARIA 狀態比沒有 ARIA 狀態更糟——它會主動誤導螢幕閱讀器使用者關於元件目前的狀況。
