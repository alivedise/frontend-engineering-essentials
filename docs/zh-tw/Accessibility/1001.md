...
**在使用 ARIA 之前，優先選用原生 HTML 元素。** `<button>` 永遠優於 `<div role="button">`，`<label>` 永遠優於 input 上的 `aria-label`，`<h2>` 永遠優於 `<div role="heading" aria-level="2">`。原生元素免費提供角色、名稱計算、鍵盤行為和焦點順序。每個僅使用 ARIA 的替代方案，都需要你重新實作瀏覽器已內建的功能，而且難免遺漏邊角情況。

**禁止（MUST NOT）對可聚焦元素或包含可聚焦子元素的元素套用 `aria-hidden="true"`。** 將元素從無障礙樹中隱藏，同時讓它保持可鍵盤到達，會產生「幽靈控制項」：鍵盤使用者可以聚焦它，但螢幕閱讀器使用者無法感知或理解它。若要同時對視覺和非視覺使用者隱藏互動內容，請使用 `display:none`，這樣也會將元素從 tab 順序中移除。

**禁止（MUST NOT）以不受支援的方式使用與元素原生語義相衝突的 ARIA 角色。** 不要在 `<button>` 上放 `role="heading"`，也不要在 `<table>` 上放 `role="button"`。角色必須用於原生語義與其不衝突的元素上。衝突的角色會產生不一致且令輔助技術使用者感到困惑的無障礙樹。

**應該（SHOULD）隨時保持 ARIA 狀態屬性與元件實際狀態同步。** `aria-expanded`、`aria-checked`、`aria-selected` 等屬性，必須在對應的視覺或互動狀態每次改變時同步更新。過時的 ARIA 狀態比沒有 ARIA 狀態更糟——它會主動誤導螢幕閱讀器使用者關於元件目前的狀況。
