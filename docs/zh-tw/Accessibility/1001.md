...
**禁止（MUST NOT）對可聚焦元素或包含可聚焦子元素的元素套用 `aria-hidden="true"`。** 將元素從無障礙樹中隱藏，同時讓它保持可鍵盤到達，會產生「幽靈控制項」：鍵盤使用者可以聚焦它，但螢幕閱讀器使用者無法感知或理解它。若要同時對視覺和非視覺使用者隱藏互動內容，你必須（MUST）使用 `display:none`，這樣也會將元素從 tab 順序中移除。
