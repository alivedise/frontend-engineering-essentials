...
| `<main>` | `main` | 地標；每頁一個 |
| `<header>` | `banner`（頁面層級時） | 當為 `body` 的直接子元素時 |
| `<footer>` | `contentinfo`（頁面層級時） | 當為 `body` 的直接子元素時 |
...
- 開發者必須加上：`tabindex="0"`、檢查 Enter（鍵碼 13）和 Space（鍵碼 32）並手動觸發 click 的 `keydown` 處理器，以及防止 Space 的預設捲動行為。
...
注意：`role="alert"` 自動隱含 `aria-live="assertive"` 和 `aria-atomic="true"`。`role="status"` 自動隱含 `aria-live="polite"` 和 `aria-atomic="true"`。你可以使用角色簡寫，或在通用容器上使用明確的 `aria-live` — 但不要在同一個元素上同時使用兩者。
