...
**應該（SHOULD）在 CI 的每個 Pull Request 中執行 axe-core，並將違規視為測試失敗而非警告。** 透過 `@axe-core/playwright` 或 `vitest-axe` 將 axe-core 整合到測試套件中，可為每個元件提供有保障的結構基線：沒有缺少的表單標籤、沒有無效的 ARIA 組合、沒有低於可偵測門檻的對比度問題。axe 零違規報告是發布的必要前提，而非充分條件——它涵蓋了大約 30–40% 的真實 WCAG 問題。

**應該（SHOULD）在發布任何互動元件前進行純鍵盤測試。** 不動滑鼠，開啟、導航、關閉並退出每個互動元件。用 Tab 遍歷表單、用 Enter 或 Space 啟動下拉選單、用 Escape 關閉 modal，並確認焦點返回到觸發元素。許多鍵盤問題——缺少 Escape 處理、modal 關閉後焦點未返回、懸停專用的工具提示——對所有自動化工具都是不可見的，只能透過手動操作發現。

**應該（SHOULD）在上線新使用者流程前，至少用一個真實螢幕閱讀器進行測試。** Windows 上的 NVDA+Chrome 免費且覆蓋最大的免費 SR 使用者群；VoiceOver+Safari 內建於 macOS 和 iOS，是行動裝置測試不可或缺的目標。螢幕閱讀器測試能回答任何工具都無法回答的問題：播報序列是否合理、live region 更新是否在正確時機觸發、元件在 browse 模式與 forms 模式下的行為是否正確？
...
### 5. 安裝了 eslint-plugin-jsx-a11y 但未強制執行
