...
> **SHOULD**（建議）在發布無障礙功能前，至少以 NVDA+Chrome 與 VoiceOver+Safari 作為主要測試組合進行驗證。
>
> **MUST NOT**（禁止）在一般內容區域使用 `role="application"`——僅在需要完整鍵盤控制的 widget 區域（例如程式碼編輯器）才使用。誤用此角色會讓螢幕閱讀器使用者無法使用 browse 模式導覽。
>
> **SHOULD**（建議）使用真實的螢幕閱讀器軟體進行測試，而非依賴瀏覽器 DevTools 模擬——DevTools 無障礙面板可顯示無障礙樹，但無法重現螢幕閱讀器的實際播報行為。
...
### 在內容區域使用 `role="application"`

```html
<!-- 不良：停用整個頁面的所有 browse 模式快捷鍵 -->
<div role="application">
  <h1>文件</h1>
  <p>歡迎使用文件...</p>
  <!-- 按 H 期望導覽標題的 SR 使用者，
       在 application 模式中，H 改為輸入字母「h」。 -->
</div>

<!-- 良好：role="application" 僅用於真正的應用 widget -->
<div role="application" aria-label="試算表編輯器">
  <!-- 一個真正具有自訂鍵盤互動模型的 widget -->
</div>
```
