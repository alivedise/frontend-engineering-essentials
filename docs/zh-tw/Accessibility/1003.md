...
> **SHOULD** 符合 WCAG AA 對比度最低要求——一般文字 4.5:1，大型文字與 UI 元件邊界 3:1。在開發期間使用自動化工具（axe、Lighthouse、DevTools 對比度比值）驗證，而非只在設計交付時驗證。
>
> **MUST NOT** 以顏色作為傳遞資訊的唯一手段。每個以顏色編碼的狀態（錯誤、警告、成功、必填、選中）都必須有非顏色的補充：圖示、標籤、圖案或結構性指示器。
>
> **SHOULD** 支援 `prefers-reduced-motion`，將所有 CSS 過渡動畫包裝在 `@media (prefers-reduced-motion: no-preference)` 區塊中，確保在作業系統設定中啟用了減少動態效果的使用者，能獲得無動態的體驗。
...
/* 白色背景上的灰色文字——2.0:1——所有文字大小均不通過 AA */
.muted-fail {
  color: #aaaaaa;
...
/* 深藍色上的白色文字——7.0:1——通過 AA 與 AAA */
.badge-pass-aaa {
  color: #ffffff;
  background-color: #003d80;
}

/* 白色背景上的深灰色文字——12.6:1——通過 AA 與 AAA */
.body-text-pass {
  color: #222222;
  background-color: #ffffff;
}

/* 黃色背景上的深色文字——4.5:1——一般文字通過 AA */
.warning-badge-pass {
  color: #3d3000;
...
有六個切片（紅、橙、黃、綠、藍、紫）的圓餅圖，對 deuteranope 使用者而言幾乎無法辨別：紅、橙、黃切片變得幾乎無從區分。

修正方式：直接在圖表區段上加資料標籤、除顏色外使用明確的圖案或紋理（斜線），並選擇色盲安全的調色盤（如 IBM Carbon 無障礙色彩組合，或專為色覺缺陷設計的 ColorBrewer 定性調色盤）。
