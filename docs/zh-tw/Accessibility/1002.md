...
**應該（SHOULD）在關鍵狀態轉換後主動管理焦點。** 強制型對話框開啟時，必須（MUST）將焦點移至其中第一個可聚焦的元素；關閉時，必須（MUST）將焦點返還給觸發元素。SPA 路由切換後，將焦點移至新頁面的標題或 live region 通知。刪除已聚焦的列表項目後，將焦點移至下一個兄弟元素、前一個兄弟元素，或若清單已空則移至容器——絕對不要讓焦點消失到 `<body>`。
