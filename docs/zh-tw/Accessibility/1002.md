...
> **MUST NOT** 使用 `outline: none` 或 `outline: 0` 而不提供可見的替代 focus 指示器。移除 outline 而不提供替代方案會使鍵盤導覽不可見，並違反 WCAG 2.4.7（Focus 可見）。
>
> **SHOULD** 在強制型對話框開啟和關閉後、在單頁應用程式路由切換後、以及在刪除有 focus 的項目後，明確管理 focus。讓 focus 去到正確的地方——不要讓它消失。
>
> **MUST NOT** 使用大於零的 `tabindex` 值（`tabindex="1"`、`tabindex="2"` 等）。正數 tabindex 值會建立覆寫 DOM 順序的自訂 tab 順序，導致鍵盤使用者的 focus 跳躍令人困惑且不線性。
...
function RouteChangeAnnouncer() {
  ...
  return (
    ...
    {/* CSS 中加入：.route-heading:focus { outline: none } */}
  );
}
```
...
### 移除 focus 指示器而不提供替代方案

```css
/* WRONG——在數千個 CSS reset 中都能看到 */
* { outline: none; }
*:focus { outline: none; }

/* 目的是移除瀏覽器有時不美觀的預設環。
   結果是鍵盤使用者得到零視覺回饋。
   WCAG 2.4.7（Level AA）要求所有可鍵盤操作的元件都有可見的 focus。 */
```

修復：使用自訂指示器設定 `:focus-visible` 樣式。絕不在全域抑制 focus 指示器而沒有替代方案。

### 開啟 modal 而不將 focus 移入其中

```js
// WRONG：modal 出現，focus 停留在其後的觸發按鈕上
function openModal() {
  modal.classList.add('visible');
  // 沒有 focus 管理——鍵盤/螢幕閱讀器使用者被困在背景
}
...
### 忘記在關閉 modal 時恢復 focus

```js
// WRONG：modal 關閉時 focus 消失到 <body>
function closeModal() {
  modal.classList.remove('visible');
  // 沒有 focus 恢復——使用者必須從頭重新導覽
}

// CORRECT：將 focus 返回觸發器
function closeModal() {
  modal.classList.remove('visible');
  openButton.focus(); // 或 lastFocusedElement.focus()
}
```
