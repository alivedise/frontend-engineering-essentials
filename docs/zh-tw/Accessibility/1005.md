...
- **必須**將每個輸入元素與可見標籤關聯 — 不是 placeholder，不是 title 屬性，不是視覺上相鄰的段落。
- **必須**透過 `aria-describedby` 將每則錯誤訊息與其輸入元素關聯，確保欄位獲得焦點時錯誤會被播報。
- **應該**在建構自訂 combobox 之前先使用原生 `<select>` 元素；ARIA combobox 模式需要大量鍵盤處理邏輯，而原生 `<select>` 已免費提供這些功能。
...
Placeholder 是補充提示文字，不是標籤。用它提供格式範例，而非欄位名稱。

### 2. 視覺上關聯標籤，卻沒有程式化關聯

```html
<!-- 錯誤：<p> 看起來像標籤，但沒有程式化連結 -->
<p>名字</p>
...
螢幕閱讀器在 forms 模式中不會讀取周圍文字 — 只會讀取聚焦元素的無障礙名稱。

### 3. Radio 群組沒有 fieldset + legend
...
```html
<!-- 錯誤：錯誤注入 DOM 時被播報（role="alert"），但焦點回到欄位時不再播報 -->
<input type="email" id="email" />
<div role="alert">請輸入有效的電子郵件地址。</div>

<!-- 正確：aria-describedby 永久連結錯誤與輸入元素 -->
<input type="email" id="email" aria-describedby="email-error" aria-invalid="true" />
<div id="email-error" role="alert">請輸入有效的電子郵件地址。</div>
```

### 5. 必填標記只有視覺星號，沒有文字替代方案
