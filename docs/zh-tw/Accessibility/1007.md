...
- **SHOULD** 遵循 WAI-ARIA APG 模式來處理互動式 widget——角色、ARIA 屬性和鍵盤互動由 APG 為每種 widget 類型定義，而非每個元件自行發明。
- **SHOULD** 優先選用 headless 元件庫（Radix UI、React Aria、Headless UI），而非從頭建立自訂無障礙元件——無障礙實作成本高昂，且失敗模式非常隱微。
...
### 7. 在 headless 函式庫已存在的情況下自行建立自訂無障礙元件

除非你有特定理由避免依賴，否則請使用 Radix UI、Headless UI 或 React Aria。這些函式庫中的無障礙實作已跨多個螢幕閱讀器和瀏覽器進行測試，處理了你尚未遇到的邊緣情況，並在 ARIA 規格演進時持續維護。從頭正確實作焦點捕捉和 ARIA 屬性管理的生產力成本，一再被低估。
