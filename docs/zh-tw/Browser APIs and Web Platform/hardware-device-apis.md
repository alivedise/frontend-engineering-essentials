...
:::info
四套兄弟 API 讓網頁與實體硬體對話：WebUSB 做原始 USB 傳輸、WebHID 負責使用 HID 報告協定的輸入類裝置、Web Serial 服務一切看起來像序列埠的東西（Arduino、3D 印表機、ESP32 開發板）、Web Bluetooth 則透過 GATT 連接 BLE 周邊。它們共享同一套安全模型：安全情境、使用者手勢、由瀏覽器繪製且一次只授權一個裝置的選擇器。它們也共享同一個政治現實。四者皆由 Chromium 主導，Mozilla 與 WebKit 發表了負面立場，理由是指紋追蹤風險、以及「本頁面可能重新編程你的裝置」根本無法在權限提示裡向一般使用者解釋清楚，因此沒有一個是 Baseline。這使它們在定義上就是漸進增強 API：對韌體燒錄器、鍵盤設定器、面向 Chrome 使用者的實驗室工具而言絕佳，作為大眾市場功能的唯一路徑則是錯的。2026 年有一個立場真的動了：Mozilla 對 Web Serial 這一項的態度軟化為中立（neutral），Firefox Nightly 隨後以選擇性開關的形式推出了它。
:::

## 背景

這些 API 填補的缺口，過去由原生「配套應用程式」填補：鍵盤的重映射工具、印表機的韌體更新器、IDE 的開發板燒錄器。每一個都意味著一次下載、一個安裝程式、一個驅動程式。Chromium 在 Chrome 56 推出 Web Bluetooth、Chrome 61 推出 WebUSB（皆為 2017 年），再於 Chrome 89（2021 年）同時推出 WebHID 與 Web Serial，其上長出了一整個生態系：瀏覽器裡的 Arduino/MicroPython 工作流、VIA/Vial 鍵盤設定器、Android 刷機工具、POS 與實驗設備儀表板。Mozilla 的標準立場把 WebUSB、WebHID 與 Web Bluetooth 記錄為負面（negative，早期分類法稱為 harmful），Apple 的 WebKit 團隊在 2020 年那份列出 16 個易被濫用於追蹤的 API 的聲明中拒絕了整個家族，MDN 則把四者全標為 limited availability、非 Baseline。Web Serial 是唯一動了的例外：Mozilla 把立場軟化為中立，並於 2026 年在 Firefox Nightly 推出這個 API。規劃時，其餘三者仍應假設是 Chromium 專屬功能。本文涵蓋共享的權限模型、每套 API 的程式碼形狀，以及安全邊界（封鎖清單與受保護類別）實際劃在哪裡。
...
flowchart TD
    ...
    CHOOSER -- 取消 --> NODEV["USB/Serial/Bluetooth：拒絕（NotFoundError）<br/>WebHID：解析為空陣列"]
...
**Web Serial** 是四者中最友善的，因為它交給你的是標準串流。從微控制器讀取以換行分隔的 JSON 遙測，需要一個 `TextDecoderStream`、一個小小的分行 transform，加一個迴圈：
...
class LineBreakTransformer {
  ...
  transform(chunk, controller) {
    ...
    this.buffer = lines.pop(); // 保留最後那行不完整的部分
...
**WebHID** 打交道的單位是*報告（report）*：裝置透過報告描述符自述的固定格式二進位封包。自訂鍵盤的設定器監聽輸入報告、送出功能/輸出報告：

```js
const [device] = await navigator.hid.requestDevice({
  filters: [{ vendorId: 0x1234, usagePage: 0xff60 }], // 廠商自訂 usage page
});
if (!device) return; // 使用者取消：WebHID 解析為空陣列
await device.open();
...
**WebUSB** 位於最底層：在 `open()`、`selectConfiguration()`、`claimInterface()` 之後，你透過 `transferIn`/`transferOut`（bulk/interrupt 端點）與 `controlTransfer` 用裝置自身的協定溝通，等於用 JavaScript 寫驅動程式。形狀仍是同一套「選擇器然後開啟」的流程；酬載邏輯則完全依裝置而定。

在這四者上，跨造訪的重連都跳過選擇器：`getDevices()`/`getPorts()` 回傳先前已授權的裝置，`connect`/`disconnect` 事件在纜線插拔時讓 UI 與實際硬體狀態同步。

## 最佳實踐

- **必須（MUST）**把四者全部當作漸進增強：特性偵測（`"hid" in navigator`）、能力不存在時隱藏入口，並保留一條有文件的原生工具後備路徑。這些 API 沒有一個是 Baseline，其中三個在 Mozilla 與 WebKit 都有現行的負面立場。
- **必須（MUST）**在點擊處理器中呼叫 `requestDevice()`/`requestPort()`，因為瞬時啟動（transient activation，一次新的使用者手勢）是硬性要求；並把使用者取消選擇器設計為正常結果：WebUSB、Web Serial 與 Web Bluetooth 以 `NotFoundError` 拒絕，WebHID 則解析為空陣列。無論哪種，取消都不是例外路徑。
- **必須（MUST）**傳入能寫多窄就多窄的 filter（`vendorId`/`productId`、HID 的 `usagePage`、BLE 的 `services`）：選擇器只列出符合的裝置，這既是更好的 UX，也減少意外暴露。把整台機器的裝置全列出來的空 filter 清單是壞味道，而 Web Bluetooth 直接拒絕它：`filters: []` 會拋出 `TypeError`，要列出附近所有裝置必須明確傳 `acceptAllDevices: true`。
...
- **應該（SHOULD）**釋放你持有的資源：`port.close()` 前先 `reader.releaseLock()`、用完 `device.close()`、拆卸時 `gatt.disconnect()`。被占用的介面或被鎖定的串流會擋住其他所有頁面，常常連 OS 一起擋。
- **應該（SHOULD）**把傳輸迴圈包進真正的錯誤處理：硬體會在傳輸中途被拔掉，而失敗會以協定進行到一半時被拒絕的 promise 現身。
- **應該（SHOULD）**記住 Windows 的 USB 現實：WebUSB 只能 claim 綁定 WinUSB 驅動的介面；裝了廠商驅動的裝置列舉得出來、claim 不進去。
- **可以（MAY）**把韌體更新流程做在 WebUSB/Web Serial 上，這是這些 API 最對口的用途（不用安裝、燒錄器永遠是最新版）；也**可以（MAY）**在 Gamepad API 的通用映射不夠用時搭配 WebHID。

## 設計思維

**選擇器就是安全模型。**不同於攝影機/定位權限（「此來源可以使用該能力」），裝置家族授予的是*此來源可以使用這一個裝置*。瀏覽器繪製選單、頁面永遠看不到使用者沒選的裝置、filter 還限制選單列出什麼。這個設計刻意犧牲探索能力（頁面無法列舉硬體來指紋化一台機器），代價是每個裝置多一次點擊，也排除了「自動連上插著的裝置」這種 UX。

**引擎為何分裂。**Chromium 的立場：有了逐裝置同意、封鎖清單與受保護類別，剩餘風險值得用來換掉驅動安裝程式。Mozilla 對 WebUSB 與 WebHID 記錄為負面的立場：頁面把裝置重新編程成攻擊平台的風險（經典示範是把開發板變成會自己打指令的 USB 鍵盤）無法用任何非專家能評估的提示傳達，而 USB 層的識別碼是指紋表面。WebKit 站在同一邊。到 2026 年為止的盤面是：Mozilla 對 WebUSB、WebHID、Web Bluetooth 維持負面立場，WebKit 反對整個家族，唯一例外是 Web Serial，Mozilla 轉為中立並推出了 Nightly 實驗。其餘三者請按 Chromium 專屬來規劃。

**封鎖清單編碼了威脅模型。**WebUSB 拒絕 claim *受保護介面類別*（音訊、HID、集線器、大量儲存、智慧卡、視訊、音訊/視訊、無線控制器），所以頁面無法透過 USB 鍵盤的介面側錄按鍵、也讀不到隨身碟。WebHID 把系統鍵盤與滑鼠從列舉中剝除，並封鎖 FIDO usage page。Web Bluetooth 的 GATT 封鎖清單拒絕 HID 服務與 FIDO 流量，所以三種傳輸之上頁面都無法網釣驗證器。共同的脈絡：這些 API 交給你*你的*小裝置，並把每一個「一旦淪陷就打破他人信任邊界」的裝置類別挖掉。
...
**baudRate 之外的序列埠選項。**`port.open()` 接受 `dataBits`（7/8）、`stopBits`（1/2）、`parity`（`"none"`/`"even"`/`"odd"`）、`flowControl`（`"none"`/`"hardware"`）與 `bufferSize`。`readable` 串流在致命錯誤時結束（`done: true`）、在可復原錯誤後產生新的串流物件。因此標準讀取迴圈是*兩層*巢狀：外層 `while (port.readable)`、內層 `reader.read()`、`finally` 裡 `releaseLock()`。

**USB 傳輸類型。**端點有四種：control（設定/組態，經 `controlTransfer`）、bulk（大量可靠傳輸）、interrupt（小而時效敏感的封包）、isochronous（頻寬保證的串流，`isochronousTransferIn/Out`）。`claimInterface()` 取得介面端點的獨占所有權，這也是 OS 驅動與網頁永遠無法共享同一裝置的原因。

**BLE 的邊界。**Web Bluetooth 只支援 BLE/GATT：沒有 Classic Bluetooth 設定檔（A2DP 音訊、SPP 序列），所以藍牙喇叭在設計上就不在範圍內。它也是四者中唯一沒有 Worker 曝露的 API（沒有 `WorkerNavigator.bluetooth`）。特徵酬載很小，受 ATT MTU（Attribute Protocol 協商出的封包大小）限制，`writeValueWithResponse`/`writeValueWithoutResponse` 則把可靠性/延遲的取捨明白攤開。標準 16 位元 UUID（`"heart_rate"`）是 Bluetooth SIG 指配號碼的速記，自訂硬體則用完整 128 位元 UUID。行動裝置的電源管理會積極斷線；正式程式碼把 `gattserverdisconnected` 加上指數退避重連當作正常流程的一部分。
...
| Firefox | 否（負面立場） | 否（負面立場） | Nightly 實驗（2026）；立場中立 | 否（負面立場） |
| Safari | 否（反對） | 否（反對） | 否（反對） | 否（反對） |
| Baseline | 否 | 否 | 否 | 否 |
...
- **2026-04** — Web Serial 在 Mozilla 的標準立場轉為中立後，以選擇性開關的形式在 Firefox Nightly 推出：這個家族第一次非 Chromium 的動靜。
- **2021** — WebHID 與 Web Serial 隨 Chrome 89 推出。
- **2017** — Web Bluetooth 隨 Chrome 56 推出；WebUSB 隨 Chrome 61 推出。
