...
你一直在用 `fetch`、`localStorage` 和 `addEventListener` 開發功能，但新需求要求在使用者離線時進行背景同步、無需輪詢的即時通訊，以及存取本地端檔案系統。每項需求各自對應到一個五年前根本不存在的瀏覽器 API。瀏覽器 API 並非單一整體——它是一個持續擴充的能力集合，每個 API 都針對一類特定問題而設計，否則就必須透過伺服器來回請求或原生應用程式才能解決。
