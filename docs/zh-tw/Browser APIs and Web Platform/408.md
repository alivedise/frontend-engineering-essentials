...
你正在開發一個 3D 商品展示器，讓使用者能從任意角度旋轉、縮放、檢視模型。CSS 3D 變換與 Canvas 2D 無法呈現讓 3D 模型看起來逼真的陰影、光照與透視效果。WebGL 透過著色器程式存取 GPU，能以互動幀率實現基於物理的渲染。WebGPU 進一步延伸至現代 GPU 運算能力，讓瀏覽器同時具備 3D 渲染與 GPU 加速計算的能力。
...
**應該（SHOULD）在瀏覽器支援允許的新專案中，優先選擇 WebGPU 而非 WebGL 2。** 使用前務必以 `navigator.gpu` 和 `requestAdapter()` 進行功能偵測；即使屬性存在，adapter 也可能為 null，表示裝置不支援 WebGPU。在 Safari 17 及更早版本、以及尚未啟用旗標的 Firefox stable 中，`navigator.gpu` 為 undefined——WebGL 2（`getContext('webgl2')`）仍是這些環境最廣泛相容的備用方案。
