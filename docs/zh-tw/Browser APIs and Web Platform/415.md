...
工程師應該（SHOULD）在需要根據當前狀態調整應用程式 UI 時，於呼叫受權限管控的
API 之前先查詢權限狀態。以相機功能為例，在渲染相機按鈕之前查詢
`{ name: 'camera' }`，可讓按鈕在 `prompt` 狀態時顯示「授予存取」的操作提示、
在 `denied` 狀態時顯示「設定」連結、在 `granted` 狀態時直接啟動相機。這消除了
「呼叫 API 並捕獲錯誤」作為發現 `denied` 狀態的唯一手段的需要。查詢也是 UI
一致性的基礎：沒有狀態感知的應用程式在已授予相機的使用者和已封鎖相機的使用者
面前呈現相同的 UI，直到後者嘗試互動才暴露問題；而狀態感知的應用程式從一開始
就為每種情況提供正確的介面，創造了更加自信且可預期的使用者體驗。在以相機、
麥克風或地理位置為核心功能的應用程式中，提前查詢並調整 UI 是基本的工程預期，
而非可選的優化。

工程師必須（MUST）明確地為 `denied` 權限狀態設計對應的 UI。當使用者封鎖某項
權限後，瀏覽器不會再次顯示授權提示，除非使用者手動更改設定。當權限處於
`denied` 狀態時仍靜默失敗的應用程式——例如顯示永遠不會結束的載入指示器——
創造了一個毫無復原路徑的破損體驗。`denied` 狀態需要明確的 UI：一條說明訊息、
一個指向瀏覽器設定的連結，以及在某些情境下的替代操作流程。設計此 UI 時，應
將其視為一個獨立的使用者旅程，而非錯誤狀態的降級展示：使用者需要了解發生了
什麼、為何需要此權限、如何修復，以及若選擇不修復是否有替代路徑——在一個設計
良好的 `denied` 狀態 UI 中，這四個問題都應得到解答。
...
`navigator.permissions` 的舊版瀏覽器，需要先進行功能偵測——`'permissions' in navigator`
——再呼叫 `query()`。跳過此防護的應用程式，在缺少該 API 的瀏覽器上將同步拋出
...
觸發權限請求的 API 是分開的；Permissions API 是狀態讀取工具，而非請求觸發工具。
...
- MDN: Permissions-Policy 標頭 — https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Permissions-Policy
