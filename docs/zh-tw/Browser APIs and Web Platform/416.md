...
你正在為一個表單密集的應用程式開發無障礙功能，讓有動作障礙的使用者可以用語音填寫欄位。從頭實作語音辨識需要語音模型、音訊處理和大量基礎設施。Web Speech API 提供了瀏覽器原生的平台語音辨識引擎介面——`SpeechRecognition` 負責語音轉文字，`SpeechSynthesis` 負責文字轉語音——無需下載模型，也不需要伺服器往返。
