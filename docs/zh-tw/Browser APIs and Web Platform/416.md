...
工程師必須（MUST）將 Web Speech API 視為漸進增強，並為所有使用它的功能提供可正常運作的
回退 UI。`SpeechRecognition` 在 Firefox 中不可用，在 Safari 中支援有限。`SpeechSynthesis`
的聲音可用性因平台而異。僅依賴語音輸入或語音輸出、而沒有鍵盤或指標操作回退的功能，會將
不支援語音的瀏覽器使用者以及聲音不可用平台的使用者排除在外。正確的模型是將語音能力視為
基礎 UI 上的增強層——基礎 UI 在沒有語音能力的情況下也能正常使用——而非頁面必須依賴的前提
條件。在設計語音功能時，問題應該是「若語音不可用，此功能如何運作」，而非「若語音不可用，
此功能顯示錯誤訊息」；後者是將語音視為必要依賴，而非增強手段，這是設計上的根本錯誤。

工程師必須（MUST）在請求麥克風權限之前，向使用者揭露語音辨識會將音訊傳送至第三方伺服器
進行處理。Chrome 中的 Web Speech API 語音辨識實作會將音訊傳送至 Google 的伺服器。這是一
項資料隱私考量，需要使用者的知情同意，尤其是在受 GDPR、HIPAA 或類似法規約束的應用程式中。
揭露必須在麥克風權限提示出現之前呈現——而非埋藏在隱私政策中——以便使用者在音訊擷取開始
前能做出知情決策。在無法允許音訊離開裝置的受監管行業應用程式中，應評估伺服器端替代方案，
例如 Whisper API，這類方案可讓應用程式完全掌控音訊的傳輸與處理位置。揭露的形式應適應功能
的使用情境：首次使用時的一次性告知對低頻使用場景是適當的；而對於長期啟用語音辨識的聽寫
工具或客服系統，每次 session 啟動前的簡短提醒是使用者持續知情的必要保障。
