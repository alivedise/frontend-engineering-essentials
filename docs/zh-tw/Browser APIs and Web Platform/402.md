...
### SHOULD 使用 AbortController 清理事件監聽器

手動追蹤監聽器參考以呼叫 `removeEventListener` 容易出錯。單一 `AbortController` 可透過中止其 signal 一次清理多個監聽器：
...
**原則：** SHOULD 使用 `AbortController` 搭配 `signal` 選項來管理事件監聽器的生命週期，尤其在擁有多個監聽器的元件中。

### SHOULD 對捲動與觸控監聽器使用 passive: true

當 `touchstart` 或 `wheel` 監聽器非 passive 時，瀏覽器必須等待 JavaScript 完成才能確認是否會呼叫 `preventDefault()`。這會阻塞捲動並造成可見的卡頓 (jank)。

**原則：** SHOULD 將捲動與觸控事件監聽器標記為 `passive: true`，除非確實需要呼叫 `preventDefault()`。

### SHOULD 對動態內容偏好事件委派

不要為每個子元素個別附加監聽器，而是在穩定的祖先上附加單一監聽器，並透過 `event.target`（搭配 `closest()`）來判斷是哪個子元素被操作。這能減少記憶體使用、簡化清理工作，並自動處理動態新增的元素。

**原則：** SHOULD 對列表、表格等重複結構，在穩定的祖先元素上使用事件委派。
...
從 DOM 移除但仍被參考的元素上的監聽器會阻止垃圾回收。這在元件頻繁掛載與卸載的 SPA 中尤其常見：

```js
// 不佳：window 上的監聽器永遠不會被移除
function initComponent(el) {
  window.addEventListener('resize', () => {
    el.style.width = window.innerWidth + 'px';
  });
  // 當 el 被移除時，監聽器 -- 及其對 el 的閉包 -- 仍然存在
}

// 良好：使用與元件生命週期綁定的 AbortController
function initComponent(el) {
  const controller = new AbortController();
  window.addEventListener('resize', () => {
    el.style.width = window.innerWidth + 'px';
  ...
  return () => controller.abort(); // 在卸載時呼叫
}
```

### 2. 捲動/觸控監聽器缺少 passive 旗標

非 passive 的 `wheel` 和 `touchmove` 監聽器會阻塞合成器執行緒，造成捲動卡頓：
...
// 良好：passive 監聽器允許流暢捲動
element.addEventListener('wheel', (e) => {
  trackScroll(e.deltaY);
  // 此處無法呼叫 e.preventDefault() -- passive 承諾
}, { passive: true });
```

### 3. stopPropagation 中斷祖先監聽器
...
### 5. Web Component 跨 Shadow DOM 的事件重定向
