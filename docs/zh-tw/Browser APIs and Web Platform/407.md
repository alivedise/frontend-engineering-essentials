...
Canvas API 提供一個完全由 JavaScript 驅動的命令式、以像素為基礎的繪圖表面——DOM 對其中繪製的內容一無所知。SVG 提供一種宣告式、整合於 DOM 的向量格式，其中每個形狀都是可存取、可設定樣式、可腳本化的元素。團隊 MUST 以 `window.devicePixelRatio` 縮放 canvas 元素並套用反向 CSS 尺寸——省略此步驟將在高 DPI 顯示器上產生模糊輸出。團隊 SHOULD 在可及性與可縮放性重要的圖示、圖表和互動式圖解中使用 SVG；團隊 SHOULD 在 DOM 開銷會降低效能的即時動畫、圖片處理和像素層級操作中使用 Canvas。

兩種渲染模型之間的差異具有架構層面的影響。Canvas 將每一幀視為空白畫布：你清除、重繪，然後重複。瀏覽器從不快取你所繪製的內容；它只知道緩衝區目前的像素值。SVG 將每一幀視為一個保留的場景圖：瀏覽器擁有元素樹，在屬性或樣式變更時重新渲染受影響的區域，並自動使臟區失效。保留模式渲染對於稀疏更新成本較低；即時模式渲染對於全場景重繪成本較低。團隊 MUST 在選定渲染技術之前，理解其使用場景需要哪種更新模式。
...
**對需要可及性的靜態和半靜態圖形使用 SVG。** 如果圖表不是每幀都在動畫——一個條形圖、一個圓餅圖、一個偶爾有懸停狀態的節點連結圖——SVG 幾乎總是更好的選擇。每個 `<rect>`、`<path>` 和 `<text>` 元素都可以攜帶 `aria-label`、`role` 和 `tabindex` 屬性。螢幕閱讀器可以枚舉它們。鍵盤使用者可以導航它們。CSS 過渡效果可以在不需要 JavaScript 的情況下對其進行動畫處理。產生等效的可及性 Canvas 圖形需要手動維護命中圖、焦點環和 ARIA 即時區域——而 SVG 免費提供了這些大量的額外程式碼。
...
`getBoundingClientRect()` 返回套用樣式表後瀏覽器分配的 canvas CSS 版面配置尺寸。乘以 `devicePixelRatio` 得到該區域佔用的實體像素數量。將 `canvas.width` 和 `canvas.height` 設定為這些值可創建具有正確解析度的緩衝區。隨後的 `ctx.scale(dpr, dpr)` 將所有繪圖座標從 CSS 像素映射回實體像素，因此在 `(100, 75)` 繪圖的程式碼仍然命中視覺上正確的位置。
...
<svg width="200" height="200" xmlns="http://www.w3.org/2000/svg"
     role="img" aria-label="Interactive circle">
...
SVG 元素是標準的 DOM 節點。`getElementById` 找到它，`addEventListener` 附加事件處理器，`setAttribute` 更新其表現屬性——瀏覽器安排只重繪受影響的區域。`<svg>` 元素上的 `role="img"` 和 `aria-label` 使圖形對螢幕閱讀器可見。這種可及性整合程度除了 SVG DOM 本質自動提供的之外，不需要任何額外程式碼。
