...
Worker 無法存取 `window`、`document`、`document.getElementById()` 或任何 DOM API。這是設計上的考量 -- 從多個執行緒存取 DOM 會需要鎖定機制 (locking)，並產生死鎖 (deadlock) / 競態條件 (race condition) 的風險。正確做法是在 Worker 中計算結果，然後傳送給主執行緒進行 DOM 更新：

```js
// 錯誤：在 Worker 內部會拋出 ReferenceError
...
self.addEventListener('message', () => {
  ...
  self.postMessage({ result }); // 由主執行緒更新 DOM
});
```

### 2. 忽略結構化複製的開銷（應使用可轉移物件）

每次 `postMessage` 呼叫都會透過結構化複製深層複製資料。對於大型 ArrayBuffer、ImageData 或型別陣列 (typed array)，這會不必要地複製數 MB 的記憶體。應使用轉移列表參數來移動所有權：

```js
// 慢：複製 50 MB 的資料（需要約 50-100 毫秒）
...
// buffer.byteLength 現在是 0 -- 所有權已移至 Worker
```

### 3. 不再需要 Worker 時未終止它

每個 Worker 消耗一個作業系統執行緒和相關記憶體（基線約 1-5 MB）。建立 Worker 而不終止它會導致資源洩漏 (resource leak)，尤其在 SPA 中於路由切換時建立 Worker 的情況下：

```js
// 不好：Worker 永遠存活，即使已離開該頁面
...
function processData(data) {
  ...
  worker.onmessage = (e) => {
    updateUI(e.data);
    worker.terminate(); // 釋放作業系統執行緒和記憶體
  };
}
```

### 4. Service Worker 快取過期內容而沒有版本策略
...
### 5. 使用 SharedArrayBuffer 卻未設定跨源隔離標頭
