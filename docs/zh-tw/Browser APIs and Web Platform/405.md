...
一個試算表應用程式在每次按鍵時都要處理數千個儲存格的公式依賴關係。這個計算在主執行緒上同步執行，每次阻塞輸入達 200 毫秒，讓應用程式感覺遲鈍無比。Web Workers 在背景執行緒中執行 JavaScript，無法存取 DOM，透過訊息傳遞與主執行緒溝通。將計算移至 Worker，主執行緒就能持續響應使用者輸入，同時讓背景執行緒負責運算。
