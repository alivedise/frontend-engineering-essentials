...
WebSocket 於 2011 年在 IETF RFC 6455 中標準化，對應的瀏覽器 API 在 2012 年前已在所有主流瀏覽器中落地。該協定從一次 HTTP/1.1 升級握手開始——瀏覽器發送 `Upgrade: websocket` 請求標頭，伺服器回應 `101 Switching Protocols`，從此刻起連線就不再是 HTTP：它變成了一個原始的分幀二進位串流，任一方均可隨時發送訊息。Server-Sent Events 的精神更為古老——HTTP/1.1 的串流自 1990 年代便已可行——但 SSE 作為 HTML Living Standard 的一部分被標準化，並在 2000 年代末期獲得了正式的瀏覽器 API（`EventSource`）。SSE 不是協定升級；它是一個普通的 HTTP 回應，以 `Content-Type: text/event-stream` 告知瀏覽器保持連線並逐步解析內容。
...
隨著串流 AI 與 LLM 應用的興起，SSE 的普及程度顯著提升。OpenAI 的 Chat Completions API 與 Anthropic 的 Messages API 都以 SSE 串流的形式逐字符傳送回應。客戶端開啟一個 `EventSource` 連線或一個串流 `fetch`，伺服器在生成每個 token 時即時推送。這是 SSE 的教科書級應用場景：初始請求後伺服器是唯一的發話方，負載為純文字，瀏覽器的內建重連機制無需任何自訂程式碼即可處理短暫的網路抖動。因此，即使對於主要從事傳統請求-回應應用的工程師，理解 SSE 也日益重要。

兩種技術還有一個值得關注的差異：背壓（backpressure）處理方式不同。WebSocket 提供 `bufferedAmount` 屬性：`ws.bufferedAmount` 回傳已排隊等待發送但尚未傳輸的位元組數。資料生產速度快於網路傳輸速度的發送方，可在呼叫 `send()` 前檢查 `bufferedAmount` 並自行進行流量控制。SSE 沒有這樣的回饋機制——伺服器寫入回應串流，TCP 流量控制是唯一的背壓訊號。對大多數應用而言這個差異不明顯，但對於伺服器可能超過客戶端處理能力的高吞吐量資料管道而言，這點相當關鍵。
...
**未分塊傳送大型二進位資料到 WebSocket。** WebSocket 將訊息視為單一邏輯單元進行分幀。一條大型訊息——數百 MB 的圖片 blob、批量資料匯出——在 `onmessage` 觸發前必須在瀏覽器中完整緩衝。這會讓連線在整個大型負載接收完成前無法處理其他訊息。對於大型二進位傳輸，應傳送固定大小的分塊序列並在接收端重組，或使用獨立的 `fetch` 請求處理批量資料，保留 WebSocket 只傳送控制訊息。
