...
你正在開發一個多人協作白板，多位使用者同時繪畫，每位使用者都應能近即時看到其他人的筆跡。每秒輪詢一次伺服器平均引入 500 毫秒延遲，並產生不必要的負載。WebSocket 建立持久的雙向連線，讓伺服器在繪圖更新發生的瞬間就能推送給所有已連線的客戶端。Server-Sent Events 則以更輕量的協定解決較簡單的單向情境——伺服器向客戶端推送更新。
