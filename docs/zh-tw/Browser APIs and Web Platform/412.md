...
工程師 MUST（必須）透過 `requestAnimationFrame` 而非 `setInterval` 或
`setTimeout` 來驅動 JavaScript 視覺動畫。計時器驅動的動畫與渲染管線完全脫節。
當渲染執行緒忙碌時，計時器回呼會堆積並在主執行緒可用後連續觸發，產生一陣
DOM 變更爆發，接著是數個空白幀。結果是使用者可感知的畫面抖動。唯一的例外情
況是：動畫更適合以 CSS（可在合成器執行緒上執行的轉場與關鍵幀動畫）或 Web
Animations API 處理，後者在底層使用與 rAF 相同的幀同步機制。任何在迴圈中讀
取 DOM 值並寫入 DOM 值以產生視覺動態的 JavaScript 程式碼，MUST（必須）以
rAF 作為排程機制。

工程師 SHOULD（應該）以經過時間的函式（`timestamp - startTime`）而非幀數來
表達動畫進度。以幀數為基礎的動畫在不同設備上以不同速度播放。以時間為基礎的
動畫，無論螢幕更新頻率為何，每幀都推進相同的實際毫秒數。這個原則延伸至任何
有狀態的動畫模型：物理模擬 SHOULD（應該）對經過時間進行積分，彈簧動畫
SHOULD（應該）根據經過秒數更新其速度與位置，而序列動畫 SHOULD（應該）以時
鐘延遲而非幀數計數來排程其轉場。rAF 時間戳記引數是動畫迴圈內經過時間的權威
來源；應從它推導所有時間狀態，而非依賴外部時鐘或手動追蹤的計數器。
