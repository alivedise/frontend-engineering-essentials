...
### MUST NOT 將密鑰儲存在 localStorage

`localStorage` 可被同一 origin 上執行的任何 JavaScript 存取。單一 XSS 漏洞即可暴露儲存在其中的所有值。驗證 token、API 金鑰和 session 密鑰絕對不可放入 localStorage 或 sessionStorage：

```js
// 危險：任何 XSS 指令碼都能讀取
localStorage.setItem('auth_token', jwt);

// 安全：HttpOnly cookie -- JavaScript 無法存取
// Set-Cookie: auth_token=<jwt>; HttpOnly; Secure; SameSite=Strict
```

使用 `HttpOnly` cookie 儲存驗證 token。瀏覽器會自動將它們附加到請求中，且 JavaScript 無法讀取或修改。

### SHOULD 對結構化或大量資料使用 IndexedDB 而非 localStorage

localStorage 僅儲存字串、是同步的（阻塞主執行緒），且限制在約 5 MB。IndexedDB 是非同步的、支援結構化複製 (structured cloning)（無需手動序列化）、提供索引以進行高效查詢，且可處理更大的資料集：

```js
// 避免：將複雜狀態序列化到 localStorage
...
將 localStorage 保留給簡單、小型的值：使用者偏好設定、功能旗標、像是側邊欄收合/展開的 UI 狀態。
...
### 1. 將驗證 token 儲存在 localStorage（XSS 漏洞）

頁面上執行的任何 JavaScript 都能讀取 localStorage。單一 XSS 注入即可竊取所有儲存的 token。請使用 HttpOnly cookie 進行驗證：

```js
// 危險：任何注入的指令碼都能讀取
localStorage.setItem('access_token', token);

// 安全：伺服器設定 HttpOnly cookie
// Set-Cookie: access_token=<token>; HttpOnly; Secure; SameSite=Strict; Path=/
```

### 2. 在高頻路徑中同步使用 localStorage
...
### 3. 未處理 QuotaExceededError

localStorage 和 IndexedDB 都可能在儲存配額耗盡時拋出例外。忽略此問題會導致應用程式崩潰：
...
### 4. Cookie 缺少 SameSite 屬性（CSRF 風險）
...
### 5. 在需要 IndexedDB 的場合使用 localStorage

將大型 JSON blob、物件陣列或二進位資料儲存在 localStorage 中需要手動序列化/反序列化，限制在約 5 MB，且在 `JSON.parse()` 大型字串時阻塞主執行緒。IndexedDB 原生處理這些情境：

```js
// 不佳：localStorage 中 2MB 的 JSON 字串，解析時阻塞主執行緒
const products = JSON.parse(localStorage.getItem('products') || '[]');

// 良好：IndexedDB 直接儲存物件、非同步、具備索引供查詢使用
