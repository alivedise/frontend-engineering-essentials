...
### SHOULD 批次處理 DOM 讀寫操作

瀏覽器採用延遲排版 (lazy layout) -- 在需要幾何資訊前不會重新計算位置與尺寸。當 JavaScript 交錯進行 DOM 讀取（如 `offsetWidth`）與 DOM 寫入（如 `style.width = ...`）時，會迫使瀏覽器在每次讀取時執行**同步排版 (synchronous layout)**。這種模式稱為**排版抖動 (layout thrashing)**，可將幀率從 60 fps 降至個位數。

**原則：** SHOULD 將 DOM 讀取與寫入分開。先執行所有讀取，再執行所有寫入。

### SHOULD 使用 DocumentFragment 批次插入

逐一將元素附加到即時 DOM 會在每次插入時觸發排版重算。`DocumentFragment` 是不屬於文件樹的輕量容器。將 fragment 附加到 DOM 時，其所有子節點會在單一操作中移入。

**原則：** SHOULD 在插入多個元素時使用 `DocumentFragment`（或現代的 `append()` 搭配多個引數）。
...
在同一迴圈中讀取幾何屬性並寫入樣式，會在每次迭代時強制同步排版。修正方式見範例 3：將讀取與寫入分開，或以 `requestAnimationFrame` 延遲寫入。

### 3. 在迴圈中執行 querySelector

`querySelector` 每次呼叫都會搜尋 DOM 樹。當結果不會改變時在迴圈中呼叫是浪費的：

```js
// 不佳：每次迭代都查詢 DOM
for (const item of data) {
  const container = document.querySelector('#list'); // 每次結果相同
  const li = document.createElement('li');
  li.textContent = item;
  container.append(li);
}
