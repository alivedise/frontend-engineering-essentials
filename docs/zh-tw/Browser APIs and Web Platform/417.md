...
**應該**移轉 `VideoFrame` 和 `AudioData` 而非複製它們。這些 WebCodecs 類型持有硬體分配記憶體的參考，且被明確設計用於基於移轉的管線。複製 `VideoFrame` 不僅浪費——它分配了潛在數兆位元組像素資料的完整副本——也表明對 API 預期用途的誤解。請始終移轉，並在接收方完成使用後始終呼叫 `VideoFrame` 的 `.close()` 以及時釋放底層記憶體。
