...
- **`ReadableStream`** — 串流資料的來源。將 `ReadableStream` 移轉至 Worker，允許在不先緩衝整個有效負載的情況下，在主執行緒外消費串流。已鎖定至讀取器的可讀串流無法移轉——先呼叫 `reader.releaseLock()` 解鎖，或完成消費後再移轉。呼叫 `.cancel()` 會銷毀串流而非解鎖，僅在確定要放棄串流時使用。相同的約束適用於已鎖定至寫入器的 `WritableStream`（解鎖使用 `writer.releaseLock()`）。
...
- **`RTCDataChannel`** — 將已建立的 WebRTC 資料通道移轉至 Worker，允許 Worker 直接發送和接收資料，無需主執行緒中介。與其他可移轉型別不同，`RTCDataChannel` 不是直接列入 `postMessage` 移轉陣列，而是呼叫專用的實例方法 `dataChannel.transfer()`，該方法回傳可移轉的控制代碼，再將控制代碼放入移轉列表：`worker.postMessage({ channel: handle }, [handle])`。**瀏覽器支援有限**：Chrome 111+ 支援；截至 2026 年 4 月，Firefox 和 Safari 均不支援。使用前請進行功能偵測：`typeof RTCDataChannel.prototype.transfer === 'function'`。在可用的情況下，好處顯著：主執行緒不再需要留在即時資料交換的熱路徑上。若不支援，退回方案是透過 `MessagePort` 中繼訊息——增加一個跳躍點，但在各處均可運作。
