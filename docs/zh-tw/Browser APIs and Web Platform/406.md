...
### 1. 元件卸載時未斷開 Observer 連接

Observer 對其回呼閉包 (closure) 和被觀察元素持有強參照 (strong reference)。在 SPA 中，未 disconnect 會造成記憶體洩漏 (memory leak) -- Observer、其閉包和所有被捕獲的變數在路由切換後仍然存活：

```js
// 錯誤：元件卸載時 Observer 洩漏
...
useEffect(() => {
  ...
  return () => observer.disconnect(); // 卸載時清理
}, []);
```

### 2. 在 Intersection Observer 存在時仍使用 scroll 監聽器

Scroll 監聯器在每幀強制同步佈局。如果你在檢查元素是否可見，請改用 Intersection Observer：
...
### 3. Intersection Observer threshold=0 遺漏掛載時已在 viewport 中的元素
...
### 4. Resize Observer 在回呼中修改被觀察元素造成迴圈錯誤
...
### 5. 在大型子樹上使用 Mutation Observer 而未過濾

以 `{ childList: true, subtree: true, attributes: true }` 觀察 `document.body` 會捕獲整個頁面的每一個 DOM 變更。在複雜的應用程式中，這可能每幀產生數千個 MutationRecord 物件，使回呼不堪負荷：
