...
你正在開發一個分析功能，記錄使用者捲動了多深以及閱讀了哪些區塊。在 scroll 事件中輪詢 `scrollTop` 每秒觸發數百次，且每次都強制進行佈局重算。`IntersectionObserver` 只在被追蹤的元素進入或離開可視區域時才觸發回呼，開銷遠小於輪詢。以 IntersectionObserver 取代 scroll 事件的輪詢，讓這個分析功能能夠精確記錄區塊可見性，而不會因為大量捲動操作而拖累頁面效能。
...
- [web.dev: Lazy Loading Images and Video](https://web.dev/articles/browser-level-image-lazy-loading) -- 使用原生和 IO 方式延遲載入圖片的最佳實踐
