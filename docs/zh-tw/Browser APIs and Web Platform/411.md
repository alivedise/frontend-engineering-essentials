...
WebTransport 的安全模型明顯比 WebSocket 更嚴格。
WebTransport 連線 MUST 透過 HTTP/3 建立至 HTTPS 來源——純 HTTP 來源會被拒絕。
伺服器必須出示瀏覽器憑證存放區信任的有效 TLS 憑證，或者連線可透過 `serverCertificateHashes` 選項使用伺服器憑證雜湊機制，繞過 CA 鏈以適用於開發環境與嵌入式裝置場景。
瀏覽器強制規定 WebTransport 連線只能開啟至同源，或透過對應的伺服器端標頭明確允許跨源存取的來源。
適用於 Fetch 請求的同源政策與類 CORS 限制，不會僅因為你使用了更底層的 API 就消失。

## 原則

WebTransport 提供建立在 HTTP/3 與 QUIC 之上的低延遲、雙向瀏覽器通訊 API，同時支援不可靠的 datagram（類 UDP，無傳遞保證）與可靠的有序串流（類 TCP，基於 QUIC 串流）。團隊 MUST NOT 在未提供 WebSocket 備援方案的情況下部署 WebTransport——截至 2026 年，WebTransport 在 Chrome 97+ 與 Edge 受到支援，但 Firefox 穩定版與 Safari（實驗性）不支援。當使用場景可容忍封包遺失，且最小化往返延遲比保證傳遞更重要時——即時遊戲、即時遙測、游標共享——團隊 SHOULD 選擇 WebTransport 而非 WebSocket。當需要訊息排序與保證傳遞時，團隊 SHOULD 使用可靠串流而非 datagram。在傳送任何資料前，團隊 MUST 等待 `transport.ready` promise——連線建立過程是非同步的。
...
```ts
// bidirectional-stream.ts — 可靠有序訊息（遊戲事件、聊天）
