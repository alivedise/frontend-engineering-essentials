...
你正在開發一個多人遊戲客戶端，玩家位置更新必須以最低延遲送達，而偶爾的封包遺失是可以接受的——重傳一個過時的位置，還不如直接丟棄它。WebSocket 保證送達與順序，這在網路不穩定時反而引入了排隊延遲。WebTransport 在 HTTP/3 上同時提供可靠串流與不可靠資料報，讓遊戲能依據每則訊息的特性選擇適合的可靠性語意。
