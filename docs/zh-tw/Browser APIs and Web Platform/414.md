...
工程師應該（SHOULD）在不需要回應、且事件將完整上下文作為訊息酬載時，使用
`BroadcastChannel` 進行跨分頁事件通知。身份驗證狀態變更（登入、登出、
token 刷新）、主題切換，以及快取失效信號，都是 BroadcastChannel 的典型使用
情境。頻道採用即發即忘（fire-and-forget）模式；若某分頁在訊息廣播時已關閉，
它將不會收到該訊息。對於需要確認（acknowledgement）的場景——例如要求所有
分頁確認已接收並處理登出事件——BroadcastChannel 不是正確的工具，因為它沒有
回應機制；此時應考慮 SharedWorker 或帶有協調邏輯的 Service Worker。使用具有
`type` 欄位的訊息信封格式，可在同一頻道上承載多種事件類型，避免為每種事件
各自建立獨立頻道所造成的命名混亂與資源浪費。

工程師應該（SHOULD）在多個分頁需要共享單一有狀態資源時使用 `SharedWorker`——
例如 WebSocket 連線、API 回應的本地快取，或共享的背景同步狀態。`SharedWorker`
避免了 N 個分頁各自對同一伺服器建立 WebSocket 連線的情況。一個 Worker 持有
一個連線，並將訊息扇形分發給已連接的分頁。Worker 的生命週期與已連接的 port
數量掛鉤：當所有已連接的分頁關閉後，Worker 即終止。選擇這兩種 API 的依據是
複雜度預算：BroadcastChannel 只需兩行程式碼即可使用，無需額外的 Worker 腳本
檔案；SharedWorker 則需要獨立的腳本檔案、正確的 connect 處理器，以及謹慎的
port 生命週期管理，但換來的是能夠持有跨越單一分頁存活時間之有狀態資源的能力。
工程師在引入 SharedWorker 前，應確認其目標環境的瀏覽器支援矩陣，並為
不支援的環境提供功能降級路徑，以維持跨平台的功能一致性。
