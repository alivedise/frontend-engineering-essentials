...
你正在審查一個 Pull Request，一位初級工程師用 `<div>` 和 `<span>` 堆砌出整個功能介面。螢幕閱讀器將頁面內容唸成一片毫無分別的文字牆，自動化無障礙稽核報告顯示零個地標區域（landmark region）。問題的核心不是樣式，而是結構本身。HTML 語意元素正是為了解決這個問題而存在：它們向瀏覽器、輔助技術和搜尋引擎傳遞語意，無需額外的屬性或 JavaScript。
