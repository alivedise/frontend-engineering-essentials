...
### 3. 在需要外部主題化時使用 Shadow DOM

```js
// 錯誤：對需要完全外部樣式的元件使用 Shadow DOM
...
**影響：** 消費者無法用既有 CSS（`.btn-primary`、utility classes 等）設定按鈕樣式。Shadow DOM 阻擋所有外部選擇器。在依賴共享 CSS 的設計系統中，該元件成為樣式死胡同。

**修正：** 完全不使用 Shadow DOM，或透過 `::part()` 和 CSS custom properties 暴露樣式接口：

```js
// 選項 A：不使用 Shadow DOM -- 可從外部完全設定樣式
class ThemedButton extends HTMLElement {
  connectedCallback() {
...
### 4. 未將屬性反映至 attributes

```js
// 錯誤：property 變更不會更新 attribute
...
**影響：** CSS 屬性選擇器（`toggle-switch[checked]`）無法運作。`querySelector('[checked]')` 找不到該元素。DevTools 不顯示目前狀態。Attribute 和 property 分歧，令消費者困惑。

**修正：** 將 properties 反映至 attributes（反之亦然）：
