...
在大型內容應用程式中，一個接受頁面資料物件並回傳完整型別 `WithContext<Product>` JSON-LD 節點的工具函式，能將結構化資料邏輯集中於單一位置，使其易於單獨測試。針對 `aggregateRating` 的條件式守衛——僅在評論數超過最低閾值時才發出該節點——可防止針對沒有評論的產品發布錯誤的結構化資料。
...
export function buildProductJsonLd(product: ProductData): WithContext<Product> {
  return {
    ...
    '@type': 'Product',
    name: product.name,
    offers: { '@type': 'Offer', price: product.price, priceCurrency: 'TWD' },
