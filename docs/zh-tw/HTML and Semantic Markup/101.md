...
### MUST：宣告 charset 與 viewport

每份 HTML 文件**必須（MUST）**在文件的前 1024 位元組內包含 `<meta charset="utf-8">` 宣告，以及用於回應式渲染的 `<meta name="viewport">` 標籤。

**理由。** 若缺少 `charset`，瀏覽器會猜測編碼，可能導致亂碼（mojibake）。若缺少 `viewport`，行動裝置瀏覽器會以桌面寬度（通常 980px）渲染頁面並縮小，導致文字無法閱讀、觸控目標過小。這兩個 meta 標籤是任何現代網頁的最低限度中繼資料。
...
### SHOULD：為關鍵第三方來源使用資源提示

文件**應該（SHOULD）**為關鍵的第三方來源（例如字型 CDN、API 端點）加入 `<link rel="preconnect">`，並為較不關鍵的來源使用 `<link rel="dns-prefetch">` 作為替代。

**理由。** 瀏覽器在取得任何跨來源資源前，必須完成 DNS 查詢、TCP 交握和 TLS 協商。`preconnect` 提前執行這三個步驟，可在關鍵路徑上節省 100-500 毫秒。`dns-prefetch` 只執行 DNS 查詢，且有更廣泛的瀏覽器支援。
...
### SHOULD：在根元素提供 lang 與 dir

`<html>` 元素**應該（SHOULD）**始終攜帶 `lang` 屬性（BCP 47 語言標籤），對於從右到左的語言，還應攜帶 `dir="rtl"` 屬性。

**理由。** 螢幕閱讀器依據 `lang` 切換語音設定檔。搜尋引擎用它進行語言索引。瀏覽器用它處理斷字、引號和拼字檢查。省略 `lang` 會迫使所有這些系統進行猜測。
...
**影響。** 行動裝置瀏覽器以約 980px 寬度渲染頁面並縮小。文字極小、觸控目標無法使用，Lighthouse 會將頁面標記為非行動裝置友善。

**修正。** 始終加入 viewport meta 標籤：
...
**影響。** 螢幕閱讀器無法判斷正確的發音規則。設定為英語的螢幕閱讀器會錯誤地朗讀法語內容。搜尋引擎無法可靠地索引頁面的語言。瀏覽器的自動翻譯功能缺少基準語言。
...
**影響。** 瀏覽器不知道資源類型，因此無法設定正確的優先級或套用內容安全政策（CSP）檢查。大多數瀏覽器會取得資源兩次 -- 一次來自 preload，一次來自實際請求 -- 導致頻寬使用量加倍。瀏覽器也會發出主控台警告。

**修正。** 始終指定 `as` 屬性，需要時加上 `crossorigin`：
...
<head>
  <meta charset="utf-8" />
  <title>My Page</title>
  <p>Welcome to my page!</p>  <!-- 這會迫使 <head> 提前關閉 -->
</head>
```

**影響。** 當瀏覽器在 `<head>` 內遇到流內容元素（如 `<p>`）時，會隱式關閉 `<head>` 並開啟 `<body>`。之後宣告的任何中繼資料都被視為 body 內容而被忽略。這可能導致樣式表遺失、資源提示失效和無效的 HTML。

**修正。** `<head>` 內只保留中繼資料元素（`<meta>`、`<title>`、`<link>`、`<style>`、`<script>`、`<base>`、`<noscript>`）。
...
- [MDN: The `<meta>` element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta) -- 所有 meta 元素屬性與標準中繼資料名稱的參考
