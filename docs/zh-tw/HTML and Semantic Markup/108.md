...
工程師必須（MUST）在每個具有 `target="_blank"` 的 `<a>` 元素上加入 `rel="noopener noreferrer"`。沒有 `rel="noopener"` 時，被開啟的頁面會透過 `window.opener` 取得開啟者的 `window` 物件參考，並可藉此將開啟者重導向至釣魚頁面，這種攻擊稱為反向 tabnapping（reverse tabnapping）。被開啟的頁面在連結當下不需要是惡意的；任何被事後入侵的跨來源頁面都可以利用這個存取管道。`noreferrer` 同時會抑制 `Referer` 標頭，防止目的地得知完整的引薦來源 URL。現代瀏覽器已將 `target="_blank"` 的預設行為改為隱含 `rel="noopener"`，但明確寫出這個屬性仍是正確的做法：它在程式碼審查中意圖清晰，適用於較舊的瀏覽器版本，並向未來的維護者傳達設計意圖。

工程師必須（MUST）在所有嵌入第三方內容的 `<iframe>` 元素上設定 `sandbox` 屬性，並僅指定明確需要的能力 token。被沙箱化的 iframe 會在獨立的瀏覽上下文中執行，無法存取嵌入頁面的 cookie、儲存空間或 JavaScript 作用域。`allow-scripts` 與 `allow-same-origin` 這兩個沙箱 token 禁止（MUST NOT）同時使用：在同源沙箱化 iframe 中執行的腳本，能夠讀取 iframe 自身的 `sandbox` 屬性，並從 DOM 中將其移除，導致瀏覽器以不受限制的模式重新載入框架。這種自我移除漏洞會完全抵消沙箱化所提供的保護，因此無論嵌入的第三方內容為何，這種組合一律視為反模式。
