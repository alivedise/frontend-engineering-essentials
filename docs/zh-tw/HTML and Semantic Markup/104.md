...
**優先使用 `<details>`/`<summary>` 實現漸進式揭示（progressive disclosure）模式。** 當內容可按需顯示或隱藏時（FAQ、可展開區塊、次要資訊），SHOULD 使用原生揭示元件，而非建立自訂 accordion。原生元素以零 JavaScript 提供鍵盤互動、無障礙播報和切換語意，消除整個類別的焦點管理錯誤。
