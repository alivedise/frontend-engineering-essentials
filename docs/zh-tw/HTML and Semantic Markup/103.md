...
你正在建構一個收集送貨地址、付款資訊和配送偏好的結帳表單。表單提交時會遺漏必填欄位，並接受明顯無效的郵遞區號，因為驗證邏輯全部寫在 submit 處理器中——而使用者只需在文字欄位中按 Enter 就能繞過它。HTML 內建的表單驗證——`required`、`pattern`、`type="email"`、`minlength`——在 submit 事件觸發之前就會執行，無需 JavaScript 即可運作，並呈現瀏覽器原生的錯誤提示 UI，完全不需要額外的程式碼。
...
- [MDN: Constraint Validation](https://developer.mozilla.org/en-US/docs/Web/HTML/Guides/Constraint_validation) -- HTML 驗證屬性和 Constraint Validation API 的參考
- [MDN: FormData API](https://developer.mozilla.org/en-US/docs/Web/API/FormData) -- 用於從表單控制項建構鍵值對的 FormData 介面參考
- [MDN: :user-valid](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Selectors/:user-valid) -- 在使用者互動後才匹配的 :user-valid pseudo-class 參考
