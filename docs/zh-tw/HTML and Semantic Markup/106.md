...
> **SHOULD** 確保核心內容與功能在無 JavaScript 的情況下仍能運作。

> **SHOULD** 使用 `inert` 屬性以宣告式方式管理焦點與互動狀態。

這些原則源自 Web 的架構：HTML 是基線傳輸格式，在任何腳本執行之前即被解析與渲染。當核心流程僅依賴 HTML 時，它們在慢速連線、限制性環境、輔助技術使用者及搜尋引擎爬蟲等情境下皆能運作。
...
### 1. 在伺服器 HTML 即可滿足的場景使用 SPA

```js
// 錯誤：對內容網站使用客戶端路由的 SPA
createBrowserRouter([
...
**影響。** 完全在客戶端渲染的內容型網站（部落格、文件、行銷頁面）要求使用者在看到任何內容之前下載、解析並執行 JavaScript bundle。First Contentful Paint 延遲數秒。搜尋引擎可能無法索引所有內容。無 JavaScript 的使用者看到空白頁面。

**修正。** 使用伺服器渲染或靜態生成。若使用框架，選擇 HTML 優先的預設值（Astro、Eleventy），或啟用 SSR/SSG（Next.js `generateStaticParams`、SvelteKit `prerender`）：

```js
// Astro -- 生成靜態 HTML，預設零 JS
---
...
**影響。** 這個實作品質不佳地重新實現了 `<details>` / `<summary>`。JavaScript 版本需要手動管理 ARIA、鍵盤處理與狀態追蹤，且在無 JavaScript 時會失效。
