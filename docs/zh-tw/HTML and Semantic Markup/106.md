...
你正在為一個支援頁面建構 FAQ 區塊，該頁面有相當大量的流量來自企業網路與低效能裝置，部分使用者的 JavaScript 受政策限制而停用。設計稿包含可展開的答案面板、破壞性操作前的確認對話框，以及一個懸浮通知徽章——這些全都是團隊過去用 JavaScript 實作的模式。若以 JavaScript 優先實作，沒有 JavaScript 的使用者將看到損壞的 UI 或完全空白的頁面。HTML 的 `<details>`、`<dialog>` 與 `popover` API 原生支援這三種互動模式，並可在能力較強的環境中疊加 JavaScript 進行增強，而無需維護兩套獨立的程式碼路徑。
