...
:::info
HTML 提供超過 100 種元素，每一種都帶有特定的意義。當你選擇 `<nav>` 而非 `<div>` 時，你不是在設定樣式 -- 你是在向瀏覽器、螢幕閱讀器（screen reader）、搜尋引擎和閱讀模式傳達意圖。語意標記（semantic markup）是網頁無障礙的基礎：它讓機器理解人類一眼就能看到的內容。
:::
...
ARIA（Accessible Rich Internet Applications，無障礙富網際網路應用程式）的存在是為了填補 HTML 沒有原生語意的空白。但 ARIA 只修改無障礙樹 -- 它不增加任何行為。`<div role="button">` 對螢幕閱讀器看起來像按鈕，但它沒有鍵盤支援、沒有焦點管理、沒有表單送出功能。原生的 `<button>` 免費提供所有這些。
...
作者**必須（MUST）**為內容的用途使用最具體的語意 HTML 元素，而非使用通用的 `<div>` 或 `<span>` 搭配 CSS 樣式或 ARIA 角色。

**理由。** 語意元素提供隱含的 ARIA 角色、鍵盤行為和瀏覽器原生功能（焦點管理、表單送出、驗證），且完全免費。使用 `<div>` 再用 JavaScript 和 ARIA 重建這些行為既容易出錯又不完整。WebAIM Million 分析發現，有 ARIA 的頁面平均比沒有 ARIA 的頁面多偵測到 70% 的無障礙錯誤 -- 不是因為 ARIA 有害，而是因為開發者用 ARIA 來修補語意 HTML 本來就能原生提供的功能。

### SHOULD NOT：在原生元素已提供相同角色時添加 ARIA

作者**不應該（SHOULD NOT）**在已經原生暴露相同語意的元素上添加 ARIA 角色、狀態或屬性。這是 ARIA 的第一條規則：「如果你能使用原生 HTML 元素或屬性，其語意和行為已內建你所需的功能，就不要重新利用元素再添加 ARIA role、state 或 property 來使其無障礙，直接使用原生元素即可。」

**理由。** 冗餘的 ARIA（例如 `<nav role="navigation">`）雖然無害但多餘 -- 它表明開發者不理解原生語意，且可能在其他地方錯誤地套用 ARIA。更糟的是，不正確的 ARIA 會覆蓋原生語意：`<nav role="banner">` 會將元素的角色從 navigation 改為 banner，破壞無障礙樹。

### MUST：維持標題層級階層

作者**必須（MUST）**維持邏輯性的標題階層（`<h1>` 到 `<h6>`），不跳過層級。每個頁面**應該（SHOULD）**只有一個 `<h1>`。

**理由。** 螢幕閱讀器使用者依靠標題層級進行導覽。從 `<h2>` 跳到 `<h4>` 會造成空缺，暗示有遺漏的小節。標題結構是螢幕閱讀器使用者的主要導覽機制 -- 根據 WebAIM 的螢幕閱讀器使用者調查，67.7% 的使用者透過標題進行導覽。

### MUST：為互動元素提供無障礙名稱

每個互動元素**必須（MUST）**有無障礙名稱（accessible name）。表單控制項使用 `<label>`。其他互動元素使用可見的文字內容、`aria-label` 或 `aria-labelledby`。

**理由。** 沒有無障礙名稱的互動元素，螢幕閱讀器只會宣告其角色（例如「按鈕」但沒有標籤）。使用者無從得知該按鈕的功能。
...
**影響。** 螢幕閱讀器看不到地標、標題或結構。使用者無法按地標或標題導覽。搜尋引擎和閱讀模式無法擷取有意義的內容。
...
**影響。** 螢幕閱讀器使用者按標題層級導覽時，預期 `<h1>` 之後是 `<h2>`。跳到 `<h4>` 暗示有三個層級的階層遺漏。使用者可能認為內容缺失或誤解頁面結構。
...
**影響。** 雖然不會破壞頁面，但冗餘的 ARIA 表明對語意的誤解，並使程式碼混亂。當開發者後來不小心覆蓋原生角色時，就有實際危害的風險。`<ul>` 上的 `role="list"` 特別不必要。
