...
### Masking as an afterthought

```typescript
// WRONG: deploy replay with no masking, add masking later
Sentry.init({
  integrations: [
    Sentry.replayIntegration({
      maskAllInputs: false, // "we'll add masking later"
      maskAllText: false,
    }),
  ],
  replaysSessionSampleRate: 1.0, // recording everything
});
```

If PII is captured before masking is added, the data is already in the replay provider's storage. GDPR erasure obligations apply immediately. The remediation cost — identifying affected users, submitting deletion requests, confirming deletion — far exceeds the cost of configuring masking before go-live.

```typescript
// CORRECT: mask by default, explicitly unmask what engineers need
Sentry.init({
  integrations: [
    Sentry.replayIntegration({
      maskAllInputs: true,  // default — leave this on
      maskAllText: false,   // true if your app renders PII in text nodes
    }),
  ],
  replaysSessionSampleRate: 0.1, // 10% of sessions, not all
  replaysOnErrorSampleRate: 1.0, // 100% of error sessions
});
```

### Enabling replay unconditionally for EU users

```typescript
// WRONG: replay starts before consent is checked
import * as Sentry from '@sentry/browser';

Sentry.init({
  dsn: '...',
  integrations: [Sentry.replayIntegration()], // no consent gate
  replaysSessionSampleRate: 0.1,
});
```

GDPR classifies session replay as personal data processing. Recording EU users without consent creates regulatory exposure. The correct pattern gates replay initialization behind a consent signal.

```typescript
// CORRECT: replay only after consent grant
Sentry.init({
  dsn: '...',
  integrations: [], // no replay at init time
});
...
### Not correlating replay with error reports

```typescript
// WRONG: LogRocket and Sentry running in parallel with no link between them
LogRocket.init('app/id');
Sentry.init({ dsn: '...' });
// No connection — engineers must search both tools separately
```

Without correlation, an engineer looking at a Sentry error must search LogRocket separately for the same user and time window. The two-step lookup defeats much of the efficiency gain from having both tools.

```typescript
// CORRECT: attach LogRocket session URL to every Sentry error scope
LogRocket.getSessionURL((sessionURL) => {
  Sentry.getCurrentScope().setExtra('logRocketSession', sessionURL);
});
```

### Omitting the correlation ID from API requests

```typescript
// WRONG: API requests have no session identifier
const response = await fetch('/api/orders');

// Backend logs show: GET /api/orders 500 — no way to find which user
```

Without a correlation ID, a backend engineer seeing an error in server logs cannot find the frontend session replay associated with it. The correlation ID is the thread that connects the frontend experience to the backend trace.

```typescript
// CORRECT: every API request carries the correlation ID
import { correlationId } from './correlation';
...
// Backend log: GET /api/orders 500 X-Correlation-ID=abc-123-def
// Engineer can find the Sentry session replay tagged with correlationId=abc-123-def
```

### Recording replay in non-production environments

```typescript
// WRONG: replay enabled in all environments
Sentry.init({
...
Staging and development environments may have test accounts with realistic-looking data, developer workflows that mimic sensitive operations, or debugging states that should not be stored in a third-party replay provider. Limit replay to production.

```typescript
// CORRECT: replay only in production
Sentry.init({
