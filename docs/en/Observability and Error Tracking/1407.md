...
**MUST treat privacy protection as the default, not an afterthought — adopt opt-in to recording rather than opt-in to masking.** If PII is captured before masking is configured, the data is already recorded and subject to GDPR erasure obligations that may be operationally impossible to satisfy. Teams MUST keep `maskAllInputs: true` in `Sentry.replayIntegration()` and MUST set `maskAllText: true` for any application that renders account numbers, email addresses, or health data in DOM text nodes.
