...
**SHOULD NOT store session replay data longer than required by your data retention policy.** Session replay data is personal data under GDPR and subject to data minimization requirements: it must not be retained beyond the period necessary for its stated purpose. Teams SHOULD configure the retention period explicitly in the replay tool's dashboard — most platforms default to 30 or 90 days, but the correct value depends on your privacy policy and the typical debugging window. Data retained beyond the policy creates erasure obligations and increases exposure in a breach. Confirm the provider's configurable retention range before selecting a platform, as some enterprise plans offer shorter minimums than the default.
