...
- SHOULD alert on error rate (%) rather than raw error count — rate normalizes for traffic volume and is meaningful across all load levels.
- SHOULD define frontend SLOs covering error rate, p75 LCP, and p75 INP, and track error budget burn rate rather than point-in-time threshold violations.
- SHOULD link every alert to a runbook URL so on-call engineers have a documented response procedure at the moment the alert fires.
- SHOULD review and prune alert rules quarterly — any alert that fires more than once per week without requiring action is a candidate for removal or tuning.
- SHOULD NOT create alerts for metrics that do not directly indicate user harm or require immediate action.
- SHOULD use both fast burn (>5x) and slow burn (>2x sustained) alerts for SLO error budgets to catch both acute incidents and chronic degradation.
...
```
Alert: Fast Burn (2% budget consumed in 1h → 5x burn rate)
...
Alert: Slow Burn (10% budget consumed in 6h → 2x burn rate)
...
### Alerting on raw error count instead of error rate

```
# WRONG: fires during every traffic spike even when error rate is healthy
Alert: JavaScript errors > 100 in last 10 minutes

# CORRECT: normalizes for traffic volume
Alert: JavaScript error rate > 2% over last 10 minutes
       (error_count / total_sessions) > 0.02
```

100 errors at 0.1% traffic is a catastrophe. 1,000 errors at 0.5% traffic is within the SLO. Raw count cannot distinguish between these.

### Setting SLO targets without error budgets

```
# WRONG: SLO as a binary pass/fail with no budget concept
"Uptime must be 99.9% or we failed."

# CORRECT: SLO with explicit error budget
"99.5% of sessions must be error-free over 30 days.
 Error budget = 0.5% * 30d * 24h * 60m = 216 minutes.
 Current budget consumed: 47 minutes (21.8% of budget, 12 days into the month)."
```

Without an error budget, every SLO breach triggers the same response regardless of severity. With a budget, teams can make informed trade-offs: a risky deployment that might consume 20 minutes of budget is a different conversation when 180 minutes remain versus when 10 minutes remain.

### Creating alerts without runbooks

```yaml
# WRONG: alert with no response documentation
annotations:
  summary: "Frontend error rate {{ $value }}% is critical"
  # no runbook_url

# CORRECT: every alert has a runbook
annotations:
  summary: "Frontend error rate is critical"
  runbook_url: "https://wiki.example.com/runbooks/frontend-error-rate-critical"
  description: "Error rate has been above 2% for 10 minutes."
```

An alert without a runbook delegates documentation to the on-call engineer at the worst possible moment. The runbook does not need to be perfect — a draft with the three most likely causes and their resolutions is far better than nothing.

### Never pruning alert rules

Alert rules accumulate. A team starts with 5, adds 3 after an incident, adds 4 more when onboarding a new tool, and six months later has 30 alert rules that fire at different rates and severities. No one knows which ones are still relevant. The quarterly review process forces discipline:

```
Quarterly Alert Review Checklist:
1. List all alert rules with their fire frequency in the past 90 days
2. For each rule that fired > 4 times: what % required action?
3. Rules with action rate < 25%: tune threshold or delete
4. Rules that never fired: still relevant? Delete if not.
5. For every rule that remains: confirm the runbook URL is current
```

### Misconfiguring fast burn vs. slow burn windows

```yaml
# WRONG: using a single short window for burn rate — misses slow chronic degradation
- alert: SLOBurnRate
  expr: slo_burn_rate_1h > 5

# CORRECT: multi-window burn rate catches both acute and chronic problems
# Fast burn: short + long window both elevated (reduces false positives)
- alert: SLOFastBurn
  expr: slo_burn_rate_1h > 5 AND slo_burn_rate_5m > 5
  for: 2m

# Slow burn: sustained elevation over a longer period
- alert: SLOSlowBurn
  expr: slo_burn_rate_6h > 2 AND slo_burn_rate_30m > 2
  for: 15m
```

A single short-window burn rate alert produces false positives during brief traffic spikes. The multi-window approach requires both a short-term spike and a longer-term trend, which dramatically reduces false positive rates.
