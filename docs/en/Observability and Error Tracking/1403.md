...
**SHOULD** sample high-volume log events — verbose user interaction traces, navigation events, and other events that fire repeatedly every session — at a configurable rate (e.g., 5–10%) to prevent overwhelming the log platform. Errors MUST NOT be sampled; every error event is diagnostically relevant. Only non-critical informational events that produce large volume at scale qualify for sampling. Implement sampling decisions per event category and store the decision for the session so a given user is consistently sampled in or out.
