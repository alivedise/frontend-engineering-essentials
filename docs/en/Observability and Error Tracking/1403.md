...
After the user authenticates, update the logger context. Rather than mutating the singleton directly, a common pattern is to use a thin context wrapper:

```ts
// src/lib/logger.ts (add setContext method to BrowserLogger)
...
A common mistake is including user-provided input directly in log messages:

```ts
// WRONG: logs the user's email address
logger.error(`Authentication failed for ${user.email}`);

// CORRECT: log the opaque user ID only
logger.error('Authentication failed', { userId: user.id, reason: 'invalid_password' });
```

Email addresses, names, and any other PII in log records create GDPR data retention obligations and expand the blast radius of a log platform data breach. Always log opaque identifiers.

### Using `fetch` for log flush on page unload

```ts
// WRONG: fetch is cancelled during page unload
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden') {
    fetch('/api/logs', { method: 'POST', body: JSON.stringify(buffer) }); // may be cancelled
  }
});

// CORRECT: sendBeacon is fire-and-forget, survives page unload
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden') {
...
Calling `fetch('/api/logs', ...)` inside a log method that is called 10 times during a single user interaction is a network anti-pattern. It generates 10 sequential HTTP requests that add latency and server load. Buffer events and flush on a timed interval or threshold.

### Using `console.log` in production

`console.log` output is not sent anywhere in a production browser environment. It appears in the user's DevTools (if they have them open) and then disappears. It cannot be queried, aggregated, or alerted on. Replace production logging with a structured logger that sends records to a log ingestion endpoint.

### Logging at `debug` level in production

Debug-level logs contain internal state, component lifecycle events, and API request details that are useful during development but produce enormous volume in production. Configure the minimum log level to `info` or `warn` in production environments to avoid unnecessary storage costs and noise in the log platform.

### Using `beforeunload` instead of `visibilitychange`

`beforeunload` is unreliable on mobile browsers (iOS Safari and many Android browsers do not consistently fire it during tab switches or app backgrounding). Use `visibilitychange` with `document.visibilityState === 'hidden'` as the authoritative page-unload signal. It fires on mobile when the browser is backgrounded and on desktop when the tab is hidden or closed.

### Missing consistent fields across log records

A log record for a payment error that includes `userId` and one for a navigation event that does not makes cross-record correlation impossible. Establish a baseline schema for all log records and enforce it in the logger implementation — not at the call site. The `userId`, `sessionId`, and `traceId` fields should be set once in the logger config and injected automatically into every record.
