...
- SHOULD initialize the OpenTelemetry `WebTracerProvider` at application startup and register auto-instrumentations for fetch and document load.
- SHOULD propagate the `traceparent` header on all `fetch` calls to outbound APIs to enable distributed trace correlation.
- SHOULD use `performance.mark()` and `performance.measure()` to time key user interactions (form submission, checkout flow, file upload, modal open/close).
- SHOULD observe Long Tasks with `PerformanceObserver` in production and report them to an APM backend or RUM tool.
- SHOULD NOT rely solely on auto-instrumentation — manual spans add semantic context (operation name, user intent, business entity IDs) that automated tooling cannot infer.
- SHOULD export traces to an OTLP-compatible backend (Datadog, Honeycomb, Grafana Tempo, Jaeger) rather than a vendor-specific SDK to avoid lock-in.
- SHOULD attach span attributes for business context: `user.id`, `order.id`, `session.id`, `feature.flag` — not PII.
...
async function handleSubmit() {
  ...
  try {
    ...
    const [measure] = performance.getEntriesByName('checkout-submit-duration')
    if (!measure) {
      performance.measure('checkout-submit-duration', 'checkout-submit-start', 'checkout-submit-end')
    }

    const [result] = performance.getEntriesByName('checkout-submit-duration')
    if (result) {
      console.info('[perf] checkout-submit-duration:', result.duration.toFixed(1), 'ms')
      // Report to your APM or analytics endpoint
      reportPerformanceMeasure('checkout-submit-duration', result.duration)
    }
  } catch (err) {
...
```ts
const exporter = new OTLPTraceExporter({
  url: 'https://trace.agent.datadoghq.com/api/v0.2/spans',
  headers: {
...
```ts
// WRONG: if an exception is thrown, span.end() is never called
...
A span that never ends is held in memory by the `BatchSpanProcessor` indefinitely and never exported. In a long-running application, leaked spans accumulate and eventually cause memory pressure.

### Propagating `traceparent` to third-party domains

```ts
// WRONG: propagating trace context to all domains
getWebAutoInstrumentations({
  '@opentelemetry/instrumentation-fetch': {
    propagateTraceHeaderCorsUrls: [/.*/],  // matches everything
  },
})

// CORRECT: only propagate to your own API domains
getWebAutoInstrumentations({
  '@opentelemetry/instrumentation-fetch': {
    propagateTraceHeaderCorsUrls: [
      /https:\/\/api\.yourcompany\.com/,
      /https:\/\/internal\.yourservice\.io/,
    ],
  },
})
```

Injecting `traceparent` into requests to Stripe, Google Analytics, or other third-party APIs sends your trace IDs to external parties and may cause CORS preflight failures if those services do not accept the header.
...
### Ignoring Long Tasks below an arbitrary threshold

```ts
// WRONG: only observing tasks above 200ms misses tasks that accumulate to cause poor INP
if (entry.duration > 200) { report(entry) }

// CORRECT: the Long Tasks API already uses the 50ms threshold from the spec
// Report all long tasks; filter by severity in the APM backend, not in the client
if (entry.duration > 50) { report(entry) }
```

The 50ms threshold is the specification-defined boundary between a "normal" task and a Long Task. Tasks between 51ms and 200ms are common INP contributors and should not be silently discarded.

### Exposing OTLP collector credentials in the browser bundle

```ts
// WRONG: API key visible in browser source
const exporter = new OTLPTraceExporter({
  url: 'https://api.honeycomb.io/v1/traces',
  headers: { 'x-honeycomb-team': 'hcaik_abc123...' },
})
```

Browser bundle contents are publicly readable. API keys in the bundle are effectively public. Instead, proxy OTLP through your own backend and authenticate server-to-collector with an environment variable:

```ts
// CORRECT: browser sends to your own backend
const exporter = new OTLPTraceExporter({ url: '/api/otlp/v1/traces' })

// Your backend forwards with the real key
// api/otlp/v1/traces.ts
fetch('https://api.honeycomb.io/v1/traces', {
  method: 'POST',
  headers: {
    'x-honeycomb-team': process.env.HONEYCOMB_API_KEY!,
    'Content-Type': req.headers['content-type'] ?? 'application/protobuf',
  },
