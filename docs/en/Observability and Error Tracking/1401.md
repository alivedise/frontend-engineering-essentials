...
**Sentry** is the dominant platform in this space. It began as an open-source Django error logger in 2010 and expanded into the canonical full-stack error tracking platform. Its dominance is earned on several axes:
...
- **Best-in-class source map support.** Sentry was the first platform to make source map upload a first-class part of the SDK workflow, with official Vite, webpack, and Rollup plugins that handle upload automatically during the build step.
...
Understanding the full Sentry workflow — SDK initialization, source map upload, breadcrumbs, user context, release tracking, noise filtering, and alerting — is the foundation for every other observability practice in this category.

## Principle

- MUST NOT commit source maps to the public bundle or serve them on the public CDN.
- SHOULD upload source maps to Sentry in CI immediately after the build step, then delete the `.map` files before CDN deploy.
- SHOULD set user context via `Sentry.setUser()` immediately after a user authenticates.
- SHOULD configure `beforeSend` to filter noise before events reach the platform.
- SHOULD create a Sentry release and associate commits with every production deploy.
- SHOULD configure alert rules to route high-severity errors to Slack or GitHub Issues within minutes.
- SHOULD use `Sentry.addBreadcrumb()` to annotate significant application events that do not produce automatic breadcrumbs.

## Design Thinking

### Why Sentry won

The error tracking market has multiple entrants — Datadog RUM, Rollbar, Bugsnag, LogRocket, Highlight — but Sentry commands the highest market share among frontend teams. The reasons are structural rather than marketing:

**Source map support as a first-class feature.** The competition largely treated source maps as an afterthought. Sentry built official build-tool integrations (`@sentry/vite-plugin`, `@sentry/webpack-plugin`) that handle the entire upload lifecycle as part of the build configuration. The friction of "upload source maps in CI" dropped from a complex multi-step shell script to a three-line plugin configuration.

**Self-hostability eliminates adoption blockers.** Enterprise teams with data sovereignty requirements, GDPR obligations, or security review processes that prohibit third-party data sharing can run Sentry on-premises. Most SaaS error tracking competitors have no self-hosted path. This makes Sentry the only choice for a significant segment of the market, which in turn funds continued investment in the SDK ecosystem.

**Release tracking and commit integration.** Sentry's release tracking model — where each deploy creates a named release, associates a commit range, and tracks which issues appeared or regressed in that release — transforms the debugging workflow. Instead of asking "when did this error start happening?", the engineer asks "which release introduced this error?" and gets an answer in one click.
...
- **Network errors from users' connections.** A user on a flaky mobile connection who loses connectivity mid-session produces `TypeError: Failed to fetch` events that are not actionable — the connection dropped, not your code.
...
Without filtering, these events flood the issue tracker, bury real errors, and consume your event quota. `beforeSend` is the callback in `Sentry.init()` that runs before each event is sent. It can inspect the event and return `null` to drop it, or modify the event before sending. A well-configured `beforeSend` can reduce inbound event volume by 60–90% without losing any signal on real errors.

### Why release tracking transforms debugging

The most expensive part of incident response is usually not the fix — it is diagnosis. "Which line is broken?" is answerable with a stacktrace. "When did this start breaking?" requires correlating error frequency over time with deployment timestamps. "Which deploy introduced this?" requires manual cross-referencing of error spikes with deployment logs.

Release tracking in Sentry answers the last two questions automatically. When a release is created and commits are associated, the platform can display:
...
This changes incident response from a multi-step forensic investigation into a single-screen lookup. The engineer sees the error, sees that it appeared in release `1.4.7`, clicks through to the commit range, and identifies the offending change in minutes.
...
**SHOULD configure alert rules on error rate rather than raw error count, and set `enabled: import.meta.env.PROD` to prevent development sessions from consuming the event quota.** A raw-count threshold fires during every traffic spike regardless of whether the application is actually broken. Teams SHOULD configure both a fast-burn alert (error rate > 2% sustained for 10 minutes) and a regression alert (resolved issue recurs in a new release) linked to a runbook URL.
...
Call `Sentry.setUser()` immediately after a successful login. This associates every subsequent error event with the authenticated user, enabling you to search for "all errors affecting user X" and to reach out to affected users.
...
Sentry automatically collects breadcrumbs for DOM clicks, XHR and fetch requests, `console.log` calls, and browser navigation events. For application-level events that do not produce automatic breadcrumbs — a user advancing a multi-step form, a WebSocket message received, a feature flag evaluated — add them manually.
...
### Committing or serving source maps publicly

Serving `.map` files from the CDN alongside the bundle exposes original source code. Check your build output for `.map` files before deploying. If using the Vite or webpack plugin, confirm that `filesToDeleteAfterUpload` is configured. If uploading via `sentry-cli`, delete `.map` files from the build directory before the CDN sync step. Verify with `ls -la dist/ | grep .map` — the output should be empty after a correct CI run.

### Omitting the `release` field in `Sentry.init()`

Without a release identifier, error events are not associated with a specific deploy. The "which release introduced this?" question has no answer in the platform. The release must be the same string passed to the plugin or `sentry-cli`. A common pattern is to inject it as an environment variable during the build step (e.g., `VITE_RELEASE=$(git rev-parse --short HEAD)`).

### Skipping `beforeSend` configuration

Without noise filtering, a new Sentry installation often reaches its event quota within the first few days of production traffic. Browser extension errors, third-party script rejections, and benign browser quirks all generate events. Configure `beforeSend` before going live, not after the quota is exhausted.
...
### Not calling `Sentry.setUser(null)` on logout

If a user logs out and another user logs in on the same browser session (common on shared computers), the second user's errors will be attributed to the first user's identity until the page is refreshed. Always call `Sentry.setUser(null)` in the logout handler.
