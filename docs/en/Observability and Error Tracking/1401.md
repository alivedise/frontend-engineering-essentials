...
**MUST set `enabled: import.meta.env.PROD` in `Sentry.init()` to prevent development sessions from consuming the event quota.** Development traffic is noise, not signal. Teams MUST ensure Sentry only sends events in production by gating initialization with the environment flag, since a misconfigured local environment can exhaust a monthly quota in hours.
