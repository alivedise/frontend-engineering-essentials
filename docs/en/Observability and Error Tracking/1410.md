...
Every state-modifying action in an admin UI MUST be recorded in an audit
log that captures the actor, the action, the affected resource, and the
before-and-after state. Error tracking for internal tools SHOULD be
configured with 100% sampling and low alert thresholds because each error
affects a known, named user and may represent a compliance risk. Performance
monitoring SHOULD focus on high-interaction workflows — bulk operations,
data tables, complex form flows — where degraded performance directly
reduces staff productivity. Internal tool observability data MUST be
stored with access controls appropriate to its sensitivity: audit logs
that capture administrator actions require at least as much access
restriction as the actions they record.
