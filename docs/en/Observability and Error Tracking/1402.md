...
**MUST use `onINP` from the `web-vitals` library for interactivity measurement — not the deprecated `onFID`.** INP replaced FID as a Core Web Vital in March 2024 and measures end-to-end interaction latency across the full page lifetime. Teams MUST migrate any existing FID instrumentation to INP; FID data in old dashboards SHOULD be treated as historical only.

**MUST report Core Web Vitals at the p75 percentile, not as averages.** Google's ranking algorithm evaluates a URL's Core Web Vitals at the 75th percentile of real Chrome sessions. An average of 2.0s LCP is compatible with a bimodal distribution where half of users experience 3.5s LCP — a severe tail problem that averages suppress. Teams MUST configure their analytics backend to use percentile aggregation (e.g., `quantile(0.75)(value)` in ClickHouse).

**SHOULD use `navigator.sendBeacon` — not `fetch` — to submit RUM metric events to the analytics endpoint.** Metrics collected late in the session, particularly INP, are at risk of being dropped if reported via `fetch`, which is cancelled when the user navigates away. Teams SHOULD also listen for `visibilitychange` and flush the metric queue when `document.visibilityState === 'hidden'`.

**SHOULD apply session-based sampling — not per-metric sampling — before sending data to the analytics endpoint.** Make the sampling decision once per session using `sessionStorage` and apply it uniformly to all metrics from that session. Per-metric sampling produces incoherent datasets where LCP and INP from the same session are reported at different rates, making correlation impossible.

**SHOULD segment RUM data by page template, device class, and browser before drawing performance conclusions.** A single aggregate p75 LCP for the entire site hides the pattern where mobile users on the checkout page have 4.5s LCP while desktop users on the landing page have 1.2s. Teams SHOULD treat field data from CrUX and their own RUM pipeline as the authoritative source for production performance decisions; Lighthouse SHOULD be treated as a development-time feedback tool only.

**MUST make sampling decisions per session, not per metric.** Make the sampling decision once per session using `sessionStorage` and apply it uniformly to all metrics from that session. Per-metric sampling produces incoherent datasets where LCP and INP from the same session are reported at different rates, making cross-metric correlation impossible. Teams SHOULD set sampling rates proportional to traffic volume: 1–2% for sites above one million daily sessions, 5–10% for 100K–1M, and 100% for sites below 100K.
...
- [Datadog RUM documentation](https://docs.datadoghq.com/real_user_monitoring/application_monitoring/browser/)
