...
Every HTTP request from the browser that is part of a user interaction
SHOULD carry a `traceparent` header that links it to a root span created
in the browser. Backend services MUST be configured to read and propagate
this header so that frontend and backend spans appear under the same trace
ID in the observability backend. Manual span creation SHOULD be limited
to meaningful user interactions — page navigations, form submissions, and
explicit data-loading operations — rather than every DOM event, which
produces trace noise without observability value.

Sampling MUST be configured at the SDK level before any span data leaves
the browser. Sending every trace from every user session to the backend
overwhelms ingestion pipelines and inflates cost. A sample rate between
1% and 10% is appropriate for high-traffic applications. 100% sampling
is reserved for development environments and for specific user sessions
flagged for targeted debugging.
...
- Creating spans for every DOM event or state mutation — span noise
  ...
  observability value
- Exporting spans directly from the browser to a SaaS backend — embeds
