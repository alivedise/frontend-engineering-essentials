...
- SHOULD attach active feature flag state to error reports as Sentry context or tags so that errors on variant code paths are diagnosable without manual reproduction.
- SHOULD tag every analytics event with the active flag variants at the time the event was captured, so A/B experiment analysis can split cohorts correctly.
- SHOULD define guardrail metrics (including Core Web Vitals, particularly LCP and INP) for every A/B experiment before launch, not after observing results.
- SHOULD NOT peek at A/B test results before the pre-specified sample size is reached — determine the required sample size before starting the experiment.
- SHOULD pre-specify the Minimum Detectable Effect (MDE) and target statistical power (80% is the standard) before calculating sample size.
- SHOULD use PostHog as the default starting point for teams that do not already have separate flag management, product analytics, and session replay systems.
...
### Not attaching flag state before rendering variant code

```ts
// WRONG: flag state attached after the variant renders — if an error occurs during
// renderTreatmentCheckout(), the Sentry event will not have the flag context yet
const variant = posthog.getFeatureFlag('checkout-v2')
renderTreatmentCheckout()
Sentry.setTag('flag.checkout-v2', String(variant))  // too late

// CORRECT: attach flag state before executing any variant-specific code
const variant = posthog.getFeatureFlag('checkout-v2')
Sentry.setTag('flag.checkout-v2', String(variant))  // set first
...
```ts
// WRONG: checking results after N days and stopping when p < 0.05 is reached
// This is p-hacking — the false positive rate is much higher than 5%

// CORRECT: pre-specify sample size and evaluate exactly once
const REQUIRED_SAMPLE_SIZE_PER_VARIANT = 3_474  // calculated before launch
// Do not look at results until both variants have >= 3,474 users
```

The pre-specified sample size must be documented in the experiment brief before the flag is launched. Stopping early — even if the result looks "obviously significant" — invalidates the statistical guarantees.

### Using the same flag key for multiple sequential experiments

```ts
// WRONG: reusing a flag key means users who participated in experiment 1
// (when 'new-header' meant one thing) are mixed with users in experiment 2
// (when 'new-header' means something else)
const variant = posthog.getFeatureFlag('new-header')

// CORRECT: use versioned or descriptive flag keys per experiment
const variant = posthog.getFeatureFlag('header-redesign-2026-q1')
```

Flag key reuse contaminates experiment cohorts. A user assigned to `control` in the first experiment may be assigned to `treatment` in the second, but their historical events still have the old assignment attached. Analysis must filter to events within the experiment's date range, or use a new key entirely.

### Not persisting flag assignments for returning users

A user must always see the same variant on every visit. If flag assignments are re-randomized on each page load, users may see the control on Monday and the treatment on Tuesday. This causes:

- Inconsistent user experience (flickering between variants)
- Polluted analytics data (a single user contributes to both cohorts)

PostHog handles persistence automatically via `localStorage+cookie`. GrowthBook requires that the same user attributes — including a stable `id` — are set on each session. For anonymous users, assign a UUID on first visit and persist it in `localStorage`.

### Shipping experiments that breach guardrails

```ts
// After the experiment reaches sample size:
// PRIMARY METRIC: conversion improved from 8.0% to 9.4% (p = 0.023) ✓
// GUARDRAIL: LCP increased from 1800ms to 3200ms in treatment cohort ✗

// WRONG: shipping because the primary metric is positive
await flagClient.setFlagToTreatmentForAll('checkout-v2')

// CORRECT: rolling back because a guardrail was breached
await flagClient.setFlagToControlForAll('checkout-v2')
// Investigate the LCP regression before re-running the experiment
```

Guardrail breaches are not warnings — they are blockers. An experiment that ships despite a guardrail breach sets a precedent that guardrails are optional, which renders them meaningless.
...
- [Sentry setContext documentation](https://docs.sentry.io/platforms/javascript/enriching-events/context/)
