...
**SHOULD tag every analytics event with all active feature flag variants at the time the event is captured.** Without per-event flag tagging, A/B experiment analysis cannot split cohorts and compare treatment against control on the same metric. Teams using PostHog benefit from automatic flag tagging on every captured event; teams using other analytics platforms MUST add flag properties explicitly to each event call.
