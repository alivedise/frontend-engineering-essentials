...
- **MUST NOT** configure `Access-Control-Allow-Origin: *` together with `Access-Control-Allow-Credentials: true`
- **MUST NOT** reflect the `Origin` header directly into `Access-Control-Allow-Origin` without checking it against an allowlist
- **SHOULD** maintain an explicit server-side origin allowlist and set `Access-Control-Allow-Origin` only when the request origin is in that list
- **SHOULD** set `SameSite=Strict` on session cookies for apps that do not require cross-site embedding
- **SHOULD** add CSRF tokens to state-changing endpoints when `SameSite=Strict` is not feasible
- **SHOULD** require a custom request header (e.g., `Content-Type: application/json` or `X-Requested-With: XMLHttpRequest`) on all state-changing API requests to force a CORS preflight for cross-origin callers
- **SHOULD** validate the `Origin` header on all state-changing requests as a secondary CSRF defense
...
**Maintain a server-side origin allowlist.** Hardcode the set of trusted origins and set `Access-Control-Allow-Origin` only when the incoming `Origin` is in that list. Do not derive the allowlist from a regex — regex-based matching is prone to bypass (`https://example.com.evil.com` matching `.*example\.com`).
...
**Validate the `Origin` header on state-changing requests as a secondary CSRF defense.** Even if the request comes without CORS (e.g., a same-origin form), checking that the `Origin` or `Referer` header matches a known good value is a cheap, layered control.

**Use `SameSite=Strict` on session cookies** for applications that do not embed cross-site or depend on OAuth redirect flows. Use `SameSite=Lax` when those flows are needed. Never use `SameSite=None` unless you are building a deliberately cross-site embedded widget (e.g., a payment iframe), and always pair it with `Secure`.

**Use CSRF tokens (synchronizer token pattern) on state-changing form submissions.** Server generates a per-session random token, includes it in the page (as a hidden field or a non-httpOnly cookie), and validates it on each state-changing request. An attacker on `evil.com` cannot read the token from your page, so they cannot forge a valid request.

**Use the double-submit cookie pattern for stateless CSRF protection in SPA APIs.** Server sets a non-httpOnly, non-Secure cookie with a random value. The JavaScript client reads that cookie and sends its value as a request header (`X-CSRF-Token`). The server compares the header value to the cookie value. An attacker on `evil.com` cannot read the cookie (SOP blocks cross-origin cookie reads), so they cannot set the matching header value.

**Require `Content-Type: application/json` on all state-changing API endpoints.** Forms submit `application/x-www-form-urlencoded` by default. Requiring JSON forces cross-origin requests to include a non-simple content type, which triggers a preflight. If the CORS policy is correct, the preflight will fail for unauthorized origins.
