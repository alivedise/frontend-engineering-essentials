...
- **MUST NOT** store access tokens or refresh tokens in `localStorage` or `sessionStorage`
- **SHOULD** store access tokens in memory (module variable or React state) and refresh tokens in `httpOnly` cookies
- **SHOULD** use `httpOnly` cookies with `Secure`, `SameSite=Strict` (or `Lax`), and `HttpOnly` attributes for refresh tokens
- **MUST** use PKCE (RFC 7636) for all OAuth 2.0 Authorization Code flows in SPAs
- **MUST NOT** use the OAuth 2.0 implicit flow
- **MUST** validate JWT expiry (`exp`), audience (`aud`), and issuer (`iss`) server-side on every request — never trust the client to enforce these
- **SHOULD** rotate refresh tokens on every use and invalidate all tokens for a user when a reuse attempt is detected
...
**Set short access token lifetimes.** 15 minutes is a widely adopted default. A shorter lifetime limits the window during which a stolen access token can be used. Adjust based on your refresh mechanism: if silent refresh is in place, 15 minutes is comfortable; if the user must re-authenticate to renew, a longer lifetime may be necessary for usability.

**Rotate refresh tokens on every use.** Each time a refresh token is used to obtain a new access token, issue a new refresh token and invalidate the previous one. This limits the replay window for a stolen refresh token: if an attacker obtains and uses a refresh token, the legitimate client's next refresh attempt will fail with the old token. The server can detect this reuse — if an already-invalidated refresh token is presented, it indicates potential compromise and all tokens for that user should be invalidated, forcing re-authentication.

**Schedule silent refresh proactively.** Attempt the refresh at 75-80% of the access token's lifetime, not after it has expired. An expired access token during a background refresh request causes a visible authentication failure. Renewing early ensures continuity and gives room for retries if the first attempt fails.

**Set all cookie security attributes.** For `httpOnly` refresh token cookies: `HttpOnly` (no JS access), `Secure` (HTTPS only), `SameSite=Strict` or `SameSite=Lax` (CSRF mitigation), `Path` scoped to the token endpoint (`/auth/token`), and `Max-Age` appropriate for the refresh token lifetime. Never omit `Secure` on a production cookie.

**Validate the JWT server-side on every request.** Check `exp` (is the token expired?), `iss` (is this token from the expected issuer?), and `aud` (is this token intended for this service?). Do not cache the validation result — a token that was valid on the last request may have been revoked or expired in the interim. Also explicitly specify the expected signing algorithm in your verification code; never allow the algorithm to be taken from the token header (this is the "algorithm confusion" attack vector).

**Never log tokens.** Access tokens and refresh tokens must not appear in application logs, error tracking payloads (Sentry breadcrumbs, Datadog traces), analytics events, or any other observability pipeline. A token in a log is a token exposed to everyone with log access. Review error handlers and request interceptors for accidental token logging, particularly in Authorization header values.

**Rotate session identifiers on login.** When a user authenticates, issue a new session identifier even if one already exists. This prevents session fixation attacks where an attacker pre-seeds a known session ID and waits for a victim to authenticate into it.
...
**Not validating JWT expiry server-side.** The server cannot trust the client not to send expired tokens. An attacker will strip the expiry check from their client code or craft requests directly. Validate `exp` on every request, server-side, without exception.

**Logging tokens in application logs or error tracking.** An Authorization header value logged by an Express middleware, a Sentry breadcrumb containing a full request headers dump, a Datadog trace with the `access_token` field — all of these expose tokens to everyone with access to those systems. Scrub token values from logs before they are written. Review request logging middleware and error handlers explicitly.
...
**Trusting the `alg` field in the JWT header.** The signing algorithm used to verify the token should be hardcoded in the server's verification logic. If the server accepts any algorithm listed in the token header, an attacker can craft a token with `"alg": "none"` (no signature required) or switch from RS256 to HS256 using the public key as the HMAC secret. Always pin the expected algorithm explicitly.

**Not invalidating the token family on reuse detection.** If a refresh token is used twice — which should be impossible under normal rotation — the correct response is not just to reject the second use. It indicates that at some point the refresh token was stolen and is being replayed. Both the attacker and the legitimate user hold copies. Invalidating all tokens for that user forces a re-authentication, preventing the attacker's copy from continuing to produce valid access tokens.
