...
SRI requires that the `<script>` or `<link>` element declaring an external
resource include an `integrity` attribute containing one or more hash values
in the form `<algorithm>-<base64-encoded-hash>`. Supported algorithms are
`sha256`, `sha384`, and `sha512`. The `crossorigin="anonymous"` attribute
MUST accompany any SRI-protected resource to ensure the browser requests the
resource via CORS without credentials, which is required for hash verification
to function correctly. If the hash check fails, the browser MUST NOT execute
the script or apply the stylesheet — the element fires an `error` event rather
than a `load` event, and the resource's bytes are discarded before any
execution occurs.
