...
Content Security Policy (CSP) is an HTTP response header that instructs the browser about which origins are allowed to load resources and what can execute on a given page. It is the browser's built-in mechanism for limiting the blast radius of a cross-site scripting (XSS) attack: even if an attacker injects malicious script into your HTML, a correctly configured CSP can prevent that script from executing. CSP does not replace input sanitization or output encoding — those remain the first line of defense — but it is the second line that contains the damage when the first line fails.

CSP is delivered as a single HTTP header with a semicolon-delimited list of directives. Each directive targets a specific resource type: `script-src` for JavaScript, `style-src` for CSS, `img-src` for images, `connect-src` for fetch and XMLHttpRequest, `font-src` for fonts, `frame-ancestors` to prevent clickjacking, `base-uri` to block base tag injection, and `form-action` to restrict where forms can submit. `default-src` acts as the fallback for any resource type not explicitly listed — but notably `base-uri` and `form-action` do NOT inherit from `default-src` and must always be set explicitly.
...
- **MUST NOT** use `'unsafe-inline'` in `script-src` without pairing it with a per-request nonce or hash that neutralizes it
- **SHOULD** deploy `Content-Security-Policy-Report-Only` before switching to enforcement mode
- **SHOULD** use `'strict-dynamic'` with nonces for `script-src` rather than maintaining a domain allowlist
- **SHOULD** set `base-uri 'none'` and `frame-ancestors 'none'` (or a specific trusted-origin list)
- **SHOULD** deliver CSP via HTTP response header rather than a `<meta>` tag
...
A misconfigured CSP silently breaks legitimate scripts for real users. It will break your login buttons, your analytics, your chat widget, your error tracking — whatever script you forgot to account for. You will not know until users start filing bugs. `Content-Security-Policy-Report-Only` is the only safe way to deploy a new policy: it reports violations without blocking anything, so you can iterate on the policy against real traffic before flipping it to enforcement. Skipping this step is how teams end up rolling back a CSP header at 2 AM.
...
2. **Third-party scripts load their own dependencies.** Your analytics script loads from `analytics.example.com`, which loads telemetry from `telemetry.example.com`, which you forgot to allowlist. Now analytics breaks, so you add `telemetry.example.com` — and the allowlist grows indefinitely.
...
`'strict-dynamic'` solves this differently: once a script is loaded via a trusted nonce or hash, any scripts it dynamically creates and inserts into the DOM are also trusted. You never need to allowlist a CDN domain again. The trust propagates from your server's explicit endorsement, not from a list of domains that can go stale.

### Why `base-uri 'none'` is easy to forget and expensive to omit

An attacker who can inject a `<base href="https://evil.example.com">` tag can redirect every relative URL on the page — form action targets, script sources, image sources — to their server. This works even if you have a strict `script-src` policy, because the base tag rewrite happens before URL resolution, and relative URLs are not subject to CSP origin checks in the same way. `base-uri 'none'` or `base-uri 'self'` eliminates this attack vector entirely. It is one directive, one value, and it is nearly always missing from CSP policies in the wild.

## Best Practices

**Generate nonces correctly.** A nonce must be cryptographically random, at least 128 bits, and must never be reused across requests. In Node.js:

```js
// Preferred: randomUUID is available in Node 14.17+ and all modern browsers
const nonce = Buffer.from(crypto.randomUUID()).toString('base64');

// Alternative: randomBytes gives you control over length
const nonce = crypto.randomBytes(16).toString('base64');
```

Never derive nonces from request data (timestamps, user IDs, IPs) — any deterministic function is vulnerable to prediction attacks.

**Always set CSP via HTTP header, not a `<meta>` tag.** The `<meta http-equiv="Content-Security-Policy">` form exists but is limited in important ways:
...
**Start from `default-src 'none'`.** Begin with everything blocked and add only what the application needs. This forces you to think through every resource type. A policy that starts from `'self'` or a permissive default tends to accumulate exceptions silently.

**Test with csp-evaluator.withgoogle.com.** Paste your policy and check for High severity warnings. Common findings: allowlisted domains with JSONP endpoints, `unsafe-inline` without nonces, missing `object-src`, missing `base-uri`.

**Deploy report-only first, always.** Even for policy updates to an already-enforcing policy, switch to report-only first in staging, then roll to production in report-only, collect data for at least 48 hours, then switch to enforcement.
...
5. **Report-only in production** — roll the report-only header to production. Monitor the violation endpoint for at least 48 hours across different browsers and geographies.
...
This sequence is not optional for customer-facing applications. Skipping straight to enforcement is how teams discover they broke the payment flow for Safari users at peak traffic.
...
### 2. Generating a nonce in Next.js middleware and setting the CSP header
...
Next.js reads the nonce from the request header it set in middleware and applies it to `<script>` tags it renders during SSR.
...
The browser sends a JSON POST to the endpoint on violations:
...
```http
Content-Security-Policy:
  script-src 'sha256-qznLcsROx4GACP2dm0UCKCzCG+HiZ1guq6ZZDob/Tng=' 'strict-dynamic';
  object-src 'none';
  base-uri 'none';
```

The hash must match the exact bytes of the script content, including whitespace. Any character difference — even a trailing newline — invalidates the hash and the script will be blocked.

## Common Mistakes

**Adding `'unsafe-inline'` as the "fix" when a script breaks under CSP.**
This is the most common mistake and the most damaging one. `'unsafe-inline'` nullifies any nonce-less CSP and returns you to zero XSS protection. The correct fix is to move the inline script to an external file, or add a nonce to it. If the script is third-party and cannot receive a nonce, `'strict-dynamic'` will usually handle its dynamically loaded dependencies once its entry point has a nonce.
...
Nonces must be generated per-request on the server. If your build step generates HTML files that are served as static assets (no server-side rendering), nonces embedded at build time will be the same for every visitor — effectively a constant, not a nonce. Use hash-based CSP for static assets, or switch to SSR for pages that need nonce-based policies.

**Deploying CSP in enforcement mode without a report-only phase.**
You will discover what you broke from user reports, not from your own testing. Any legitimate resource you did not account for — a third-party widget, a browser extension script, a dynamically injected analytics tag — will silently break in production.
