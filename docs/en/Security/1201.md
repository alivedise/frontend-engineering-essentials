...
**MUST generate nonces correctly.** A nonce MUST be cryptographically random, at least 128 bits, and MUST never be reused across requests. In Node.js:
...
**MUST deploy report-only first, always.** Even for policy updates to an already-enforcing policy, you MUST switch to report-only first in staging, then roll to production in report-only, collect data for at least 48 hours, then switch to enforcement.
