...
**MUST generate nonces correctly.** A nonce MUST be cryptographically random, at least 128 bits, and MUST never be reused across requests. Use `crypto.randomUUID()` (Node 14.17+) or `crypto.randomBytes(16)` converted to base64. Never derive nonces from request data (timestamps, user IDs, IPs) — any deterministic function is vulnerable to prediction attacks.
