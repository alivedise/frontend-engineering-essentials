...
Structured data also has a maintenance dimension that is easy to underestimate. A product page that launches with accurate JSON-LD can become inaccurate as the product's price changes, reviews accumulate, or the product goes out of stock. Teams that implement structured data in a one-time setup without a synchronization strategy create technical debt that silently degrades SEO performance over time. The implementation architecture must treat structured data with the same discipline as any other data layer: generated programmatically, kept synchronized with the source of truth, and monitored in production.

## Principle

Engineers MUST use JSON-LD as the format for all new structured data implementations and MUST place the `<script type="application/ld+json">` element in the server-rendered HTML, not inject it via client-side JavaScript after page load. Search engine crawlers that do not execute JavaScript will not find structured data injected after the initial HTML response. Frameworks such as Next.js, Nuxt, and Astro all provide mechanisms to inject elements into `<head>` during server-side rendering; engineers MUST use these mechanisms rather than relying on client-side `useEffect` or `onMounted` hooks.

Engineers MUST NOT publish structured data that misrepresents page content. This includes fabricating review scores, claiming a page is a product page when it is an informational page, adding aggregate ratings that do not reflect actual user reviews, or marking up content as an article authored by a real person when no such article exists. The Schema.org types are semantic contracts with search engines. Violations of this contract result in manual actions from Google's quality team that suppress rich results across all pages of the affected domain, not only the offending page.
...
The following pattern shows how to generate JSON-LD from a data object in Next.js App Router. The same principle applies to Nuxt and Astro with minor syntax differences.
...
Using the `schema-dts` package (`npm install schema-dts`) provides TypeScript types for all Schema.org entities. The `WithContext<Product>` type adds the `@context` property to the standard `Product` type, giving compile-time checking for property names and value shapes. If `reviewCount` is zero, the `aggregateRating` property is omitted rather than emitting an `AggregateRating` with zero reviews, which would violate Google's policies.
...
The `@context` declares the vocabulary namespace. Every nested object with `@type` uses Schema.org types. The `offers.price` and `offers.priceCurrency` are required properties for Google's Product rich result. The `aggregateRating.reviewCount` must reflect the actual number of reviews on the page or sourced from the same data pipeline — not a hardcoded number that drifts over time.
...
The Google rich results documentation specifies required and recommended properties for each type. For `Product`, `offers.price` and `offers.priceCurrency` are required for rich result eligibility. For `AggregateRating`, `ratingValue` and `reviewCount` (or `ratingCount`) are required. Omitting required properties does not cause a validation error in the Schema Markup Validator — the vocabulary itself treats most properties as optional — but it does cause Google to withhold the rich result for that page. Engineers who validate only against Schema.org without consulting Google's rich results requirements miss these gaps.
