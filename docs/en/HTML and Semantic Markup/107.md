...
In large content applications, a utility function that accepts the page data object and returns the fully typed `WithContext<Product>` JSON-LD node concentrates the structured data logic in one place, making it straightforward to test in isolation. The conditional guard on `aggregateRating` — emit the node only when the review count exceeds a minimum threshold — prevents erroneous structured data from being published for products with no reviews.

```ts
import type { WithContext, Product } from 'schema-dts';

export function buildProductJsonLd(product: ProductData): WithContext<Product> {
  return {
