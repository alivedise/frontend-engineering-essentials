...
> **SHOULD** ensure core content and functionality work without JavaScript.

> **SHOULD** use the `inert` attribute to manage focus and interaction state declaratively.

These principles follow from the web's architecture: HTML is the baseline transport format, parsed and rendered before any script executes. When core flows depend only on HTML, they work on slow connections, in restrictive environments, for assistive technology users, and for search engine crawlers.
...
### 1. SPAs Where Server HTML Would Suffice

```js
// WRONG: SPA with client-side routing for a content site
createBrowserRouter([
...
**Impact.** A content-heavy site (blog, documentation, marketing) rendered entirely on the client requires users to download, parse, and execute a JavaScript bundle before seeing any content. First Contentful Paint is delayed by seconds. Search engines may not index all content. Users without JavaScript see a blank page.

**Fix.** Use server rendering or static generation. If using a framework, choose one with HTML-first defaults (Astro, Eleventy) or enable SSR/SSG (Next.js `generateStaticParams`, SvelteKit `prerender`):

```js
// Astro -- generates static HTML, zero JS by default
---
...
### 2. JS for Things HTML Does Natively

```js
// WRONG: JavaScript toggle implementation
const btn = document.querySelector('.toggle-btn');
const content = document.querySelector('.toggle-content');
btn.addEventListener('click', () => {
  content.hidden = !content.hidden;
  btn.setAttribute('aria-expanded', !content.hidden);
});
```

**Impact.** This reimplements `<details>` / `<summary>` poorly. The JavaScript version requires manual ARIA management, keyboard handling, and state tracking. It breaks without JavaScript.

**Fix.** Use the native HTML element:

```html
<details>
  <summary>Toggle Content</summary>
  <p>This content expands and collapses natively.</p>
</details>
```

### 3. Not Testing With JavaScript Disabled
...
### 4. Overusing data-* as State Management
...
### 5. autofocus on SPA Route Changes
