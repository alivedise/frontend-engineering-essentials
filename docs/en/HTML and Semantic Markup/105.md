...
### 3. Shadow DOM when you need external theming

```js
// WRONG: using Shadow DOM for a component that needs full external styling
...
**Impact.** Consumers cannot style the button with their existing CSS (`.btn-primary`, utility classes, etc.). Shadow DOM blocks all external selectors. The component becomes a styling dead end in design systems that rely on shared CSS.

**Fix.** Either skip Shadow DOM entirely, or expose styling hooks with `::part()` and CSS custom properties:

```js
// Option A: No Shadow DOM -- fully styleable from outside
class ThemedButton extends HTMLElement {
  connectedCallback() {
    this.innerHTML = `<button class="themed-btn"><slot></slot></button>`;
  }
}

// Option B: Shadow DOM with explicit styling hooks
class ThemedButton extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: 'open' });
...
### 4. Not reflecting attributes to properties

```js
// WRONG: property changes do not update the attribute
...
**Impact.** CSS attribute selectors (`toggle-switch[checked]`) do not work. `querySelector('[checked]')` cannot find the element. DevTools do not show the current state. The attribute and property diverge, confusing consumers.

**Fix.** Reflect properties to attributes (and vice versa):
...
### 5. Forgetting customElements.define()
