...
Engineers MUST add `rel="noopener noreferrer"` to every `<a>` element with `target="_blank"`. Without `rel="noopener"`, the opened page receives a reference to the opener's `window` object via `window.opener`, which it can use to redirect the opener to a phishing page — an attack known as reverse tabnapping. The opened page does not need to be malicious at the time of linking; any cross-origin page that is later compromised can exploit this access. `noreferrer` additionally suppresses the `Referer` header, preventing the destination from learning the full URL of the referring page. Modern browsers default `target="_blank"` to imply `rel="noopener"`, but the explicit attribute remains the correct pattern: it is unambiguous in code review, works in older browser versions, and signals intent to future maintainers.

Engineers MUST set the `sandbox` attribute on all `<iframe>` elements that embed third-party content, specifying only the capability tokens explicitly required. A sandboxed iframe runs in a separate browsing context with no access to the embedding page's cookies, storage, or JavaScript scope. The `allow-scripts` and `allow-same-origin` sandbox tokens MUST NOT be combined: a script running in a same-origin sandboxed iframe can read the iframe's own `sandbox` attribute and remove it from the DOM, causing the browser to reload the frame without restrictions. This self-removal vulnerability completely negates the protection that sandboxing is intended to provide, so the combination is treated as an anti-pattern regardless of the specific third-party content being embedded.
