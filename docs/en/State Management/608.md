...
### Missing cancelQueries Before the Optimistic Write

A common implementation error is applying the optimistic update to the cache without first
canceling in-flight queries for the same key. If a background refetch is in progress when
`onMutate` fires and that refetch resolves after the optimistic write, TanStack Query's default
behavior is to merge the stale server response into the cache — overwriting the optimistic value
with pre-mutation data. The result is a visible flicker: the UI updates optimistically, briefly
reverts when the stale refetch lands, and then updates again when `onSettled` triggers a fresh
refetch. To the user this looks like a double-update bug.

```js
// WRONG: missing cancelQueries — stale refetch can overwrite optimistic state
onMutate: async (postId) => {
  const previous = queryClient.getQueryData(['post', postId]);
...
// CORRECT: cancel in-flight queries before reading snapshot and applying optimistic update
onMutate: async (postId) => {
  await queryClient.cancelQueries({ queryKey: ['post', postId] });
  const previous = queryClient.getQueryData(['post', postId]);
  queryClient.setQueryData(['post', postId], old => ({ ...old, likes: old.likes + 1 }));
...
The `await` is non-negotiable. The underlying cancellation is asynchronous; skipping the await
leaves a race condition open even if `cancelQueries` is called.

### Silent Rollback Without User Feedback

Implementing rollback correctly — restoring the snapshot in `onError` — is only half of the
requirement. An `onError` handler that calls `queryClient.setQueryData(key, context.previous)` and
nothing else produces a UI that silently snaps back. From the user's perspective: they clicked, the
UI changed, and then it changed back without explanation. This reads unambiguously as a bug, not as
error handling. It erodes trust, causes users to re-attempt the action repeatedly, and generates
support tickets.

```js
// WRONG: restores state but gives user no feedback
onError: (_err, postId, context) => {
  queryClient.setQueryData(['post', postId], context.previous);
  // Silent — user sees the UI revert and has no idea why
},

// CORRECT: restores state AND notifies the user
onError: (_err, postId, context) => {
  queryClient.setQueryData(['post', postId], context.previous);
  toast.error('Failed to like post — please try again');
},
...
Every `onError` handler for an optimistic update must display a notification that explains what
happened and gives the user a path forward. "Unable to update — please try again" is the minimum
viable message. Specific messages with retry affordances are strongly preferred.

### Applying Optimistic Updates to High-Risk Mutations

The like button and the payment button are not the same kind of mutation, even though both trigger
a POST request. The like button has a near-zero failure rate for application-level reasons;
optimistic confirmation costs nothing and saves 150-300ms of perceived latency per interaction.
The payment button may fail for insufficient funds, expired cards, fraud flags, and service
degradation; the failure rate is high enough that optimistic confirmation of payment creates a
harmful user experience.

```
// WRONG: optimistically confirming a payment
onMutate: async (paymentData) => {
  // Never show "Payment successful" before the server confirms
  setPaymentStatus('success'); // User sees this before server responds
  return {};
},

// CORRECT: use a loading state for high-stakes mutations
// Show a spinner, disable the submit button, wait for server confirmation
// Only display success or failure after the server responds
```

Showing "Payment successful" and then rolling back to "Payment failed" forces users to re-evaluate
whether their money was charged, damages trust in the application, and may require support
intervention. For high-stakes or frequently-failing mutations, loading states with clear progress
indicators are the correct choice. Optimistic updates are a precision tool for high-confidence,
