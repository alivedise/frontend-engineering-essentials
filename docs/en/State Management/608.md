...
**MUST NOT apply optimistic updates to mutations with unpredictable outcomes.** Payments, inventory
reservations, permission changes, and file uploads carry failure rates too high for optimistic
confirmation and consequence severity too significant to risk displaying a false positive. When a
mutation's server-side outcome cannot be reliably predicted from the client's current state, the
correct pattern is a loading state — not an optimistic one — so that the UI reflects genuine
uncertainty until the server responds.
