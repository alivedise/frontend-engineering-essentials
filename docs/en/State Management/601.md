...
### 2. Storing Derived Values in State

```jsx
// WRONG -- redundant state
const [items, setItems] = useState([1, 2, 3]);
const [total, setTotal] = useState(6); // derived from items

function addItem(n) {
  const newItems = [...items, n];
  setItems(newItems);
  setTotal(newItems.reduce((a, b) => a + b, 0)); // must keep in sync
}
...
If a value can be computed from existing state, it is not state -- it is a derivation. Storing it separately creates synchronization bugs. In React, compute during render. In Vue, use `computed()`. In Solid, use a derived signal.

### 3. Too Many Individual useState Calls
...
### 5. Lifting State Prematurely

```
Before lifting state, ask:
1. Does more than one component actually need this state right now?
2. Or am I lifting "just in case" some future component might need it?

If only one component uses the state, keep it local.
Lift only when a second consumer appears.
```

Premature lifting violates co-location, increases coupling, and makes both the parent and children harder to understand. "You Aren't Gonna Need It" (YAGNI) applies to state placement just as much as it applies to code.
