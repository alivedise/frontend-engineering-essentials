...
**Use `useReducer` over `useState` for multi-field state with complex transitions.** When a component has three or more related `useState` calls, or when the next state depends on the previous state in non-trivial ways, SHOULD consolidate into `useReducer`. A reducer centralizes transition logic, prevents impossible intermediate states, and makes each state change explicit and auditable.
