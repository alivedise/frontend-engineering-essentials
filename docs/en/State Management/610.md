...
The correct maximum history depth depends on the expected editing session length and the memory
cost per entry. For snapshot-based histories, each entry's cost is the size of the full state
object at that point. For patch-based histories, each entry's cost is the size of the diff for
that operation — typically kilobytes for text edits and bytes for simple property changes.
A useful heuristic is to set the limit such that the worst-case history memory footprint — maximum
depth times maximum entry size — remains under 50MB. For most productivity applications, 100 steps
with snapshot-per-keystroke merging and patch-based storage easily satisfies this constraint.
Monitor history stack memory in production using `performance.measureUserAgentSpecificMemory()` or
by periodically sampling the stack's serialized size, and tune the limit based on observed usage
patterns rather than a single design-time guess. Document the chosen limit and its rationale in a
comment adjacent to the constant so that future maintainers understand why the value is what it is
rather than treating it as an arbitrary magic number.
