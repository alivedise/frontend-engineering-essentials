...
:::info
State management libraries are tools with opinions. Each one embeds a philosophy about how state should be structured, updated, and observed. Redux treats state changes as recorded events. MobX treats state as a living spreadsheet. Jotai builds state from the bottom up, one atom at a time. Choosing a library is a bet on which mental model your team will sustain over years of maintenance. Understanding the philosophy behind each library matters more than memorizing its API.
:::
