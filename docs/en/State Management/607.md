...
Derived state and async state use the same mechanism: mutate the proxy, and every subscriber that read the mutated property re-renders. There is no reducer or selector, and no separate vocabulary for "this update happens to be asynchronous." `refreshPrice` reads like an ordinary async function because it is one.
...
Nanostores minimizes bundle size and framework lock-in:
