...
### 1. Using Redux for Simple Applications

```tsx
// OVER-ENGINEERED -- Redux for a todo app with 3 components
// configureStore, createSlice, Provider, useSelector, useDispatch
// for state that never leaves one page and has no complex update logic

// APPROPRIATE -- local state or a lightweight library
function TodoApp() {
  const [todos, setTodos] = useState([]);
  // Simple enough. No library needed.
}
```

Redux pays dividends in large applications with complex state interactions, middleware needs, and team coordination. For a small app with local state, it adds indirection without benefit. As Dan Abramov wrote: "Don't use Redux until you have problems with vanilla React."

### 2. Using Raw Redux Instead of Redux Toolkit
...
### 3. Mixing Multiple State Libraries Without Clear Boundaries
...
### 4. Not Using Framework-Native Solutions First
...
### 5. Choosing Based on Hype Instead of Actual Needs
