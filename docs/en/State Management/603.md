...
> **SHOULD NOT** use context as a state manager for frequently-changing state.

Context is a **transport mechanism**. It solves the problem of "how does a value get from A to D without passing through B and C?" It does not solve the problem of "how do I efficiently update state and re-render only the affected components?" That is the job of a state management library.

When you put rapidly changing values (form input, cursor position, animation frame) into React Context, every consumer of that context re-renders on every change -- regardless of whether the specific piece of data they read actually changed. This is the **re-render-all trap**, and it is the single most common performance mistake with React Context.

Vue's `provide`/`inject` avoids this trap by default because Vue's reactivity system tracks dependencies at a fine-grained level. But even in Vue, context is best reserved for values that change infrequently: configuration, services, and cross-cutting concerns.
...
### 1. Using Context for High-Frequency State (React)

```jsx
// WRONG -- form input in context triggers re-renders across the entire tree
...
// Every keystroke in the name field re-renders ALL consumers,
// including components that only read email or bio.
```

Form state changes on every keystroke. Putting it in React Context means every consumer re-renders on every keystroke. Use local state, a form library (React Hook Form, Formik), or a state management library instead.

### 2. Not Splitting Contexts

Combining unrelated values (theme, auth, locale, feature flags) into a single context creates unnecessary coupling. When the user logs in, the theme consumer re-renders. When the locale changes, the auth consumer re-renders. Always split contexts by rate-of-change and domain.

### 3. Providing a New Object Reference Every Render

```jsx
// WRONG -- new object on every render, even if mode has not changed
function ThemeProvider({ children }) {
  const [mode, setMode] = useState('light');
  return (
    <ThemeContext value={{ mode, setMode }}>
...
Without `useMemo`, every render of `ThemeProvider` creates a new object, causing all consumers to re-render even when `mode` has not changed.

### 4. Confusing Svelte Context (Not Reactive) with Vue provide/inject (Reactive)
...
### 5. Using Context When a State Library Is Needed
