...
> **SHOULD** make the URL the source of truth for any state that should be shareable, bookmarkable, or survive a page refresh.

If a user can share a link and the recipient should see the same view -- the same filters, the same pagination, the same selected tab -- that state belongs in the URL. Component-local state (`useState`, `ref()`) is ephemeral: it vanishes on refresh and cannot be shared. URL state is durable: it persists across sessions, devices, and users.

This does not mean *all* state belongs in the URL. Modal open/closed, hover effects, animation progress, and form draft content are ephemeral UI state that should remain in component state. The guideline: **if the state changes the answer to "what is the user looking at?", it belongs in the URL. If it changes "how the page currently looks", it belongs in the component.**
...
### 1. Keeping Shareable State in Component Instead of URL

```tsx
// WRONG -- filters are lost on refresh, cannot be shared
function ProductList() {
  const [category, setCategory] = useState('all');
  const [sort, setSort] = useState('price');
  // URL stays at /products regardless of filter state
}

// CORRECT -- filters live in the URL
function ProductList() {
  const [searchParams, setSearchParams] = useSearchParams();
  const category = searchParams.get('category') ?? 'all';
  const sort = searchParams.get('sort') ?? 'price';
  // URL: /products?category=shoes&sort=price -- shareable, bookmarkable
}
```

If a colleague asks "can you send me the link to the shoes sorted by price?", the first version cannot produce that link. The second version already has it in the address bar.

### 2. Not Syncing URL with State on Browser Back
...
### 3. Encoding Complex State in URL
...
### 4. Not Using URLSearchParams (Manual String Parsing)
