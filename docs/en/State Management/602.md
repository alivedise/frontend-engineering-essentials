...
> **SHOULD** derive values from existing state rather than synchronizing separate state variables.

This principle maps directly to the **Single Source of Truth** rule. When you store a derived value as independent state, you create a second source of truth that must be kept in sync with the first. Every sync point is a potential bug.

### The Synchronization Tax

Consider a product list with a search filter. The "synchronized" approach stores both the full list and the filtered list:

```
State A (products) --[manual sync]--> State B (filteredProducts)
State C (searchQuery) --[manual sync]--> State B (filteredProducts)
```

Every time `products` or `searchQuery` changes, you must remember to update `filteredProducts`. Miss one update path and the UI shows stale data. The "derived" approach eliminates State B entirely:

```
State A (products) --\
                      +--> Derivation --> filteredProducts (computed)
State C (searchQuery) --/
```

There is no second state to synchronize. The derived value is always correct by construction.
...
**Simplicity.** Derived state has no setter, no update logic, no event handler. It is a declaration: "this value is defined as X." Synchronized state requires imperative glue code: "when A changes, also update B."
...
- Svelte `$derived`: uses push-pull reactivity -- dependents are notified of changes (push), but the value is not recalculated until read (pull).
...
This is not a flaw in React but a consequence of its rendering model. React components are functions that re-execute from top to bottom. `useMemo` is an optimization within that model: "do not recompute this value if the inputs have not changed." It is not a reactive primitive like Vue's `computed()` or Solid's `createMemo`.
...
Key points:
- `filteredProducts` derives from `products`, `searchQuery`, and `selectedCategory` -- no separate state needed.
- `totalPrice` derives from `filteredProducts` -- a chain of derivations.
...
- Dependencies are tracked at runtime, not compile time -- no dependency array needed.
...
Key points:
- In Solid, a plain derived function (like `displayLabel`) works for cheap computations -- no memoization needed.
...
### 1. Syncing State Manually Instead of Deriving
...
function addItem(n) {
  const next = [...items, n];
  setItems(next);
  setTotal(next.reduce((a, b) => a + b, 0)); // must remember to sync
}

function removeItem(index) {
  const next = items.filter((_, i) => i !== index);
  setItems(next);
  // BUG: forgot to update total here!
}

// CORRECT -- derive the value
const [items, setItems] = useState([1, 2, 3]);
const total = items.reduce((a, b) => a + b, 0); // always correct
```

The synchronized version has a bug: `removeItem` updates `items` but forgets to update `total`. The derived version cannot have this bug because `total` is recalculated from `items` on every render.

### 2. Expensive Computation Without Memoization
