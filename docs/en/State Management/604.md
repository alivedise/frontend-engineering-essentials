...
This is the fundamental challenge of **server state**: the client never owns the truth. It holds a cache, a local copy that may or may not reflect reality. The cache will become stale; the real questions are *how quickly* and *what you do about it*.
...
Dedicated server-state libraries, such as TanStack Query, SWR, Nuxt's `useFetch`, and framework-level data loading (Remix loaders, SvelteKit `load`, React Server Components), exist because this class of problems is distinct from client-state management and deserves specialized tooling.
...
This works for one endpoint. Now multiply it by every API call in your application. You need a reducer, three action types, a thunk, and a selector for every endpoint. You also need to answer:
...
This is the **stale-while-revalidate** pattern: serve the cache instantly, then silently update it in the background. The user sees content immediately, and it corrects itself moments later if anything changed. This approach is borrowed from HTTP caching semantics (RFC 5861).
...
- **React Server Components** execute on the server. They can `await` database queries or API calls directly. The result is streamed as rendered HTML: no client-side fetch, loading state, or cache entry is needed for the initial load.
...
These approaches complement TanStack Query and SWR rather than replace them. Server-rendered data handles the initial load; client-side cache libraries handle subsequent interactions, mutations, and real-time updates.
...
Loading state, error boundaries for fetch failures (Remix already provides route-level error boundaries), and client-side cache management are all unnecessary here. The data arrives with the HTML. After mutations via Remix `action` functions, loaders automatically revalidate.
...
- [TanStack Query: Overview](https://tanstack.com/query/v5/docs/framework/react/overview): official documentation covering queries, mutations, and caching
- [TanStack Query: Query Invalidation](https://tanstack.com/query/v5/docs/framework/react/guides/query-invalidation): guide to invalidating and refetching queries
- [TanStack Query: Optimistic Updates](https://tanstack.com/query/v5/docs/react/guides/optimistic-updates): patterns for instant UI updates with rollback
...
- [Remix: Data Loading](https://remix.run/docs/en/main/guides/data-loading): server-side loaders and automatic revalidation
