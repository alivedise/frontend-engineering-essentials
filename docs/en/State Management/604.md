...
> **SHOULD NOT** store server responses in client state managers (Redux, Zustand, Pinia) as the source of truth.

Server data belongs in a server-state cache managed by a purpose-built library. Client state managers are designed for synchronous, client-owned data -- theme preferences, sidebar open/closed, form drafts. Mixing the two creates a maintenance burden: you must manually track loading, error, staleness, refetching, and cache invalidation for every endpoint. Dedicated server-state tools handle all of this automatically.
...
### 1. Storing API Responses in Redux/Zustand as Source of Truth

```js
// WRONG -- manual server state in a client store
...
// You must manually handle: staleness, refetching, deduplication,
// cache invalidation after mutations, retry logic, garbage collection.
// TanStack Query handles all of this with a single useQuery call.
```

Use client stores for client-owned state (theme, sidebar, form drafts). Use TanStack Query, SWR, or framework-level data loading for server-owned state.
...
### 3. No Cache Invalidation After Mutation
...
### 5. Waterfall Fetching (Sequential When Parallel Is Possible)
