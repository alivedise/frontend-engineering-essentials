...
Engineers SHOULD use a form library for any form with three or more fields, validation
requirements, or async submission. The threshold is not about the number of state variables but
about the validation and submission lifecycle. A form with required fields, format validation,
server-side errors, and loading states during submission has enough lifecycle complexity to benefit
from a library's tested, consistent handling of each phase. Without a library, the engineer must
reimplement this lifecycle manually and consistently across every form in the product — an approach
that introduces divergence, bugs, and maintenance overhead as forms accumulate.

Engineers MUST validate on both client and server and MUST NOT rely solely on client-side form
validation for security-relevant input. Client-side validation provides immediate feedback to honest
users; it does not prevent malicious submissions. API endpoints that accept form submissions must
validate the same rules independently. A Zod schema can be shared between the frontend resolver and
the backend route handler, making dual validation cost-effective and keeping both sides in sync when
...
`mode: 'onTouched'`. Engineers must not render error messages for fields that have neither been
touched nor had a submit attempted, regardless of whether the schema would fail validation on the
current value.
