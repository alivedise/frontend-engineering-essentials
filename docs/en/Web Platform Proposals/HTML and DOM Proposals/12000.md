...
The blue node is the category overview. Gray range nodes are currently empty of drafted sub-articles; the planned topics are shown as gray leaves. Sub-articles for the 12000 range are the scope of a follow-up specification under the FEE project — this overview is the entry point until those sub-articles exist. Readers tracking specific proposals should use the References section's links to WHATWG and WICG until the corresponding sub-article is drafted.

The category's current emptiness reflects the pace at which HTML and DOM proposals graduate relative to the pace at which FEE catches up. Popover (reached Baseline 2025), Invokers (landing in 2025), and Document Picture-in-Picture (shipping in Chromium with Safari 18.3 support added) are all strong candidates for near-term sub-article writing, followed by graduation to FEE-100 or FEE-400 when they meet the cross-engine stability bar.

The reading order when the sub-articles exist will roughly follow: Popover first (the most mature and the highest traffic), Invokers second (built on Popover conceptually), Navigation API third (affects application architecture at a broader level), then Document PiP and Speculation Rules as narrower-audience features. This is the reading order for a developer encountering the HTML/DOM proposal landscape for the first time — start with what shipped universally, work backward toward less mature proposals.

Beyond the currently planned sub-articles, the 12000 range has room for features that have not yet reached WICG or WHATWG. Observable primitives (the `Observable` interface now under WICG discussion) may eventually arrive; view-source-level changes (the proposed `source` element for structured data) could populate the Elements range. The range's reserved space accommodates future proposals without renumbering existing articles.

## Tracks in this range

| Range | Topic | Example proposals |
|-------|-------|-------------------|
| 12001-12099 | Elements & attributes | Popover API, Invokers (`commandfor`), scoped custom element registry |
| 12100-12199 | DOM APIs & events | Document Picture-in-Picture, Web Locks, observable primitives |
| 12200-12299 | Navigation & routing | Navigation API, URL Pattern API |
| 12300-12399 | Performance & loading | Speculation Rules, Prerender, Priority Hints |
| 12400-12499 | Security & privacy | Trusted Types, Fenced Frames, Permissions Policy extensions |
| 12500-12999 | Reserved | — |

The range assignments follow reader-facing categorization. Popover sits under Elements because its primary author-facing surface is the `popover` attribute; Speculation Rules sits under Performance because its primary purpose is prefetch/prerender control. A proposal that spans multiple tracks (the Navigation API covers both navigation and loading) is placed under the track that matches its primary author-facing concept, with a cross-reference to the secondary track in its Related FEEs section.
