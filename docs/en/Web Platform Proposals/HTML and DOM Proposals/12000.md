...
The five sub-articles cover distinct surfaces of the HTML platform. FEE-12001 documents the `command` and `commandfor` attributes, which turn imperative click-handler wiring into declarative markup on any `<button>`. FEE-12002 documents `CustomElementRegistry` construction and `attachShadow({ registry })`, the namespace-scoping mechanism for custom element definitions. FEE-12003 documents `documentPictureInPicture.requestWindow()`, which promotes arbitrary HTML into a floating always-on-top window. FEE-12004 documents `window.navigation`, the platform routing primitive that SPA frameworks now layer over. FEE-12005 documents `<script type="speculationrules">`, the declarative prefetch and prerender governance replacement for `<link rel="prerender">`.

The articles are independent — a reader can pick any one and understand it without the others. Cross-references appear in each article's Internal References section where the proposals genuinely interact. Invoker Commands and Popover interact directly because `command="toggle-popover"` is the most common invoker use case; the Navigation API and Speculation Rules interact where a framework uses both for route-level prefetching; Document Picture-in-Picture and the Navigation API interact where PiP-aware routes need to preserve window state.

Different audiences enter this range at different articles, driven by the problem they already have:

- **Application engineer surveying UI primitives** — starts with Invoker Commands. The declarative `command` / `commandfor` model is the feature closest to everyday form and dialog work, and its integration with Popover covers the most common overlay patterns.
...
- **Framework author touching the router** — starts with the Navigation API. The platform primitive replaces the history-based router layer that every SPA framework has reimplemented for over a decade.
...
The reading-order recommendation above (the numerical order) is the ordering for someone surveying the range without a specific pre-existing problem. Readers with a problem already in hand should jump directly to the matching article above and treat the others as optional background.
...
The next candidate for graduation after Popover is likely Invoker Commands once Firefox drops the flag; the article at 12001 will graduate to FEE-100 range and a stub will stay in place. Scoped Custom Element Registry follows a similar path if Firefox ships it, which would put 12002 next in the graduation queue. Document Picture-in-Picture, Navigation API, and Speculation Rules depend on Safari and Firefox shipping and are less predictable.
...
The category's shallow depth reflects the pace at which HTML and DOM proposals graduate relative to the pace at which FEE catches up. Invokers and Document Picture-in-Picture are strong candidates for graduation to FEE-100 or FEE-400 when they meet the cross-engine stability bar. Popover graduated to Baseline 2025 and is tracked for a dedicated stable-range FEE in a separate spec.

The suggested reading order follows the numbering. Invoker Commands comes first as the most immediately useful declarative pattern; Scoped Custom Element Registry second for multi-owner namespace scoping; Document Picture-in-Picture third for the new window capability; Navigation API fourth for SPA routing at the platform level; Speculation Rules last for prefetch and prerender governance. A developer new to the HTML/DOM proposal landscape can read them in order to build a mental model — the simpler declarative additions first, architectural-scope features afterward.
...
Sub-articles are numbered sequentially, in roughly the order a reader encounters the proposals in practice. Invoker Commands and Scoped Custom Element Registry are element and attribute-level additions; Document Picture-in-Picture is a windowing API; the Navigation API covers SPA routing; Speculation Rules governs prefetch and prerender hints. Each article's Internal References section cross-links the proposals that interact with each other (for example, Invoker Commands and Popover).
