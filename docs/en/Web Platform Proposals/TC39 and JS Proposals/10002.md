...
JavaScript is the last major language to add explicit resource management syntax:
...
This ordering is intentional: the `finally` block may still reference the resources (for logging, flushing, etc.), so disposal happens after `finally` completes.
...
using conn: DatabaseConnection = new DatabaseConnection(url);
...
```js
import { AsyncDisposableStack } from 'core-js'; // or native in Node.js 22+
