...
- **`setTimeout(fn)` / `setInterval(fn)`**: the callback inherits the context at the `setTimeout()` call site. (The TC39 proposal specifies this behavior at the language level, formalizing what modern Node.js `AsyncLocalStorage` already does in practice. Early Node.js versions had inconsistencies with timer propagation; the TC39 spec eliminates this ambiguity.)
