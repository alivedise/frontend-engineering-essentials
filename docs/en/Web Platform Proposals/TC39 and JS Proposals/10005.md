...
state: draft
slug: signals-reactive-primitives-in-the-language
level: senior
reviewed: tone
...
SolidJS uses fine-grained reactive tracking to remove the virtual DOM entirely. A component never re-renders: SolidJS compiles templates to DOM nodes and attaches signals directly to DOM properties, so when a signal changes, only the specific DOM attribute or text node that depends on it updates. There is no diffing and no component re-render.
...
Angular adopted signals in version 17, giving it a reactivity primitive that tracks exactly what changed. Angular had relied on Zone.js, a library that monkey-patches browser APIs to trigger change detection after any async operation, since its inception. Zone.js was effective but opaque: it made every `setTimeout`, `Promise`, and `fetch` call trigger a full change detection pass, with no way to know what actually changed.
...
Combined, these properties guarantee that any call to `.get()` returns a value that is consistent with the current state of all transitive source signals. This is exactly what the spreadsheet model guarantees.
...
### Polyfill integration recipe: debounced microtask flush

The `signal-polyfill` README ships a canonical `effect()` recipe that frameworks plug into. It illustrates how to wire `Signal.subtle.Watcher` into a real scheduler with debouncing so the Watcher fires once per microtask regardless of how many writes land in the same tick.
...
2. Add the signals the boundary cares about to the Watcher's set; the notify callback fires the first time any of them transitions to dirty.
3. Inside notify, debounce with a `needsEnqueue` flag and `queueMicrotask` so the Watcher fires once per microtask regardless of how many writes land in the same tick.
4. In the microtask, drain `w.getPending()` by calling `.get()` on each entry to recompute it, then call `w.watch()` to re-arm the Watcher for the next dirty transition.
5. Hand the new values off to whatever rendering or effect mechanism the framework owns. The proposal stops at the dependency graph; rendering is the framework's job.

Lit demonstrates the pattern end-to-end: `SignalWatcher` is a mixin that owns steps 1-4 internally, and step 5 is "trigger an update on the host element."
