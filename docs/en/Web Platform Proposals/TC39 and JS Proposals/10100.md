...
**Note on indexing:** The proposal originally included an `.indexed()` method that would pair each value with its index. It was not included in ECMAScript 2025. To add indices, use an external counter — unlike `Array.prototype.map`, iterator `.map()` only receives the value, not the index:
...
Iterator helpers reached Stage 4 before ECMAScript 2025 (published June 2025) and shipped as part of that release. The proposal was championed by Michael Ficarra, Yulia Startsev, Lea Verou, and others. It sat at Stage 3 for several years while browser implementations caught up.
