...
:::info
You can now chain `.map()`, `.filter()`, and `.take()` directly on any iterator, including generators and `Set`s, without first spreading into an array.
:::
...
JavaScript has always had a composition problem with iterables. Arrays have rich transformation methods: `.map()`, `.filter()`, `.reduce()`. Those methods only work on arrays. Any other iterable (a `Set`, a `Map`, a generator, a `NodeList`) must first be materialized into an array before you can do anything useful with it.
...
Every generator that needs an early exit gets this loop written out by hand, and re-reviewed each time it appears. Libraries like Lodash and Ramda have filled this gap for arrays. `_.chain()` in Lodash is a well-known workaround, but it requires a runtime dependency and operates on arrays, not iterators.
...
### A worked case: paginated records

Your team receives large paginated datasets from an API. Rather than pulling all pages at once, a senior engineer has written an async generator that yields records lazily as they arrive:
...
You need the first 10 active premium users from this stream. `Iterator.from()` only accepts synchronous iterables, and `fetchRecords` is an `async function*`, so `Iterator.from(fetchRecords('/users'))` cannot be used here. `Iterator.from()` treats whatever `next()` returns as an already-resolved `{ done, value }` result; for an async generator, `next()` actually returns a `Promise`, so `value` reads as `undefined` and the first call to the `.filter()` predicate throws `TypeError: Cannot read properties of undefined (reading 'active')`.

The proposal that would let you write a clean chain against an async source, AsyncIterator helpers, is a separate TC39 proposal. As of mid-2026 it is at Stage 2, is not part of ES2025, and has not shipped in any browser (see Browser Support & Version Reference). Until it ships, an async source still needs a manual loop with an early exit:
...
Iterator helpers do solve this exact problem today for a synchronous source, for example a generator over records already loaded in memory. Example 1 below runs the same filter-then-cap chain end to end.
...
### 1. Filtering and capping a synchronous record stream
...
function* cachedRecords() {
  ...
  // a real dataset would keep yielding further records
}
...
This is the filter-then-cap chain from Context, running against a synchronous generator. `.take(10)` stops the chain once it has collected 10 matches or the generator is exhausted, whichever happens first. The equivalent chain for an asynchronous source such as `fetchRecords` is not available today; see Context and Browser Support & Version Reference.

### 2. Infinite Fibonacci with lazy filtering
...
### 3. Deduplicating and transforming with `Iterator.from(Set)`
...
### 4. Mapping over a Map's entries
...
### 5. Lodash equivalent for comparison
...
### 6. Early exit with `.find()` and `.some()`
...
**Engineers SHOULD NOT convert an existing `.filter().map()` chain on a small, already in-memory array to iterator helpers purely for performance.** For small finite arrays, the eager Array methods are usually faster and simpler. The win from iterator helpers is avoiding wasted work on large, infinite, or early-exited sources, not raw throughput; see Design Thinking for the trade-off.
...
JavaScript took the opposite approach. Array methods, introduced in ES5, were designed for small, in-memory collections in an era when "big data in the browser" was not a real concern. The methods are eager: every element is transformed before returning. The `Symbol.iterator` protocol was added in ES6, but it was only the low-level protocol. No high-level helpers came with it.

So arrays got a method library and every other iterable got only the raw `next()` protocol. Iterator helpers collapse this gap.

### When Array methods still win

Laziness is not free. Each iterator helper allocates a small helper object and drives the chain through the `next()` protocol one value at a time, so for a short, finite array that is already sitting in memory, `Iterator.from(arr).filter(fn).map(fn).toArray()` typically does more work than `arr.filter(fn).map(fn)`. Community benchmarks comparing the two show iterator helpers pulling ahead once the source is large, the transformation chain is deep, or the consumer exits early with `.take()` or `.find()`. For small in-memory arrays, plain Array methods stay faster and simpler. Reach for iterator helpers to avoid wasted work on data you do not need in full, not as a drop-in performance upgrade for existing `.filter().map()` chains on small arrays.
...
`_.chain()` wraps the array in a lazy wrapper that defers execution until `.value()` is called, though Lodash's lazy evaluation and shortcut fusion only kick in for chains long enough to qualify. The parity with iterator helpers is closest for long chains and weakest for small arrays. The bigger structural difference is that `_.chain()` still operates on arrays, not on the iterator protocol itself: it cannot consume generators or non-array iterables natively. Iterator helpers are the native, protocol-level version of this pattern.

### Infinite sequences are now practical

Before iterator helpers, an infinite sequence was mostly a conceptual tool. It was useful for explaining generators but awkward to use in real code. Now, a developer can express "give me the first N even squares from the natural numbers" as a direct pipeline:
...
The generator never produces the infinite list. Each step pulls one value at a time: `.take(5)` requests 5, `.filter()` pulls from `.map()` until 5 pass, `.map()` pulls from `naturals()` one value at a time.
...
**Note on indexing:** `.map()`, `.filter()`, `.flatMap()`, `.forEach()`, `.some()`, `.every()`, and `.find()` all receive `(element, index)`, the same second argument that `Array.prototype` methods receive. To pair a value with its index, use `iterator.map((v, i) => [i, v])`. The proposal originally included a standalone method (named `asIndexedPairs`, later `indexed`) that would pair each value with its index; it was dropped before ECMAScript 2025 because the `index` parameter on `.map()` and the other methods already covers that use case.
...
## Browser Support & Version Reference
...
Or use the `core-js` polyfill with `useBuiltIns: 'usage'` in Babel. The polyfill patches `Iterator.prototype` and adds `Iterator.from()`. Per MDN's compatibility data, native support for every helper method (`.map()`, `.filter()`, `.take()`, `.drop()`, `.flatMap()`, `.forEach()`, `.some()`, `.every()`, `.find()`, `.reduce()`, `.toArray()`, and `Iterator.from()`) landed in Chrome 122, Firefox 131, and Safari 18.4, matching the status block above.

### Sync vs. async iterator helpers

Everything in this article is the synchronous `Iterator` proposal, the one that shipped in ES2025. A separate proposal, [AsyncIterator helpers](https://github.com/tc39/proposal-async-iterator-helpers), would add the same `.filter()`/`.map()`/`.take()` family to async iterators and `async function*` generators. As of mid-2026 it sits at TC39 Stage 2 and has not shipped in any browser. Until it ships, an async source needs the manual `for await` loop shown in Context.

### TC39 Stage history

Iterator helpers reached Stage 4 before ECMAScript 2025 (published June 2025) and shipped as part of that release. The proposal was authored by Gus Caplan, Michael Ficarra, Adam Vandolder, Jason Orendorff, and Kevin Gibbons, and championed through TC39 by Michael Ficarra and Yulia Startsev. It sat at Stage 3 for several years while browser implementations caught up, reaching Stage 4 at the October 2024 plenary.
...
- [TC39 proposal-iterator-helpers](https://github.com/tc39/proposal-iterator-helpers) — proposal repository with spec text, authors, and champions
- [TC39 proposal-async-iterator-helpers](https://github.com/tc39/proposal-async-iterator-helpers) — the separate, still-Stage-2 proposal for async iterator helpers
