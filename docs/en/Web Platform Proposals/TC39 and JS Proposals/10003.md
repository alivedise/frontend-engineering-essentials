...
This proposal is at TC39 Stage 2. It is not available in any browser or engine without a transpiler.
The syntax is still actively evolving and **may change before Stage 4**. Always consult the [TC39 proposal repository](https://github.com/tc39/proposal-pattern-matching) for the latest syntax. Do not use in production without a Babel transpiler and an understanding of the stability risks.
:::

## Context

JavaScript's `switch` statement has served as the primary multi-branch dispatch mechanism for decades. It works for simple equality tests, but falls apart quickly once you need to branch on the *structure* of a value. Four specific problems make `switch` inadequate for modern JavaScript:
...
// Requires @babel/plugin-proposal-pattern-matching (Stage 2 — syntax may change)
async function handleApiResponse(res) {
  return match (res) {
...
// Avoid — guard is hidden inside the arm body
...
**Do NOT use `match` in production without a Babel transpiler.** The proposal is at Stage 2. The syntax has changed significantly from earlier drafts and may change again before Stage 4. Babel plugin availability does not indicate stability — it means the syntax is available for experimentation and feedback.
...
```js
// Requires: @babel/plugin-proposal-pattern-matching
// Stage 2 — syntax subject to change before Stage 4
...
function classifyNumber(n) {
  return match (n) {
    ...
    // or combinator: either condition matches
...
Note: `0` uses `SameValueZero` (matches both `+0` and `-0`). Use `+0` or `-0` explicitly for signed zero matching.
...
match (obj) {
  ...
  // Shorthand: { foo } is equivalent to { foo: void }
  // (tests existence of 'foo' but doesn't bind or test its value)
  when { foo }: '...';
}
```
