...
`addInitializer` queues a function to run during instance construction, after all field initializers have run. The queued function receives the instance as `this`.

Key rules:
- It can be called from any decorator kind except `class`.
