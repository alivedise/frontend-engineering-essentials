...
:::info
Take control of which CSS wins — not by fighting specificity on every selector, but by declaring which layer of rules outranks which other layer.
:::
...
The root of the problem is that all author styles — every `<link>` tag and `<style>` block you write — live in a single undifferentiated "author layer" in the CSS cascade. You cannot declare "these rules are framework rules" or "these rules are overrides" in a way that the browser respects. You can only compete on selector weight.
...
This is the control that CSS developers have needed for 25 years.

## Scenario

You are integrating a design system into a legacy enterprise application. The design system provides a `.button` component class. The legacy application has accumulated years of stylesheet debt, including selectors like `#sidebar .button`, `#app .button.button--primary`, and various other high-specificity rules that target buttons in specific contexts.

Every time the design system renders a button in the sidebar, the legacy stylesheet wins. The selector `#sidebar .button` has specificity `(1,1,0)` — one ID and one class — while the design system's `.button` has specificity `(0,1,0)`. The design system's styles are silently overridden.

The traditional fix is to escalate the design system's selectors to match or beat the legacy specificity. Add an ID wrapper. Repeat the class. Add `!important` to every property. Each of these "fixes" increases the specificity debt and makes the next conflict harder to resolve.

With `@layer`, you assign the legacy rules to a `legacy` layer and the design system rules to a `design-system` layer. You declare `@layer legacy, design-system` at the top of your stylesheet. The design system layer now has higher priority than the legacy layer. The design system's `.button` beats `#sidebar .button` in the legacy layer — not because of selector weight, but because of layer order.

No selectors were modified. No `!important` was used. The layers declare intent.
...
- SHOULD NOT put all your styles in layers without a deliberate strategy. Unlayered styles — styles not inside any `@layer` block — are treated as the implicit highest-priority layer for normal rules. If you layer some rules but not others, the unlayered rules will beat everything.
...
- SHOULD treat unlayered styles as the implicit top-priority escape hatch. When you need a legacy override to win without touching layer declarations, leave it unlayered — it will beat all layered rules automatically.
...
CSS was designed as a document stylesheet language. The original cascade model — user-agent styles, then user styles, then author styles, with specificity as the tiebreaker — was appropriate for a world where a single author wrote a single stylesheet for a single document.

The author origin was never subdivided. All author styles — your first `<link>`, your last `<style>`, your inline style minus the `!important` exception — competed in a flat pool. There was no mechanism to say "these rules are lower priority within my own code." The only levers were selector weight and source order.

As the web grew into an application platform, this flat model created a recurring class of bugs: third-party CSS overrides your CSS, your CSS overrides a third-party design system, a legacy rule overrides a new rule written by a developer who forgot the legacy rule existed. Every large CSS codebase eventually develops "specificity debt" — an increasing arms race of heavier selectors and `!important` declarations applied as bandages.

The CSS Working Group recognized this problem in the early 2010s. Various proposals for "cascade control" were discussed. The `@layer` mechanism — named layers with explicit priority declarations — was specified in CSS Cascading and Inheritance Level 5, with stable browser implementations arriving in 2022.
...
`@layer` does not make `!important` obsolete, but it changes how `!important` interacts with layers. For `!important` rules, the layer priority is reversed: `!important` rules in earlier (lower-priority) layers beat `!important` rules in later (higher-priority) layers. This mirrors how user agent and user `!important` rules work in the broader cascade — it is not an anomaly, it is a consistent application of the cascade model. Understanding this reversal is essential for codebases that mix layers and `!important`.
...
In the layered model, you reason about layers first. If rule A is in layer `utilities` and rule B is in layer `components`, and `utilities` is declared after `components`, then rule A wins — full stop — regardless of specificity. Specificity only matters when two competing rules are in the same layer.
...
The selector `#app .button.button--primary` has specificity `(1,3,0)`. The selector `.bg-red` has specificity `(0,1,0)`. Under the old model, `.bg-red` would lose badly. Under `@layer`, `.bg-red` wins because `utilities` is declared after `components`.
...
The `@import url() layer()` syntax layers the entire external stylesheet without modifying it. Any rule from normalize.css lives in `reset` — the lowest layer — so your own styles always win.

### Legacy integration: design system over legacy selectors

This is the scenario from the earlier section, implemented with `@layer`.
...
The design system's `.button` rule wins in every context — not because its selector is heavier, but because `design-system` is declared after `legacy`. The legacy stylesheet was not modified.
...
/* Unlayered — beats all layered rules, no matter what */
.modal--open {
  display: flex;
  align-items: center;
...
The `.modal--open` class is unlayered. It beats `.modal` from `@layer components` even though `.modal` was defined there. Use unlayered styles intentionally — they are the escape hatch for cases where you need a rule to win unconditionally.
...
There is no direct `@supports` test for `@layer`. The correct approach is to check the `CSSLayerBlockRule` or `CSSLayerStatementRule` interfaces in JavaScript:
