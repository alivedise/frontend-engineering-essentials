...
Modern browsers no longer delegate form control rendering to OS widgets — Chrome and Firefox use their own rendering engines. However, `<textarea>` and `<input>` remain **replaced elements** under the CSS specification, and until `field-sizing`, the CSS intrinsic sizing model had no mechanism to size them by their text content.
