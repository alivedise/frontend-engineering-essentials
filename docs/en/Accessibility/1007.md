...
- **SHOULD** follow WAI-ARIA APG patterns for interactive widgets — roles, ARIA attributes, and keyboard interactions are defined by the APG for each widget type, not invented per component.
- **SHOULD** prefer headless component libraries (Radix UI, React Aria, Headless UI) over building custom accessible components from scratch — the accessibility implementation cost is high and the failure modes are subtle.
...
### 7. Building custom accessible components when headless libraries exist

Unless you have a specific reason to avoid the dependency, use Radix UI, Headless UI, or React Aria. The accessibility implementation in these libraries has been tested across multiple screen readers and browsers, handles edge cases you have not encountered yet, and is maintained as ARIA specifications evolve. The productivity cost of implementing focus trap and ARIA attribute management correctly from scratch is consistently underestimated.
