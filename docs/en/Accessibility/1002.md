...
**MUST NOT use `outline: none` or `outline: 0` without providing a visible alternative focus indicator.** Removing the browser's default focus ring without a replacement makes keyboard navigation invisible for all users who depend on it, directly violating WCAG 2.4.7 (Focus Visible). MUST use `:focus-visible` to provide a custom ring that appears for keyboard users without showing on mouse clicks.

**MUST manage focus explicitly after key state transitions.** When a modal dialog opens, you MUST move focus to the first focusable element inside it; when it closes, MUST return focus to the trigger element. After SPA route changes, move focus to the new page heading or a live region announcement. After deleting a focused list item, move focus to the next sibling, the previous sibling, or the list container — never let focus disappear to `<body>`.

**MUST NOT use `tabindex` values greater than zero.** Positive tabindex values (`tabindex="1"`, `tabindex="2"`, etc.) create a two-phase focus order that visits all positive-tabindex elements before naturally focusable elements, overriding DOM order. The result is a non-linear, unpredictable Tab sequence that is disorienting for keyboard users. SHOULD use `tabindex="0"` to add elements to the natural tab order at their DOM position, and `tabindex="-1"` for programmatic focus only.
...
> **Note on import paths:** The example below uses the `@react-aria/*` hooks distribution (e.g. `@react-aria/focus`, `@react-aria/dialog`). Newer projects can use `react-aria-components` instead, which provides a higher-level component-based API and consolidates all hooks under a single package.
...
Add this to your stylesheet to suppress the focus ring on the heading when it is focused programmatically (not by keyboard):

```css
/* Suppress focus ring on programmatically-focused heading (not triggered by keyboard) */
.route-heading:focus {
  outline: none;
}
```
...
### Designing hover-only interactions that exclude keyboard users

Using `:hover` to reveal interactive elements (tooltips, dropdowns, card actions) makes those interactions inaccessible to keyboard users. Keyboard users cannot trigger hover states. Every `:hover` interaction must have a corresponding `:focus` or `:focus-within` equivalent so that keyboard focus on or within the element reveals the same content.

```css
/* WRONG: actions only appear on mouse hover — keyboard users never see them */
.card:hover .card-actions {
  display: block;
}

/* CORRECT: reveal on hover and when focus is anywhere inside the card */
.card:hover .card-actions,
...
- [:focus-visible CSS Selector — MDN Web Docs](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Selectors/:focus-visible)
