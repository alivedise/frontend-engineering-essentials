...
**SHOULD manage focus explicitly after key state transitions.** When a modal dialog opens, you MUST move focus to the first focusable element inside it; when it closes, MUST return focus to the trigger element. After SPA route changes, move focus to the new page heading or a live region announcement. After deleting a focused list item, move focus to the next sibling, the previous sibling, or the list container — never let focus disappear to `<body>`.
