...
- **MUST** associate every input with a visible label — not a placeholder, not a title attribute, not a visually adjacent paragraph.
- **MUST** associate every error message with its input via `aria-describedby` so the error is announced when the field receives focus.
- **SHOULD** use the native `<select>` element before building a custom combobox; the ARIA combobox pattern requires extensive keyboard handling that native `<select>` provides for free.
...
Placeholder is supplementary hint text, not a label. Use it for format examples, not field names.

### 2. Visually associating a label without programmatic association

```html
<!-- Wrong: the <p> looks like a label but is not programmatically connected -->
<p>First name</p>
...
Screen readers in forms mode do not read surrounding text — they only read the accessible name of the focused element.

### 3. Radio group without fieldset + legend
...
### 4. Error message not associated with input

```html
<!-- Wrong: error announced when injected into DOM (role="alert") but never re-announced on focus -->
<input type="email" id="email" />
<div role="alert">Please enter a valid email.</div>

<!-- Right: aria-describedby links the error to the input permanently -->
<input type="email" id="email" aria-describedby="email-error" aria-invalid="true" />
<div id="email-error" role="alert">Please enter a valid email.</div>
```

### 5. Marking only visual asterisk as required without text alternative
...
### 6. Disabling native form validation without providing an accessible alternative
...
### 7. Forgetting autocomplete on personal data fields
