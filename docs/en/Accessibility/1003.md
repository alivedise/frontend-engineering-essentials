...
- **Users with vestibular disorders**: Animations, parallax effects, and motion can cause vertigo and nausea in users with vestibular dysfunction. Roughly 35% of US adults over 40 show measurable vestibular dysfunction on balance testing (Agrawal et al., 2009), much of it asymptomatic. The `prefers-reduced-motion` media query allows these users to opt out.
...
- **Deuteranopia** (a form of red-green color blindness, most common): Absence of M-cone (medium-wavelength, green-sensitive) function; the milder anomalous-trichromacy form is deuteranomaly. Together, this deutan category affects approximately 6% of males. Red and green appear as shades of yellow-brown.
- **Protanopia**: Absence of L-cone (long-wavelength, red-sensitive) function; the milder anomalous-trichromacy form is protanomaly. Together, this protan category affects approximately 2% of males. Red appears very dark; red-green discrimination is lost.
...
WCAG 1.4.1 sits at Level A, the floor every WCAG-conformant site must clear before AA or AAA even come into consideration. Relying on color alone violates that floor; it is not a higher-level refinement teams can defer.
...
Two more recent CSS features reduce the boilerplate this requires. The `color-scheme` property, set as `color-scheme: light dark` on `:root`, opts form controls, scrollbars, and other user-agent-rendered UI into the matching scheme, so native widgets don't stay light (and unreadable) inside an otherwise dark page. The `light-dark()` function, now Baseline (widely available across browsers since May 2024), lets a single custom-property declaration carry both values, for example `--color-bg: light-dark(#ffffff, #111827);`, instead of duplicating the whole token block inside a `prefers-color-scheme: dark` block the way Example 3 below does.

### Reduced motion: prefers-reduced-motion

The `prefers-reduced-motion` CSS media query reflects whether the user has requested reduced motion in their operating system accessibility settings. Vestibular disorders affect the inner ear's balance and spatial-orientation systems. Animations and parallax can trigger real physical symptoms: dizziness, nausea, and disorientation.
...
Windows High Contrast Mode (WHCM) is an accessibility feature that overrides all author-defined colors with a system-enforced palette of approximately seven color keywords (`ButtonText`, `Canvas`, `CanvasText`, `Highlight`, `HighlightText`, `LinkText`, `GrayText`). CSS `background-image`, `box-shadow`, `text-shadow`, and most gradient effects are suppressed. The `forced-colors` media query, introduced in 2020, allows authors to detect this mode and adjust styles accordingly. macOS and iOS expose a comparable but distinct signal, the `prefers-contrast: more` media query, which reflects the platform's "Increase Contrast" accessibility setting without forcing a fixed system palette the way WHCM does.
...
**SHOULD meet WCAG AA contrast minimums throughout the interface.** Normal text requires a 4.5:1 contrast ratio against its background; large text (18pt or 14pt bold) and UI component boundaries such as form field borders and focus indicators require 3:1. SHOULD use automated tooling (axe-core, Lighthouse, or the Chrome DevTools color picker) to verify contrast ratios against rendered output during development, not only at design hand-off.

**MUST NOT use color as the only means of conveying information.** Every color-coded state (error, warning, success, required, selected) MUST carry a non-color supplement: an icon, a text label, a pattern, or a structural indicator. Approximately 8% of males have a form of color vision deficiency and cannot reliably distinguish red from green; a red error border with no icon or label is invisible as an error signal for these users.

**SHOULD support `prefers-reduced-motion` by gating all CSS transitions and animations inside a `@media (prefers-reduced-motion: no-preference)` block.** This approach makes no animation the default and treats animation as opt-in for users who have not requested reduction. This is the safer default compared to overriding animations in a `reduce` block. MUST also gate JavaScript-driven animations on `matchMedia('(prefers-reduced-motion: reduce)').matches` before triggering them.

## Design Thinking

### Why contrast ratios are mathematical

The WCAG contrast ratio formula derives from the **relative luminance** model of human vision. Relative luminance (L) for an sRGB color is computed by:
...
The **4.5:1 ratio threshold** was chosen because it approximates the contrast sensitivity required for a person with 20/40 vision (the standard for legal driving in many jurisdictions) to read normal-size text under typical office lighting. 20/40 vision means the person can read at 20 feet what a person with normal vision can read at 40 feet, and it corresponds to roughly a 1.5x loss of contrast sensitivity relative to normal vision: WCAG 1.4.3's own rationale combines the 3:1 minimum recommended by ISO 9241-3 with that 1.5x factor (3 x 1.5 = 4.5:1), a level of loss common with age-related low vision.

The **7:1 AAA threshold** is calibrated for users with roughly 20/80 vision. WCAG's rationale for SC 1.4.6 states that users with greater vision loss than that usually rely on magnifying or contrast-enhancing assistive technology rather than on higher-contrast text alone.

The relative-luminance formula is a coarse model, not a per-pixel perceptual guarantee. It is well documented to mispredict contrast for color pairs near the light and dark extremes: WCAG 2.x can overstate the readability of some dark-on-dark combinations enough that a pair reports a passing ratio while remaining hard to read in practice. APCA (the Accessible Perceptual Contrast Algorithm) was built as a more perceptually accurate replacement and was explored as a candidate for WCAG 3, but the W3C removed it from the WCAG 3 working draft in 2023 for lack of working-group consensus and has not settled on a replacement algorithm as of 2026. Until WCAG 3 ships a new model, the 4.5:1 and 7:1 ratios remain the enforceable, testable targets for WCAG 2.x conformance.
...
A form with red-highlighted required fields (no asterisk, no label, no icon) communicates nothing useful to a deuteranope. A dashboard with green positive trends and red negative trends is indistinguishable to a protanope. A red-only error border excludes the same population: roughly 8% of male users who cannot reliably distinguish red from green.

WCAG 1.4.1 sits at Level A, the minimum, because the W3C treats color-only information as a basic exclusion mechanism, not a matter of visual style.

The design solution requires no creative compromise: an icon or label costs nothing, takes one line of HTML, and simultaneously helps all users. Most people read labels faster than they decode color semantics.

### Why prefers-reduced-motion is underused

The `prefers-reduced-motion` media query has broad browser support (all modern browsers since 2019), but adoption trails the standard. The HTTP Archive Web Almanac's Accessibility chapter measured usage on roughly 34% of both desktop and mobile sites in 2022, rising to roughly half of mobile sites (50% mobile, 49% desktop) by 2024. A large share of animated interfaces still ship without it. The gap is cultural: animation is associated with polish and brand quality, and accessibility is mentally bucketed as a separate concern.

Named vestibular disorders include benign paroxysmal positional vertigo (BPPV), vestibular migraine, Meniere's disease, and labyrinthitis. For users with these conditions, unexpected parallax scrolling, autoplay hero animations, or loading spinners that move continuously can trigger vertigo that lasts hours.
...
- [FEE-1000: Accessibility Overview](/en/Accessibility/1000)
- [FEE-207: CSS Custom Properties & Theming](/en/CSS%20and%20Layout%20Systems/207)
- [FEE-1006: Testing Accessibility: axe, Lighthouse & Screen Readers](/en/Accessibility/1006)
