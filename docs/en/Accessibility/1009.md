...
"Reduced motion" does not mean "no animation." It means animations that could cause harm or
distraction should be reduced or replaced with instant transitions or simpler alternatives.
...
concentration on the primary task, turning ambient decoration into an active obstacle.

## Principle

Engineers MUST provide a `prefers-reduced-motion: reduce` media query fallback for all animations
that involve movement, parallax, scaling, or rotation. The fallback MUST reduce or eliminate the
motion. The conventional implementation is to write the reduced-motion state as the default CSS
rule and the animated state inside a `@media (prefers-reduced-motion: no-preference)` block, so
that users who have not adjusted their system settings and users who have enabled Reduce Motion
both receive a safe baseline. The value `transition-duration: 0.01ms` is the conventional choice
for "instant transition" in the reduced-motion fallback — it removes the perceptible animation
without eliminating the transition mechanism that JavaScript may depend on via `transitionend`
events.

Engineers MUST NOT publish content that flashes more than three times per second in a region
covering more than 25% of the screen. WCAG 2.3.1 (Three Flashes or Below Threshold) is a Level A
requirement. Content that violates this criterion can trigger seizures in users with photosensitive
epilepsy. This prohibition applies to all content types: CSS animations, JavaScript-driven
animations, animated images, videos, and embedded third-party content. Testing recorded content
against this criterion requires a tool such as the Photosensitive Epilepsy Analysis Tool (PEAT) or
the Harding Flash and Pattern Analyser. Visual inspection alone is not sufficient to detect
violations, because the human eye cannot reliably count flashes in the 3-per-second range; tooling
is required.
...
substitutions. The trigger is not visual change per se — a color change, a fade, or an opacity
transition rarely causes vestibular response — but rather the perception of objects moving through
space. Animations that suggest motion in three-dimensional space (zooms, perspective transforms,
rotation) are more likely to be triggers than animations that suggest two-dimensional change (color,
opacity, blur). When replacing a triggering animation with a reduced-motion alternative, the goal
is to preserve the communicative intent of the animation — "this element is appearing," "this element
is being removed" — while substituting a perceptually flat change (a fade) for a spatially
suggestive one (a slide or zoom).
...
WCAG 2.2.2 (Pause, Stop, Hide) requires that auto-playing content — carousels, hero animations,
background videos, looping illustrations — that starts automatically and lasts more than five
seconds must be pausable, stoppable, or hideable by the user. The pause or stop control must be
keyboard-accessible, and it must be visible without requiring the user to hover or focus into the
animated region to discover it. A looping background video in a hero section with no visible pause
...
"Pause background video" or an icon with accessible name — that toggles the video's `paused` state
and updates its own label to reflect the current state ("Play background video" when paused). The
button must be reachable by keyboard in the natural tab order of the page, appearing before the
