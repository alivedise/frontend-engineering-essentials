...
| `<header>` | `banner` (at page level) | When a direct child of `body` |
| `<footer>` | `contentinfo` (at page level) | When a direct child of `body` |
...
> **SHOULD** use native HTML elements and attributes before reaching for ARIA. A `<button>` is always preferable to `<div role="button">`. A `<label>` is always preferable to `aria-label` on an input. A `<h2>` is always preferable to `<div role="heading" aria-level="2">`.
>
> **MUST NOT** apply `aria-hidden="true"` to an element that is focusable or contains focusable children. Hiding an element from the accessibility tree while leaving it reachable by keyboard creates a ghost control that keyboard users can focus but screen reader users cannot perceive or understand.
>
> **MUST NOT** use ARIA to apply a role that contradicts the native semantics of the element in a way that is unsupported. For example, do not put `role="heading"` on a `<button>` — roles must be used on elements whose native semantics do not conflict.
>
> **SHOULD** keep ARIA state attributes (`aria-expanded`, `aria-checked`, `aria-selected`, etc.) synchronized with the actual visual and interactive state of the component at all times. Stale ARIA state is worse than no ARIA state — it actively misinforms AT users.
...
- The developer must add: `tabindex="0"`, `keydown` handler checking for Enter (key code 13) and Space (key code 32) to manually fire a click, and prevent the default scroll behavior of Space.
...
Note: `role="alert"` automatically implies `aria-live="assertive"` and `aria-atomic="true"`. `role="status"` automatically implies `aria-live="polite"` and `aria-atomic="true"`. You can use either the role shorthand or explicit `aria-live` on a generic container — but not both on the same element.
