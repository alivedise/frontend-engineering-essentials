...
> **SHOULD** test with at least NVDA+Chrome and VoiceOver+Safari as the primary testing matrix before shipping accessible features.
>
> **MUST NOT** use `role="application"` on content regions — reserve it only for widget regions that require complete keyboard control (e.g., a code editor). Misusing it strips browse-mode navigation from screen reader users.
>
> **SHOULD** test with real screen reader software, not browser DevTools simulations — DevTools accessibility panels show the accessibility tree but cannot replicate screen reader announcement behavior.
...
### Using `role="application"` on content regions

```html
<!-- Bad: Disables all browse-mode shortcuts for the entire page -->
<div role="application">
  <h1>Documentation</h1>
  <p>Welcome to the docs...</p>
  <!-- SR user pressing H expects to navigate headings.
       In application mode, H types the letter "h" instead. -->
</div>

<!-- Good: role="application" only on actual application widgets -->
<div role="application" aria-label="Spreadsheet editor">
  <!-- A genuine widget with its own keyboard interaction model -->
</div>
```
