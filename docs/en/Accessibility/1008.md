...
Engineers MUST write UI text in plain language, avoiding jargon, unexplained acronyms, and complex
sentence structures. Error messages must explain what went wrong in plain terms, what the user
should do next, and — where relevant — why the action failed. An error message that reads "Request
failed: 422 Unprocessable Entity" provides no useful information to any user; "Your email address
must be in the format name@example.com" is actionable. Plain language is not a matter of dumbing
down content — it is a matter of respecting the user's time and cognitive resources, which are
finite for every user and more constrained for users with cognitive disabilities.

Engineers MUST NOT require users to solve cognitive puzzles to authenticate when less cognitively
demanding alternatives exist. WCAG 3.3.8 requires that authentication not depend on a cognitive
function test — including transcribing distorted text (CAPTCHA), performing mental arithmetic, or
memorizing multi-step sequences — unless an alternative authentication method is available. Password
managers and copy-paste must be allowed for all authentication fields. Blocking autofill or paste in
password fields forces users to hold credentials in working memory and type them character by
character — a process that is cognitively demanding for everyone and functionally prohibitive for
users with memory impairments or motor difficulties.
