...
**SHOULD run axe-core in CI on every pull request and treat violations as test failures, not warnings.** Integrating axe-core via `@axe-core/playwright` or `vitest-axe` in the test suite gives every component a guaranteed structural baseline: no missing form labels, no invalid ARIA combinations, no contrast failures below the detectable threshold. A 0-violations axe report is a necessary precondition for shipping, not a sufficiency claim — it covers roughly 30–40% of real-world WCAG issues.

**SHOULD test with keyboard only before shipping any interactive component.** Open, navigate, close, and dismiss every interactive widget without touching the mouse. Tab through forms, activate dropdowns with Enter or Space, close modals with Escape, and verify that focus returns to the trigger element. Many keyboard failures — missing Escape handling, focus not returning after modal close, hover-only tooltips — are invisible to every automated tool and only discoverable through manual interaction.

**SHOULD test with at least one real screen reader before launching new user flows.** NVDA+Chrome on Windows is free and covers the largest free SR user base; VoiceOver+Safari is built into macOS and iOS and is non-negotiable for mobile. Screen reader testing answers questions that no tool can: does the announce sequence make sense, do live region updates fire at the right moment, does the component behave correctly in browse mode versus forms mode?
...
### 5. eslint-plugin-jsx-a11y installed but not enforced
