...
Teams MUST declare all shared framework singletons — `react`, `react-dom`, and any shared router (`react-router-dom`, `@tanstack/router`) — as `singleton: true` in the Module Federation `shared` configuration of both the host shell and every remote. Module Federation's runtime sharing mechanism resolves a single version of each singleton when multiple remotes expose the same package; without `singleton: true`, each remote bundles its own copy, and the page ends up with two or more independent React instances. Two React instances on the same page produce React context failures (context provided by one instance is invisible to components rendered by another), "invalid hook call" errors (hooks rely on a React-internal dispatcher that is not shared across instances), and hydration mismatches in SSR contexts. The `singleton: true` flag is not a performance optimization — it is a correctness requirement.

Teams MUST also specify `requiredVersion` alongside `singleton: true`. The version constraint ensures that Module Federation's shared scope negotiation can detect when a remote ships a version of React that is incompatible with the host's requirement and fall back gracefully, rather than silently loading a mismatched version. The `requiredVersion` field accepts semver range strings (`'^18.0.0'`), and the Module Federation runtime will warn at load time when the constraint is violated. Teams MUST treat these warnings as errors in CI and block deployments that introduce version mismatches in singleton packages.

Teams MUST NOT couple micro-frontends via direct cross-remote imports of state stores, context providers, or internal modules. An import of the form `import { useCartStore } from 'checkout/store'` in the catalog remote creates a build-time dependency on the checkout remote's internal implementation. When the checkout team refactors their store, renames the export, or changes its shape, they cannot do so without coordinating with every remote that depends on it — which defeats the entire purpose of independent deployability. The only safe cross-remote import is of explicitly exposed, versioned, public contracts: UI components, typed event payloads, or shared utility functions that the exposing team commits to maintaining as stable APIs.

Inter-MFE communication SHOULD be implemented through a custom event bus backed by the browser's native `EventTarget` API, or through a minimal shared contract published as a versioned package. The event bus approach keeps the communication channel in the browser's event system, which is already shared across all scripts on the page, requires no Module Federation configuration, and carries no framework dependency. The shared contract approach (a small `@acme/mfe-contracts` npm package that exports typed event payloads and store interfaces) is appropriate when the communication surface is large enough to warrant explicit versioning and changelog discipline.

Teams SHOULD NOT adopt micro-frontends for applications with fewer than three independent teams or fewer than twenty routes. Below this threshold, the deployment independence that micro-frontends provide is not worth the operational cost: maintaining separate CI pipelines, coordinating `remoteEntry.js` URL changes across environments, managing shared dependency version compatibility, and debugging cross-remote runtime failures requires engineering time that a team below the threshold would spend more productively on product features. The monorepo monolith, potentially with internal package boundaries enforced by lint rules, provides the same code organization benefits with none of the runtime complexity.
...
**Not declaring `singleton: true` for React and the router.** This is the most impactful mistake in Module Federation configuration. Without `singleton: true`, each remote that depends on React will bundle its own copy. Two React instances on the same page cannot share context — a context provider from the host's React instance is invisible to consumers using the remote's React instance. The `useEffect`, `useState`, and `useContext` hooks rely on a React-internal dispatcher that is not shared between instances, producing the "Invalid hook call" error. The fix is straightforward but requires touching every remote's `webpack.config.js`: add `singleton: true` and `requiredVersion` to every entry in the `shared` config.
...
**Hard-coding remote URLs in webpack config without environment substitution.** Hard-coding `checkout@https://checkout.acme.com/remoteEntry.js` in `webpack.config.js` makes it impossible to run the shell against a staging or local checkout remote without modifying the config file. Use `process.env.CHECKOUT_REMOTE_URL` with a default value for production, and inject the variable in CI per environment. This pattern also enables canary deployments: a staging environment can point to `checkout@https://checkout-canary.acme.com/remoteEntry.js` while production continues to use the stable URL.
