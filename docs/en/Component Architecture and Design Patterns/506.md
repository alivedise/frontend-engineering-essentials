...
An error boundary MUST implement `getDerivedStateFromError` to transition the component into its error state when a child throws. It SHOULD also implement `componentDidCatch` to report the error to an observability service — a boundary that renders a fallback but does not report the error is operationally blind: errors become invisible until users call support. Teams MUST NOT swallow errors silently; `componentDidCatch` MUST call the configured observability endpoint (Sentry, Datadog, New Relic, or an internal service) on every caught error, including the `info.componentStack` parameter, which provides the exact render path from the root to the component that threw.

Teams MUST place error boundaries at feature-subtree granularity. A single root-level boundary is not a resilience strategy — it is a slightly less catastrophic version of no boundary at all: the entire application's UI is replaced by a fallback for any single widget failure. The correct mental model is that each independently functional piece of UI represents a resilience unit. Each resilience unit MUST have its own boundary. Non-critical widgets (charts, recommendations, ads, activity feeds) SHOULD use a boundary with a retry-capable fallback or a silent empty state. Critical-path components (navigation, layout, authentication gates) SHOULD use a high-tree boundary with a minimal fallback that gives the user enough context to understand what happened and what to do next.

Error boundaries do NOT catch errors thrown in event handlers (`onClick`, `onChange`, `onSubmit`), in asynchronous code (`setTimeout`, `setInterval`, `requestAnimationFrame`), in Promise chains or `async/await` outside a render path, in server-side rendering, or in errors thrown inside the error boundary component itself. Teams MUST handle these failure modes explicitly: event handler errors require `try/catch` inside the handler; Promise rejections require `.catch()` or `try/catch` inside `async` functions with local error state. Mixing these two failure modes — expecting event handler errors to be caught by a boundary — is one of the most common resilience mistakes in React codebases.
...
**Single root-level boundary.** Wrapping the entire application in one `<ErrorBoundary>` at the `App` root means that when any widget — a chart, a recommendation carousel, an activity feed — throws a rendering error, the entire application's UI is replaced by the root fallback. From the user's perspective, the application has crashed. The fix is systematic: audit every independently valuable UI component and ensure it has its own boundary. The root boundary remains as a last-resort safety net, but it should almost never activate in a correctly structured application.
...
**Silently swallowing errors.** An error boundary that renders a fallback but does not call `componentDidCatch` with an observability service means production crashes are invisible. Users see a degraded experience, the engineering team sees nothing, and the error accumulates silently until enough users complain. Always connect the `onError` callback to a real observability pipeline. Even in development and staging environments, at minimum log the error and component stack to the console so the development team can see what boundaries are activating.
