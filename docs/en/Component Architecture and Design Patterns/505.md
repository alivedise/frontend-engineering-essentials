...
level: mid
slug: higher-order-components-cross-cutting-logic
reviewed: tone
...
As React applications grow, certain behaviors recur across unrelated components: authentication checks before rendering a page, analytics event firing when a user views a section, theme injection, permission gates, error boundaries, feature flag toggles. These concerns cut across the component tree horizontally rather than following any parent-child relationship. The challenge is implementing each concern once and applying it consistently, without duplicating logic or tangling unrelated concerns into individual components, while keeping the component tree legible to the debugging tools developers rely on every day.
...
**MUST be a pure function with respect to its component argument.** Given the same `WrappedComponent`, a HOC MUST return a component with the same augmented behavior on every invocation. It MUST NOT mutate the `WrappedComponent`'s prototype or `defaultProps`. Doing so causes unpredictable behavior when the same base component is passed to multiple HOCs or used directly alongside its wrapped form.

**MUST NOT stack more than two HOCs on a single component.** Each HOC layer adds a wrapper element to the React tree, multiplies DevTools noise, and creates a prop-tunneling channel. When more than two cross-cutting concerns must be applied, extract a single multi-concern wrapper that composes the concerns internally, or restructure the concerns as hooks the component calls directly.

**SHOULD prefer custom hooks for cross-cutting non-render concerns.** Analytics, auth state reads, permission checks, data prefetching, and store subscriptions do not require DOM wrapping. Hooks compose without the prop-tunneling tax HOC stacking introduces and produce no anonymous entries in React DevTools. MUST use HOCs when the concern requires wrapping or replacing the rendered output.
...
Alternative B is usually the cleanest: the component's source code reads as a plain list of the concerns it applies, without wrapper nesting, DevTools noise, or prop-tunneling. Alternative A is appropriate when the concern truly must wrap the render output (theme context injection in this example must surround the component, which is a render-level requirement) and when consolidating to avoid stacking is still worthwhile.
...
- [FEE-503: Headless Components & Renderless Patterns](./503.md): shares the motivation of keeping cross-cutting logic out of component bodies
