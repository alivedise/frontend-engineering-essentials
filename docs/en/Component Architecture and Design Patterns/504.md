...
A fourth dimension is developer ergonomics during testing. Controlled components are easier to test: the test author provides the value as a prop and asserts on what renders. Uncontrolled components require either simulating user input (typing into the field) or reading the value from a ref after interaction. For simple components used in isolated test suites, either works. For components tested with Testing Library, the preference is almost always to simulate real user input rather than to pre-set internal state, which means the controlled vs. uncontrolled distinction matters less in tests than it does in production code — the test interacts with the component the same way a user would, regardless of the control model.

A fifth dimension is TypeScript ergonomics. A fully controlled API narrows the type of `value` to the actual value type and requires `onChange`; the TypeScript compiler enforces both. A dual-mode API typically uses an overloaded signature or discriminated union to express that `defaultValue` is allowed only when `value` is absent, and that `onChange` is optional when uncontrolled. Libraries that implement this typing correctly prevent callers from passing both `value` and
`defaultValue` at compile time — a static guard against the most common dual-mode misuse.
Achieving this with TypeScript requires either function overloads (one overload for controlled,
one for uncontrolled) or a conditional generic type that makes `value` and `onChange` required
when `defaultValue` is omitted. The implementation is non-trivial but the payoff is
significant: callers receive a compile-time error rather than a runtime warning.
...
**Prefer local state when a component has a single, permanent caller and will never need external control.** `useControllableState` adds a layer of abstraction that pays off when multiple callers need different control strategies. When a component is a one-off leaf used in exactly one place and the product design makes external control implausible, keeping state entirely local — with no `defaultProp` escape hatch — is simpler, easier to read, and carries less cognitive overhead. Add dual-mode support when a second concrete use case appears, not in anticipation of one that may never arrive.
...
**MUST derive the displayed value entirely from props in controlled mode.** A controlled component MUST NOT hold a local copy of the value in internal state. Every user interaction MUST be communicated upward through the event callback; the parent is solely responsible for deciding whether and how to update the prop. If the parent does not update the prop, the component renders the same value again — this is intentional, not a bug.

**MUST accept `defaultValue` for initialization in uncontrolled mode and read it exactly once on mount.** The component MUST NOT re-read `defaultValue` on subsequent renders. Changing `defaultValue` after mount has no effect, and callers MUST NOT rely on it to reset the component.

**MUST NOT pass both a controlled prop and the corresponding `defaultValue` to the same component instance.** Doing so creates indeterminate behavior. When in controlled mode, the component MUST NOT update internal state in response to user interactions — the prop is the only source of truth, and every change MUST go through the callback.
