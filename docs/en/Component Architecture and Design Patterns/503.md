...
<template>
  ...
  <slot
    ...
    :getItemProps="(item) => ({
      role: 'option',
      onClick: () => { emit('select', item); query = item.label; isOpen = false; },
    })"
  />
</template>
```
...
**Over-headless-ing.** A button that always looks the same across the entire product does not need a headless layer. A modal that has one design across a single application does not need a headless layer. The pattern exists to handle real variation, not theoretical future variation. Build the specific component first; extract headless only when a second visually distinct context actually exists.

**Omitting ARIA prop factories.** If the hook returns only `isOpen` and `toggle` without ARIA attributes, every consumer must independently research the WAI-ARIA combobox pattern, implement `aria-expanded`, `aria-haspopup`, `aria-controls`, `aria-activedescendant`, and keyboard handlers. Bugs will diverge across consumers. ARIA wiring is exactly the behavior that the headless layer should centralize.
