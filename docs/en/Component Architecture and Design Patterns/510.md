...
state: draft
slug: compound-component-pattern
...
component needing to pass the active state explicitly to each sub-component. The pattern lets the
consumer rearrange the component's internal parts freely.
...
The compound component pattern is a specific application of the provider/consumer context model,
where the provider is a component and the consumers are its designated sub-components. It is
distinct from generic context providers that supply global state: a compound component context is
scoped to a specific component tree and has no meaning outside it. A `<Tabs>` context does not
leak beyond the `<Tabs>` boundary; two `<Tabs>` instances on the same page each manage their own
independent context. The context guard pattern (covered below) enforces this scoping. Without it,
a compound component's context can drift into a global state management solution that
happens to live inside a component.
...
`<Tabs tabs={[...]} panels={[...]} />`. This is compact for simple cases, but it is rigid: the
consumer cannot insert additional elements between the tab list and the panels, customize the
order of sub-components, or omit a sub-component that the design does not require. If the
...
context outside that tree, for example by calling `useTabsContext()` in a component that renders
at the root level, produces `undefined` rather than a context object. The resulting error
...
explicit type assertions. This means the guard is both a runtime safety net and the mechanism
that makes TypeScript infer the correct type for all hook consumers, eliminating the need for `!`
non-null assertions scattered throughout sub-component implementations.
...
clear in JSX that `Tabs.Panel` is part of the `Tabs` family rather than a standalone component
that happens to share a name prefix. IDEs that support property autocompletion will surface the
...
- **Passing shared state as props instead of context.** Threading an `activeTab` prop through
  ...
  a conventional parent/child hierarchy, not a compound component. The consumer loses
...
- **Omitting the context guard and using a non-null default value instead.** Setting
  ...
  guard pattern, a `null` default plus a throwing hook, is the correct approach.
...
- **Exposing internal sub-components as public exports without the static property relationship.**
  ...
  ancestor. This is precisely the situation the context guard exists to catch at runtime.
