...
Engineers SHOULD use the compound component pattern for stateful UI components with multiple
moving parts that users of the component need to control individually — tabs, accordions, select
menus, dialogs, menus, command palettes, and similar structures. The alternative — a single
component that accepts all configuration as props — produces prop explosion and limits layout
flexibility. A `<Tabs tabs={tabConfig} renderPanel={renderPanel} activeIndex={activeIndex}
onTabChange={onTabChange} tabListClassName={tabListClassName} panelClassName={panelClassName} />`
signature grows unbounded as requirements evolve. The compound pattern gives consumers structural
control while keeping behavioral state internal to the parent. The consumer's code reads as markup
that describes intent, not as a data structure that configures a black box.

Engineers MUST NOT use compound component context to share state between arbitrary components
that are not in a known parent/child relationship. Compound component context is implementation
state, not application state. Accessing a `<Tabs>` context from a component that is not a
descendant of a `<Tabs>` is an error that indicates the context is being misused as a global
store. Compound component contexts MUST throw a descriptive error when consumed outside their
intended parent. Engineers who need to share state across distant components that do not share a
meaningful parent boundary SHOULD reach for a dedicated state management solution — Zustand,
Jotai, React Query, or similar — rather than misusing a compound component context for that
purpose.
