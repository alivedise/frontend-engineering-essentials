...
**Triggering unnecessary re-renders (coarse-grained).** In React, creating new objects or arrays in render (`style={{color: 'red'}}`, `items.filter(...)`) creates new references every render, defeating `React.memo`. Memoize or hoist stable references.
