...
A fifth design consideration is the `asChild` pattern, popularized by Radix UI. Rather than
making a component polymorphic via an `as` prop — which requires TypeScript gymnastics to
properly type the ref and all forwarded props — `asChild` accepts any single React element as
`children` and merges the component's behavioral props onto it using `cloneElement` or a
slot-based merge utility. The consumer writes `<Button asChild><a href="/page">Go</a></Button>`,
and the `Button`'s `onClick`, `aria-*`, and class handling are applied to the `<a>` without
the `Button` needing to know which element it is rendering. The type system benefit is that the
consumer explicitly provides the element, so TypeScript can infer its props correctly without
a generic. The limitation is that `asChild` is not self-evident to consumers who are unfamiliar
with the pattern, so it SHOULD be documented clearly at the component level.
...
**Document polymorphic component intent with a JSDoc comment on the `as` prop.** TypeScript IDE hints surface the type signature of `as`, but they do not explain why the prop exists or which elements are semantically appropriate. A concise JSDoc comment — for example, `/** The root element to render. Use 'a' or a router Link component when the button must navigate. Defaults to 'button'. */` — surfaces in IntelliSense at the callsite, reducing the need to consult the component's source or documentation page. This is especially valuable for teams where the component library is consumed by engineers who did not write it. Include any semantic constraints directly in the comment: if only `'button'`, `'a'`, and `typeof Link` are reasonable values, say so explicitly rather than leaving the `ElementType` union to speak for itself.

**Version the `as` prop's accepted types conservatively.** Widening `as` from `'button' | 'a'` to `ElementType` is non-breaking; narrowing it back is breaking. Start with the smallest set of element types that covers all real use cases, and expand the union as concrete needs arise. This keeps the TypeScript error messages useful — `Type '"div"' is not assignable` is more actionable than `Type '"div"' is not assignable to type 'ElementType'` with no further constraint.

**MUST forward all props not explicitly consumed onto the underlying native element using `{...rest}`.** Consuming all props internally breaks ARIA attributes, `data-testid` attributes used by test suites, and native event handlers that callers depend on. Consuming all props is not a protective measure — it is a trap.

**MUST support `ref` forwarding on any component that wraps a native DOM element.** A component that does not forward its ref forces callers to use `document.querySelector`, which is fragile and fails in SSR contexts.

**MUST NOT make every component polymorphic by default.** The `as`/`asChild` pattern should be adopted only where the need for element flexibility is real and recurring. Premature polymorphism adds implementation and type complexity with no benefit for components whose root element never legitimately changes.
