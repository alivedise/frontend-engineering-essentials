...
Understanding what a component API should accomplish requires understanding what it should not accomplish. A component API should not be a vehicle for encoding application-specific business logic — that is the caller's responsibility. It should not replicate the HTML attribute vocabulary — that is what `{...rest}` forwarding is for. It should not use boolean flags to encode mutually exclusive states — that is what enum props and TypeScript union types are for. And it should not assume that the root element is always the same HTML element — that is what the polymorphic `as` and `asChild` patterns are for. A component API that does all these things correctly is simultaneously simpler, more powerful, and easier to evolve than one that does not.
...
A component's API is also a promise about stability — and a constraint on the implementation. Props are harder to remove than to add. A prop that was added in haste to satisfy a single use case becomes a compatibility obligation that persists across major versions. The design work happens before the first consumer arrives: thinking through which props are genuinely necessary, which behaviors should be composed externally rather than encoded as props, which HTML attributes should pass through automatically, and which elements the component may legitimately need to become. The patterns in this article are not mechanical transformations to apply by rote — they are frameworks for having these design conversations early, before the API hardens.
