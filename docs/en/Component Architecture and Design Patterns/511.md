...
### The re-render cost of context values

Before exploring mitigation patterns, it is worth understanding exactly how a context value
change propagates to re-renders. When a component calls `Context.Provider` with a `value` prop,
React compares the new value to the previous value using `Object.is` semantics — the same
reference equality check used for memoized props. If the reference has changed, React schedules
a re-render for every component in the tree that has subscribed to this context via `useContext`
or `use(Context)`. This happens regardless of whether the new value is semantically equivalent to
the previous one.

The implication is that a provider whose value object is re-created on each render — even if the
contents are identical — re-renders all its consumers on every provider render. A theme provider
that passes `{ theme: 'dark', toggleTheme }` as a new object literal on every render will cause
every themed component to re-render whenever any ancestor of the provider renders. This is not a
React bug; it is the expected behavior of `Object.is` comparison applied to object references.
The developer's responsibility is to ensure that the context value's reference is stable when its
contents are stable, which is what `useMemo` provides.
