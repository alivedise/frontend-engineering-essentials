...
> **SHOULD** use Conventional Commits format for all commit messages. The format (`type(scope): description`) is the machine-readable input that unlocks deterministic version calculation and automated CHANGELOG generation.

> **SHOULD** automate version bumps and CHANGELOG generation rather than doing either manually. Use `semantic-release` for single-package repositories and applications; use `@changesets/cli` for monorepos or libraries with multiple independently versioned packages.

> **SHOULD** publish packages from CI only, never from a developer's local machine. A CI-published package is reproducible, auditable, and not subject to the differences between developer environments.

> **MUST NOT** manually edit the version field in `package.json` or write CHANGELOG entries by hand once automated tooling is in place. Manual edits will be overwritten by the tooling and create confusing conflicts.

> **SHOULD** use publish-on-tag rather than publish-on-merge. An explicit git tag represents deliberate release intent. Publishing on every merge to `main` makes every commit a potential release and removes the ability to batch changes.
...
`semantic-release` analyzes a list of these structured messages and applies a deterministic rule set: any commit with type `feat` triggers a minor version bump; any commit with type `fix`, `perf`, or `refactor` triggers a patch bump; any commit with a `BREAKING CHANGE:` footer or a `!` after the type triggers a major version bump. The highest applicable bump across all commits since the last release determines the new version. There is no ambiguity, no committee decision, no "should this be a 1.5 or a 2.0?" debate. The commit messages made the decision at the time of the change.
...
The `changesets/action` bot maintains a single open PR titled "chore: version packages" that accumulates all pending changesets. The PR body shows a summary of which packages will be bumped by how much and what will appear in each CHANGELOG. When this PR is merged, the action runs `pnpm changeset publish`, which:

1. Runs `pnpm changeset version` — bumps `package.json` versions and writes CHANGELOG entries
2. Publishes all packages with a new version to npm
3. Creates git tags for each published package (e.g., `@myorg/button@2.1.0`)
4. Creates GitHub Releases for each tag
...
`semantic-release` and changesets both need to read git tags and commit history to determine what has changed since the last release. Using the default `actions/checkout` with `fetch-depth: 1` (shallow clone) means the tooling cannot see previous release tags. Always set `fetch-depth: 0` in workflows that run release tooling.
...
**Publishing from a developer's local machine.**
Local publishes bypass CI checks, use the developer's local build environment, are not reproducible, and create packages that cannot be traced to a specific git commit in the same way CI-built packages can. Once automated publishing is in place, revoke personal npm publish access to the package and require all publishes to go through the CI workflow.

**Not skipping CI on the release commit.**
`@semantic-release/git` pushes a commit back to the default branch (with updated `CHANGELOG.md` and `package.json`). If the workflow triggers on every push to that branch and the release commit does not include `[skip ci]`, the release workflow runs again on the release commit — finds no new releasable commits since it is now past the tag — and exits harmlessly, but wastes CI minutes. Add `[skip ci]` to the release commit message template in `.releaserc.json`.
