...
Edge functions SHOULD be used for logic that is latency-sensitive and
stateless or near-stateless: authentication checks from a JWT, geolocation-
based redirects, A/B test assignment, and HTML response rewriting. Edge
functions MUST NOT be used for operations that require a database round trip
unless the database has edge-native replication (Cloudflare D1, Neon, PlanetScale).
Geo-partitioned data MUST comply with the data residency requirements of
the applicable jurisdiction — data that must remain in a specific region
MUST NOT be processed or stored by edge functions running outside that region.
