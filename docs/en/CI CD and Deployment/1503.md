...
**MUST NOT hardcode API base URLs or environment-specific configuration in frontend source code.** If `VITE_API_URL` is a compile-time constant rather than an environment variable, every preview deployment will call the same hardcoded endpoint — usually production. Teams MUST use environment variables for all values that differ between production, preview, and development contexts, including analytics IDs, OAuth redirect URIs, and feature flag service endpoints.

**SHOULD enable preview deployments for all pull requests that affect UI.** Stakeholders, designers, and QA SHOULD be able to review the real build at a real URL rather than relying on screenshots or local setup instructions. Managed platforms (Vercel, Netlify, Cloudflare Pages) implement this automatically once connected; self-hosted setups SHOULD replicate the same workflow using GitHub Actions with a deploy step and a PR comment bot.

**SHOULD update the PR comment in place on subsequent pushes rather than creating a new comment.** A bot that creates a new comment on each push clutters the PR thread with stale URLs. Teams SHOULD check for an existing bot comment and update it in place so the PR always shows a single, current preview URL.

**SHOULD add `X-Robots-Tag: noindex` headers to all preview deployments.** Preview URLs are publicly accessible and will be indexed by search engines unless explicitly blocked. Polluting search results with development builds is a correctness problem, not merely cosmetic. Apply the noindex header at the platform level so every preview deployment is protected automatically.

**SHOULD add a teardown workflow triggered on `pull_request` with `types: [closed]` for self-hosted preview environments.** Managed platforms (Vercel, Netlify) tear down preview environments automatically when a PR closes. Self-hosted setups SHOULD replicate this behavior to avoid accumulating hosting costs and namespace pollution as closed PRs leave orphaned deployments behind.

**SHOULD NOT grant production-level secrets to preview workflows triggered by fork PRs.** GitHub does not expose repository secrets to workflows triggered by pull requests from forks by default. Teams SHOULD use environment-level secrets with required reviewers, or `pull_request_target` with explicit trust boundaries, rather than granting unscoped secret access to any fork that opens a PR.
...
- [FEE-1504: Production Deployment: Static Hosting & CDN](1504.md)
