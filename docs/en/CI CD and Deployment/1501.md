...
**MUST include `runner.os` in the cache key.** Node modules contain platform-specific binaries and native addons that differ between Linux and macOS. A cache created on a Linux runner MUST NOT be offered to a macOS runner, because `node_modules` built for one OS will not function correctly on the other. Always compose the key as `node-modules-${{ runner.os }}-...-${{ hashFiles('**/pnpm-lock.yaml') }}`.

**MUST set `cancel-in-progress: true` for PR workflows and MUST NOT set it on the main branch.** When a developer pushes a corrected commit, the superseded run SHOULD be cancelled immediately to avoid wasting runner minutes and polluting the PR status list. However, cancelling runs on `main` means some commits are never fully tested, which defeats the purpose of post-merge CI. Use `cancel-in-progress: ${{ github.ref != 'refs/heads/main' }}` in a single workflow file to handle both cases correctly.
