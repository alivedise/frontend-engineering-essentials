...
There is one nuance: for workflows triggered by pushes to `main` (post-merge), cancellation is usually wrong. You want every commit to `main` to be fully tested and potentially deployed. The correct pattern is to apply `cancel-in-progress: true` only on PR-triggered runs, and omit it (or set it to `false`) for `main` branch runs. The conditional expression <code v-pre>cancel-in-progress: ${{ github.ref != 'refs/heads/main' }}</code> handles this in a single workflow file.
...
**MUST cache `node_modules` keyed by the lockfile hash, and MUST pair every cache restore with `--frozen-lockfile`.** A cold `pnpm install` takes 60–90 seconds; a cache hit takes 3–5 seconds. Keying on `hashFiles('**/pnpm-lock.yaml')` makes invalidation correct-by-construction — when any dependency changes, the lockfile changes, the hash changes, and the cache is busted automatically. Without `--frozen-lockfile`, a partial restore-key match can silently install packages that differ from the committed lockfile.

**MUST include `runner.os` in the cache key.** Node modules contain platform-specific binaries and native addons that differ between Linux and macOS. A cache created on a Linux runner MUST NOT be offered to a macOS runner, because `node_modules` built for one OS will not function correctly on the other. Always compose the key as <code v-pre>node-modules-${{ runner.os }}-...-${{ hashFiles('**/pnpm-lock.yaml') }}</code>.

**MUST set `cancel-in-progress: true` for PR workflows and MUST NOT set it on the main branch.** When a developer pushes a corrected commit, the superseded run MUST be cancelled immediately to avoid wasting runner minutes and polluting the PR status list. However, cancelling runs on `main` means some commits are never fully tested, which defeats the purpose of post-merge CI. Use <code v-pre>cancel-in-progress: ${{ github.ref != 'refs/heads/main' }}</code> in a single workflow file to handle both cases correctly.

**SHOULD add `workflow_dispatch` to all CI workflows.** Without this trigger, re-running a workflow against a branch without pushing a new commit is impossible. Teams SHOULD include it as a zero-cost escape hatch for debugging runner environment issues or validating fixes without polluting git history.

**SHOULD pin tool versions rather than using `latest`.** Using `version: latest` for `pnpm/action-setup` or `actions/setup-node` means a major version release can silently break CI without any change to your own code. Teams SHOULD pin to a major version (`version: 9`) and upgrade deliberately.
