...
> **SHOULD** use feature flags for high-risk features. A flag allows the feature to be disabled within seconds without any deployment, providing a safety net that no rollback strategy can match in speed.

> **SHOULD** set a removal date when creating a feature flag. Flags that outlive their purpose become permanent conditional branches in the codebase — increasing complexity, blocking refactoring, and creating confusion about which code path is canonical.
...
import { LaunchDarklyClientProvider } from '@launchdarkly/openfeature-node-provider';
...
OpenFeature.setProvider(new LaunchDarklyClientProvider(ldClient));
...
**Deploying and releasing simultaneously for every change.**
Without feature flags, every merged PR is immediately visible to all users. High-risk changes — checkout redesigns, search overhauls, pricing page changes — should be merged behind a flag and released independently of the deployment. Treating deployment and release as the same event removes the ability to control exposure.

**Using canary without session affinity.**
Routing a user to the new version for some requests and the old version for others within the same session breaks the application. JavaScript bundles, API versions, and application state are not interchangeable between deployments. Always configure sticky sessions when implementing canary splits.

**Never removing feature flags.**
A flag that remains in the codebase after reaching 100% rollout is dead code with extra steps. The code path it guards is no longer conditional — it is always active — but the conditional remains, obscuring that fact. Schedule removal tasks at flag creation time, not after.

**Evaluating flags server-side but not client-side (or vice versa).**
In server-rendered applications, flag evaluation must be consistent between server and client renders. If the server renders the new checkout (flag on) but the client re-renders the old checkout (flag evaluated again on mount with a different result), the application shows a flash of wrong content. Ensure the flag evaluation context is hydrated correctly from server to client.

**Treating a flag platform outage as a hard failure.**
Feature flag SDKs fetch flag definitions from a remote service. If that service is unavailable, the SDK should fall back to a locally cached value or a safe default — not throw an exception that crashes the application. Always define a safe default value when evaluating a flag. Most SDKs accept this as the second argument to the evaluation call.

**Blue-green with database migrations that are not backward compatible.**
For applications with a database, blue-green requires that the new version and old version can both operate against the same database schema simultaneously. A migration that removes a column that the old version reads will break rollback. Always use the expand-contract migration pattern: add the new column (expand), migrate data, deploy the new application version, then remove the old column (contract) only after the old version is gone.
