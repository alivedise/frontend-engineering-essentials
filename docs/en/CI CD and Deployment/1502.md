...
> **SHOULD** run lint and type-check before tests — fail-fast ordering surfaces cheap signals first and avoids wasting expensive runner minutes on code that does not pass static analysis.

> **SHOULD** run `tsc --noEmit` as an explicit, separate CI step from the build — modern bundlers like esbuild and SWC transpile without type checking, meaning a successful build does not imply type correctness.
...
**Running tests before lint and type-check.** Tests are significantly more expensive than static analysis. Placing tests first in a sequential pipeline means a developer waits 2 minutes to discover a lint error that a 15-second job would have caught immediately.

**Assuming a successful build implies type correctness.** Vite, Next.js (when using SWC), and esbuild-based setups all transpile TypeScript without running the type checker. Removing `tsc --noEmit` from CI leaves the project without a type-safety gate.

**Setting coverage thresholds too high and gating on them.** A hard 80% threshold incentivizes writing low-value tests that touch lines without asserting behavior. Teams that gate on coverage percentage frequently end up with worse test suites than teams that gate on zero — because the floor creates the wrong optimization target.
...
**Not uploading build artifacts.** If the build job succeeds but produces no artifact upload, downstream jobs (size-limit, deployment preview) must rebuild from source. Uploading the `dist/` directory as a GitHub Actions artifact and restoring it in downstream jobs saves the full build time on every run.

**Skipping `--frozen-lockfile` on CI installs.** Without `--frozen-lockfile` (pnpm) or `--ci` (npm), the package manager may resolve dependency ranges differently than the lockfile specifies. This produces non-deterministic installs — the CI environment may have different package versions than the developer's machine. Always pass `--frozen-lockfile` on CI.

**Not setting `fetch-depth: 0` for affected commands.** Nx and Turborepo need full git history to compute the diff between the current branch and the base. Without `fetch-depth: 0` in `actions/checkout`, the repository is a shallow clone and the base commit may not exist, causing the affected computation to fall back to running everything.
