...
**MUST set `Cache-Control: no-cache` on HTML entry points and MUST NOT apply long TTLs to them.** `index.html` always has the same URL. Caching it aggressively means users with a cached copy will not receive new deployments until the TTL expires — potentially one year later. Teams MUST configure the CDN to send `no-cache` on `index.html` so browsers revalidate with the origin on each load, while still benefiting from 304 Not Modified responses when nothing has changed.

**MUST set `Cache-Control: public, max-age=31536000, immutable` on content-hashed assets.** JavaScript bundles, CSS files, and images with content hashes in their filenames are safe to cache permanently because the URL changes when the content changes. A browser that cached `main.a3f8c2.js` will never serve a stale file; if the bundle changes, the new URL is `main.d91b4f.js`. Teams MUST ensure the bundler has content hashing enabled and MUST NOT disable it.

**SHOULD use platforms that provide atomic deployment guarantees.** Without atomicity, there is a window during deployment when some CDN edge nodes serve files from the new version and others from the old, producing broken applications for users who land during the transition. Vercel, Netlify, and Cloudflare Pages all implement atomic deployments. Teams using AWS S3 directly SHOULD upload hashed assets before `index.html` to minimize the unsafe window, and SHOULD consider versioned S3 prefixes with a CloudFront origin switch for true atomicity.

**MUST upload files in the correct order when deploying to S3.** Hashed assets MUST be uploaded before `index.html`. If `index.html` goes live before its referenced JavaScript bundles exist in the bucket, users who request the page immediately after deployment will receive 404 errors for the new bundle URLs. Upload assets first, then `index.html` last.

**MUST invalidate CloudFront's edge cache for `index.html` after every S3 deployment.** CloudFront caches responses at edge nodes. Uploading a new `index.html` to S3 without invalidating the edge cache means users continue receiving the old version until the TTL expires. Teams MUST run `aws cloudfront create-invalidation --paths "/index.html"` as the final step of every S3+CloudFront deployment.

**SHOULD use a managed platform rather than raw S3 for teams without existing AWS infrastructure.** Setting up a production-grade S3 + CloudFront deployment requires configuring an S3 bucket, CloudFront distribution, origin access control, a custom domain in Route 53, ACM certificate, cache behaviors, and CI invalidation steps. Each step is an opportunity for misconfiguration. Managed platforms (Vercel, Netlify, Cloudflare Pages) SHOULD be preferred when the team lacks AWS expertise, as they provide correct defaults for cache headers, HTTPS, atomic deployments, and preview environments at near-zero engineering cost.
...
- MDN Web Docs — [Cache-Control](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Cache-Control): authoritative reference for all `Cache-Control` directives including `no-cache`, `immutable`, and `max-age`
- Vercel Documentation — [Cache Control Headers](https://vercel.com/docs/headers): how Vercel's edge network applies and respects cache headers, and how to configure them via `vercel.json`
- Netlify Documentation — [Headers](https://docs.netlify.com/manage/routing/headers/): configuring custom response headers in `netlify.toml`, including cache header recommendations for SPAs
