...
Static assets with content-hash filenames MUST be served with long-lived
`Cache-Control` headers (`max-age=31536000, immutable`). The HTML entry
point — the document that references all assets — MUST be served with a
short `max-age` or with `no-cache` to ensure browsers fetch a fresh copy
after each deployment. CDN purge MUST be triggered for the HTML entry point
as part of the deployment pipeline, before the deployment is considered
complete. Teams MUST NOT rely on users clearing their browser cache as
a workaround for failed invalidation.
