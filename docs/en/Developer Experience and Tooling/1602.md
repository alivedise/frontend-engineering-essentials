...
Start with no `prettier.config.js`. Prettier's defaults are well-chosen: 80-character print width, double quotes, semicolons, trailing commas everywhere (the "all" setting, which is the v3 default). For many teams these defaults are acceptable without modification. Accepting defaults means zero config surface — nothing to debate, nothing to maintain, nothing to diverge between environments.
...
- `trailingComma: "es5"` — opt back to ES5-only trailing commas if the codebase must support environments where function-parameter trailing commas break (rare; Prettier v3 defaults to `"all"`).
