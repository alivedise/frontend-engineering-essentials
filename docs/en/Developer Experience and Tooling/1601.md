...
**MUST use `eslint.config.js` with explicit plugin imports in new projects.** The legacy `.eslintrc` format is deprecated as of ESLint 9. Teams MUST define separate config objects for source files, test files, and config files, each with their own rule sets, so that rule scope is explicit and directory-traversal cascade behavior is eliminated.

**MUST enable type-aware rules with `parserOptions.projectService: true`.** This is the modern API for typescript-eslint v8+, replacing the older `parserOptions.project` that required specifying tsconfig paths manually. Teams MUST also set `tsconfigRootDir: import.meta.dirname` alongside `projectService` to anchor tsconfig discovery to the directory containing `eslint.config.js`; without this, the discovered tsconfig may differ depending on the directory from which `eslint` is invoked.

**MUST install the full standard plugin set for React and TypeScript projects.** A well-rounded configuration includes `eslint-plugin-react` for component rules, `eslint-plugin-react-hooks` to enforce the Rules of Hooks, `eslint-plugin-jsx-a11y` for accessibility coverage, `eslint-plugin-security` to catch XSS and injection patterns, and `eslint-plugin-unicorn` for modern JavaScript idiom enforcement. Teams MUST NOT selectively omit plugins from this set — the combined coverage catches more than any individual plugin provides.

**MUST apply `error` severity to rules that block shipping and `off` to explicitly disabled rules.** Rules that catch bugs — unused variables, console statements in production builds, explicit `any`, and floating promises — MUST be set to `error`, not `warn`. Teams MUST NOT leave rules absent from the config; an explicit `"rule-name": "off"` is a documented decision, while an absent rule is ambiguous.

**SHOULD use `FlatCompat` when migrating an existing `.eslintrc`-based configuration.** The `@eslint/eslintrc` package provides a `FlatCompat` utility that wraps a legacy config into a flat config object, allowing incremental migration without rewriting everything at once. Teams SHOULD migrate individual plugins and rules to native flat config objects one at a time, removing each `compat.extends()` call as the corresponding migration is complete.

**MUST run `eslint --max-warnings 0` in CI.** This flag treats any warning as a failure, preventing the accumulation of unaddressed warnings that developers learn to ignore locally. Combined with a lint-staged pre-commit hook, this creates two enforcement gates: warnings MUST be resolved before commit, and any that slip through MUST be caught in CI before merge.
