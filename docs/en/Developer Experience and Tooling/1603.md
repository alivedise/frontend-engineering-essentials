...
For pnpm projects, `prepare` does not run automatically on `pnpm install` by default since pnpm v8. Add `enable-pre-post-scripts=true` to `.npmrc` to restore this behavior, or instruct developers to run `pnpm prepare` manually after their first install.
