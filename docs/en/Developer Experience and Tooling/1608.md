...
Debugging SHOULD begin with a specific hypothesis rather than exploratory
logging. The hypothesis determines which DevTools panel and which breakpoint
type to use. Breakpoints SHOULD be preferred over `console.log` for
interactive debugging because they stop execution at the exact point of
interest, expose the full call stack, and allow live inspection of all
in-scope variables without modifying the source. The Performance panel
MUST be used for diagnosing any interaction that feels slow before
optimizing code, because perceived slowness is frequently caused by
long tasks outside the code path the developer suspects.
