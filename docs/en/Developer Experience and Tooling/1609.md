...
**SHOULD** manage per-project environment variables with `direnv` and a committed `.envrc` file that contains non-secret values, sourcing secrets from a secrets manager or a gitignored `.envrc.local` file. Centralizing configuration in a committed `.envrc` eliminates "works on my machine" issues caused by missing or differently named environment variables across developer machines.

**MUST** make onboarding automatable to a single command or a linear sequence of commands that a new developer can follow without asking for help. A bootstrap script that installs the correct Node version, starts local services, installs dependencies, and runs database migrations verifies that the environment setup is reproducible and signals that the team treats environment setup as a first-class concern.
