...
Teams MUST adopt a commit message convention and enforce it mechanically with commitlint. A convention enforced only by social pressure — code review comments, README instructions — is a convention that will be applied inconsistently, especially under time pressure.

The `type` field MUST be chosen with precision. `feat` means a user-visible feature addition. `fix` means a bug correction. `chore` means maintenance work that is neither a feature nor a bug fix. Misusing `feat` for refactors or `fix` for chores corrupts the automated version bump signal: a `chore` commit mislabeled as `feat` will trigger an unintended minor version release.

Breaking changes MUST be marked with both the `!` suffix in the header and a `BREAKING CHANGE:` footer. The `!` is a visual signal to human readers; the footer is the machine-readable description that tools like `semantic-release` read to include migration instructions in the changelog. Using only one of the two is insufficient.

Scopes SHOULD be enumerated in the commitlint configuration. An unconstrained scope field accumulates drift: `Auth`, `auth`, `authentication`, `login` are four ways to write the same scope, none of which match each other in filters and changelogs. A fixed enum forces discipline and keeps scope values consistent across the commit history.

Commit descriptions MUST be written in the imperative mood, lowercase, without a trailing period. "add input validation to login form" is correct; "Added input validation" is not. The description completes the sentence "If applied, this commit will..." — the imperative form fits naturally into that sentence; the past tense does not.

Header length MUST be capped. 72 characters is the conventional limit, derived from the git log display width and email client wrapping behavior. Longer headers are truncated in tooling and difficult to read in `git log --oneline`.
...
The eight types defined by `@commitlint/config-conventional` have precise meanings:
...
`@semantic-release/commit-analyzer` uses the `@commitlint/config-conventional` type mapping by default. Only `feat`, `fix`, and `BREAKING CHANGE` trigger releases. All other types are included in the changelog if generated, but do not trigger version bumps.
...
flowchart TD
    ...
    C -- Yes --> E{Is scope defined and in allowed scopes?}
    E -- No --> F[Hook rejects: scope not in enum]
    E -- Yes --> G{Is description non-empty and header <= 72 chars?}
...
### Treating scope as a free-text field

A commitlint configuration without `scope-enum` treats scope as unconstrained. Over time, `auth`, `Auth`, `authentication`, `login`, and `user-login` all appear in the commit history, all referring to the same feature area. The changelog grouping becomes meaningless, and filtering commits by scope in tools like `git log --grep` requires knowing every spelling variant.

The fix: define `scope-enum` with the allowed list for your repository. Start with the obvious ones and add more as the codebase grows. A pull request that adds a new feature area is also a pull request that adds a scope to `scope-enum`.

### Using `feat` for refactors

Refactoring existing code is not a feature. It does not add user-visible functionality. Labeling it `feat` causes `semantic-release` to bump the minor version, signaling to consumers that new functionality was added — which did not happen. Consumers who read changelogs to decide upgrade timing are misled.

The correct type for refactoring is `refactor`. This type does not trigger a version bump because, by definition, a refactor is a behavior-preserving change. If a refactor inadvertently exposes new API surface, the new surface is the feature and should be separately committed as `feat`.
...
### Marking breaking changes with only one mechanism

A breaking change commit with `!` in the header but no `BREAKING CHANGE:` footer is visually prominent but not parseable by automated tools. `semantic-release` detects the major version bump from the `!`, but the changelog entry for the breaking change will be empty — no description of what broke, no migration path.

A breaking change commit with a `BREAKING CHANGE:` footer but no `!` is parseable but not visually prominent. Developers scanning git log will not see the warning.

Use both. The `!` suffix is for humans reading the log; the `BREAKING CHANGE:` footer is for tools generating changelogs and for humans reading changelogs. Both audiences need both signals.
