...
The "Debug Vitest: Current File" configuration is particularly useful: it runs only the test file currently open in the editor, with full breakpoint support. With `--pool=forks`, Vitest runs in a single process that VS Code's Node.js debugger can attach to. Breakpoints in test files resolve reliably in this mode.
