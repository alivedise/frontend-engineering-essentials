...
| Many downstream microservices behind each screen | Build BFFs; aggregation demand makes the pattern essential at that scale | One more service per experience to deploy, monitor, and keep on call | Newman; SoundCloud runs dozens at hundreds of millions of requests per hour |
