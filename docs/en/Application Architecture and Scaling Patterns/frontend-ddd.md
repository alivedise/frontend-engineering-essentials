...
One upstream endpoint, two context-local models, one deliberately tiny shared kernel (a small model fragment two contexts co-own and change together). The two "Product" types agree on identity and money and on nothing else, and that disagreement is the design.
...
The anti-corruption layer is a mapping function at the context's edge. The upstream DTO (data transfer object, the wire-format shape) is shaped by the backend's storage and its own history; the ACL translates it into this context's language and absorbs upstream changes at one point:
...
export function add(a: Money, b: Money): Money {
  if (a.currency !== b.currency) throw new CurrencyMismatch(a.currency, b.currency); // domain error, defined beside Money
...
- **SHOULD** align context boundaries with team and language boundaries rather than with technical layers; contexts follow where the vocabulary changes, which usually follows where the teams are. The polyseme test finds the seams: when the same word means different things in two meetings, a boundary sits between them.
...
**The enemy is the innocent shared types folder.** A shared `types/` or `models/` package with one `Product` interface that has accumulated dozens of optional fields is the standard failure shape of a scaled frontend monorepo: checkout, catalog, search, and admin each pushed their needs into the one shared shape. Every field ends up nullable somewhere and no one dares delete anything, so a change for one team breaks another. Bounded contexts are the named alternative: several small models that are each internally consistent, plus explicit translation where they meet. The counterintuitive move is accepting duplication of the *word* to avoid coupling of the *model*.

**Strategic transfers, tactical mostly does not.** Aggregates and repositories answer persistence questions (transactional consistency, identity across a database round-trip) that the browser has outsourced to the server and to query caches. The frontend's hard problems are the strategic ones: whose word wins, where models may differ, who translates. Judged by trying to build aggregates in React, frontend DDD looks like ceremony; judged from context mapping, it answers the questions the frontend actually has.

**Conway's law runs through the middle of this.** Conway's law says that systems mirror the communication structures of the organizations that build them, and Fowler notes that context boundaries in practice follow human culture: the model changes where the language changes, and the language changes where the teams change. A shared model across teams with different vocabularies forces cross-team sign-off on every field change; a context-per-team split with an ACL keeps those changes local, and a boundary linter keeps them that way.

## Deep Dive

**Context relationships, translated to frontend reality.** DDD names the ways two contexts can relate, and a frontend consuming an API it does not own is, in DDD terms, *downstream*. Being a **conformist** (adopting the upstream model as your own) is cheap while the API is stable and yours to mirror; the moment the upstream model turns awkward for the UI (storage-shaped nesting, enum codes, nullable sprawl), the **anti-corruption layer** buys a context-local model for the cost of one mapping file. **Customer/supplier** describes the healthier org where the frontend's needs feed the API's roadmap, and a **published language** is what a versioned, documented API schema (OpenAPI, GraphQL SDL) provides: a stable, shared representation neither side owns unilaterally.

**TypeScript as the language-enforcement medium.** Structural typing undermines context separation by default: any object with the right fields *is* a `Sku`. Branded types restore nominal-style boundaries (`type Sku = string & { readonly __brand: "Sku" }`), making an ACL not just conventional but type-checked; a raw `product_id` string does not compile where a `Sku` is required. Combined with package-level `exports` maps that hide internals, the compiler and the module resolver jointly enforce what DDD calls the context boundary.
...
| A context needs independent deploys and ownership | Context per micro-frontend | Promote the boundary to a build/deploy boundary (see [FEE-1901](/micro-frontend-architecture)) |
...
- [Clean & Hexagonal Architecture in the Frontend](/clean-hexagonal-frontend)
