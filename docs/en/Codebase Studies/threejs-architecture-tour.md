...
:::info
This tour reads three.js at tag **r172 (2024-12-31)** through five lenses: class hierarchy, module decomposition, public API surface, render loop, and resource lifecycle. Five subsystem-root classes (`Object3D`, `BufferGeometry`, `Material`, `Texture`, `Node`) all extend a single `EventDispatcher` base and fan out into parallel inheritance trees that never cross-merge below the root, with a sixth large independent tree rooted at `Loader`. The same one-line `dispose()` event-fire contract used by the original geometry/material/texture trio was carried forward into the newer `Node` shader-graph subsystem (54 subclasses at r172). The library does not own the per-frame loop. It owns one tick of it, exposed as a synchronous `render(scene, camera)`.
:::

## Context

Three.js is the WebGL/WebGPU 3D rendering library maintained by mrdoob (Ricardo Cabello) and a long-running contributor pool. The repository at tag `r172` packs **685 `.js` files** under `src/`, organized into **16 responsibility-named subdirectories** plus a small set of top-level entry/aggregator files (`Three.js`, `Three.Core.js`, `Three.WebGPU.js`, `Three.WebGPU.Nodes.js`, `Three.TSL.js`, `Three.Legacy.js`, `constants.js`, `utils.js`). Two domains, `renderers/` and `nodes/`, held roughly 65% of the source files at r172, split between the legacy WebGL backend and the node-graph plus WebGPU stack. r172 shipped 2024-12-31; three.js has since reached r185 (2026-07-01), roughly 13 releases later, and the WebGPU/node path has continued to develop since this snapshot. The counts, line numbers, and excerpts below describe r172, not the current tip.
...
The architectural angle that makes the codebase interesting is generational: the original four resource-owning classes (`BufferGeometry`, `Material`, `Texture`, `WebGLRenderTarget`) established conventions in the WebGL era, and the same conventions reappear verbatim on the `Node` base class introduced for TSL (Three.js Shading Language, the node-graph shader authoring layer that feeds both `NodeMaterial` and the WebGPU backend).
...
`src/Three.js` is the entire package entry file at r172 (lines 1-9):
...
A sixth large tree sits outside the EventDispatcher forest: `Loader` is its own root with 13 direct subclasses (the file-format loaders) and does not extend `EventDispatcher` at r172. The class declaration in `src/loaders/Loader.js` line 3 reads `class Loader {` with no `extends` clause, confirming Loader is structurally independent from the event substrate. The independence tracks with what a `Loader` actually holds: it owns no observable or GPU-disposable state of its own. It parses input and hands back geometries, textures, or materials that are themselves `EventDispatcher` subclasses and get disposed on their own, so a `Loader` has nothing to publish and no reason to inherit the pub/sub root.
...
Two domains, `renderers/` and `nodes/`, hold roughly 65% of the 685-file source tree. The other 14 are small and tight: `scenes/` is 3 files, `audio/` is 5, `cameras/` is 6, matching the inheritance picture of small subtrees of subsystem-root classes plus a few helpers.
...
**Imperative Per-Frame Entry.** The renderer exposes a synchronous `render(scene, camera)` method that the host application calls once per frame; the library never owns the loop, only one tick of it. A lens scan for `render`/`tick`/`update`/`step`/`loop` method and function definitions across the r172 `src/` tree surfaces 82 hits (a deduplicated count of definition sites, not a raw `grep` line count), concentrated in three giants: `WebGLRenderer.js` (2955 LOC), `common/Renderer.js` (2860 LOC), and `nodes/core/NodeBuilder.js` (2426 LOC).
...
The user-facing reason this matters is stated directly in the official manual: three.js does not garbage-collect WebGL-side resources on its own, so an application that stops referencing a geometry, material, or texture without calling `dispose()` leaks GPU memory ([three.js manual, "How to dispose of objects"](https://threejs.org/manual/en/how-to-dispose-of-objects.html)). The event-fire contract this lens documents is the mechanism the manual's warning is about.

A lens scan for `dispose`/`destroy`/`release`/`close` call sites across the r172 source tree surfaces 180 hits (a deduplicated count of call sites, not a raw `grep` line count). The classic resource-owning classes (`BufferGeometry`, `Material`, `Texture`, `WebGLRenderTarget`) all use the one-line dispose-event shape, unchanged in r172. `src/core/BufferGeometry.js` lines 1105-1109:
...
Byte-identical body. There are **54 `extends Node` declarations** across `src/nodes/` at r172 (verified via `grep -rnE 'extends Node\b' src/nodes/`; a plain substring match on `"extends Node"` over-counts to 57 by also catching `extends NodeParser`, `extends NodeFunction`, and `extends NodeVar`, none of which are `Node` subclasses), and every one of the 54 inherits this dispose contract. Node subclasses added years after the WebGL renderer inherit the same one-line dispose contract.
...
**What to look for elsewhere:** a `dispose()` method whose entire body is `this.dispatchEvent({ type: 'dispose' })`, present on classes that own GPU/native resources, with no equivalent on the universal scene-graph or event-dispatcher base. A second strong signal (the one this lens highlights) is the same one-line shape reappearing verbatim on a newer subsystem's base class, here `Node` for shader-nodes. That reappearance is evidence the team treats the contract as a reusable shape carried across architectural generations.
...
- [FEE-1810 Three.js — The Dispose Lifecycle Contract](/en/Codebase%20Studies/threejs-dispose-lifecycle) — the single-pattern satellite that establishes the dispose-event contract on the classic four resource classes; this tour adds the generational evidence (54 `Node` subclasses inherit the same shape).
