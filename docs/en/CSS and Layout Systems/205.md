...
### 5. Not using @layer to order third-party vs. application CSS

**The mistake:** Importing third-party CSS (component libraries, resets) without layer ordering, then fighting specificity battles:
...
.button { background: navy; }  /* May lose to the library's .button */
```

Without `@layer`, the cascade order depends on source order and specificity. Third-party styles often use high-specificity selectors that are hard to override without `!important`.

**The fix:** Use `@layer` to declare explicit cascade ordering:
...
@layer app {
  .button { background: navy; }  /* Always wins over third-party */
}

@layer overrides {
  /* Emergency overrides go here */
}
```
