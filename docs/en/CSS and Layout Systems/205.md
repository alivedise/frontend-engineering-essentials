...
**Use CSS Modules as the default for component-scoped styles.** CSS Modules produce a single unhashed class per rule in source but a unique identifier at build time, giving you low-specificity selectors (no attribute overhead, no inline style specificity), zero runtime cost, and deterministic output. SHOULD be the starting point for any React or framework-agnostic codebase. Only deviate toward runtime CSS-in-JS when dynamic theming requirements cannot be satisfied by CSS custom properties, or toward Shadow DOM when distributing components to unknown consumers.
