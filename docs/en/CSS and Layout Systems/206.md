...
### 1. Choosing a framework before defining the design system

**The mistake:** A team picks Tailwind CSS because it is popular, then discovers their design system uses a 4px spacing scale while Tailwind defaults to a 4px base but with a specific set of named stops. They fight the defaults throughout the project:

```html
<!-- Fighting Tailwind's spacing scale with arbitrary values -->
<div class="p-[18px] gap-[22px] mt-[14px]">
```

When most of your classes use arbitrary values (`[18px]`), you have lost the consistency benefit that justified using a framework in the first place.

**The fix:** Define your design system tokens first (spacing scale, color palette, type scale, breakpoints). Then evaluate which framework best accommodates those tokens. Tailwind is highly configurable; Open Props provides raw tokens you can adopt directly; Panda CSS lets you define tokens with type safety.

### 2. Fighting Tailwind defaults instead of customizing the config
...
### 3. Bootstrap grid for everything when CSS Grid is simpler
...
### 4. Multiple CSS frameworks in one project
...
### 5. Not measuring runtime CSS-in-JS bundle impact
