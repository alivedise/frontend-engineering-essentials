...
Layout containment creates a new block formatting context: floats and margins
stay contained within the element's boundary rather than bleeding out or
collapsing through. Nothing inside the element affects the layout position of
elements outside it. The browser can stop its layout recalculation at this
element's boundary: work done inside the contained subtree does not propagate
upward to the document root or sideways to sibling elements. For a component
that updates frequently, such as a data grid that
...
are typically leaf nodes in the layout tree. It is still worth knowing when
...
off-screen. The visual clipping differs from `overflow: hidden`: it does not
affect the element's geometry for layout purposes, but it does tell the
...
behavior is the same as adding `position: relative` to the element, a pattern
already common for scoping stacking contexts, so the practical impact is
usually neutral.
...
those elements into view, the browser renders them on demand. This differs from
lazy-loading in the image or resource sense: no network requests are deferred.
Instead, the browser skips the work of computing layout and paint for content
