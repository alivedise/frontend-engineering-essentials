...
Engineers SHOULD apply `contain: layout paint` to components that function as
self-contained rendering units — carousels, data tables, rich text editors,
infinite scroll items, and similar widgets whose internal changes do not affect
sibling layout. The containment hints allow the browser to limit recalculation
scope to the contained subtree. The benefit is proportional to the complexity of
the contained subtree and the frequency of updates; a static, rarely-updated
component gains little from containment and adding it indiscriminately is not a
meaningful optimization.

Engineers MUST set `contain-intrinsic-size` alongside `content-visibility: auto`
on any element large enough that its absence would cause layout shift when the
element transitions from skipped to rendered. Without `contain-intrinsic-size`,
the browser gives skipped elements zero size; when they are rendered on scroll,
the document height changes and the scroll position jumps. The `auto <length>`
form of `contain-intrinsic-size` memoizes the last known actual size of the
element so the placeholder remains accurate after the first render pass.
