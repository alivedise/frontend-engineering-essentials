...
Also beware of unintentional stacking contexts. Adding `transform`, `opacity < 1`, or `filter` to an ancestor creates a stacking context that traps all descendant z-indexes. This is the most common reason "z-index: 9999 does not work." [FEE-211 Stacking Contexts, Paint Order & the Top Layer](./stacking-contexts-paint-order-top-layer.md) covers the complete trigger list and paint order.
...
| [FEE-211 Stacking Contexts, Paint Order & the Top Layer](./stacking-contexts-paint-order-top-layer.md) | Full treatment of the stacking model introduced in this article -- the complete stacking-context trigger list, the CSS 2 Appendix E paint order, and the top layer as the alternative to z-index escalation. |
...
- [MDN: display](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Properties/display) -- Full reference for the display property including the two-value outer/inner syntax
