...
Engineers SHOULD limit `backdrop-filter` to a small number of concurrently
visible elements on any given page. Each element with `backdrop-filter` requires
the browser to composite the layers beneath it during every repaint, which
increases GPU load. On mobile devices with limited GPU resources, excessive use
of `backdrop-filter` causes frame drops during scroll and animation. A single
modal backdrop, one sticky header blur, or a small tooltip blur are typical
acceptable uses; applying it to every card in a large list is not.

Engineers MUST NOT rely on `mix-blend-mode` alone to communicate meaning to
users. Blending modes alter the visual appearance of text and images in ways
that can reduce contrast below WCAG accessibility thresholds, particularly when
the background color shifts due to content scrolling beneath a blended element.
If `mix-blend-mode` is used for decorative purposes on text, engineers MUST
verify contrast ratios against all likely background values and not only against
the background present in the initial state.
