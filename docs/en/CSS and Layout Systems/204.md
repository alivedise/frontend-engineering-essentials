...
state: draft
level: mid
slug: responsive-design-and-container-queries
---
...
Media queries became the dominant tool for responsive design. Developers defined viewport breakpoints, typically targeting phone, tablet, and desktop widths, and wrote CSS rules for each range. Frameworks like Bootstrap codified this into a standard set of breakpoints (576px, 768px, 992px, and 1200px; Bootstrap 5 added a fifth tier at 1400px) that became industry conventions.

But media queries have a fundamental limitation: they query the **viewport**, not the **component's container**. A card component inside a narrow sidebar receives the same "desktop" styles as a card in a wide main content area, because both exist in the same viewport. Developers worked around this with ad-hoc class overrides (`.sidebar .card { ... }`), but this couples components to their page context instead of keeping them reusable. That is why a sidebar widget that looks right in a page's main content area breaks when it is reused in a narrower auxiliary column: it was always styled relative to the viewport, not to the space its own parent actually gives it.

**Container queries** (`@container`), specified in the W3C CSS Containment Module Level 3 and shipped in all major browsers by February 2023, solve this problem. They allow components to query the size of their nearest containment context rather than the viewport. A card component can define its own responsive behavior, such as switching between a horizontal and a vertical layout, based solely on how much space its parent gives it.

Alongside container queries, modern responsive CSS includes **fluid typography** with `clamp()`, **responsive images** with `srcset` and `<picture>`, **new viewport units** (`svh`, `dvh`, `lvh`) that account for mobile browser chrome, **style queries** (`@container style()`) that adapt a component to a container's custom properties rather than its size, and **logical properties** that make layouts internationalization-ready.
...
Now the sidebar card stays vertical because its container is only 240px, while the main content card goes horizontal because its container has enough room. The card component is portable: the same CSS produces a different layout depending on which container it is placed in.
...
The `cqi` unit (container query inline) is 1% of the container's inline size, analogous to `vw` for the viewport. Combined with `clamp()`, it enables fluid typography that scales with the component's container rather than the viewport. Rather than hand-tuning these `clamp()` values by eyeballing a `vw` coefficient, tools like Utopia (utopia.fyi) generate a fluid type scale directly from a minimum and maximum viewport width paired with a minimum and maximum font size.
...
### 6. Style queries: reacting to a container's custom properties

Container queries are not limited to size. Style queries let a component respond to a CSS custom property set on an ancestor container, independent of that container's dimensions:
...
/* A density toggle sets a custom property on the container ... */
.card-wrapper[data-density="compact"] {
  --density: compact;
}

/* ...and a style query reads it from any descendant */
...
`@container style()` matches on a container's custom property values, as shown above. Style queries with custom properties have shipped in Chromium since version 111 (2023); check current browser support before relying on them as more than a progressive enhancement.
...
**Place breakpoints where content breaks.** Breakpoints SHOULD go where your specific content breaks, such as a line of text growing too long or a card getting cramped, rather than at arbitrary device widths like 768px or 1024px. Content-driven breakpoints keep working as new device sizes appear; device-width breakpoints need a new override each time a new screen size becomes common.
...
Ethan Marcotte's 2010 approach was built on a key insight: the viewport is the one constant you can query. At the time, CSS had no mechanism to ask "how wide is my parent?"; it could only ask "how wide is the screen?" That constraint was enough to move web design from fixed-width layouts (typically 960px) to fluid ones, and it meant every breakpoint decision for the next decade followed the viewport's lead.
...
This worked for page-level layout but created problems at the component level. A reusable `<Card>` component might need to be compact in a sidebar but full-width in a main content area, at the same viewport width. Media queries cannot express this, because they have no concept of the component's container.

### Container queries as paradigm shift

Container queries invert the responsive model. Instead of asking "how wide is the viewport?", a component asks "how wide is my container?" Three consequences follow:

**Components become truly portable.** A card component with container query styles works correctly whether placed in a 300px sidebar, a 600px content area, or a 1200px full-width section, with no knowledge of viewport size or page layout required.
...
### How frameworks handle container queries
...
Container queries change this. Tailwind added container queries in v3.2 (October 2022) through the official `@tailwindcss/container-queries` plugin. Core support landed in Tailwind v4.0 (January 2025): `@container` plus `@sm:`, `@md:`, `@lg:`, and `@max-*` variants work without installing a plugin. Component-level responsive utilities are shifting toward container-based queries as this support matures, while page-level layout remains viewport-based. The distinction matters: use media queries for **page layout decisions** (sidebar visible? single or multi-column?), and container queries for **component adaptation** (card horizontal or vertical? show subtitle or not?).
...
### 2. Assuming `em` breakpoints help with browser zoom

**The mistake:** Treating `em`/`rem` breakpoints as unambiguously more accessible than `px`, on the assumption that they respond to browser zoom while `px` does not:

```css
@media (min-width: 768px) { /* behaves the same as 48em under browser zoom */ }
```

Under a browser's page zoom, `px` and `em`/`rem` media queries trigger at the same effective point: `48em` evaluates to 768 CSS pixels at the default 16px root font size, so zooming to 200% shifts a `768px` query and a `48em` query identically. The setting that actually differs by unit is the browser's default font-size preference, a separate control from zoom, found in browser accessibility settings. If a user raises that default from 16px to 20px, `1rem` becomes 20px, so a `48em` breakpoint now triggers at 960px while a `768px` breakpoint still triggers at 768px.

**The fix:** Use `em`/`rem` breakpoints when you want layout to respond to a user's default font-size setting, a real need for readers who rely on a larger base text size. Adam Wathan's widely-cited case for `px` rested on a Safari em-media-query bug that was fixed in Safari 15, after which he switched to recommending `em`; Zell Liew's cross-browser tests reach the same conclusion. `em` is the safer default, though `px` breakpoints remain common in practice (Tailwind, for one, defaults to `px`). Pick one convention and apply it consistently within a codebase.

```css
/* 48em triggers at 768px only when the browser's default font size is 16px */
@media (min-width: 48em) { ... }
```
...
- [CSS & Layout Systems Overview](/en/CSS%20and%20Layout%20Systems/200): parent overview describing where responsive design and container queries fit in the modern CSS layout landscape.
- [Flexbox & Grid](/en/CSS%20and%20Layout%20Systems/203): the layout systems whose grid definitions and flex directions media queries and container queries adapt.
- [CSS Containment & `contain`](/en/CSS%20and%20Layout%20Systems/209): `container-type` is one of the containment primitives defined by this spec family, which container queries are built on.
- [Modern CSS Frameworks](/en/CSS%20and%20Layout%20Systems/206): covers the Tailwind, Bootstrap, and other framework breakpoint systems referenced in this article's Design Thinking section.
- [CSS Custom Properties & Theming](/en/CSS%20and%20Layout%20Systems/207): custom properties serve as design tokens for breakpoints, spacing, and fluid type scales, and combine with style queries for context-aware theming.
...
- [W3C CSS Containment Module Level 3](https://www.w3.org/TR/css-contain-3/). The specification defining container queries, `container-type`, `container-name`, container query length units (`cqw`, `cqi`, `cqb`, etc.), and style queries.
...
- [MDN: CSS Container Queries](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Containment/Container_queries). Comprehensive guide to container queries including `@container`, `container-type`, container query units, named containers, and style queries.
- [MDN: @container](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/At-rules/@container). Reference documentation for the `@container` at-rule syntax, including size and style queries.
...
- [MDN: Responsive Images](https://developer.mozilla.org/en-US/docs/Web/HTML/Guides/Responsive_images). Guide to `srcset`, `sizes`, and the `<picture>` element for responsive image loading.
...
- [Trys Mudford and James Gilyead, "Utopia: Fluid Responsive Design"](https://utopia.fyi/). Community-standard calculator and methodology for generating `clamp()`-based fluid type and space scales from a min/max viewport and min/max size pair.
- [Zell Liew, "PX, EM or REM Media Queries?" (2016)](https://zellwk.com/blog/media-query-units/). Tested comparison of how `px`, `em`, and `rem` media queries behave under browser zoom and under changes to the browser's default font size.
- [Adam Wathan, "Don't Use Em for Media Queries" (2017)](https://adamwathan.me/dont-use-em-for-media-queries/). The counterpoint case for `px` breakpoints, citing a since-fixed Safari bug with `em`/`rem` media queries.
