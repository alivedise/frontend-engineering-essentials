...
### 1. Using only viewport breakpoints for component design

**The mistake:** Styling reusable components with media queries tied to viewport widths:

```css
/* Every card switches to horizontal at 768px viewport,
   even cards in narrow containers */
@media (min-width: 768px) {
  .card { flex-direction: row; }
}
```

The card behaves identically regardless of its container size. In a 250px sidebar on a 1400px desktop, the card gets the "wide" styles -- causing overflow or cramped layouts.

**The fix:** Use container queries for component adaptation:
...
Reserve media queries for page-level layout decisions (showing/hiding a sidebar, switching column count).

### 2. Forgetting container-type on the parent
...
### 3. Using px breakpoints instead of em (zoom accessibility)
...
### 4. No fallbacks for older browsers without container query support
...
### 5. Hardcoding breakpoint values instead of using design tokens
