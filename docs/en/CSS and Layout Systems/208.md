...
Engineers MUST use `grid-template-rows: subgrid` or
`grid-template-columns: subgrid` when a nested element needs its children to
align to the parent grid's tracks. Using fixed heights or JavaScript-measured
alignment to achieve the same result is fragile — content changes break the
layout silently, and the fix requires re-running measurement logic or manually
updating hard-coded values in multiple places. Subgrid is the semantic solution
designed for exactly this problem and MUST be preferred over workarounds in any
environment where subgrid is supported.

Engineers SHOULD provide a `@supports (grid-template-rows: subgrid)` fallback
for any layout where subgrid alignment is a significant UX concern and the
application must support browsers that predate subgrid. The fallback need not
be identical to the subgrid layout; a stacked column layout or a layout that
accepts minor misalignment across cards is sufficient. The goal is a functional
layout for all users, not pixel-perfect alignment for all users. The fallback
SHOULD be authored first as the baseline rule, with the subgrid enhancement
layered inside the `@supports` block so the cascade remains predictable and
the enhancement cannot break the fallback.
