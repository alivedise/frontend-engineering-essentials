...
A PWA that passes the Lighthouse audit in development but silently fails to update in production is worse than no PWA — users are served stale content indefinitely with no indication that a newer version exists. Teams MUST implement an explicit service worker update strategy before shipping to production. The browser's default update behavior — new SW waits for all tabs to close before activating — is not a strategy. It is an assumption that tabs close frequently, which is false for most web applications. Long-lived tabs are the norm in productivity tools, dashboards, and any application that users keep open throughout their workday. Under the default behavior, such users may remain on the previous version of the application for days or weeks after a new version is deployed.

Teams MUST use versioned cache names and delete old caches on `activate`. Cache names that do not include a version string cannot be reliably cleaned up when a new service worker activates: the `activate` handler has no way to distinguish which caches are stale if they all use the same name. Cache names SHOULD follow the pattern `name-vN` (for example `static-v3`, `runtime-v3`), where `N` is incremented whenever the caching strategy or asset set changes in a way that requires a clean slate. Teams SHOULD audit their installed PWA on a physical iOS device before shipping — macOS Safari and the iOS simulator do not replicate the push permission restrictions and background API limitations that apply on real iOS hardware.
...
**Run Lighthouse against a production build, not the dev server.** Vite's dev server does not register a service worker. `next dev` stubs or disables SW behavior. Running Lighthouse against a dev server will produce misleading results — it may report a passing PWA score against an application that has no real service worker, or fail checks that would pass in production. Always run the audit against `pnpm build && pnpm preview` or the equivalent production build command, and use Chrome's incognito mode to avoid cached state from previous test runs.

**Use the prompt-to-reload pattern as the default update strategy.** `skipWaiting()` without a user prompt is safe only when the new SW's cache strategy is backward-compatible with in-flight requests and the asset set has not changed in a way that would break an active session. For most applications, the safer default is to post `UPDATE_AVAILABLE` from the service worker's `install` event and show a non-intrusive banner that gives users control over when to reload.

**Increment cache version strings whenever the caching strategy changes.** Do not use cache names without a version suffix. A cache named `static` cannot be distinguished from a stale cache named `static` by the `activate` handler. Use `static-v1`, `static-v2`, and so on, and delete all caches matching the namespace prefix except the current version during `activate`.

**Precache the `start_url` explicitly.** The Lighthouse offline check navigates to `start_url` with the network disabled. If `start_url` is not in the precache, this check fails. More importantly, users who install the PWA to their home screen and then launch it while offline will see a browser error page if `start_url` is not cached. This is the single most common cause of a broken home screen install experience.

**Verify Digital Asset Links before submitting to the Play Store.** If the `.well-known/assetlinks.json` file is missing, malformed, or returns the wrong certificate fingerprint, Chrome will display the URL bar inside the installed TWA. Users who see the URL bar inside a Play Store-installed "app" will conclude that the app is broken or low-quality. Verify the file using the Digital Asset Links API (`https://digitalassetlinks.googleapis.com/v1/statements:list?source.web.site=https://yourdomain.com&relation=delegate_permission/common.handle_all_urls`) before submitting the APK.

**Test on a physical iOS device.** macOS Safari and the iOS Simulator do not accurately replicate the home-screen installation requirement for push notifications, the absence of Background Sync, or the behavior of the Web App Manifest in installed PWA context. Keep at least one physical iPhone in the testing rotation for any PWA that targets iOS users.

**Avoid caching opaque responses without size limits.** An opaque response (a cross-origin response fetched without CORS) reports a size of 0 bytes to the Cache Storage API but occupies real storage. Browsers may pad the actual storage usage of opaque responses. Caching many opaque responses without a size or count limit can exhaust the browser's cache storage quota, at which point the browser begins evicting cache entries — possibly the wrong ones. If cross-origin assets must be cached, configure a maximum number of entries or a maximum age using Workbox's `ExpirationPlugin`.

**Provide a meaningful offline fallback page.** The Lighthouse audit accepts a blank page as an offline response, but users deserve better. Create a dedicated `/offline.html` page that is precached during the SW install phase, explains that the user is offline, and offers navigation to cached content if available. When the SW cannot fulfill a navigation request from the network and has no cached version, fall back to this page rather than returning a generic browser error.
...
**Running the Lighthouse PWA audit against the development server.** Vite's dev server and most framework dev servers either disable the service worker entirely or replace it with a stub. A Lighthouse audit against `localhost:5173` (Vite) will fail the service worker check or report against a no-op SW that does not represent production behavior. Always audit `localhost:4173` (Vite preview) or a deployed production URL.
...
**Not testing on a physical iOS device.** macOS Safari does not enforce the home-screen install requirement for Web Push. Testing push notification subscription on macOS will succeed, but the same code will silently fail when a user opens the application in Safari on an iPhone because `Notification.requestPermission()` is ignored in the browser tab context. Always verify the push subscription flow on a physical iPhone running iOS 16.4 or later.

**Not setting up Digital Asset Links before Play Store submission.** If `assetlinks.json` is missing or incorrect, Chrome displays the URL bar inside the installed TWA. The URL bar is a signal to users that something is wrong with the packaging. It is not correctable without publishing an updated APK. Verify the Digital Asset Links configuration before the first submission to the Play Store.

**Using the same cache name across versions.** A cache named `app-cache` cannot be distinguished from a previous version's cache named `app-cache`. The `activate` handler has no way to know whether the cache is current or stale. If the new SW uses `app-cache` to store its precached assets, it may find entries from the previous version already present and skip fetching the updated assets. Version the cache names and clean up old versions in `activate`.

**Caching opaque responses without an expiration policy.** Cross-origin responses fetched without a CORS header are stored as opaque responses. The Cache Storage API reports their size as 0 bytes, but browsers may count them against the storage quota at a padded size (Chrome historically padded opaque responses to 7 MB for quota accounting purposes). Caching many opaque responses without a maximum entry count or TTL can exhaust the browser's storage quota for the origin, triggering eviction of other cached assets. If cross-origin caching is necessary, configure an entry limit using Workbox's `ExpirationPlugin`.
