...
A push notification is a re-engagement tool, not a real-time data channel. Teams MUST obtain explicit permission before subscribing users to push notifications. The browser permission dialog is a one-time, high-stakes gate: a user who clicks "Block" is permanently prevented from receiving notifications from that origin until they manually navigate to their browser's site settings and reset the permission. This means a poorly timed permission prompt does not merely fail to get permission — it actively destroys any future chance of ever obtaining it. For this reason, teams MUST NOT call `Notification.requestPermission()` on page load or from a component mount effect. Permission SHOULD be requested only after the user has had a clearly positive interaction with the application and has signaled intent by clicking a button in the application's own UI — a two-step flow that gives the browser dialog a contextual reason to appear.

VAPID is not optional. Teams MUST generate a VAPID key pair and configure the server to sign all push requests with VAPID credentials. All modern push services — including FCM (Chrome, Edge), Mozilla's push service (Firefox), and Apple's APNs-backed service (Safari 16.4+) — require VAPID. The public VAPID key is passed to `PushManager.subscribe()` in the browser; the private key is kept on the server and used to sign each push request. The key pair is generated once and stored securely. The service worker's `push` event handler MUST call `event.waitUntil()` wrapping a Promise that resolves after `showNotification()` completes; calling `showNotification()` without `event.waitUntil()` may result in the notification never appearing because the browser may terminate the service worker before the asynchronous display operation finishes. Similarly, the `notificationclick` handler MUST call `event.waitUntil()` wrapping any async work such as `clients.openWindow()`, for the same reason.
...
**Request permission only from a user-initiated action, after positive engagement.** Place a "Enable notifications" button in a contextually relevant location — the order confirmation page, the chat interface, the settings page — rather than on an entry screen. Explain clearly what the user will be notified about before they tap the button. Call `Notification.requestPermission()` only from the click handler of that button. Never call it on mount or scroll.

**Convert the VAPID public key before passing it to subscribe().** The `applicationServerKey` parameter requires a `Uint8Array`, but the VAPID public key generated by `web-push generate-vapid-keys` is a URL-safe Base64 string. The conversion involves stripping URL-safe characters, adding padding, running `atob()`, and mapping character codes to bytes. This conversion is a standard utility that every push implementation needs; the example section below provides the canonical implementation.

**Always set the `tag` field on notifications.** Without `tag`, every push notification creates a new entry in the notification drawer, even if they all relate to the same entity (the same chat conversation, the same order). A user who receives ten order status updates sees ten separate notifications. Set `tag` to a string that uniquely identifies the entity the notification is about (for example, `order-${orderId}` or `chat-${conversationId}`). The most recent notification for that tag replaces all previous ones.

**Handle expired and invalid subscriptions on the server.** When the push service returns HTTP 404 (subscription not found) or HTTP 410 (subscription has expired and should not be retried), your server MUST delete the corresponding subscription from its database. Continuing to send to a dead endpoint wastes server resources and may result in the push service rate-limiting your application. The `web-push` library's `sendNotification()` rejects with a `WebPushError` whose `statusCode` property can be inspected for this purpose.

**Implement `notificationclick` — it is not optional.** A push notification that the user taps and nothing happens is a significantly worse experience than not receiving the notification at all. The user has interrupted what they were doing to tap the notification; if the result is silence, their trust in your application's notifications decreases immediately. The `notificationclick` handler must close the notification and navigate the user to the relevant page, either by focusing an existing window at that URL or by opening a new one.

**Send the minimal payload needed.** The Web Push payload is encrypted and has a size limit of 4096 bytes after encryption overhead. Treat the push payload as a pointer to a resource, not the resource itself. Send the notification title, body, URL, and tag — not the full entity data. The client retrieves fresh data from the server when it opens. This pattern also avoids stale data: a notification payload is generated at send time, but the user may open the notification minutes or hours later. Data fetched on open is always fresh.

**Provide a subscription management UI.** Users who enabled push notifications should be able to disable them from within your application without needing to find browser settings. Implement an unsubscribe flow: call `subscription.unsubscribe()` on the client, and send a DELETE request to your server to remove the stored subscription. Without this, users who want to stop notifications are left with no obvious recourse except revoking permission in browser settings — a hostile experience that may lead to permanent blocks.

**Test with browser DevTools.** Chrome DevTools Application panel has a Push panel under Service Workers that allows sending a test push message without a server. Use this to verify that the service worker `push` handler and the `notificationclick` handler work correctly before wiring up the server. Firefox has equivalent developer tooling. Always test the `notificationclick` handler by clicking the notification in the OS notification center, not just by triggering the event from DevTools.
...
**Calling `Notification.requestPermission()` on page load.** The permission dialog appears before the user has any context for why the site wants to send them notifications. Rejection rates for unsolicited permission prompts are well above 90% in most analytics data. Worse, a denial is permanent: the user will never receive push notifications from that origin again unless they manually reset the permission. Show an in-app prompt first; call `requestPermission()` only from the click handler of an opt-in button.

**Not implementing `notificationclick`.** A notification that the user taps and produces no result is worse than no notification. Implement the handler. Always call `event.notification.close()` at the start, and always call `event.waitUntil()` around the async navigation logic.

**Not calling `event.waitUntil()` in the `push` handler.** Without `event.waitUntil()`, the browser may terminate the service worker before `showNotification()` resolves, resulting in the notification never appearing. Always wrap the `showNotification()` call in `event.waitUntil()`.

**Not calling `event.waitUntil()` in the `notificationclick` handler.** The same issue applies. `clients.matchAll()` and `clients.openWindow()` are async. If the service worker is killed before they resolve, no navigation occurs. Wrap the entire async chain in `event.waitUntil()`.
...
**Not setting `tag`.** Without a tag, every push sent for the same entity (the same order, the same chat conversation) creates a new, separate notification in the drawer. A user who receives five order status updates in quick succession sees five separate notifications. Set `tag` to deduplicate — a new notification with the same tag replaces the previous one.

**Not handling expired subscriptions on the server.** Push services return HTTP 410 when a subscription has been permanently revoked (user cleared browser data or blocked notifications). Servers that ignore this response and continue sending to the dead endpoint waste resources and may accumulate a large number of stale subscriptions in their database over time. Catch `WebPushError` with `statusCode` 404 or 410 and delete the subscription.
