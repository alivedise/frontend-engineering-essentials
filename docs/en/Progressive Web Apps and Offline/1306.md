...
# [FEE-1306] Web Push Notifications

:::info
A push notification is a message delivered to a user's device by the browser, sent by a server, even when the web application is not open in any tab. It closes a long-standing gap with native apps, which have been able to send timely messages to users for over a decade while the web could only surface information when someone actively visited a page. The Push API (a W3C specification) and the Web Push protocol it rides on (IETF RFC 8030) bring this re-engagement surface to Chrome, Firefox, and Safari (16.1+ on macOS, 16.4+ on iOS) without requiring a native app install.
:::

## Context

Native apps have offered push messaging since the early 2010s; the web lacked an equivalent until the Push API matured across Chrome (2015), Firefox, and eventually Safari on macOS (Safari 16.1, 2022) and iOS (Safari 16.4, March 2023, Home Screen apps only). This article covers the imperative Web Push flow end to end: subscribing a user, storing the subscription, sending a message from your server, and handling delivery and interaction in a service worker. Background Sync, a related but distinct capability for deferring writes until connectivity returns rather than notifying users of events, is covered separately in FEE-1305 and appears here only in the decision table below.

The browser never receives messages directly from your server. Instead, your server sends messages through an intermediary service operated by the browser vendor: Firebase Cloud Messaging (FCM) for Chrome, Mozilla's push service for Firefox, and Apple's APNs-backed service for Safari on iOS and macOS 13+. Your server communicates with these services using RFC 8030 ("Generic Event Delivery Using HTTP Push"), authenticated by VAPID (RFC 8292). Because the push service absorbs the connection, your server does not need to track sockets or scale a WebSocket tier just to send notifications; the trade-off is latency. There is no guaranteed delivery time: the push service may batch or delay delivery on low-power devices, and if the subscription endpoint becomes invalid, delivery silently fails. Push notifications are a re-engagement tool, not a real-time data channel; the decision table below covers when Server-Sent Events or WebSocket (FEE-403) is the correct alternative instead.
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    ...
    // Maximum 2 actions in Chrome. Not supported on iOS Safari.
...
**SHOULD re-subscribe when the push service rotates a subscription.** The push service can invalidate and replace a subscription without the user ever unsubscribing. `PushSubscription.expirationTime` signals a planned expiry, and the service worker's `pushsubscriptionchange` event fires when the push service forces a rotation. Listen for this event, call `subscribe()` again with the same `applicationServerKey`, and POST the new subscription to your server so it replaces the stale one. Support is uneven: Firefox fires `pushsubscriptionchange` reliably, Chrome does not, so periodic re-validation of the stored subscription on app open is the pragmatic complement rather than relying on the event alone.
...
**MUST** call `event.waitUntil()` in the `notificationclick` handler, wrapping any asynchronous work such as `clients.matchAll()` or `clients.openWindow()`. Without `event.waitUntil()`, the browser terminates the service worker before the async chain completes, so the intended window never opens or focuses and the user who tapped the notification gets no visible response.

**SHOULD send the minimal payload needed.** RFC 8030 caps the encrypted push message at 4096 bytes, which leaves roughly 4 KB minus encryption overhead for your plaintext (about 3993 bytes with the standard `aes128gcm` content encoding). Send only what the notification needs to render: the title, body, URL, and tag. The client fetches full data from the server when it opens. This also avoids stale data, since the payload is generated at send time but the user may open the notification minutes or hours later.

**SHOULD provide a subscription management UI.** Users who enabled push notifications SHOULD be able to disable them from within your application without needing to find browser settings. Implement an unsubscribe flow: call `subscription.unsubscribe()` on the client, and send a DELETE request to your server to remove the stored subscription. Without this, users who want to stop notifications have no obvious recourse except revoking permission in browser settings, which is a hostile experience that may lead to permanent blocks.
...
Browsers also enforce this directly, beyond etiquette. On iOS the request must originate from a user gesture or it fails outright, and Firefox similarly suppresses prompts that are not user-initiated. Chrome takes a different approach: it demotes sites with high dismissal rates to a quieter, easy-to-miss permission UI.

The wrong pattern: calling `Notification.requestPermission()` on page load rather than from a user gesture, for example inside a `useEffect` on mount. This is the single most common push notification mistake. The user has no idea what the site is or what it would notify them about. The rejection rate for unsolicited permission requests is extremely high.

The right pattern: show a soft in-app prompt first, an HTML element you control completely, such as a banner. It says something like "Stay updated on your order: tap 'Enable Notifications' to get instant order status updates." When the user taps "Enable Notifications" in your UI, your code calls `Notification.requestPermission()`. The browser dialog appears at a moment when the user has opted into it, in a context that explains why they would want it.

A common variation that should be avoided: showing a custom dialog that asks "Would you like to receive notifications?" with Yes/No buttons, and then calling `requestPermission()` when the user clicks Yes. This creates a confusing double-prompt: the user says yes to your dialog, and then the browser immediately asks the same question again. Use the in-app banner to describe the value proposition, and wire the button directly to `requestPermission()` instead of to another confirmation dialog.
...
- `tag`: A string that deduplicates notifications. If a new notification arrives with the same `tag` as an existing notification, the existing notification is replaced rather than a second notification being added. Always set `tag` for notifications that represent an ongoing state, such as a chat conversation. Without it, rapid pushes create a stack of separate notifications, which users experience as spam.
- `renotify`: A boolean (default `false`). When `true`, replacing a tagged notification plays the sound and vibration as if it were a new notification. Set this to `true` when the replacement represents genuinely new content the user should be alerted to, even if it replaces a prior notification about the same topic. `renotify: true` throws a `TypeError` if used without also setting `tag`.
- `actions`: An array of action objects, each with an `action` string identifier and a `title` string. The maximum number of actions is user-agent defined via `Notification.maxActions`; Chrome currently allows 2. These render as buttons inside the notification on Android and some desktop browsers. The selected action is available in `notificationclick` via `event.action`.
...
Safari shipped Web Push on macOS in Safari 16.1 (macOS Ventura, 2022); iOS and iPadOS followed in Safari 16.4 (March 2023), restricted to Home Screen web apps. On iOS, push notifications from the web require the user to add the site to their Home Screen first (as a Progressive Web App). In a Safari browser tab on iOS, the Notification and Push APIs are not exposed at all: `window.Notification` is `undefined`, and feature detection (as in `getPushPermissionState()` above) correctly reports them as unsupported rather than a call silently failing. The user must first tap the Share button and select "Add to Home Screen," and then open the resulting Home Screen icon. Only the installed Home Screen app gets the APIs, and only then can it request push permission and subscribe.

This constraint has a direct impact on how you design the subscription flow for iOS users. `navigator.standalone` (a non-standard but widely supported property) returns `true` when the page is running as an installed Home Screen app. Use this as a gate: if `navigator.standalone !== true` on iOS, show an informational prompt explaining that the user must add the app to their Home Screen before enabling notifications, rather than calling `requestPermission()`, which does not exist in an ordinary Safari tab. Even once installed, the permission request must still come from a user gesture, the same constraint the browsers apply everywhere else.

Additionally, iOS Safari's push notifications have some option restrictions: `actions` are not supported (notification action buttons do not appear), the `badge` option (the Android status-bar glyph) is ignored, and the `icon` must be pre-declared in the Web App Manifest rather than being settable at notification time via `showNotification()`. App-icon badge counts are still available on iOS 16.4+ Home Screen apps through the separate Badging API; see [FEE-1317: Badging API](/en/Progressive%20Web%20Apps%20and%20Offline/badging-api).

Safari 18.4 (iOS/iPadOS) and 18.5 (macOS) added a second delivery path, Declarative Web Push, which renders a notification directly from a JSON payload without booting a service worker. It uses the same RFC 8030 transport and `applicationServerKey` model described above, so the subscription flow in this article still applies. See FEE-1316 for the payload format and how to support both delivery paths from a single send.

## Common Mistakes

**Using push for real-time updates when the app is open.** Push notifications are for reaching users when the app is closed. If the user has the application open, delivering a push notification creates a redundant pop-up alongside whatever in-app update mechanism is already running. Teams commonly reach for push because it is already implemented; see the decision table above for when Server-Sent Events or WebSocket (FEE-403) is the correct tool instead.

**Storing the VAPID private key in client-side code.** The private key signs push requests on the server. If it is embedded in a Vite env var that gets bundled into the frontend, anyone can read it from the browser and send unauthorized push notifications to all of your subscribers. The private key must only ever exist on the server, loaded from a server-side environment variable.

## Related Topics

- [FEE-1305: Offline UX Patterns](/en/Progressive%20Web%20Apps%20and%20Offline/1305) (Background Sync for reliable write delivery without push)
- [FEE-1302: Service Workers](/en/Progressive%20Web%20Apps%20and%20Offline/1302) (push and notificationclick handlers run in the SW context)
- [FEE-1316: Declarative Web Push](/en/Progressive%20Web%20Apps%20and%20Offline/declarative-web-push) (Safari's service-worker-free delivery path over the same RFC 8030 transport)
- [FEE-403: Fetch, Streams & Network APIs](/en/Browser%20APIs%20and%20Web%20Platform/403) (SSE and WebSocket as alternatives for real-time in-app updates)
...
- M. Thomson, E. Damaggio, and B. Raymor, "Generic Event Delivery Using HTTP Push," RFC 8030, IETF (2016). https://www.rfc-editor.org/rfc/rfc8030
- M. Thomson, "Message Encryption for Web Push," RFC 8291, IETF (2017). https://www.rfc-editor.org/rfc/rfc8291
- M. Thomson and P. Beverloo, "Voluntary Application Server Identification (VAPID) for Web Push," RFC 8292, IETF (2017). https://www.rfc-editor.org/rfc/rfc8292
