...
**Version static cache names and delete old caches on activate.** Define `const STATIC_CACHE = 'static-v1'` and `const DYNAMIC_CACHE = 'dynamic-v1'` at the top of the service worker file. In the `activate` handler, delete every cache whose name is not in the set of current cache names. Increment the version string whenever the precache asset list changes. This ensures that assets cached by a previous service worker version are cleaned up immediately rather than consuming storage indefinitely.

**Always clone the response before putting it in the cache.** A `Response` object's body is a readable stream that can only be consumed once. If you return `response` to `respondWith` and then try to call `response.clone()` afterward, the body has already been read and the clone is empty. Clone immediately: `const clone = response.clone(); cache.put(request, clone); return response;`. The clone goes into the cache; the original goes to the page.

**Apply Cache-First only to resources with content-hashed URLs.** The content hash in the URL is the invariant that makes Cache-First safe. Without it, Cache-First is a correctness hazard. When reviewing service worker code, the simplest correctness check is to look at every `cacheFirst()` call site and confirm that the URLs it handles contain a hash segment.

**Use Network-First as the safe default for anything not explicitly classified.** A resource that was not intentionally categorised is better served with a fresh network response than with a potentially stale cached one. Network-First is not as performant as Cache-First or Stale-While-Revalidate for that resource, but it is never incorrect. Correctness first, then performance optimisation once the routing is verified.

**Do not cache opaque responses from cross-origin no-cors requests.** Check `response.type === 'opaque'` before calling `cache.put()` and skip the cache for opaque responses. If the cross-origin resource needs to be cached, request CORS headers from the server and fetch with `mode: 'cors'`.

**Implement a bounded eviction policy for dynamic caches.** For any cache populated at runtime (not at install time), define a maximum number of entries and enforce it after each `cache.put()`. Retrieve all cache keys with `cache.keys()`, sort by insertion order if needed, and delete entries beyond the limit. Without this, a high-traffic application will grow its dynamic cache without bound.

**Test offline behaviour explicitly, not just cache hit paths.** The Network-First fallback to cache, the Cache-Only error on a missing precache entry, and the Stale-While-Revalidate background update all have paths that only execute when the network is unavailable. Use Chrome DevTools Network panel's "Offline" mode to verify that each strategy behaves as specified when network requests fail, not just when they succeed.

**Scope each strategy function to a single named cache.** Passing the cache name as a parameter to each strategy function (`cacheFirst(request, STATIC_CACHE)`, `networkFirst(request, DYNAMIC_CACHE)`) rather than hardcoding it inside the function body makes the cache ownership visible at the call site and prevents accidental cross-cache pollution. Versioned static assets and runtime-fetched API responses belong in separate named caches precisely so they can be versioned and cleaned up independently. If a new deployment bumps `STATIC_CACHE` to `static-v2` but `DYNAMIC_CACHE` stays at `dynamic-v1`, the activate handler deletes only `static-v1`; the dynamic cache carries over unchanged. This granularity is only achievable if the cache names are explicit at every call site.

**Guard against caching error responses.** A `fetch()` that resolves (does not throw) can still return a 4xx or 5xx response. A `cache.put()` that stores a 404 response will serve a 404 on every subsequent cache hit for that URL until the cache is cleared, which makes the resource appear permanently broken. Before calling `cache.put()`, check `response.ok` (`response.status >= 200 && response.status < 300`) and skip the cache if it is false. The network error path -- where `fetch()` throws -- is a separate code path handled by the `catch` block in `networkFirst`. The response-level error path -- where `fetch()` resolves with a non-OK status -- requires an explicit `if (response.ok)` guard before `cache.put()`.
...
**No cache eviction policy for dynamic caches.** A service worker that caches every response it handles will grow the dynamic cache indefinitely. On devices with limited storage quotas (older Android devices, private browsing mode), the browser will eventually evict all caches for the origin simultaneously, destroying the precache and the dynamic cache together. Implement a bounded LRU eviction after each `cache.put()` in dynamic caches: open the cache, retrieve all keys, and delete the oldest entries beyond the limit.

**Caching opaque responses from cross-origin no-cors requests.** Opaque responses always have `response.status === 0` regardless of whether the underlying server returned 200 or 500. Caching a 0-status response that was actually a server error means the service worker will serve the error response -- which may have an empty body -- to every subsequent request for that resource, with no visible error. Always check `response.type !== 'opaque'` before calling `cache.put()`, or fetch with `mode: 'cors'` when the server supports it.

**Not cloning the response before caching.** Calling `response.clone()` is not optional. A `Response` body is a readable stream, and it can only be consumed once. If the service worker passes the response to `respondWith()` and then attempts to read the body again for `cache.put()`, the second read returns an empty stream. The clone must be created before either copy is consumed: `const clone = response.clone()` immediately after the `await fetch()` call, before any other operation on the response.
