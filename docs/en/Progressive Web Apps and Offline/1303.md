...
### Why caching strategy is an architectural decision, not a configuration choice

The temptation is to treat caching strategy as a detail to revisit after the service worker is working. In practice, the wrong strategy for a resource category produces bugs that are difficult to diagnose: users see stale pricing, mutations silently fail to reach the server, or updates never appear without a hard reload. These bugs are not visible in development -- where the network is fast and the cache is empty -- and only manifest under production conditions with warm caches and intermittent connectivity. Choosing the right strategy per resource category before writing the fetch handler eliminates this class of problem at the design level.

The five canonical strategies map cleanly onto a decision tree driven by three questions: Does the URL encode the content (via hash or version)? Is the request a mutation? Is freshness mandatory on every load? Answering these questions per resource category produces the strategy assignment mechanically, before any code is written. The fetch handler then becomes an implementation of a pre-existing decision, not a place where decisions are made implicitly.

A common mistake is applying a single strategy globally -- either one blanket Cache-First rule for performance, or a blanket Network-First rule for correctness. Neither extreme is appropriate. Cache-First applied globally will cache mutation responses and API errors. Network-First applied globally eliminates the performance benefit of caching versioned assets, where Cache-First would serve them at zero latency with no correctness risk. The correct approach is explicit per-resource-category routing, where each category is assigned the strategy that fits its freshness and correctness requirements independently.
