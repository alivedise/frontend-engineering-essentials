...
Installability is a progressive enhancement and MUST NOT be used to gate core functionality. Users who decline to install the app, or whose browsers do not support installation, MUST receive a fully functional web experience. The manifest and the install prompt exist to improve convenience and engagement for returning users -- they are not a distribution paywall.

Teams MUST provide maskable icons at both 192x192 and 512x512 pixels, and the `start_url` declared in the manifest MUST be cached by the registered service worker. An installed app that fails to load offline presents a broken experience that is indistinguishable from a broken native app. A manifest that is technically present but references uncached assets or incorrectly sized icons SHOULD be treated as invalid; Lighthouse will report it as such. The `display` field MUST NOT be set to `"browser"` if the team intends to support installation -- `"browser"` mode explicitly opts out of the standalone launch experience and suppresses `beforeinstallprompt` in Chromium-based browsers.
...
**Link the manifest and theme-color from every HTML page.** The `<link rel="manifest" href="/manifest.json" />` and `<meta name="theme-color" content="#3b82f6" />` tags must appear in the `<head>` of every page in the app, not only the root. Browsers evaluate installability criteria on each page load; a manifest that is missing from some pages causes inconsistent behavior for users who enter the app through a deep link.

**Use `/?source=pwa` in `start_url` for analytics attribution.** The query parameter is ignored by the app's routing logic but is captured by analytics as a distinct traffic source. This lets teams measure how often installed users open the app from the home screen versus the browser, and whether installed users have different retention or conversion rates than browser users.

**Defer the install prompt until after a meaningful conversion.** The user's first interaction with the app is the worst moment to show an install prompt. They have not yet understood what the app does or why they would want it on their home screen. Wait until the user has completed an action that signals intent -- saved a document, added a task, logged in -- before surfacing the install button.

**Test icon appearance with maskable.app before shipping.** Provide both `"purpose": "any"` and `"purpose": "maskable"` entries separately. Verify that the maskable variant looks correct when clipped to a circle. Use the tool to check that no meaningful content lands in the outer 20% of the icon.

**Validate the manifest with Lighthouse before launch.** Lighthouse's PWA audit checks manifest completeness, icon availability, `start_url` reachability, and service worker registration. Running the audit in CI or in Chrome DevTools before shipping catches issues that would otherwise prevent `beforeinstallprompt` from firing.

**Handle the `appinstalled` event to update UI state.** After the user accepts the install prompt, the browser fires `appinstalled` on `window`. Listen for this event to hide the install button and optionally show a confirmation message. This ensures the button does not persist in the UI after the app is already on the home screen.
...
**Icon entries missing `"purpose": "maskable"`.** Without maskable icons, Android and some desktop environments apply a default background and crop the standard icon into a circle, often clipping logos or text that sit near the icon's edges. Provide separate icon entries for `"any"` and `"maskable"` purposes, and validate the maskable variant at maskable.app before shipping.

**Setting `"display": "browser"`.** This explicitly opts out of standalone mode and prevents `beforeinstallprompt` from firing in Chromium-based browsers. Teams that intend to support installation must use `"standalone"`, `"minimal-ui"`, or `"fullscreen"`.

**Showing the install prompt on first page load.** The user has not seen the app's value. Prompting immediately produces low acceptance rates and a poor first impression. Defer the custom install button until the user has completed at least one meaningful action.

**Not handling `appinstalled`.** If the install button remains visible after the user has already installed the app, it creates confusion and suggests the UI is not responding. Always listen for `appinstalled` on `window` and remove or hide the button when it fires.

**Using `"purpose": "any maskable"` on a single icon entry.** This tells the browser the same image serves both purposes. On platforms that do not clip icons, the maskable image's outer padding will appear as visible whitespace. Provide separate entries for each purpose.

**Missing the manifest link on deep-linked pages.** If `<link rel="manifest">` is only present on the root page, users who enter the app through a direct URL (email link, search result) may not trigger installability evaluation on that page load. Include the manifest link in the `<head>` of every page.
