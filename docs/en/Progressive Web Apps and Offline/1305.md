...
Offline is a state, not an error. Applications MUST communicate network state clearly to users and MUST NOT display a blank screen or a raw fetch error when the network is unavailable. Every application that registers a service worker SHOULD define an explicit offline fallback — at minimum a cached page that tells the user they are offline and identifies which functionality remains available. The offline fallback page MUST be precached during the service worker's `install` event; relying on a runtime cache hit means the fallback is not guaranteed to be available when it is needed.

Applications SHOULD prefer optimistic UI and deferred synchronization over blocking the user from taking action while offline. A user who cannot submit a form because the network is down has had their work interrupted; a user whose submission was accepted optimistically and queued for sync has had a seamless experience. When a queued write fails permanently after retries, the application MUST roll back local state and notify the user — silent data loss is worse than a visible error. The sync queue MUST be persisted to IndexedDB so that it survives page reloads and service worker restarts. Manual retry buttons MUST be provided as a fallback for browsers that do not support the Background Sync API, since iOS Safari and some Firefox configurations do not support it.
...
**Precache the offline fallback page unconditionally.** Add `/offline.html` to the service worker's `install` event cache open call. Do not rely on a runtime cache entry for the fallback. The fallback must be available before any user visit occurs.

**Always test the offline fallback in DevTools.** Open DevTools, go to Application > Service Workers, check "Offline," navigate to a route that is not cached. The offline page should appear. If the browser's default error page appears, the fallback is not working. Test this before shipping.

**Distinguish the offline indicator from the offline fallback.** The online/offline event listener drives a UI indicator (a banner, a status chip) that tells the user their current network state on the current page. The offline fallback is a full-page replacement for a navigation request that fails. These are two different mechanisms serving two different purposes; implementing one does not implement the other.

**Keep the sync queue in IndexedDB, not sessionStorage or memory.** A queue stored in memory is lost on page refresh. A queue stored in sessionStorage is lost when the browser tab closes. Only IndexedDB persists across both page reloads and service worker restarts. The `idb` library (Jake Archibald) provides a clean Promise-based wrapper around the raw IndexedDB API and reduces the boilerplate significantly.

**Register the sync tag after the IndexedDB write, not before.** The sync event can fire almost immediately after registration if the network is available. If the tag is registered before the data is in IndexedDB, the handler will run with an empty queue and return a resolved Promise, which the browser interprets as success. The queued data is then orphaned — it sits in IndexedDB with no registered sync tag to process it.

**Notify the user of queue status on both ends.** When a write is queued offline, show a message: "Saved locally. Will submit when you're back online." When the sync completes (the service worker posts a `postMessage` to the page), update the UI: "Submitted successfully." If the user never sees the second confirmation, they may submit the form again when online, creating a duplicate. The full round-trip feedback loop — queued acknowledgment and sync confirmation — prevents duplicates and builds trust.

**Provide a manual retry button for all browsers.** Background Sync is a progressive enhancement, not a baseline. Wrap the sync registration in a feature detection check (`'SyncManager' in window.ServiceWorkerRegistration.prototype`) and always render a "Retry" button in the UI for users on unsupported browsers. The retry button reads the IndexedDB queue and replays it directly from the page when clicked.

**Roll back optimistic state on permanent failure.** Define what "permanent failure" means for each operation: typically, a non-retriable HTTP status code (400, 401, 403, 404, 409, 422) or exhaustion of a retry budget. When permanent failure is detected, revert the local state change and show the user a clear error message with the option to try again. Never silently discard the failed write.
...
**Not precaching `/offline.html` in the `install` event.** If the offline fallback is only added to the cache at runtime — when the user first navigates to the page while online — then a user who installs the PWA and immediately loses connectivity before their first navigation will see the browser's default error page, not the application's fallback. Precaching in `install` is the only guarantee.

**Registering the Background Sync tag before the IndexedDB write completes.** The sync event can fire within milliseconds of registration if connectivity is available. If the tag is registered before the `db.add()` call resolves, the handler reads an empty store, returns a resolved Promise, and the browser marks the sync as complete. The data is then stuck in IndexedDB permanently — it will never be retried because the registration is gone. Always `await db.add(...)` before calling `reg.sync.register(...)`.

**Silently discarding optimistic updates when sync fails.** If a user submits a form, sees a success state (optimistic), loses connectivity, and the background sync permanently fails (server returns 409, or retries are exhausted), the application must roll back the success state and notify the user. Failing silently is worse than an error message — the user believes their action succeeded and will be confused when the result never appears.

**Assuming Background Sync is universally available.** It is not. iOS Safari does not support it. Firefox support is limited. Any application that uses Background Sync without a manual retry fallback is broken for a significant fraction of mobile users. Always wrap sync registration in feature detection and always provide a retry button.
