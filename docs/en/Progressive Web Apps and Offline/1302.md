...
A service worker is a network proxy, and teams MUST treat it as middleware: the caching behaviour is a consequence of the fetch handler's logic, not the purpose of registering a service worker. Teams MUST register a `fetch` event handler whenever they register a service worker that is intended to support installability -- Chrome requires a `fetch` handler for the `beforeinstallprompt` event to fire. A service worker registered without a `fetch` handler satisfies neither the installability criterion nor any meaningful network interception goal, and SHOULD NOT be shipped.

Teams MUST call `event.waitUntil()` inside both the `install` and `activate` event handlers whenever those handlers perform asynchronous work. The browser may terminate the service worker thread before a promise settles if `waitUntil` is not called, silently killing cache writes or cleanup operations mid-execution. Teams SHOULD call `self.clients.claim()` inside the `activate` handler when they want the new service worker to take control of existing open tabs immediately rather than waiting for the next navigation.
...
**Always check for service worker support before registering.** The `'serviceWorker' in navigator` guard prevents runtime errors in environments where the API is not available, including some older browsers and non-secure contexts where the service worker API is blocked. Register inside a `DOMContentLoaded` or load event listener to avoid blocking the critical rendering path. Wrapping the registration in a `window.addEventListener('load', ...)` callback is idiomatic: it ensures the service worker registration network request does not compete with the page's critical resources during the initial load.

**Keep the service worker file small and import heavy logic via `importScripts()` or dynamic imports.** The service worker file is parsed and evaluated each time the browser checks for updates. A large file increases the update-check overhead. Move large handler registries, fixture data (for MSW-style setups), or complex routing tables to separate modules imported inside the service worker, so the main file remains a thin entry point whose byte-comparison check resolves quickly.

**Call `event.waitUntil()` with every async operation in `install` and `activate` handlers.** Without `waitUntil`, the browser may terminate the service worker thread before the async work completes. Use `Promise.all()` to wrap multiple concurrent async operations into a single promise passed to `waitUntil`.

**Version cache names explicitly and delete old caches in the `activate` handler.** A service worker that never cleans up old caches will accumulate stale entries across deployments. The pattern is to define `const CACHE_NAME = 'app-v1'` at the top of the file, and in the `activate` handler delete every cache whose key does not match `CACHE_NAME`. Increment the version string when the precache asset list changes.

**Use `self.clients.claim()` in `activate` when you need immediate control.** Without `clients.claim()`, a newly activated service worker only controls pages that navigate after activation. Existing open tabs continue to be served by the previous service worker until they are reloaded. Call `clients.claim()` when you need the new service worker to take control of all open tabs immediately.

**Send auth tokens via `postMessage`, not via the SW file itself.** The service worker file is not a secure storage location for secrets. The correct pattern is to send the token from the page to the service worker using `navigator.serviceWorker.controller.postMessage({ type: 'SET_AUTH_TOKEN', token })` and store it in a variable in the SW scope. The token is not persisted across SW restarts -- resend it on each page load.

**In the fetch handler, guard carefully on request URL and method before modifying requests.** A fetch handler that unconditionally intercepts all requests will catch navigation requests, cross-origin requests, and browser-internal requests. Test `event.request.mode`, `event.request.method`, and `new URL(event.request.url).origin` before deciding whether to respond with a modified request. Failing to guard correctly can break navigation, font loading, and analytics calls in unexpected ways. A common safe pattern is to early-return for any request whose origin does not match `self.location.origin`, which limits the handler's effect to same-origin requests only.

**Test the service worker update flow deliberately, not just the happy path.** Most service worker bugs surface not during initial installation but during the update cycle: the new SW is installed but the old one is still active, the new cache is populated with different asset names than the old one, or `clients.claim()` causes a tab to switch service worker versions mid-session. Write an explicit test scenario where you open the app in one tab, deploy an updated service worker file, open a second tab, and verify that the first tab continues to work correctly while the second tab operates under the new service worker. The Chrome DevTools Application panel's "waiting" indicator is the observable signal to check.

**Avoid `skipWaiting()` in production without a user-initiated trigger.** Calling `skipWaiting()` unconditionally in the `install` handler causes the new service worker to activate while old service worker-controlled tabs are still open. If the new cache strategy is incompatible with assets those tabs have already loaded, it can produce broken states mid-session. Prefer exposing a "reload to update" prompt driven by `registration.waiting` instead.
...
**`waitUntil()` omitted from install or activate handlers.** Without `waitUntil`, the browser considers the handler synchronously complete and may terminate the service worker thread before async cache operations finish. The symptom is intermittent: the cache will sometimes be populated correctly (when the browser happens to keep the SW thread alive long enough) and sometimes not. Always pass a promise to `waitUntil`, even if it wraps a single `caches.open()` call.

**Registering a service worker without checking `'serviceWorker' in navigator`.** Calling `navigator.serviceWorker.register()` in a browser that does not support service workers -- including HTTP contexts (non-localhost), some privacy-focused configurations, and older Safari versions -- throws a runtime error. Always guard the registration call with the feature-detection check.
