...
The app shell is the minimal set of HTML, CSS, and JavaScript that MUST be
pre-cached and served offline-first by the service worker. Content within the
shell boundary — navigation, layout chrome, loading states — MUST render
correctly without any network request. Route-level content outside the shell
boundary SHOULD use network-first or stale-while-revalidate strategies because
it changes frequently and must reflect current data. The shell boundary SHOULD
be as small as possible: every byte of JavaScript and CSS in the shell adds
to the pre-cache payload and to the critical rendering path on first load.
