...
### Why update strategy is a product decision, not just an engineering detail

The service worker update lifecycle sits at the intersection of engineering
architecture and product experience. The decision between cooperative prompts
and `skipWaiting` is not purely technical — it encodes assumptions about users:
how long they keep tabs open, how much they tolerate interruption, and how much
risk the team accepts from mixed-version states. A team that deploys several
times per day and whose users routinely leave the application open in a pinned
tab faces a fundamentally different update problem than a team that deploys
once a week. Treating the default browser behavior as acceptable — which leaves
users on stale versions indefinitely — is itself a product decision, typically
an uninformed one. Making the update strategy explicit forces the team to
articulate what "running on a stale version" means for their users: is it an
inconvenience, a data integrity risk, or a security exposure? The answer
determines the correct update mechanism.
