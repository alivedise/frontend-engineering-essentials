...
**SHOULD choose `generateSW` first and switch to `injectManifest` only when needed.** The `generateSW` mode is fully declarative — all caching behaviour is expressed in `vite.config.ts` as data, not code. Teams MUST NOT use `generateSW` when the service worker needs custom request-mutation logic such as auth token injection, A/B routing, or custom analytics instrumentation; `injectManifest` mode MUST be used for those cases.
