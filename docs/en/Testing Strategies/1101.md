...
**SHOULD** use Vitest for all new Vite-based projects. It shares your existing `vite.config.ts`, supports native ESM, and delivers substantially faster cold starts than Jest.

**SHOULD NOT** rely on snapshot tests as a substitute for assertion-based tests. A snapshot that renders a full component JSX tree will be silently approved by anyone running `vitest --update-snapshots` without reading it. Prefer explicit assertions over what the user sees.
...
**Resetting mocks in the wrong place.** `vi.fn()` accumulates call history. If you check `expect(mockFn).toHaveBeenCalledOnce()` in test B, and test A called the same mock once, your assertion will pass for the wrong reason. Use `mockClear()` in `beforeEach`, or configure `clearMocks: true` globally in `vitest.config.ts`.
...
**Approving snapshots without reading them.** Running `vitest --update-snapshots` after a snapshot failure without examining the diff is the most common way snapshots erode test quality. If you cannot articulate why the snapshot changed, the test is not giving you confidence — it is giving you a false sense of coverage.

**Targeting coverage thresholds as a goal.** An `expect(fn()).toBeTruthy()` assertion covers the function for coverage purposes but asserts almost nothing meaningful. Coverage is a signal for uncovered code, not a proof of test quality. Aim for tests that would fail if you introduced a realistic bug, not tests that push the percentage up.
