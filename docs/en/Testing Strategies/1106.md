...
**Using full-page screenshots instead of scoped element screenshots.** A full-page screenshot for a component test is like using `document.body.innerHTML` in a unit test: you get far more noise than signal. A dynamic sidebar, a cookie banner, or a footer with live data will cause false positives on every run. Always scope screenshots to the specific component or region you care about.

**Committing baselines generated on developer machines.** A baseline generated on macOS with Retina display and a specific font smoothing setting will not match a screenshot from Ubuntu on a CI runner. The result is permanent false positives. Establish a policy: baselines are always generated and stored from CI. If you need to update a baseline, push a commit to CI and trigger the reference capture there, or use Docker locally to match the CI rendering environment.

**Setting the threshold too low or too high.** A threshold of 0 pixel differences will fail constantly on every anti-aliased edge. A threshold of 10% will let substantial layout regressions through. Start with `maxDiffPixelRatio: 0.01` (1%) and calibrate based on the noise you observe in your specific rendering environment. Different components may need different thresholds: a text-heavy component is noisier than a solid color block.

**Not disabling animations before capture.** A rotating loading spinner captured at a random point in its animation cycle produces a different screenshot every time. Any CSS `transition` or `animation` that is running when the screenshot is taken introduces timing-dependent variance. Always inject `animation-duration: 0ms` for visual regression captures, or use `waitForTimeout` to let animations complete — though timeouts introduce their own flakiness.
