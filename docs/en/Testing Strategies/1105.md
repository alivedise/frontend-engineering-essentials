...
**Skipping the Red phase.** Running a test that passes immediately is not a red-green cycle — it's just a green test. Before trusting a test, confirm it can fail. If you cannot produce a failure from a new test, either the test is testing the wrong thing or the implementation already exists. A test that cannot fail is providing no value.

**Writing too much code to reach green.** The discipline is minimum code to pass. If the test expects `add(1, 2)` to return `3`, the correct first implementation is `return 3`. Force yourself to write the next test that breaks the hardcoded return. This sounds pedantic, but it teaches you to let the tests drive the design incrementally rather than implementing in one shot.

**Treating the refactor step as optional.** Technical debt accumulates in code that has green tests but no refactor discipline. The green tests are the safety net for refactoring — refusing to use the safety net means carrying the full weight of accumulating debt forward.
...
**Conflating spiking with TDD.** When you genuinely don't know how to solve a problem, write exploratory code without tests. That is a spike: it produces knowledge, not production code. Throw it away after the exploration, then use TDD to implement the real solution. Treating a spike as a TDD session produces code you don't understand well enough to test-drive.
