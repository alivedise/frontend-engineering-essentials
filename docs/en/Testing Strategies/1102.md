...
**PREFER `getByRole` AND `getByLabelText` OVER `getByTestId` AS YOUR FIRST QUERY STRATEGY.** The Testing Library documentation designates `getByRole` as the top-priority query because it targets the same signals screen readers and keyboard users rely on. If `getByRole('button', { name: 'Submit' })` cannot find your button, the button either lacks an accessible role or lacks an accessible name — a real accessibility defect, not just a test inconvenience. You MUST NOT reach for `getByTestId` until all semantic queries are exhausted.

**USE `userEvent.setup()` INSTEAD OF `fireEvent` FOR ALL INTERACTION TESTS.** `userEvent` dispatches the full browser event sequence — pointerdown, mousedown, focus, keydown, input, keyup, change, blur — that a real user's action produces. `fireEvent` dispatches a single synthetic event and MUST NOT be used when the component relies on per-keystroke logic, focus management, or `onMouseDown` rather than `onClick`. In RTL v14+, you SHOULD call `userEvent.setup()` at the top of each test to maintain pointer and selection state across multiple interactions.

**ASSERT ON OBSERVABLE DOM OUTPUT, NOT ON INTERNAL STATE OR REFS.** A test that reads `component.state.isLoading` or a React ref value is coupled to implementation details that break whenever you extract logic to a custom hook or rename an internal variable, even when the rendered UI is identical. Tests MUST verify only what the user observes — rendered text, ARIA attributes, disabled state, and visible error messages.

**USE `queryBy*` FOR ABSENCE ASSERTIONS AND `findBy*` FOR ASYNC CONTENT.** `getBy*` throws immediately when no match is found, so `expect(screen.getByRole('alert')).not.toBeInTheDocument()` will throw before the `expect` runs if the element is absent. You MUST use `queryBy*` for absence assertions. Similarly, `findBy*` returns a Promise and MUST be awaited; forgetting `await` causes the assertion to evaluate an unresolved Promise object, which is always truthy.
