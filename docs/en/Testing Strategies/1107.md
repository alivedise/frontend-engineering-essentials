...
Tests SHOULD verify behavior observable by users or consumers of the module, not internal implementation details. Tests SHOULD be maintainable: a test that breaks on every harmless refactor creates noise without adding confidence, and a noisy test suite is one that gets ignored.

A test suite that takes 30 minutes to run will be skipped. A suite that fails every other run on a flaky network call will be ignored. Speed and reliability of the suite are first-class engineering concerns, not afterthoughts.
...
**Treating coverage as a quality gate.** Mandating 80% line coverage in CI incentivizes engineers to write tests that execute code without asserting anything meaningful. The coverage number goes up; the confidence does not. Use coverage to find uncovered files, not to certify test quality.

**Writing tests that mirror implementation.** A test that calls `form.validate()`, then checks `form._errors.length`, then asserts `form._isValid === true` is testing the form's internal state machine, not its behavior. When the internal property names change during a refactor, the test breaks. Rewrite these tests to test what the form does from the outside: does it show an error message? Does it submit? Does it disable the submit button while processing?
...
**Allowing flaky tests to stay skipped.** A test marked `.skip` is not a passing test — it is an ignored test. Every skipped test represents a behavior that CI no longer verifies. Either fix the flakiness (deterministic mocking, stable selectors, removing real timers) or delete the test and write a better one. A growing list of skipped tests is a sign of test suite rot.

**Skipping mutation testing because it is slow.** Mutation testing does not need to run on every commit. Running it weekly in a scheduled CI job, or running it only on the files changed in a PR (using Stryker's `--since` flag), provides high-signal feedback at manageable cost. Skipping it entirely leaves a category of test quality gaps permanently invisible.
