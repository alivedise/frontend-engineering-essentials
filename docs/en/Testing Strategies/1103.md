...
**SHOULD** use `data-testid` attributes for E2E selectors rather than CSS class names or element structure. CSS classes change with UI redesigns; `data-testid` attributes are an explicit contract between the component and the test.

**SHOULD** configure test retries on CI (`--retries 2` for Playwright CLI, `retries: 2` in Playwright config, or `retries: { runMode: 2 }` in Cypress config). Network timing and CI environment variance cause intermittent failures that are not real bugs; retries absorb one-off flakiness without masking systematic failures.

**SHOULD** prefer auto-waiting assertions (`expect(locator).toBeVisible()` in Playwright, `cy.get(...).should('be.visible')` in Cypress) over sleeping or polling. Both frameworks automatically wait for elements to appear before acting; explicit sleeps are a smell that indicates a timing assumption the framework should be handling.

**SHOULD NOT** share mutable state across E2E tests. Each test should set up and tear down its own data or use test isolation mechanisms (unique user per test, database transaction rollback, test fixtures).
...
**Using CSS class selectors in E2E tests.** `cy.get('.btn-primary')` or `page.locator('.submit-button')` breaks when the design system renames the class, when the button gains a modifier class, or when the class is removed entirely in a refactor. Switch to `data-testid` attributes or accessible locators (`getByRole`, `getByLabel`) which are explicitly stable.

**Not configuring retries on CI.** A single flaky failure that blocks a deploy is more disruptive than the cost of two extra retries. Set `retries: 2` on CI as a baseline. If a specific test retries more than occasionally, investigate the root cause — it will be a real timing bug or a test isolation problem.

**Writing tests that depend on execution order.** Test A creates a user. Test B expects that user to exist. When the test runner parallelises or shuffles tests, Test B fails intermittently. Each test must create its own data in a `beforeEach` hook or use test fixtures.

**Sleeping instead of asserting.** `page.waitForTimeout(2000)` is a fixed sleep that makes tests slower and still fails when the operation takes 2001ms. Use `expect(locator).toBeVisible()` or `cy.get(...).should('be.visible')`, both of which poll until the condition is true or the timeout expires. The framework handles the timing.
