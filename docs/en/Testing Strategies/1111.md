...
Engineers MUST investigate and fix flaky tests rather than configuring CI to retry them. Retry
configuration silences the symptom without addressing the cause, reduces confidence in test
results, and increases the probability that a real regression is masked by a "this test sometimes
fails" assumption. Retry is appropriate only as a temporary measure while a flaky test is
quarantined and actively diagnosed.

Engineers SHOULD quarantine flaky tests — move them to a separate test suite that does not block
CI — when the root cause is not immediately diagnosable. A quarantined test is labeled with a
tracking comment and issue reference, does not block CI, and is tracked in the team's backlog.
This is preferable to leaving the flaky test in the main suite (where it erodes trust) or
deleting it (where a real bug may go undetected).
