...
Design tokens are the single source of truth for all visual decisions in a product. Every hardcoded color, spacing value, or typography size in component code is a design token waiting to be extracted. Teams MUST establish a three-tier token hierarchy — primitive, semantic, component — before writing component styles. A flat token namespace is not a token system; it is a rename table that provides nominal benefit while blocking the architectural flexibility that justifies the investment.

Token authoring MUST use the W3C DTCG format (`$value` and `$type` properties). Teams MUST NOT use the legacy Style Dictionary v3 format (`value` and `type` without the `$` prefix) for new token files; the `$`-prefixed format is the interoperability standard, and tools that consume tokens without format negotiation will increasingly expect it.

Semantic tokens MUST reference primitive tokens by alias rather than copying raw values. Copying raw values into semantic tokens defeats the entire purpose of the hierarchy — when the primitive changes, every semantic token that copied the value becomes stale. Alias references (`{color.blue.500}`) propagate changes automatically; copied values do not.

Component code MUST reference only semantic tokens, never primitive tokens directly. This constraint is what enables rebranding and theming: swapping which primitive a semantic token aliases is a one-line change in the token source that propagates to every component consuming that semantic token. Bypassing semantic tokens in component code breaks this propagation chain.

The build pipeline SHOULD use `outputReferences: true` in Style Dictionary's CSS platform configuration. With `outputReferences: true`, the generated CSS variables preserve the reference chain — `--button-background: var(--color-action-primary)` — rather than resolving to the final value `--button-background: #3b82f6`. Preserving references enables runtime theming via CSS overrides; resolving them eliminates it.

Token files and component packages MUST be versioned in lockstep. A token package update that renames a semantic token is a breaking change for every component that references that token. Token stability commitments are API stability commitments.
...
**Referencing primitive tokens directly in component code.** `color: var(--color-blue-500)` in a component stylesheet is the most common token violation. When the brand primary shifts from blue-500 to indigo-500, every component that hardcoded `--color-blue-500` must be hunted down and updated manually. Referencing semantic tokens — `color: var(--color-action-primary)` — means the update happens in the token source and propagates automatically.

**Outputting `px` instead of `rem` for spacing and typography.** CSS pixels do not respect the user's browser font-size preference. A user who sets their browser default to 20px for readability sees no benefit from that preference when all spacing and typography values are hardcoded in `px`. Using `rem` output means the component layout scales proportionally with the user's preference, which is an accessibility requirement, not a preference.
...
**Treating the DTCG format as optional.** The DTCG format is the interoperability standard between design tools and engineering tools. Figma Variables uses DTCG for its import and export. Style Dictionary 4.x is built around DTCG. Starting with the legacy `value` / `type` format (no `$` prefix) and deferring migration creates a conversion burden at the worst possible time — when the team has existing consumers and tooling dependencies that must all be migrated simultaneously.
...
- [W3C Design Token Community Group — Format Specification](https://tr.designtokens.org/format/) — The normative specification for the DTCG JSON format, including all defined `$type` values, composite token structures, and alias reference syntax.
