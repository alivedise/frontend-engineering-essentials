...
A component without a story is a component without a contract. Stories MUST document every meaningful state — not just the happy path. Teams MUST co-locate story files with component files.

The happy path is the state every developer knows. The states that fail in production are the ones no one documented: `disabled` when an async operation is pending, `error` when the network call fails, `empty` when the list has zero items, `loading` when data is being fetched. These states have specific visual requirements — specific enough that someone in a design review approved them. They need to exist as stories, because a story is the only durable record of "this is what the component should look like in this state."

Co-location is not a stylistic preference — it is a maintenance convention that prevents documentation drift. When `Button.stories.tsx` lives in the same directory as `Button.tsx`, every developer who opens the component sees the story immediately. When stories live in a separate `__stories__` directory or a separate `storybook/` tree, they are invisible during routine component work and are the first thing to fall out of sync when the component is refactored.

Teams SHOULD add `@storybook/addon-a11y` to every Storybook configuration. Accessibility issues are easier to catch at development time than in QA or after a complaint. The addon runs an automated axe-core audit on every story render, surfacing issues like missing labels, insufficient contrast ratios, and invalid ARIA attributes in the Storybook UI itself.
...
**Writing only a Default story.** The Default story documents the happy path. It does not document what the component looks like when disabled, when loading, when an error occurs. These are the states that have specific visual requirements and are the first to regress silently. Write stories for every meaningful state from the beginning.
...
**Not co-locating stories with components.** When stories live in a separate directory from the components they document, they become invisible during routine component work. Developers edit the component, forget that a story exists, and push changes that break the story. Co-location is the convention that keeps stories visible and maintained.
