...
- **Radix UI** — a suite of unstyled, accessible primitives for React. Each primitive ships with full keyboard support and ARIA semantics, zero default styles, and a composition API built around the Radix `asChild` pattern that lets consumers forward their own element types without breaking accessibility. As of late 2024, Radix UI has over 9 million weekly npm downloads.
