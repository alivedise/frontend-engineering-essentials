...
Icons are UI components, not image assets. An icon delivered as a React component participates in the type system, accepts props, and is tree-shaken. Teams MUST NOT deliver UI icons as `<img>` tags — `<img>` cannot be styled with `currentColor`, cannot receive CSS color changes on hover/focus, and requires `alt` text management on every usage site.

The `currentColor` capability is the key constraint. A button that changes color on hover or focus — `text-primary hover:text-primary-hover` — propagates that color to any SVG child that uses `currentColor` as its `stroke` or `fill`. An `<img>` receives no CSS inheritance and cannot participate in this. Every icon-in-a-button that is an `<img>` requires a separate explicit hover and focus style, duplicating the button's own hover styles.

Teams SHOULD prefer `lucide-react` as the default icon library for React projects. It provides over 1,500 icons with a consistent design language (2px stroke, 24x24 viewBox), tree-shakeable named exports, and TypeScript props. It is the default icon set in shadcn/ui (FEE-903), making it the low-friction choice for projects already using that stack.
...
The failure mode: `import * as Icons from 'lucide-react'`. This defeats tree-shaking in every bundler. The full icon library — all 1,500+ icons — is included in the bundle. Avoid namespace imports for icon libraries.
...
function Icon({ icon: IconComponent, size = 24, label, className, ...rest }: IconProps) {
  if (label) {
    // Meaningful icon: title provides accessible name, role="img"
    return (
      <IconComponent
        width={size}
        height={size}
...
**`width` and `height` HTML attributes for scalable icons.** HTML attributes set a fixed pixel size on the SVG element. They cannot be overridden by CSS without `!important`. Use CSS — Tailwind `h-4 w-4`, CSS `width: 1rem; height: 1rem`, or CSS custom properties — for icons that need to be responsive, scale with text, or be overridden by consumers.
