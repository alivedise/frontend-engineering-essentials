...
**`width` and `height` HTML attributes for scalable icons.** HTML attributes set a fixed pixel size on the SVG element. They ignore `em` units, responsive utilities, and CSS custom properties — a `width="20"` attribute does not scale when the surrounding font size changes. Use CSS — Tailwind `h-4 w-4`, CSS `width: 1rem; height: 1rem`, or a `size` prop that sets CSS `width`/`height` — for icons that need to be responsive or scale with text.
