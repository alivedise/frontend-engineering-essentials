...
Dark mode is a token-level concern, not a component-level concern. Components MUST NOT contain hardcoded color values. Every color MUST reference a semantic CSS custom property. When this contract is honored, switching themes requires overriding a set of CSS variables — zero component changes, zero JavaScript re-renders.

The separation of semantic tokens from their resolved values is what makes theming mechanically simple. A component that renders `color: var(--color-text-primary)` does not know whether that resolves to `hsl(222 47% 11%)` in light mode or `hsl(210 40% 98%)` in dark mode. That is the correct state of affairs. The component owns its semantic intent; the theme owns the resolved value.

Primitive tokens — raw values like `--color-blue-500: hsl(217 91% 60%)` — MUST NOT have theme-specific overrides. Primitive tokens represent values, not intentions. Only semantic tokens — `--color-interactive-default: var(--color-blue-500)` — need dark mode variants, because only semantic tokens express intentions that can change across themes.

Teams SHOULD implement both `prefers-color-scheme` detection (for users who have not set a preference in the app) and a class-based toggle (for users who want to override their system setting). These two mechanisms serve different users. Ignoring the first breaks the experience for users who set a system-wide dark mode preference. Ignoring the second makes it impossible for users to use a different mode in the app than their OS preference.
...
**Using `transition: all` for the theme switch.** `transition: all` animates every CSS property that changes — including layout properties. A component that changes its width when content changes between themes (e.g., a nav that reflows) will produce unexpected layout animation. Specify only color-related properties: `transition: color 150ms ease, background-color 150ms ease, border-color 150ms ease`.
