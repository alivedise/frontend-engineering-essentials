...
Engineers SHOULD implement multi-brand support through the design token layer,
not through component-level branching. A component that contains
`if (brand === 'brandA') ...` becomes harder to maintain with each new brand
and requires component changes when visual changes are needed. A component that
references only `var(--color-action-primary)` requires no changes when a new
brand is added; only a new token file is required. The token layer is the
contract surface between shared engineering and brand-specific configuration;
keeping component code free of brand conditionals is the invariant that makes
this contract viable at scale.

Engineers MUST establish a clear token contract between the design system and
brand teams: a defined set of semantic token names that every brand must provide
values for. The contract is the interface between the shared component layer and
the brand layer. Adding a new semantic token to the contract without a
corresponding default value is a breaking change for all brands that do not
define it. The contract should be versioned alongside the component library;
breaking changes — adding a required token, renaming an existing token —
require a major version bump and a migration guide for every brand team
