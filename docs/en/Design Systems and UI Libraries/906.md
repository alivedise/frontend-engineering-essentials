...
A shared component library is a public API. Every change that removes a prop, changes default behavior, or requires a new peer dependency MUST be treated as a breaking change and communicated as a major version bump. Teams MUST NOT publish patch versions for API changes.

Semantic versioning is not bureaucracy — it is the communication mechanism that allows consumers to upgrade on their own schedule. A major version bump is a signal: this version requires active migration work. A minor version bump is a signal: new functionality is available, upgrade is safe. A patch version bump is a signal: a bug was fixed, upgrade is safe and recommended. When a breaking API change is published as a patch, consumers who upgrade expecting safety break unexpectedly. They lose trust in the library's versioning and stop upgrading promptly, which means they stop receiving bug fixes and security patches.

The strict rule for UI libraries: removing or renaming a prop is a major version bump. Changing a prop's default value is a major version bump. Requiring a new major version of a peer dependency is a major version bump. Adding an optional prop is a minor bump. Adding a new component is a minor bump. Fixing a visual bug without changing props or behavior is a patch bump.

Teams SHOULD use a CI check that fails the pipeline when a PR modifies a package without an accompanying changeset file. This makes the versioning process enforceable rather than dependent on individual developer memory.
...
**Missing `"sideEffects": false`.** Without this flag, bundlers assume every module has side effects and cannot tree-shake the package. A consumer importing one component gets the entire library. The bundle size impact is proportional to the library size.

**Not verifying the published tarball.** The `dist/` directory is missing, source files are included, or `node_modules` accidentally ended up in the tarball. Run `npm pack --dry-run` locally and check the file list before every release. Look for: `dist/` is present, `src/` is absent, `node_modules/` is absent.
