...
Engineers SHOULD express component variants using a variant configuration
library (CVA, `tailwind-variants`) rather than inline conditional className
logic. A component with three or more independent variant dimensions requires
combinatorial conditional logic that grows quadratically with each new
dimension; a variant configuration object is linear in the number of dimensions
and produces a self-documenting API surface. The variant configuration approach
also generates TypeScript types automatically, making the variant API
statically verifiable without manual type writing.

Engineers MUST reference semantic design tokens in component styles, not
primitive token values directly. A button setting
`background-color: var(--color-blue-600)` is coupled to a specific color and
must be updated whenever the palette changes. A button setting
`background-color: var(--color-action-primary)` is coupled to a semantic
concept that survives theme changes, brand updates, and dark mode toggling
without any modification to the component's source code.
