...
### 1. Not disconnecting observers on component unmount

Observers hold strong references to their callback closures and observed elements. In SPAs, failing to disconnect creates memory leaks -- the observer, its closure, and all captured variables survive route changes:

```js
// WRONG: Observer leaks when component unmounts
function mountWidget(container) {
  const observer = new IntersectionObserver(callback);
  observer.observe(container);
  // No cleanup -- observer lives forever
}
...
### 2. Using scroll listeners when Intersection Observer exists
...
### 3. Intersection Observer threshold=0 missing elements already in viewport on mount
...
### 4. Resize Observer loop error from modifying observed elements in callback
...
### 5. Mutation Observer on large subtrees without filtering

Observing `document.body` with `{ childList: true, subtree: true, attributes: true }` captures every DOM mutation on the entire page. This can generate thousands of MutationRecord objects per frame in complex applications, overwhelming the callback:

```js
// WRONG: Observes every mutation in the entire document
...
// CORRECT: Scope observation to the smallest necessary subtree
// and filter attributes to only what you need
...
- [web.dev: Lazy Loading Images and Video](https://web.dev/articles/lazy-loading-images) -- Best practices for image lazy loading with native and IO approaches
