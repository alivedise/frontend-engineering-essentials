...
WebGL exposes OpenGL ES 2.0/3.0 to the browser, enabling GPU-accelerated 2D and 3D rendering through a programmable shader pipeline written in GLSL. WebGPU is the modern successor — a lower-overhead API modelled on Vulkan/Metal/D3D12, supporting both rendering and GPU compute via WGSL shaders. Teams SHOULD NOT choose raw WebGL or WebGPU when a library abstraction (Three.js, Babylon.js, TWGL) would provide the same result at a fraction of the implementation cost — the native APIs are appropriate when fine-grained GPU pipeline control is required. Teams SHOULD prefer WebGPU for new projects where browser support permits (`navigator.gpu` feature-detect required); WebGL 2 (`getContext('webgl2')`) remains the broadest-compatibility fallback.

GPU resource objects — `GPUBuffer`, `GPUTexture`, `WebGLProgram`, and `WebGLBuffer` — MUST be explicitly destroyed or deleted when no longer needed. The JavaScript garbage collector does not reclaim GPU memory; only explicit calls to `gpuBuffer.destroy()`, `gpuTexture.destroy()`, `gl.deleteProgram()`, and `gl.deleteBuffer()` release the underlying driver handles. Failure to do so causes GPU memory leaks that are invisible to heap profilers and will eventually degrade rendering performance or crash the GPU process.
