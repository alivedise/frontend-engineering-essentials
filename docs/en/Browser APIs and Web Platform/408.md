...
| Texture sampling | `texture2D(sampler, uv)` | `textureSample(t, s, uv)` |
...
The vertex shader receives a `vec2` position attribute and passes it through to clip space by appending `z=0` and `w=1`. The fragment shader discards the position and outputs a hard-coded RGBA blue. A Vertex Array Object captures the buffer binding and attribute layout so the draw call only needs to bind the VAO and program.
