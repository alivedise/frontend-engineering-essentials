...
The following mistakes are consistently observed in production code and code review. Each is subtle enough to pass casual inspection.

**Listing a `TypedArray` view instead of its backing `ArrayBuffer` in the transfer list.**
`worker.postMessage({ pixels }, [pixels])` does nothing — `pixels` is a `Uint8ClampedArray` view, not an `ArrayBuffer`. The browser silently falls back to structured clone, the sender is not neutered, and the receive side gets a copy. The fix: `worker.postMessage({ pixels }, [pixels.buffer])`. The comment in Example 1 above flags exactly this mistake.

**Accessing the transferred object after `postMessage` returns.**
`postMessage` is synchronous up to the neutering step: by the time the call returns, the object is already neutered. Code that saves a local reference and reads it "later this tick" will see `byteLength === 0` or throw. The fix: extract any needed values *before* calling `postMessage`.

**Forgetting `port.start()` when using `addEventListener` on a `MessagePort`.**
`MessagePort` is in "paused" state by default. Messages are enqueued but not dispatched until the port is started. Assigning to `port.onmessage` starts it automatically; calling `port.addEventListener('message', handler)` does not. Without `port.start()` the port silently accumulates messages. This is the source of most "my MessageChannel isn't working" bugs.

**Not calling `.close()` on `VideoFrame` after the worker is done processing.**
Hardware-backed media memory sits outside the JS heap. GC does not reclaim it. Each unclosed `VideoFrame` permanently leaks its allocation (typically 4–16 MB for 1080p). A worker that processes 30 fps and skips `.close()` will exhaust available memory in seconds. Always wrap `VideoFrame` consumption in a `try/finally` block with `.close()` in the `finally` branch.

**Transferring into a terminated or errored worker.**
`postMessage` on a terminated worker throws a `DOMException`. Transfer into an errored worker succeeds syntactically — the message is posted and the object neutered on the sender — but the worker never processes it. The object is lost. Always check worker liveness before transferring irreplaceable data, or design pipelines so the sender retains a copy until the worker acknowledges receipt.
