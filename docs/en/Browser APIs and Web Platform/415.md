...
Engineers SHOULD query permission state before invoking a permission-gated API
when the application's UI needs to adapt to the current state. For a camera
feature, querying `{ name: 'camera' }` before rendering the camera button allows
the button to show a "grant access" affordance for `prompt` state, a "settings"
link for `denied` state, and direct camera activation for `granted` state. This
eliminates calling the API and catching the error as the only way to discover the
`denied` state.

Engineers MUST design for the `denied` permission state explicitly. When a user
blocks a permission, the browser will not show the prompt again unless the user
manually changes the setting. An application that silently fails when a permission
is `denied` — showing a loading spinner that never resolves — creates a broken
experience with no recovery path. The `denied` state requires explicit UI: a
message, a link to browser settings, and in some cases an alternative flow.
...
update their UI in response — for example, enabling a camera button when the user
...
API is a state-reading facility, not a request-triggering one. This confusion
most often surfaces in applications that add `navigator.permissions.query()` as
an optimization to an existing permission flow without fully understanding its
read-only nature, resulting in a query call that correctly reads state but
provides no permission dialog to the user, while the actual prompt-triggering
call is either missing or placed incorrectly in the flow.
