...
You are building an accessibility feature for a form-heavy application where users with motor impairments can fill in fields by voice. Implementing speech recognition from scratch requires a speech model, audio processing, and significant infrastructure. The Web Speech API provides a browser-native interface to the platform's speech recognition engine — `SpeechRecognition` for voice-to-text and `SpeechSynthesis` for text-to-voice — without requiring your own speech model, audio pipeline, or dedicated speech infrastructure.
...
**MUST disclose to users that speech recognition sends audio to a third-party server for processing, before requesting microphone permission.** Chrome's Web Speech API implementation transmits audio to Google's servers; this is not an on-device process. The disclosure must be visible and proximate to the microphone control, and must appear before the browser's permission prompt — not buried in a privacy policy. Applications subject to GDPR, HIPAA, or enterprise data residency requirements that cannot allow audio to leave the device must evaluate server-side alternatives such as the Whisper API, where the audio transport path and processing location are entirely under the application's control.
