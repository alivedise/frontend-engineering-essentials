...
### 1. Attempting to access the DOM from a Worker

Workers have no access to `window`, `document`, `document.getElementById()`, or any DOM API. This is by design -- DOM access from multiple threads would require locking and create deadlock/race-condition hazards. Instead, compute the result in the worker and send it to the main thread for DOM updates:

```js
// WRONG: This throws ReferenceError inside a worker
self.addEventListener('message', () => {
  document.getElementById('result').textContent = 'Done'; // ERROR
});

// CORRECT: Send result back to main thread
self.addEventListener('message', () => {
  const result = expensiveCalculation();
  self.postMessage({ result }); // Main thread updates DOM
});
```

### 2. Ignoring structured cloning overhead (use transferable objects)

Every `postMessage` call deep-copies the data via structured cloning. For large ArrayBuffers, ImageData, or typed arrays, this copies megabytes of memory unnecessarily. Use the transfer list parameter to move ownership instead:

```js
// SLOW: Clones 50 MB of data (takes ~50-100ms)
const buffer = new ArrayBuffer(50 * 1024 * 1024);
worker.postMessage({ buffer });

// FAST: Transfers ownership in O(1) time
const buffer = new ArrayBuffer(50 * 1024 * 1024);
worker.postMessage({ buffer }, [buffer]);
// buffer.byteLength is now 0 -- ownership moved to worker
```

### 3. Not terminating workers when they are no longer needed

Each worker consumes an OS thread and associated memory (~1-5 MB baseline). Creating workers without terminating them leads to resource leaks, especially in SPAs that create workers on route changes:

```js
// BAD: Worker lives forever, even after navigating away from the page
...
// GOOD: Terminate after receiving the result
function processData(data) {
  const worker = new Worker('/processor.js');
  worker.postMessage(data);
  worker.onmessage = (e) => {
    updateUI(e.data);
    worker.terminate(); // Free OS thread and memory
  };
}
```

### 4. Service Worker caching stale content without a versioning strategy
...
### 5. Using SharedArrayBuffer without cross-origin isolation headers
