...
The security model of WebTransport is notably stricter than WebSocket.
WebTransport connections MUST be made to HTTPS origins over HTTP/3 — plain HTTP origins are rejected.
The server must present a valid TLS certificate trusted by the browser's certificate store, or the connection can alternatively use a server certificate hash mechanism (`serverCertificateHashes` option) that bypasses the CA chain for development and embedded device scenarios.
The browser enforces that the WebTransport connection is only opened to the same origin or to origins that explicitly permit cross-origin access via a corresponding server-side header.
The same-origin policy and CORS-like restrictions that apply to Fetch requests do not disappear simply because you are using a lower-level API.

## Principle

WebTransport provides a browser API for low-latency, bidirectional communication built on HTTP/3 and QUIC. It supports both unreliable datagrams (UDP-like, no delivery guarantee) and reliable ordered streams (TCP-like, over QUIC streams). Teams MUST NOT deploy WebTransport without a WebSocket fallback — as of 2026, WebTransport is supported in Chrome 97+ and Edge but not in Firefox stable or Safari (experimental). Teams SHOULD choose WebTransport over WebSocket when the use case can tolerate packet loss and minimising round-trip latency matters more than guaranteed delivery — real-time games, live telemetry, cursor sharing. Teams SHOULD use reliable streams (not datagrams) when message ordering and delivery are required. Teams MUST await the `transport.ready` promise before sending data — the connection setup is asynchronous.
...
async function detectTransport(wtUrl: string, wsUrl: string): Promise<TransportTier> {
...
async function useBidirectionalStream(transport: WebTransport): Promise<void> {
  ...
  // Do NOT close writer if you intend to keep sending; close only when done
  writer.releaseLock();
  ...
  try {
    ...
    reader.releaseLock();
    // Close the writable side when done
