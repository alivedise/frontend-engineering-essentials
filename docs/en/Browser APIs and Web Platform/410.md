...
Separately, the Clipboard API and HTML Drag-and-Drop API provide two more pathways through which files reach a web app. `navigator.clipboard` offers a fully promise-based interface for reading and writing text, images, and arbitrary data types, replacing the legacy `document.execCommand('copy')` approach that was never officially standardized and whose behavior varied across browsers. Drag-and-drop allows users to drag files from the OS file manager onto a designated drop zone in the page; the mechanism is built into every modern browser and requires only a `dragover` handler that calls `event.preventDefault()` and a `drop` handler that reads `event.dataTransfer.files`. Understanding all three pathways — file input, File System Access API, and drag-and-drop — and knowing when each is appropriate is essential for building any workflow that involves local file handling.

Understanding the browser's security model around file access is prerequisite knowledge before choosing an API. `<input type="file">` is the most permissive in terms of browser support and the most constrained in capability: the user selects a file, the browser hands your code a `File` object, and after that there is no connection between the object and the on-disk file. You cannot write back; you cannot re-read the file without asking the user again. The File System Access API inverts this trade-off: it provides genuine persistence (the handle can be stored and later used to reopen the file without another picker interaction) but requires explicit user consent at the OS level and carries the security implications of giving a web page write access to the file system. The clipboard and drag-and-drop pathways sit between these extremes — both require a user gesture, neither requires an explicit permission dialog for basic operations, and both produce `File` objects (in the case of drag-and-drop) or `Blob` objects (in the case of clipboard reads) that feed the same downstream pipeline.

## Principle

The browser provides three independent mechanisms for reading user-provided files: the `<input type="file">` element (universal, widest support), the promise-based `File` methods `.text()` / `.arrayBuffer()` / `.stream()` (modern replacement for `FileReader`), and the File System Access API (`showOpenFilePicker` / `showSaveFilePicker`) which grants persistent read/write access to files the user selects. Teams MUST implement a `<input type="file">` fallback when using the File System Access API — Safari support is partial (read-only as of 2025). Teams MUST handle the async permission prompt in `showOpenFilePicker` — the call requires a transient user gesture and rejects if called programmatically or if the user denies. Teams SHOULD prefer `navigator.clipboard.writeText` / `readText` over the deprecated `document.execCommand('copy')`.
