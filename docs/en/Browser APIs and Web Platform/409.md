...
WebSocket was standardised in IETF RFC 6455 (2011) and the corresponding browser API landed in all major browsers by 2012. The protocol begins with an HTTP/1.1 upgrade handshake — the browser sends an `Upgrade: websocket` request header, the server responds with `101 Switching Protocols`, and from that point the connection is no longer HTTP: it is a raw framed binary stream where either side can send messages at any time. Server-Sent Events are older in spirit — HTTP/1.1 streaming has been possible since the 1990s — but were standardised as part of the HTML Living Standard and given a formal browser API (`EventSource`) in the late 2000s. SSE is not a protocol upgrade; it is a plain HTTP response with `Content-Type: text/event-stream` that the browser keeps open and parses incrementally.
...
The popularity of SSE has grown notably with the rise of streaming AI and LLM applications. Services like OpenAI's Chat Completions API and Anthropic's Messages API deliver token-by-token responses as SSE streams. The client opens a single `EventSource` connection or a streaming `fetch`, and the server flushes each token as it is generated. This is a textbook SSE use case: the server is the only speaker after the initial request, the payload is text, and the browser's built-in reconnection handles transient network hiccups without any custom code. Understanding SSE is therefore increasingly relevant even for engineers who think of themselves as primarily working on traditional request-response applications.

There is also a practical difference in how each technology handles backpressure. WebSocket gives the sender a `bufferedAmount` property: `ws.bufferedAmount` returns the number of bytes queued for transmission that have not yet been sent. A sender that produces data faster than the network can carry it can inspect `bufferedAmount` before calling `send()` and apply its own throttling. SSE has no such feedback mechanism — the server writes to the response stream and the underlying TCP flow control is the only backpressure signal. For most applications this distinction is invisible, but it matters for high-throughput data pipelines where the server might outpace the client's ability to process events.

## Principle

WebSockets provide a full-duplex, persistent TCP connection where either the browser or server may send messages at any time after the initial HTTP upgrade handshake. Server-Sent Events (SSE) provide a unidirectional server-to-browser stream over a standard HTTP/1.1 or HTTP/2 connection using the `EventSource` API. Teams MUST implement exponential backoff reconnection for WebSocket connections — the protocol does not guarantee persistence and network interruptions will close the socket. Teams SHOULD prefer SSE for read-only server push (live feeds, notifications, streaming AI/LLM responses) because it uses standard HTTP, works through all proxies and load balancers, and the browser handles reconnection automatically via `Last-Event-ID`. Teams SHOULD use WebSocket only when the browser also needs to send frequent messages to the server.
