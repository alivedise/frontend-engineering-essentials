...
**Batch DOM reads before writes to prevent layout thrashing.** The browser defers layout recalculation until geometry information is actually needed. When JavaScript interleaves reads (e.g., `offsetWidth`, `getBoundingClientRect()`) with writes (e.g., `style.width = ...`), it forces a synchronous layout on every read — a pattern called layout thrashing that can drop frame rates from 60 fps to single digits. SHOULD collect all geometry reads first, then apply all style writes in a second pass.

**Use `DocumentFragment` for bulk DOM insertions.** Appending elements to the live DOM one at a time triggers a layout recalculation for each insertion. A `DocumentFragment` is not part of the active document tree, so appending children to it incurs no layout cost. When the fragment is inserted, all its children move into the DOM atomically in a single operation. SHOULD use `document.createDocumentFragment()` (or the modern `append()` with multiple node arguments) whenever inserting more than one element.

**Prefer `querySelector` and `querySelectorAll` over older APIs.** `getElementById`, `getElementsByTagName`, and `getElementsByClassName` return live HTMLCollections that update as the DOM changes, which can produce subtle bugs in loops. `querySelector` and `querySelectorAll` return a static result and accept any CSS selector, making them more predictable and expressive. SHOULD default to the `querySelector` family for new code; AVOID older collection APIs unless a live collection is explicitly needed.

**Cache DOM references in long-lived code.** `querySelector` traverses the entire subtree on every call. Calling it inside a loop or a frequently invoked function when the target element does not change wastes CPU cycles. SHOULD query the DOM once, store the result in a variable, and reuse it. AVOID querying the same stable element repeatedly across multiple function calls or render cycles.
...
### 2. Not using closest() for event delegation
...
### 3. Forgetting that DOM operations are synchronous and block rendering
