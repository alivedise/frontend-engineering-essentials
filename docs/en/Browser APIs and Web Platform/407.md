...
The SVG element is a standard DOM node. `getElementById` finds it, `addEventListener` attaches event handlers, and `setAttribute` updates its presentation attribute — the browser schedules a repaint of only the affected region. The `role="img"` and `aria-label` on the `<svg>` element make the graphic visible to screen readers. This level of accessibility integration requires zero additional code beyond what SVG's DOM nature provides automatically.
