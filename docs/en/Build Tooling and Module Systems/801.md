...
Webpack remains appropriate for projects that require Module Federation, depend on loaders with no Vite equivalent, or have a large existing Webpack configuration that would be costly to migrate. For every other new greenfield application, Vite's native-ESM dev server provides dramatically faster startup and HMR with no meaningful production trade-off.

**Use Rollup (or a Rollup-based tool such as `tsup`) when publishing packages to npm.**
Rollup's output preserves module boundary information in a form that downstream bundlers can statically analyze for tree shaking. Webpack's runtime wrapper (`__webpack_require__`) hides that information, preventing consumers from eliminating unused exports. A published Webpack bundle forces every downstream consumer to ship dead code they cannot remove.
...
- **Using Webpack for a new greenfield app when Vite is available.**
  Webpack remains valid for teams with existing complex config, module federation requirements,
  or specific loader dependencies. For new apps without those constraints, Vite provides
  dramatically better DX with no meaningful trade-off.

- **Publishing a Webpack bundle to npm.**
  Webpack's runtime wrapper prevents downstream bundlers from tree-shaking your exports.
  Always use Rollup (or a Rollup-based tool like `tsup`) when publishing packages.
...
- **Forgetting to mark peer dependencies as `external` in Rollup library configs.**
  If React ends up bundled inside your component library, consumers may load two copies of React,
  causing hooks to throw because the instances do not match.
