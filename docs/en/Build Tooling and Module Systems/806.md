...
**MUST NOT** put secrets (API keys, private auth tokens, database passwords, signing keys) in
frontend environment variables. They are substituted into the bundle at build time and are
publicly readable by anyone with DevTools.

**MUST** use the framework-specific prefix (`VITE_*` for Vite, `NEXT_PUBLIC_*` for Next.js)
as an explicit opt-in for every variable you choose to expose to the client bundle.

**SHOULD** commit `.env` and mode-specific `.env.production` files that contain
non-sensitive defaults. **MUST NOT** commit `.env.local` — it is gitignored by convention and
exists specifically for secrets and local overrides that should never leave the developer's
machine.

**SHOULD** use GitHub Actions `secrets` (not `vars`) for any sensitive value injected at
build time in CI. Secrets are masked in logs; vars are not.
