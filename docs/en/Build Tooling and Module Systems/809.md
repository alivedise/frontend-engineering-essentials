...
Engineers MUST author library code as ES modules — using `"type": "module"` in
`package.json` or the `.mjs` file extension — and MUST use named exports rather than
default-exporting a single object. Tree-shaking requires statically analyzable
import/export bindings. An `export default { foo, bar, baz }` is opaque: the bundler sees
a single default export that is an object, and because property access on that object
cannot be statically resolved to specific exports, the entire object must be included when
the default is imported. Named exports such as `export function foo() {}` and
`export function bar() {}` allow the bundler to include `foo` without including `bar` when
only `foo` is imported. CommonJS modules (`module.exports`, `require`) cannot be
tree-shaken at all; the dynamic nature of the CommonJS module system means the bundler
cannot determine the export surface without executing the module.

Engineers MUST declare a `"sideEffects"` field in the `package.json` of every package
intended to be tree-shakeable. Omitting this field forces bundlers to treat the package
defensively and include all reachable modules even when their exports are unreferenced.
The value SHOULD be `false` when the package contains no side-effectful modules, or an
array of glob patterns when specific files — CSS imports, polyfills, global registrations
— must be preserved. Application-level `package.json` files SHOULD also include this
field for internal modules so that the application's own bundler can eliminate unused
internal code with the same confidence it applies to declared packages.
...
side-effect-free. Never annotate a call that writes to a global, fires a network request,
or mutates shared state — the bundler will silently drop the call and the side effect will
not occur.
