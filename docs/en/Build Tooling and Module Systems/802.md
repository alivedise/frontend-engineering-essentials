...
- SHOULD use source maps in development (type: `eval-source-map` or `eval-cheap-module-source-map`)
- SHOULD NOT expose full source maps in production (use `hidden-source-map` or equivalent; upload to an error tracker, do not serve to end users)
