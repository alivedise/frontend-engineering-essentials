...
Establishing conventions for how schemas and their inferred types are named and co-located prevents the fragmentation that emerges when each engineer places them differently.
A consistent pattern is to declare the schema as `const entitySchema = z.object({ ... })` and the inferred type as `type Entity = z.infer<typeof entitySchema>` in the same file, immediately below the schema.
This co-location makes the relationship between schema and type visible to readers and ensures that the two are always updated together.
Placing inferred types in a separate `types.ts` file re-introduces the drift risk that schema-as-source-of-truth is intended to eliminate, because the type import can be updated independently of the schema, and vice versa.
When a schema is used across multiple features, the co-located schema-and-type file belongs in the closest shared ancestor of those features in the project's directory structure, following the same proximity principle that applies to shared components and utilities.
Enforcing this convention via an ESLint rule that flags `z.infer` usages in files that do not also contain the corresponding `z.object` or schema declaration — or that import the inferred type from a file that does not also export the schema — provides an automated guardrail against the pattern fragmenting as the codebase grows.
