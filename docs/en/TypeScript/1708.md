...
**Using `parse` with try/catch in every fetch call site instead of `safeParse`** is covered as the third common mistake below. Preceding that, the first two mistakes address the most fundamental misunderstandings about what TypeScript types guarantee and what a schema library is for.
