...
**Version scope:** This article covers patterns for React 17 and 18. React 19 introduced a new ref model that passes `ref` as a regular prop without `forwardRef`, and simplified the ref API. The `forwardRef` patterns in this article remain valid for codebases on React 17-18 and for backwards-compatible components, but React 19 projects should consult the React 19 migration guide for the updated ref approach.
