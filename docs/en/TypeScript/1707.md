...
Refs add a third distinct typing concern. `useRef` has two signatures that return different types depending on the initial value, and choosing the wrong signature for a DOM ref produces a ref type that React cannot safely assign to a DOM element. The distinction is subtle — a missing `null` initial value changes the entire semantics of the ref — and it is one of the most common sources of TypeScript errors in React code that are resolved by trial and error rather than by understanding the underlying model. The same conceptual gap that causes the wrong `useRef` call also causes the wrong generic type on `forwardRef`, so understanding the ref typing model once resolves both problems at once.

Generic components add a fourth concern that the `React.FC` limitation makes visible: a component's type signature sometimes needs to carry a type parameter that flows through from the input data to the rendered output, and TypeScript's generic function syntax is the only tool for this. A `<Select<User>>` that constrains its `value` and `options` to the `User` type, or a `<List<Product>>` that gives `renderItem` access to `Product`-typed items, must be a generic function, not a component typed with a fixed props interface. Understanding that components are just functions, and that TypeScript's full generic function syntax is available for them, unlocks this capability.

## Principle

Engineers MUST NOT annotate function components using `React.FC`. The annotation adds no type safety benefit over typing the props parameter directly, implicitly included `children` in props across pre-React-18 code in ways that the React 18 types removed, prevents components from being typed as generics, and introduces an indirection layer that makes the component's type signature harder to read. The correct annotation is the function's props parameter itself: `function MyComponent({ name }: MyComponentProps)`. When a component needs to accept `children`, it MUST declare `children: React.ReactNode` explicitly in its props interface rather than relying on any annotation to include it implicitly.

Engineers MUST type React event handler props using React's synthetic event types — `React.ChangeEvent<T>`, `React.MouseEvent<T>`, `React.FormEvent<T>` — rather than native DOM event types. Synthetic event types model React's cross-browser event abstraction; native DOM types reflect browser-specific implementations and are structurally different from what React passes to handlers. Using native event types either produces a type error at the JSX level or silently narrows the handler's contract away from what React actually passes at runtime. Components SHOULD derive their prop types from `React.ComponentProps<'element'>` when wrapping native HTML elements, rather than enumerating HTML attributes manually. Engineers MUST use `useRef<T>(null)` with an explicit element type and `null` initial value for any ref that will be attached to a DOM element; omitting the `null` initial value changes the returned ref's type from `React.RefObject<T>` to `React.MutableRefObject<T | undefined>`, which is incompatible with the `ref` prop on JSX elements.
