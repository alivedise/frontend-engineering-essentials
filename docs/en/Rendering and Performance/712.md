...
Engineers MUST place all `<link rel="stylesheet">` elements in the `<head>` and
all `<script>` elements without `defer` or `async` at the bottom of `<body>`,
or use `defer` on any `<script>` placed in the `<head>`. A stylesheet in the
`<body>` causes a Flash of Unstyled Content (FOUC) and a mid-render block; a
`<script>` in the `<head>` without `defer` blocks HTML parsing and delays all
content rendering. The `defer` attribute on scripts preserves execution order,
defers execution until after HTML parsing completes, and does not block
parsing — making it the default choice for application scripts. These two
placement rules together account for the most common sources of avoidable
render-blocking delay. Linting tools and code review checklists should include
checks for blocking script placement as a standard quality gate, because the
pattern reappears frequently as new third-party scripts and integrations are
added to a page over time.

Engineers SHOULD eliminate or defer stylesheets that are not required for
above-the-fold rendering. A site's entire CSS being render-blocking means that
styling for modals, footers, animations, and other below-the-fold components
delays first paint for every user on every page load. Critical CSS inlining —
embedding the styles required for above-the-fold content in a `<style>` tag
and loading the full stylesheet as non-blocking using `media="print"` or a
JavaScript-based async technique — is the optimization that directly reduces
FCP for CSS-heavy pages. The benefit compounds on connections where stylesheet
downloads take significant time, because the critical CSS is already inlined
in the HTML response and requires no additional round-trip.
