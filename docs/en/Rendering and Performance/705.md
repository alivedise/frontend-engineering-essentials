...
- SHOULD lazy-load below-fold components, images, and routes; only the above-fold critical path should be part of the initial bundle.
- SHOULD mark packages as `sideEffects: false` in `package.json` when all exports are pure functions with no global side effects.
- SHOULD use route-based splitting as the first and highest-impact splitting strategy before applying component-level splitting.
- SHOULD avoid splitting too aggressively: each extra chunk adds a network round-trip; prefer prefetching over splitting for small components.
- SHOULD use `<Suspense>` (React) or `<Suspense>` / loading slots (Vue) to handle the loading state of lazy components gracefully.
- SHOULD verify tree shaking is working by checking the production bundle with a bundle analyzer before assuming unused imports are eliminated.
...
- **Wrapping the entire page in a single Suspense boundary.** If one large `<Suspense>` wraps all lazy children, none of them render until the slowest chunk loads. Use multiple, tightly scoped Suspense boundaries so each section can render independently as its chunk arrives.
...
- **Assuming tree shaking works without verifying it.** CommonJS libraries (many older npm packages) cannot be tree shaken regardless of what you import. Run a bundle analyzer (Vite's `rollup-plugin-visualizer`, webpack-bundle-analyzer) and check whether unused exports from your heaviest dependencies are actually being eliminated.

- **Forgetting `"sideEffects": false` in a library's package.json.** Without this field, bundlers conservatively keep every imported module in full, even if only one function is used. This is the most common reason tree shaking "doesn't work" for utility libraries.

- **Lazy loading above-fold content.** Lazy loading images or components that are immediately visible on load (hero sections, navigation, above-fold text) delays LCP and makes the page feel broken. Only resources genuinely below the fold benefit from lazy loading.
