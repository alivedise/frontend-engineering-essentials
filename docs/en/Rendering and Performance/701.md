...
The key insight is that this is not a ranking -- it is a toolbox. A well-architected application often uses multiple strategies simultaneously. A Next.js e-commerce site might use SSG for the homepage, ISR for product pages, SSR with streaming for search results, and CSR for the shopping cart -- all within the same codebase.
...
### 1. Using CSR for content-heavy, SEO-dependent sites

Building a public blog, documentation site, or e-commerce storefront as a pure CSR SPA means search engines see an empty page (or must wait for JS execution). Even though Googlebot can execute JavaScript, there is a significant delay in indexing, and other search engines may not execute JS at all. The fix: use SSG or SSR for any content that must be discoverable.

### 2. Using SSR for content that rarely changes

Rendering a marketing page or blog post on every single request wastes server compute and increases TTFB for no benefit. If the content is the same for every visitor and changes only when the team publishes updates, SSG (or ISR for large sites) delivers the same HTML faster and cheaper.

### 3. Not using streaming when the page has slow data sources

Traditional SSR blocks the entire response until all data is fetched. If one API call takes 3 seconds, the user stares at a blank screen for 3 seconds. Streaming SSR sends the page shell immediately and fills in slow sections as they resolve. Not streaming when you could is leaving FCP on the table.

### 4. ISR without a revalidation strategy

Setting `revalidate: 3600` and forgetting about it means users might see content up to an hour old. For time-sensitive content (event pages, flash sales), this can be damaging. The fix: use on-demand revalidation (webhook-triggered) for critical content, and time-based revalidation only for content where staleness is acceptable. Always think about the worst-case staleness a user could encounter.

### 5. Hydration mismatch from server/client divergence
...
- [Rendering: Server Components -- Next.js Docs](https://nextjs.org/docs/app/building-your-application/rendering/server-components) -- Next.js documentation on React Server Components, streaming, and the App Router rendering model
