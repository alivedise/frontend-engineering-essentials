...
**Measure in the field (CrUX) not just lab (Lighthouse).** MUST NOT rely solely on Lighthouse scores to assess Core Web Vitals. Lighthouse runs on a simulated mid-tier device with throttled network; it cannot capture the diversity of real users on low-end Android phones, congested connections, or heavy browser extension environments. SHOULD deploy the web-vitals library to collect LCP, INP, and CLS from real sessions and verify improvements in CrUX after every major change. Lab data guides debugging; field data at the 75th percentile is the authoritative record and what Google Search ranking uses.

**Prioritize LCP resource discovery with `<link rel="preload">`.** MUST ensure the LCP image is present in the initial HTML (not injected by JavaScript) and SHOULD add `<link rel="preload" as="image">` in the document `<head>` along with `fetchpriority="high"` on the `<img>` element. This combination ensures the browser discovers and fetches the LCP candidate before it encounters the tag during HTML parsing, minimizing the gap between TTFB and LCP paint. In Next.js, the `priority` prop on `<Image>` handles both automatically.

**Reduce INP by moving long tasks off the main thread.** SHOULD break any JavaScript task that runs longer than 50 ms into smaller chunks using `scheduler.yield()`, `setTimeout(..., 0)`, or `requestIdleCallback`, so the browser can process pending user input between chunks. SHOULD use `startTransition` and `useDeferredValue` (React 18+) to mark expensive re-renders as non-urgent, allowing the browser to paint an interaction response before completing the update. Long tasks are the primary cause of poor INP scores.

**Avoid CLS from unsized images and embeds.** MUST set explicit `width` and `height` attributes on every `<img>` and `<iframe>` so the browser can reserve space using the element's aspect ratio before the resource loads. SHOULD also reserve space for dynamically injected content (banners, ads, cookie dialogs) with `min-height` placeholders or `position: fixed` overlays that do not shift document flow. AVOID animating `top`, `left`, `height`, or `width` properties, which trigger layout shifts; use `transform` instead.
