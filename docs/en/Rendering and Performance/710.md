...
The practical distinction matters when diagnosing unexpected layer proliferation.
Discovering that a page has promoted 80 elements to compositor layers — far more
than the small set of animated elements — often traces back to a cascade of
stacking-context-creating properties applied to ancestor elements for unrelated
reasons: a `z-index` rule that required `position: relative`, a `filter` for a
subtle drop shadow, or a `transform: translateZ(0)` hack applied years earlier
to work around a rendering bug that no longer exists in any current browser.
Each of these implicitly creates a stacking context; in some rendering engine
versions and configurations, that stacking context results in layer promotion.
Auditing the Layers panel after any significant CSS restructuring and removing
obsolete `transform: translateZ(0)` and `will-change` applications that were
added as workarounds — rather than intentional performance optimization — is a
routine maintenance task that prevents layer count from accumulating silently
across many feature branches.
A useful heuristic for code review is to treat any new occurrence of
`will-change` in a static CSS rule — not in a `:hover` state, not applied via
JavaScript immediately before animation — as a signal that the engineer may not
be aware of the memory cost, and to prompt a discussion about whether dynamic
