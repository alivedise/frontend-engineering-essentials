...
| Server → client hints | Not supported | Server Push (deprecated) | 103 Early Hints (recommended) |
...
`nextHopProtocol` reflects the protocol used for the actual network fetch, not what the page was served over. A resource fetched from a CDN edge over HTTP/2 reports `'h2'` even if the origin-to-CDN leg is HTTP/1.1. This makes it useful for diagnosing what the browser sees, but not sufficient for auditing the full path.

To build a complete protocol audit, combine `nextHopProtocol` data from the browser with CDN access logs (which record the protocol used for origin fetches) and origin server access logs (which record the protocol used for CDN-to-origin fetches). A discrepancy between `nextHopProtocol === 'h2'` in the browser and `HTTP/1.1` in origin logs is the signature of the CDN downgrade problem described in the Common Mistakes section. Many CDN dashboards expose a "Protocol" dimension in their analytics, making this audit possible without log parsing.
