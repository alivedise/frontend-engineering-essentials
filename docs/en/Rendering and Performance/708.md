...
:::info
The HTTP version running between the browser and server changes how resources load in parallel and how a single lost packet affects other in-flight requests. It also determines whether fine-grained code splitting is a performance win or a regression. Understanding the differences between HTTP/1.1, HTTP/2, and HTTP/3 is essential for making correct bundling, preloading, and CDN configuration decisions.
:::
...
The adoption trajectory of each version matters for decision-making. As of Cloudflare Radar's 2025 Year in Review, roughly 50% of web requests use HTTP/2, 21% use HTTP/3, and the remaining 29% are still HTTP/1.x. HTTP/2 is supported by virtually all CDNs and origin servers, so most teams can assume it is available; HTTP/3 is now a meaningful slice of live traffic rather than a browser-support curiosity. HTTP/3 is enabled by default in Chrome, Edge, Firefox, and Safari/iOS 16+, so effectively all current browsers support it. Both newer versions require TLS in practice: no browser implements cleartext HTTP/2 (h2c) or HTTP/3. The implication for frontend teams is that TLS is not optional. Any origin still served over HTTP must be migrated before it can benefit from multiplexing. The good news is that modern certificate providers such as Let's Encrypt and Cloudflare have made free, auto-renewing TLS certificates routine, removing the cost barrier that kept many origins on HTTP/1.1 through the 2010s.
...
The 103 status is informational. The browser does not wait for it and does not render it. It immediately begins fetching the hinted resources while the server executes its database queries or template rendering. On a page with 200 ms of server processing time and 50 ms RTT, Early Hints can recover most of that 200 ms by overlapping the network fetch of CSS and JS with the server's work. This is the main advantage over traditional preload headers, which arrive with the 200 response after server processing has already completed. This full benefit applies in Chrome, Edge, and Firefox 123+ (Firefox shipped Early Hints `preconnect` in Firefox 120 but did not add `preload` support until Firefox 123). Safari 17+ only acts on `rel=preconnect` in a 103 response and ignores `rel=preload`, so the `link: preload` hints in the example above only warm the connection for Safari users rather than fetching the CSS and JS ahead of time.
...
A practical heuristic for choosing between preload and Early Hints: if server processing time is under 50 ms (for example, static file serving), preload headers are sufficient; if the server must execute database queries or complex template rendering, Early Hints hide that latency more effectively. On mobile, with 100 ms of server processing time and 80 ms RTT, Early Hints can improve First Contentful Paint (FCP) by 15–25%. This is worth validating with production RUM data before a broader rollout.
...
**Split bundles granularly only when HTTP/2+ is confirmed end-to-end, and group small modules rather than splitting to one file each.** Under HTTP/2, splitting vendor libraries into separate cacheable chunks is usually a win: a `vendor-react` chunk cached for 30 days does not re-download when only application code changes. Under HTTP/1.1, the same split creates 24 extra requests that exhaust the six-connection pool. Even under HTTP/2, splitting too finely can backfire: Khan Academy found that many small files compress worse than one larger bundle and add per-request overhead, so group related small modules into chunks of tens of kilobytes rather than one file per component. The correct default for greenfield projects is to assume HTTP/2 on modern CDNs, group chunks at a sensible granularity, and keep an explicit fallback build configuration for HTTP/1.1 legacy segments.
...
**Audit HPACK / QPACK compression benefit when working with many small requests.** HTTP/2 HPACK and HTTP/3 QPACK compress request headers across streams on the same connection. When all 30 module chunks share the same origin, `Host`, `Accept-Encoding`, and cookie headers are sent once and delta-compressed for subsequent requests. Cloudflare's production measurements found this cuts ingress (request) header bytes substantially: about 53% less total ingress traffic overall, with request (ingress) headers compressed by about 76% on average.
...
Frontend engineers routinely make bundling and preloading decisions without knowing what HTTP version will deliver those resources to the user. This is a category error. The optimal strategy for HTTP/1.1 (few large files, aggressive inlining) is nearly the opposite of the optimal strategy for HTTP/2 (many small cacheable files, minimal inlining). A team that skips verifying the protocol version at every hop, from origin to CDN edge to load balancer, will get some of those decisions wrong.

The verification step is not a one-time activity. CDN configurations change, origin servers are upgraded, load balancers are swapped, and protocol negotiation settings silently regress. Establishing protocol verification as a standard item in performance audits keeps the bundle strategy aligned with the delivery protocol as infrastructure evolves. A useful audit checks three things: `nextHopProtocol` for representative resources, CDN origin connection settings in the CDN admin dashboard, and whether `Alt-Svc` headers are present in responses.

A useful audit heuristic: when any infrastructure change lands (CDN provider switch, origin server upgrade, load balancer replacement, TLS certificate renewal), re-verify protocol negotiation before the change is considered complete. Infrastructure teams routinely make changes that inadvertently downgrade HTTP/2 to HTTP/1.1 at the CDN-to-origin hop, for example by enabling a feature that requires a custom protocol the origin does not support. Catching this class of regression in the same PR or deployment that introduced the change is far cheaper than diagnosing it weeks later when perceived performance has silently worsened.
...
HTTP/2 multiplexing eliminates the connection-count constraint by flowing all requests through one TCP connection as independent streams. Each stream has its own header block and flow-control window. The server can interleave frames from different streams at a granularity of 16 KB, so a slow-arriving large chunk does not block a fast-arriving small one at the HTTP layer. This is why fine-grained module splitting becomes viable. One chunk per route, per component, or per vendor library each loads in parallel on the same connection, shares the same header-compression table, and can be cached independently. When a user revisits the page and only one module has changed, only that module's chunk is fetched; the rest are served from cache.

This is not an unconditional argument for maximum granularity, though. Compression algorithms such as gzip and Brotli find repeated byte patterns more easily across one large payload than across many small files split at arbitrary boundaries, because each file compresses independently and loses the cross-file redundancy a single bundle would have captured. Khan Academy's engineering team measured this when it re-architected its build to serve individual JavaScript source files over HTTP/2: splitting 28 JavaScript packages into 296 individual files grew the total compressed payload by 2.5% (from about 647 KB to 663 KB), even though the uncompressed size dropped by 5.7%, and the added per-file request overhead produced a slower, less consistent loading waterfall than the bundled version. The practical middle ground is to group related small modules into chunks of tens of kilobytes rather than splitting all the way down to one file per component.

HTTP/3's per-stream loss recovery takes the multiplexing benefit further. On HTTP/2 over TCP, a single lost packet freezes all streams because the TCP stack requires in-order delivery. A 1% packet loss rate means a stream-blocking retransmit event occurs roughly every 100 packets, frequent enough to measurably degrade performance on mobile networks, where loss rates in this range are common. HTTP/3/QUIC tracks sequence numbers per stream and only stalls the stream that owns the lost packet. In practice, this translates to more consistent load times under degraded network conditions, which is where real users often operate.

### How HTTP version affects the browser's resource prioritization

HTTP/2 and HTTP/3 give browsers more levers to express resource priority. Both versions support a priority field on each stream that signals to the server which responses should be sent first when bandwidth is constrained. Browsers use this to ensure that render-blocking CSS arrives before non-critical images. Under HTTP/1.1, the browser could influence priority only indirectly, by controlling which TCP connections it opened first. This was a blunt instrument: it depended on connection timing rather than explicit signaling.
...
| Bundle granularity preference | Few large bundles | Many small modules, grouped for compression | Many small modules, grouped for compression |
...
| Adoption (2026) | Legacy; CDNs still serve it | ~50% of requests; virtually all CDNs and origins | ~21% of requests; all modern browsers |
...
**Ignoring the impact of third-party scripts on protocol negotiation.** Third-party scripts (analytics, A/B testing, chat widgets) are loaded from separate origins. Each separate origin establishes its own connection, with its own protocol negotiation and TLS handshake. If that third party's CDN is still on HTTP/1.1, the connection is also limited to the usual queue of up to six connections. A page that loads its own resources over HTTP/3 but pulls in ten third-party scripts from origins that still use HTTP/1.1 is subject to HTTP/1.1 constraints for those scripts. Use `nextHopProtocol` on third-party resource entries to audit whether critical third-party scripts are being served over an efficient protocol.
...
- [FEE-705: Code Splitting, Lazy Loading & Tree Shaking](./705.md)
- [FEE-801: Bundlers: Webpack, Vite, Rollup & esbuild](../Build%20Tooling%20and%20Module%20Systems/801.md)
