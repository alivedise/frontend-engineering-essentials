...
The verification step is not a one-time activity. CDN configurations change, origin servers are upgraded, load balancers are swapped, and protocol negotiation settings silently regress. Establishing protocol verification as a standard item in performance audits — checking `nextHopProtocol` for representative resources, reviewing CDN origin connection settings in the CDN admin dashboard, and confirming that `Alt-Svc` headers are present in responses — ensures that the bundle strategy continues to match the delivery protocol as the infrastructure evolves.
