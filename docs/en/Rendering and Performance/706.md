...
- SHOULD use WebP as the default image format for broad browser compatibility; SHOULD offer AVIF as a next-generation alternative where build pipeline or CDN supports format negotiation.
- SHOULD use `srcset` and `sizes` on every `<img>` to serve appropriately sized images for the user's viewport and display density.
- SHOULD add `loading="lazy"` to all below-the-fold images to defer their network requests until the user scrolls near them.
- MUST add `fetchpriority="high"` to the LCP image (the largest image in the initial viewport) to ensure the browser fetches it at the highest priority.
- MUST add descriptive `alt` text to every image for accessibility and SEO.
- SHOULD set explicit `width` and `height` (or `aspect-ratio` in CSS) on every image to prevent Cumulative Layout Shift while the image loads.
- SHOULD use framework image components (Next.js `<Image>`, Nuxt `<NuxtImg>`, Astro `<Image>`) in framework projects -- they enforce correct behavior by default.
...
- **Using JPEG or PNG when WebP would suffice.** WebP has universal browser support and reduces file size by 25--40% at equivalent quality. There is no reason to default to JPEG for new image uploads in 2024 and beyond, unless a legacy system requires it.

- **Lazy-loading the LCP image.** The LCP image is by definition in the initial viewport. Adding `loading="lazy"` to it delays the fetch until the browser has parsed enough of the page to determine that the image is in the viewport -- significantly increasing LCP. The LCP image should have `fetchpriority="high"` and no `loading` attribute (or `loading="eager"`).

- **Not adding explicit width and height to images.** Without dimensions, the browser allocates no space for the image during layout. When the image loads, the surrounding content shifts, generating CLS. Setting `width` and `height` (matching the intrinsic size or the displayed aspect ratio) allows the browser to reserve the correct space immediately.
...
- **Skipping format negotiation in the picture element or CDN.** Generating WebP but not AVIF (or vice versa) leaves compression gains on the table. If the CDN or build pipeline supports AVIF encoding, always offer AVIF as the first source and WebP as the fallback.
