...
**MUST clean up event listeners and subscriptions on component unmount.** Every `addEventListener`, `subscribe`, or `observe` call MUST have a corresponding removal when the component is destroyed. Leaked listeners keep their closure -- and everything it references -- alive indefinitely.

**SHOULD use AbortController for listener cleanup.** Pass an `AbortSignal` to `addEventListener` so that calling `controller.abort()` removes all associated listeners in one call. This eliminates the need to keep references to individual handler functions.

**SHOULD use WeakMap for associating metadata with DOM elements.** When you need to attach data to DOM nodes (caching computed values, storing state), use a `WeakMap` with the node as key. This allows the node and its metadata to be collected together when the node is removed from the document.

**SHOULD NOT rely on GC timing for correctness.** Garbage collection is non-deterministic. Never write code that depends on finalizers running promptly or `WeakRef.deref()` returning `undefined` at a specific time.
...
### 2. Adding event listeners without removal in SPA navigation
...
### 3. Closures capturing more scope than needed
...
### 4. setInterval without clearInterval on unmount
