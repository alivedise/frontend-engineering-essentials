...
Every value in a JavaScript program occupies memory. A common mental model separates primitives on the stack from objects and closures on the heap, but engines like V8 do not implement it literally: most values, including many primitives, live on the heap, and the stack mainly holds call frames and references into it. What the language specification does guarantee is that memory management is automatic. The engine decides when to reclaim unreachable objects; the developer manages reachability, and the engine handles deallocation.
...
Understanding memory management spans three levels:

- The language model: allocation and reachability.
- The engine implementation: V8's generational collector.
- The application patterns: framework lifecycle cleanup and leak prevention.

A single-page application's memory usage grows steadily as the user navigates between routes, and never falls back to its baseline even after returning to the home page. The garbage collector cannot reclaim objects that are still referenced. In practice, that usually means an event listener still attached to a DOM node that was removed from the tree, or a timer that was never cleared. The Example section below walks through locating the specific retained object with a heap snapshot, and the Design Thinking section covers how mark-and-sweep decides what counts as reachable in the first place.
...
**How the leak happens:** A module-scope variable holds a reference to a DOM node that was removed from the document. The GC traces from `window` to the module variable to the detached node, and finds it reachable, so it cannot be collected. The node's entire subtree (children, text, attributes) stays alive with it.

## Example

### 1. Locating the leak: heap snapshot comparison

Code fixes are easy to write once you know which object is retained. Finding that object is a separate skill, and Chrome DevTools' Memory panel is built for exactly this:

1. Open DevTools > Memory, select **Heap snapshot**, and take a baseline snapshot right after the page loads.
2. Perform the suspected leaking action several times, for example navigate to the route and back five times, so any retained objects show up as a clear multiple rather than a single hard-to-spot instance.
3. Take a second snapshot and switch its view to **Comparison**. This lists constructors by the delta in live instance count between the two snapshots; a constructor whose count grew by roughly the number of repetitions is the leak candidate.
4. In that snapshot, filter by "Detached" in the class filter, or pick "Objects retained by detached nodes" from the Summary view's constructor dropdown, to isolate DOM nodes that were removed from the document but are still retained. Selecting one shows its retainer chain: the path of references from a GC root down to that node, the same `window` to module-variable to node chain shown in the diagram above.

For production monitoring rather than local debugging, the standardized `performance.measureUserAgentSpecificMemory()` API reports a page's aggregate memory usage, including iframes and workers, without DevTools attached. It requires the page to be cross-origin isolated (`self.crossOriginIsolated === true` via `Cross-Origin-Opener-Policy` and `Cross-Origin-Embedder-Policy` headers).

### 2. Memory leak from forgotten event listener -- fix with AbortController
...
### 3. WeakMap for metadata caching on DOM elements
...
### 4. React useEffect cleanup pattern
...
All modern JavaScript engines use **mark-and-sweep** as their primary collection algorithm. The algorithm has two phases:

1. **Mark phase.** The collector starts from a set of *GC roots*:
   - the global object (`window` or `globalThis`)
   - the call stack (local variables in active functions)
   - CPU registers

   From each root, it recursively follows every reference, marking each reachable object as "alive."
...
**Why not reference counting?** Early engines and some languages, such as Swift's ARC, use reference counting: each object tracks how many references point to it, and is freed when the count reaches zero. Its fatal flaw is circular references. Two objects that reference each other both have a count of 1 even when no root can reach either one, so ARC leaks them unless the developer breaks the cycle with a `weak` or `unowned` annotation. (CPython also uses reference counting, but pairs it with a generational cycle collector that reclaims exactly these cycles, so it does not share this specific flaw.) Mark-and-sweep sidesteps the problem entirely because it traces from roots: circular islands of objects with no root path are correctly identified as garbage regardless of their internal reference counts.

### Generational GC in V8

V8 (Chrome, Node.js, Deno) implements a generational garbage collector based on the **generational hypothesis**: most objects die young. A temporary array created during a function call or an intermediate string from template literal concatenation typically becomes unreachable almost immediately after creation.

V8 divides the heap into two generations:

**Young generation (Scavenger / Minor GC).** A small, configurable region using a semi-space design. Historically V8 defaulted this to 16 MB per semi-space on 64-bit platforms (8 MB on 32-bit); newer V8 versions instead scale the default dynamically with the memory available to the process, which can land considerably lower in constrained environments such as containers or serverless functions. Half the space is the "nursery" where new objects are allocated; the other half is empty. When the nursery fills up:
...
This is extremely fast because most objects in the nursery are already dead: the collector only copies the small number of survivors, which keeps Minor GC pauses far shorter than a full heap scan would take.
...
However, side effects that reach outside the reactive system (DOM listeners, network connections, timers) still require explicit cleanup. Signals automate unsubscribing from computed values; stopping a timer or closing a socket remains the developer's job.

### TC39 Explicit Resource Management

Explicit Resource Management reached Stage 4 in May 2025 and is part of ECMAScript 2027. It introduces the `using` keyword for deterministic cleanup, already shipped in Chrome 134+:
...
- **FinalizationRegistry**: Registers a callback that fires *after* an object is garbage-collected. Useful for releasing external resources such as closing sockets or freeing GPU buffers, but the timing is non-deterministic: the callback may run much later than expected, or not at all.

**Important caveat from the spec**: "Correct code must not depend on WeakRef/FinalizationRegistry behavior." Treat them purely as optimization hints; correctness-critical logic needs a deterministic mechanism instead.
...
- [MDN -- Performance: measureUserAgentSpecificMemory()](https://developer.mozilla.org/en-US/docs/Web/API/Performance/measureUserAgentSpecificMemory) -- standardized API for measuring a page's aggregate memory usage in production, requires a cross-origin isolated context
- [Nolan Lawson -- Fixing memory leaks in web applications](https://nolanlawson.com/2020/02/19/fixing-memory-leaks-in-web-applications/) -- independent, practitioner-level case studies of closure and listener leaks in single-page applications
...
- [Node.js -- Command-line API: `--max-semi-space-size`](https://nodejs.org/api/cli.html#--max-semi-space-sizesize-in-mib) -- V8 flag documentation for the young-generation semi-space size and its platform defaults
...
- [TC39 -- Explicit Resource Management proposal](https://github.com/tc39/proposal-explicit-resource-management) -- Stage 4 specification (part of ECMAScript 2027) for `using`, `await using`, `Symbol.dispose`, and `Symbol.asyncDispose`
