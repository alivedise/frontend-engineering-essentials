...
All four weak primitives -- `WeakMap`, `WeakSet`, `WeakRef`, and `FinalizationRegistry` -- share a common constraint: keys and registered targets must be objects (or, in newer engine versions, certain non-registered symbols). Primitives like strings, numbers, and booleans are not valid keys for `WeakMap` or `WeakSet`. The reason is that primitives are value types: there is no identity to hold weakly, no object to track for reachability. The restriction is enforced at runtime with a `TypeError`.

ES2023 extended `WeakMap` and `WeakSet` to also accept registered symbols as keys, in addition to objects. Registered symbols are the result of `Symbol.for(key)` -- they have a global identity and can be looked up by key. Unregistered symbols created with `Symbol()` are not valid keys, because they do not have the kind of global identity that the engine can track for GC purposes. This extension is a minor addition to the original design; the primary use case for weak collections remains object keys.

## Principle

Engineers SHOULD use `WeakMap` rather than `Map` when associating private metadata or caches with objects that have a lifetime independent of the metadata. A `Map<HTMLElement, EventData>` retains every element seen indefinitely, growing without bound in any application that creates and discards elements. A `WeakMap<HTMLElement, EventData>` releases entries automatically when elements are collected, making the memory leak structurally impossible without requiring any explicit cleanup code from the caller. The choice between `Map` and `WeakMap` is not primarily a performance decision; it is a correctness decision about ownership and lifetime.

Engineers MUST NOT use `WeakRef` or `FinalizationRegistry` as a substitute for explicit resource cleanup. GC timing is non-deterministic. A `WeakRef` target may persist for the lifetime of the process or be collected in milliseconds -- there is no guarantee either way, and the specification deliberately does not promise the delivery order or timing of finalization callbacks. These APIs are for caches where stale entries are tolerable, and for detecting missed cleanup in debug tooling. File handles, network connections, event listener removal, and timer cancellation require explicit lifecycle management regardless of whether weak references are involved.
