...
Engineers SHOULD use `Proxy` to centralize cross-cutting concerns —
validation, change notification, access logging, lazy initialization — rather
than duplicating this logic in every setter or method. The value of Proxy is
that it intercepts operations at the object level: a single `set` trap covers
every property on the target, without enumerating properties at proxy creation
time. This makes Proxy appropriate for targets with dynamic property sets,
where the shape of the object is not known in advance and new properties may
be added after the proxy is created.

Engineers MUST use `Reflect` methods inside Proxy traps to perform default
operations, rather than accessing the target directly. Directly accessing
`target[prop]` inside a `get` trap breaks for objects with accessor properties
or prototype chains. `Reflect.get(target, prop, receiver)` correctly propagates
the receiver, which is essential for inherited getters to work correctly
through a Proxy. When the trap does not need to modify the operation at all,
it should call the corresponding `Reflect` method and return its result
directly, preserving all invariants the spec requires for that operation.
...
through explicit, discoverable code.
