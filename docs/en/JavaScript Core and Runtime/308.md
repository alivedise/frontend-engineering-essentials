...
**Prefer `for...of` over the manual iterator protocol.** SHOULD consume iterables with `for...of`, spread syntax, or destructuring rather than calling `.next()` manually. When a `for...of` loop exits early via `break` or `return`, the runtime automatically calls `iterator.return()` on the iterator, allowing it to release resources held in `finally` blocks. Manual `.next()` calls do not trigger this cleanup unless you call `.return()` explicitly.

**Always use `try/finally` in generators that hold resources.** MUST wrap resource acquisition in a generator's `try` block and place the corresponding release in `finally`. The `finally` block runs when the generator completes normally, when it throws, and when `generator.return()` is called externally — including the call made by `for...of` on early exit. Without `finally`, resources such as open connections, file handles, or subscriptions will leak when the consumer breaks early.

**Use `Symbol.asyncIterator` for streaming and push-based data sources.** SHOULD implement `Symbol.asyncIterator` on objects that produce values asynchronously over time — paginated API responses, WebSocket message streams, SSE connections, and ReadableStream wrappers. An async iterable is consumable with `for await...of`, which provides pull-based backpressure: the producer only advances when the consumer calls `next()`, preventing unbounded buffering.

**Avoid infinite generators without explicit termination conditions for the consumer.** SHOULD pair every infinite generator (`while (true) { yield ... }`) with a consumer-side limit such as `break`, a `take(n)` wrapper, or a termination signal. An infinite generator that is never terminated prevents the generator function from being garbage-collected and can accumulate state indefinitely. When an infinite generator is intended, document the termination contract clearly.
...
### Generator delegation with `yield*` and method propagation

`yield*` delegates iteration to another iterable or generator, forwarding each yielded value to the outer consumer. What is less obvious is that all three generator control methods -- `next()`, `throw()`, and `return()` -- are forwarded through the delegation chain to the innermost generator.

**`return()` propagation:** When the outer consumer calls `generator.return(value)`, `yield*` forwards the `return()` call to the currently delegated iterator. If that iterator defines `return()`, it is invoked, which triggers any `finally` block inside the inner generator. The inner generator closes, and control returns to the outer generator, which then also closes. This means that a `break` or early `return` in a `for...of` loop correctly cleans up resources across all levels of a `yield*` chain.

**`throw()` propagation:** When the consumer calls `generator.throw(error)`, `yield*` forwards the `throw()` call to the delegated iterator. If the inner generator has a `try/catch` at the current `yield`, it can catch the error and continue. If the inner generator does not handle it, the error propagates back to the outer generator's `yield*` expression, where the outer generator's `try/catch` can handle it. If neither handles it, it becomes an unhandled exception.

**`yield*` return value:** The expression `yield* innerGen()` itself evaluates to the return value of the inner generator (the value in `{ value: ..., done: true }` from the inner generator's final `return` statement). This allows composing pipelines where each generator stage can pass a result to the next.

```js
function* inner() {
  yield 1;
  yield 2;
  return "inner done"; // the return value of the inner generator
}
...
// "inner returned: inner done" is logged during iteration
```

Understanding this propagation model matters when composing generator pipelines that manage resources: a `break` at the top of the chain correctly threads `return()` all the way down, triggering `finally` blocks in every level.
