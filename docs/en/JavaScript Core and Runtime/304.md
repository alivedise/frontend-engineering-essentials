...
MDN states: "Strict equality is almost always the correct comparison operation to use." Use `===` for every equality check unless you have a specific reason not to. Configure ESLint's `eqeqeq` rule to enforce this automatically.

```js
// Correct: strict equality -- no type surprises
user.role === "admin";
count === 0;

// Acceptable exception: == null checks for both null and undefined
...
Rely on `Number()`, `String()`, and `Boolean()` for intentional conversions rather than implicit coercion. Explicit conversion makes the intent clear to readers and prevents the coercion rules from silently doing something unexpected.

```js
// Implicit -- fragile, relies on coercion rules
const n = +"42";         // number (unary +)
const s = "" + 42;       // string (+ concatenation)
const b = !!value;       // boolean (double negation)
...
The global `isNaN()` coerces its argument to a number before testing, so `isNaN("blabla")` returns `true` even though `"blabla"` is a string, not `NaN`. `Number.isNaN()` does not coerce: it returns `true` only when the argument is of type Number and its value is `NaN`.

```js
isNaN("blabla");          // true  -- string coerced to NaN, misleading
isNaN(undefined);         // true  -- undefined coerced to NaN, misleading

Number.isNaN("blabla");   // false -- not a Number at all
Number.isNaN(undefined);  // false -- not a Number at all
Number.isNaN(NaN);        // true  -- correct
Number.isNaN(0 / 0);      // true  -- correct
```
