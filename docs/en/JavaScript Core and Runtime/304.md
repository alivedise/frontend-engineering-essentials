...
### 1. Using `==` when `===` is intended

```js
// Bug: user input is always a string
function isAdmin(role) {
  return role == 1;  // "1" == 1 is true -- accidentally works, but fragile
}

// Fix: explicit conversion or strict equality
function isAdmin(role) {
  return Number(role) === 1;
}
```

But remember: `== null` is the accepted exception:
...
### 2. Truthy-checking when you mean non-nullish
...
### 3. String concatenation with `+` when addition intended
...
### 4. Assuming `typeof null === "object"` is intentional
...
### 5. Not using `Object.is()` when `-0` and `NaN` comparison matters
