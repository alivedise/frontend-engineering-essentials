...
A form validation function uses `==` to check whether a field value equals zero, and it passes for empty strings. A conditional using `+` to concatenate a number and a string produces `"12"` instead of `3`. Both behaviours are predictable once JavaScript's coercion rules are understood, but they produce bugs that are difficult to find through inspection alone. Knowing when the language coerces values and how `==` differs from `===` lets you write comparisons that behave as intended.
...
**SHOULD use `Object.is()` when `-0` or `NaN` semantics matter.** In numerical algorithms, map key comparisons, or SameValue comparisons such as `Array.prototype.includes`, `Object.is()` gives the mathematically correct answer where `===` does not: `Object.is(NaN, NaN)` is `true` and `Object.is(-0, +0)` is `false`. Use `Object.is()` wherever distinguishing negative zero from positive zero, or reliably detecting `NaN` without coercion, is a correctness requirement rather than a stylistic preference.
