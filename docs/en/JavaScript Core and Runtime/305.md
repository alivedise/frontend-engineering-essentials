...
**MUST handle async errors explicitly.** Every promise chain MUST have a `.catch()` or be inside a `try/catch` within an `async` function. Unhandled rejections are logged by the runtime but do not prevent the application from continuing in a corrupted state.

**SHOULD use error `cause` to preserve context.** When catching and re-throwing, wrap with `new Error('message', { cause: originalError })` so that the full error chain is available for debugging. Losing the original error is one of the most common mistakes in production code.

**SHOULD NOT use empty catch blocks.** Swallowing errors silently makes bugs invisible. If you intentionally ignore an error, add a comment explaining why.

**SHOULD define error boundaries at UI composition boundaries.** Each independently meaningful section of UI (a sidebar, a data table, a form) should be wrapped in a framework error boundary so that a crash in one section does not take down the entire page.
...
### 1. Empty catch blocks that silently swallow errors

```js
// Bug: error is completely hidden -- debugging is impossible
try {
  processData(input);
} catch (err) {
  // "I'll add logging later" -- you won't
}

// Fix: at minimum, log the error. If intentionally ignoring, document why.
try {
  processData(input);
} catch (err) {
...
// If truly intentional (rare):
try {
  localStorage.setItem("preference", value);
} catch {
  // Intentionally ignored: localStorage may be full or disabled in private browsing.
  // The app works fine without persisting this preference.
}
```
...
### 3. Forgetting that async functions return rejected promises
...
```js
// Bug: rejected promise with no handler
fetch("/api/data").then((res) => res.json());
// If fetch fails, unhandledrejection fires silently

// Fix: always attach a .catch() or use await in try/catch
fetch("/api/data")
  .then((res) => res.json())
  .catch((err) => handleError(err));

// Also: add a global safety net during development
window.addEventListener("unhandledrejection", (event) => {
...
### 5. Losing error context by wrapping without cause
