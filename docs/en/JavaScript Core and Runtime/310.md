...
Engineers SHOULD use `Symbol()` as property keys when adding metadata or
protocol hooks to objects shared across library and application code, where
property name collisions would silently corrupt behavior. A library that
attaches processing state using `__processed` risks collision with application
code using the same key: both silently read and write the same slot, and the
bug appears only when behavior diverges in ways that are extremely difficult
to trace back to a naming collision. A Symbol key has zero collision
probability — not low probability, but zero — because the key itself is the
unique identity. No other code can produce that Symbol without holding a
reference to it. The description string passed to `Symbol('desc')` appears in
`sym.toString()` and in debugger output, but has no effect on identity,
equality, or lookup behavior.

Engineers MUST NOT use `Symbol.for()` as a substitute for `Symbol()` when
the intent is uniqueness. `Symbol.for(key)` returns the same Symbol for the
same key from the global registry — it is explicitly shared, not unique. Any
code in any realm that calls `Symbol.for('myLib.state')` receives the
identical Symbol, including unrelated code that chose the same registry
string. If a third-party package independently uses the same `Symbol.for`
key string, it gains full access to any property the library stored under
that key. Use `Symbol()` for private unique keys; use `Symbol.for()` only
when intentional cross-realm sharing is required, and document the key
string as a stable public interface, because it becomes a shared namespace
that any caller can observe or modify.
