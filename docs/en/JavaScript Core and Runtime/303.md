...
**Modifying Object.prototype pollutes all objects.** Adding properties to `Object.prototype` affects every object in the runtime, including object literals and maps. Third-party code that uses `for...in` without `hasOwnProperty` guards will break. Never add to `Object.prototype` in production code.
