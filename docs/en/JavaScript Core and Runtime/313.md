...
You are passing a complex object with nested arrays and `Date` values from a Web Worker back to the main thread. `JSON.parse(JSON.stringify(obj))` works for plain objects but converts `Date` instances to strings and drops `undefined` values. The structured clone algorithm — used internally by `postMessage` and exposed directly as `structuredClone()` — performs a deep copy that preserves `Date`, `Map`, `Set`, `ArrayBuffer`, and other non-JSON-serialisable types.
...
### Choosing between structuredClone(), shallow copy, and custom clone

The correct cloning strategy depends on the shape of the data and the requirements
of the operation. Shallow copy (`Object.assign` or spread) is correct when the data
is flat or when shared references to nested objects are intentional — for example,
when merging configuration objects where nested sub-objects are treated as opaque
references. `structuredClone()` is correct when the data contains nested mutable
objects that must not be shared between the original and the copy — for example,
when deriving a new state object from an existing one before mutation. A custom
`clone()` method is correct when the data is a class instance with behavior that
must be preserved, or when the cloning logic is non-trivial (e.g., the clone needs
to generate a new unique ID rather than copying the source's ID).

The common mistake is reaching for a general-purpose utility when a more precise
tool is appropriate. A flat object being used as a record of primitive key-value
pairs does not need `structuredClone()` — spread is sufficient and faster. An
immutable value object that is never mutated does not need to be cloned at all —
it can be shared by reference safely. Applying `structuredClone()` universally as
a defensive habit adds unnecessary allocation cost for cases where a cheaper
strategy is safe. Understanding the data model — whether nested values can be
mutated, whether instances must retain their prototype, whether the result will
cross a serialization boundary — determines which strategy is appropriate.
...
**MUST NOT use `JSON.parse(JSON.stringify(obj))` as a general-purpose deep clone.** The JSON round-trip is a string serialization format, not a cloning mechanism: it silently drops `undefined` properties, converts `Date` objects to ISO strings, converts `Map` and `Set` instances to plain empty objects, and throws synchronously on circular references. The resulting object appears structurally plausible while carrying corrupted values, making bugs difficult to trace back to the clone site. `structuredClone()` is the correct replacement for all plain-data deep-clone use cases. The JSON round-trip remains appropriate only when the goal is to produce a JSON-safe string representation for transmission or storage — not for in-memory duplication.
