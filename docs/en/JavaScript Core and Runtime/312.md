...
Engineers MUST use arrow functions for callbacks and event handlers that need
to close over the enclosing class or function's `this`, rather than using
`.bind(this)` at the call site or assigning `const self = this`. Arrow functions
capture `this` lexically, making the intent explicit in the function definition
itself: the source of `this` is visible at the point where the function is
written, not at the point where it is called. Arrow functions cannot be rebound
by `call`, `apply`, or `bind` — this is a feature, not a limitation, because it
removes one more source of accidental rebinding. Code that relies on an arrow's
`this` being stable has that guarantee enforced by the language, not by
convention.

Engineers MUST NOT assume that a method extracted from an object retains its
`this` binding. `const fn = obj.method; fn()` invokes `fn` without an implicit
receiver; `this` inside `fn` resolves to `undefined` in strict mode and to the
global object in non-strict mode. This applies universally: assigning methods to
variables, passing them as callbacks, storing them in arrays, or destructuring
them from an object — all of these extract the function from its original
receiver and cause the binding to be lost. The only ways to preserve the
binding are to call the method in-place as `obj.method()`, to use
`.bind(obj)` to create a permanently-bound function, or to use an arrow wrapper
that captures `obj` in the closure.
