...
:::info
JavaScript ran without a module system for its first 20 years. Scripts shared everything through the global object, and the community invented CommonJS, AMD, and UMD to fill the gap. ES Modules (ESM) -- standardized in ES2015 and refined ever since -- finally gave the language a native, static, asynchronous module system. Understanding how ESM works, why it was designed with static structure, and how it coexists with CommonJS is essential for writing code that bundlers can optimize, browsers can load efficiently, and teams can maintain at scale.
:::

## Context

Before ES2015, JavaScript had no language-level module system. Node.js adopted CommonJS (CJS) in 2009, giving server-side code `require()` and `module.exports`. Browsers relied on concatenation, AMD (`define`/`require`), or UMD wrappers. Each approach had tradeoffs:

- **CommonJS** -- synchronous, dynamic, works well in Node where modules are loaded from the local filesystem. Not directly usable in browsers without bundling.
- **AMD (RequireJS)** -- asynchronous, browser-friendly, but verbose callback syntax and a separate ecosystem.
- **UMD** -- a wrapper pattern that detects the environment and works with both CJS and AMD. Complex boilerplate, no static analysis.

ES2015 introduced `import`/`export` as syntax -- part of the language grammar, not a runtime API. This distinction is critical: because imports and exports are declarative and top-level, tools can analyze the module graph at build time without executing the code.
...
**SHOULD prefer ESM over CJS for new code.** ESM enables static analysis, tree-shaking, and native browser loading. CJS remains necessary for legacy dependencies and certain Node.js tooling, but new packages and application code SHOULD use ESM.

**MUST mark side-effect-free packages with `"sideEffects": false` in `package.json`.** Without this hint, bundlers cannot safely remove unused exports from your library. A side effect is code that executes at import time and affects global state (polyfills, CSS imports, registration calls). If your package has none, declare it explicitly.

**SHOULD use named exports over default exports for library code.** Named exports enable better tree-shaking, IDE auto-import, and refactoring support. Default exports are convenient for single-purpose modules (a React component file) but cause friction when a library re-exports many items.

**MUST NOT use `require()` in ESM files.** Mixing module systems in a single file causes confusing errors. If you need to load a CJS dependency from ESM, use `import` (Node.js can import CJS) or `createRequire` from `node:module` as a last resort.

**SHOULD use dynamic `import()` for code splitting.** Static imports load everything upfront. Use `import()` to split code at route boundaries, feature flags, or heavy-dependency boundaries, reducing initial load time.

## Design Thinking

### Why ESM was designed with static structure

The fundamental design choice of ESM is that the module graph can be determined statically -- at parse time, without executing any code. This is not an accident; it was the primary design goal.
...
For production, Vite uses Rollup (or Rolldown in Vite 6+) to produce optimized bundles with full tree-shaking and code splitting.
...
**React** -- `React.lazy()` wraps a dynamic import for component-level splitting:
...
**Vue** -- `defineAsyncComponent()` provides the same pattern:
...
**Next.js** also provides `next/dynamic` for component-level splitting and automatically splits per-page, meaning each route only loads its own JavaScript.

### CJS to ESM migration

The migration from CJS to ESM in the Node.js ecosystem introduces several challenges:
...
**Conditional exports** (`package.json` `"exports"` field) solve the dual package hazard by letting you specify different entry points for different contexts:
...
### 1. Circular dependencies causing undefined imports

```js
// a.js
import { b } from "./b.js";
console.log("a.js loaded, b =", b);  // b is undefined!
export const a = "A";

// b.js
import { a } from "./a.js";
console.log("b.js loaded, a =", a);  // a is "A" (if b.js executes second)
export const b = "B";

// ESM uses live bindings -- 'b' is a reference to b.js's export slot.
// But if a.js is evaluated first, b.js has not yet executed, so the
// binding exists but has not been initialized.
//
// CJS is worse: require() returns a COPY of module.exports at the time
// of the call. If the module hasn't finished executing, you get a
// partial object.
//
// FIX: restructure to eliminate the cycle, or move the access to a
// function that runs after both modules have initialized.
export function getB() {
  return b; // accessed later, after module evaluation completes
}
```

### 2. Mixing default and named exports inconsistently

```js
// utils.js -- confusing: exports both default and named
export default function formatDate(d) { /* ... */ }
export function parseDate(s) { /* ... */ }

// consumer.js -- easy to get wrong
import formatDate from "./utils.js";            // works (default)
import { formatDate } from "./utils.js";         // ERROR: no named export 'formatDate'
import { default as formatDate } from "./utils.js"; // works but awkward

// FIX: use ONLY named exports for multi-export modules
export function formatDate(d) { /* ... */ }
export function parseDate(s) { /* ... */ }

// consumer.js -- clear and consistent
import { formatDate, parseDate } from "./utils.js";
```

### 3. Dynamic import() with template literals defeating static analysis
...
### 4. Top-level await creating waterfall loading
...
### 5. Assuming tree-shaking removes all unused code (sideEffects field)
...
- [Node.js -- ECMAScript modules](https://nodejs.org/api/esm.html) -- Node.js ESM support, interoperability with CJS, conditional exports, and `"type": "module"`
