...
You are adding a utility function that is used across twelve files in a codebase that started as a single `<script>` tag and grew organically. Functions are attached to a global namespace object and the load order of script tags determines what is available. ES Modules — `import` and `export` with static analysis — let bundlers eliminate unused code, enable circular dependency detection, and give each file an explicit dependency declaration rather than an implicit global dependency.
...
**Avoid circular dependencies between modules.** SHOULD structure modules so that no import cycle exists. In ES modules, imported bindings are live references to export slots — if module A imports from B and B imports from A, and A is evaluated first, B's exported value is not yet initialized when A's top-level code runs, producing a `ReferenceError`. Where a cycle cannot be eliminated, move the access to a function called after both modules have fully evaluated.
