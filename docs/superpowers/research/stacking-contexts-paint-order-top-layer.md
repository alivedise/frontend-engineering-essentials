...
- Adversarial-review addendum (2026-07-24): MDN's trigger list itemizes the independent transform properties and mask longhands verbatim: "Element with any of the following properties with a value other than `none`: transform, scale, rotate, translate, filter, backdrop-filter, perspective, clip-path, mask / mask-image / mask-border." (same Stacking_context URL as Claim 3, re-fetched and verified during adversarial review). The article's Visual table includes `scale` / `rotate` / `translate` and the mask longhands on this basis.
