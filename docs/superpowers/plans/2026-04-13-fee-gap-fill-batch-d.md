...
- [ ] **Step 1: Write EN article**
  ...
  **`## Principle`:**
  ...
  Engineers MUST memoize context values that are object or array literals to prevent consumers from re-rendering on every render of the provider component. `<Context.Provider value={{ user, setUser }}>` creates a new object on every render of the component that contains this JSX. Every consumer re-renders whenever this provider renders, regardless of whether `user` or `setUser` changed. `useMemo(() => ({ user, setUser }), [user, setUser])` limits re-renders to when the actual values change.
